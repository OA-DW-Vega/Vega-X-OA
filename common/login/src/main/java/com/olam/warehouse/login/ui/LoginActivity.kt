        ...
        data?.let { response ->
            when (response.status) {
                Resource.Status.SUCCESS -> {
                    //hideLoading()
                    PreferenceHelper.save(Constants.IS_LOGIN,true)
                    vm.saveToken(response.data)
                    vm.getUserDetail(getDeviceID(this), this)
                }
...
//                    }
    ...
    private fun moveToHomeActivty() {
        if (!intent.getBooleanExtra(IS_LOGGED_IN, false)) makeUnReceivedNotification(this)
        ...
        try {
            for (listOfFile in listOfFiles!!) {
                if (listOfFile.isFile) {
                    Timber.d("installApk: %s", listOfFile.name)
