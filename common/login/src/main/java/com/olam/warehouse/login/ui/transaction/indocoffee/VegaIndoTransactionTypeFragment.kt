...
class VegaIndoTransactionTypeFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        vm.weighBridgeOfflineCount.observe(this, Observer { enableGrnBlock(it) })
        vm.getOfflineWeighBridgeDetailCount()

        vm.dispatchItemCountLocal.observe(this, Observer { updateMtntUI(it) })
        vm.getDispatchWithLineItemCount()

        vm.exportSalesLocal.observe(this, Observer { updateExportSalesUI(it) })
