...
class VegaTransactionFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        // Get Receiving Data
        vm.receiveWithLineItemLocal.observe(this, Observer {
            if (it != null) {
                receivingList.clear()
        ...
        // Get Quality Data
        vm.weighBridge.observe(this, Observer {
            if (it != null) {
                qualityList.clear()
                /*val list = it.sortedByDescending { item ->
        ...
        // Get Mtnt Data
        vm.mtntWithLineItem.observe(this, Observer {
            if (it != null) {
                mtntList.clear()
                /*val list = it.sortedByDescending { item ->
        ...
        // Get Processing Data
        vm.rminLots.observe(this, Observer {
            if (it != null) {
                rminList.clear()
                rminList.addAll(it)
                if (rminList.size > 0) {
    ...
    private fun setUpAdapter(receivingList: ArrayList<VegaReceivingWithLineItems>) {
        binding.rvReceiving.setUp(this.receivingList, R.layout.item_transaction, { it, pos ->
            val receiving = it.receiving
            tvWeighBridgeId.text =
            ...
            tvType.text = receiving.weighBridgeType
            tvErrorMsg.text = receiving.syncStatusMsg
            tvErrorMsg1.text = receiving.syncStatusMsg
            tvDirection.text = receiving.truckDirection
            if (receiving.isProgress) pbItemLoading.visible() else pbItemLoading.gone()
            ...
            tvViewWeighBridge.setOnClickListener { view ->
                ...
                if (receiving.status.equals(Status.RECEVING_COMPLETED)) popupMenu.menu.findItem(R.id.action_Sync)
                    .isVisible = false
    ...
    private fun setUpQualityAdapter(qualityList: ArrayList<VegaQualityWBDetails>) {
        binding.rvQuality.setUp(qualityList, R.layout.item_transaction, { it, pos ->
            tvWeighBridgeId.text = if (it.weighBridgeId.isNullOrEmpty()) it.wbTempId else it.weighBridgeId
            when (it.status) {
            ...
            tvType.text = it.weighBridgeType
            tvErrorMsg.text = it.message
            tvErrorMsg1.text = it.message
            if (it.isProgress) pbItemLoading.visible() else pbItemLoading.gone()
            tvDirection.gone()
            ...
            ivDeleteData.setOnClickListener { view -> showItemDeleteDialogQuality(it) }
            tvViewWeighBridge.setOnClickListener { view ->
                val popupMenu: PopupMenu = PopupMenu(requireActivity(), view)
                popupMenu.menuInflater.inflate(R.menu.transaction_menu, popupMenu.menu)
    ...
    private fun setUpMtntAdapter(mtntList: ArrayList<VegaMtntWithLineItems>) {
        binding.rvMtnt.setUp(mtntList, R.layout.item_transaction, { it, pos ->
            ...
            tvType.text = mtnt.weighBridgeType
            tvErrorMsg.text = mtnt.syncStatusMsg
            tvErrorMsg1.text = mtnt.syncStatusMsg
            tvDirection.text = mtnt.truckDirection
            if (mtnt.isProgress) pbItemLoading.visible() else pbItemLoading.gone()
    ...
    private fun setUpDispatchAdapter(dispatchList: ArrayList<VegaDispatchWithLots>) {
        binding.rvDispatch.setUp(dispatchList, R.layout.item_transaction, { it, pos ->
            ...
            ivDeleteData.setOnClickListener { view -> showItemDeleteDialogDispatch(it) }
            tvViewWeighBridge.setOnClickListener { view ->
                val popupMenu: PopupMenu = PopupMenu(requireActivity(), view)
                popupMenu.menuInflater.inflate(R.menu.transaction_menu, popupMenu.menu)
                    ...
                    when (item.itemId) {
                        ...
                        R.id.action_Sync -> {
                            when (dispatch.batchNumber?.isNotEmpty()) {
                                true -> startDispatchSync(dispatch.weighBridgeId, pos, dispatch.batchNumber.toString())
    ...
    private fun setUpProcessingAdapter(rminList: ArrayList<VegaRminBomWithLots>) {
        binding.rvProcessing.setUp(rminList, R.layout.item_transaction, { it, pos ->
            val rmin = it.rminPo
            tvwbLabel.text = "CfgNo"
            tvwbTypeLabel.text = "Stage"
            tvDirectionLabel.text = "Material"
            tvDateLabel.text = "Batch"
            tvDate.text = rmin.batchNumber
            tvWeighBridgeId.text = rmin.cfgNo
            when (rmin.status) {
                Status.RMIN_COMPLETED -> tvSyncStatus.text = "Success"
                else -> tvSyncStatus.text = rmin.status.toString()
            }
            tvType.text = rmin.processingStage
            tvErrorMsg.text = rmin.syncStatusMsg
            tvErrorMsg1.text = rmin.syncStatusMsg
            tvDirection.text = rmin.materialName
            if (rmin.isProgress) pbItemLoading.visible() else pbItemLoading.gone()
            tvErrorMsg.setOnClickListener {
                tvErrorMsg.gone()
                tvErrorMsg1.visible()
            }
