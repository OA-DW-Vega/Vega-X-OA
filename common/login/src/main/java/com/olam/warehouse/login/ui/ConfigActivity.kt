...
import com.olam.warehouse.login.utils.makeUnReceivedNotification
...
class ConfigActivity : HomeBaseActivity() {
    ...
    private fun adapterSetup(){
        binding.rvReceptionType.setUpAdapter(
            receptionItems,
            R.layout.item_reception,
            ItemReceptionBinding::inflate,
            { it, pos, bindingItem ->
                bindingItem.tvReceptionItem.text = it.receptionName
                when (it.selectedProducts.size) {
                    0 -> bindingItem.tvProductItem.text =
                        if (it.products?.size!! == 1) it.products!![0] else it.product
                    else -> bindingItem.tvProductItem.text =
                        it.selectedProducts.toString().replace("[", "").replace("]", "")
                }
                bindingItem.cbReception.isChecked = it.isChecked!!
                bindingItem.cbReception.setOnCheckedChangeListener { _, isChecked ->
                    ...
                    if (isChecked) {
                        if (it.products?.size == 1) {
                            ...
                            MaterialDialog(context).show {
                                listItemsMultiChoice(items = it.products) { _, _, items ->
                                    items.forEach { text ->
                                        keys.forEach { it1 ->
                                            if (it1.contains(
                                                    it.receptionName?.plus("_").plus(conapny_code)
                                                        .plus(
                                                            "_".plus(
                                                                text.toString()
                                                            )
    ...
    private fun moveToHomeActivity() {
        ...
        selectedKeys.forEachIndexed { index, it ->
            if (it.split("_")[0].contains("DO")) {
                ...
                list.forEach {
                    if (valueOfEnum(it.replace(" ", "_")) == UserRoles.SUPPLIER) SplitInstall.download(this, "doreceiving", dolistener)
                    else if (valueOfEnum(it.replace(" ", "_")) == UserRoles.QUALITY) SplitInstall.download(this, "odquality", dolistener)
                }
                ...
                list.forEach { it1 ->
                    when (valueOfEnum(it1)) {
                        ...
                        UserRoles.MTNT -> {
                                if(it.split("_")[2].contains("CASH")){
                                    SplitInstall.download(
                                        this,
                                        "dispatch",
                                        vegalistener
                                    )
                                }else if(it.split("_")[2].contains("COCO") ){
                                    SplitInstall.download(
                                        this,
                                        "mtntcocoa",
                                        vegalistener
                        ...
                        UserRoles.INVOICE -> SplitInstall.download(
                            this,
                            "invoicenicaragua",
                            vegalistener
                        )
            ...
            if (index.equals(selectedKeys.size - 1)) {
                ...
                makeUnReceivedNotification(this)
