 ...
 */
class VegaTransMasterSyncDialog(private val updateStatus: (Boolean) -> Unit) :
    BottomSheetDialogFragment() {

    private lateinit var binding: BottomSheetTransMasterBinding
    private var isSyncDone: Boolean = false
    /*companion object {
        fun newInstance() = VegaTransMasterSyncDialog().putArgs {
        }
    }*/
...
//                                setOffline()
                                ...
                                binding.tvStatusValue.setTextColor(resources.getColor(com.olam.warehouse.presentation.R.color.green))
                                ...
                                val transList = Gson().fromJson<List<TransactionMaster>>(
...
//                                fliptab.selectRightTab(true)
                                workInfo.outputData.getString(TRANS_OUTPUT_DATA)
                                    ?.let {
                                        ...
                                        binding.tvStatusValue.setTextColor(resources.getColor(com.olam.warehouse.presentation.R.color.red))
                                        activity?.toast(it)
                                    }
                                dismissNotification(activity?.applicationContext!!)
//                                hideCustomLoading()
...
//                                hideCustomLoading()
    ...
    private fun updateItemCounts(transList: List<TransactionMaster>, count: String) {
        if (transList.size > 0) {
            binding.tvPOCount.text =
                transList.get(0).masterDTO.purchaseOrderList?.size.toString().plus(" items")
            binding.tvInvoiceCount.text =
                transList.get(0).masterDTO.invoiceDetails?.size.toString().plus(" items")
            binding.tvGrnReprintCount.text =
                transList.get(0).masterDTO.receiptReprintDto?.size.toString().plus(" items")
            binding.tvGrnPriceCount.text =
                transList.get(0).masterDTO.grnPriceDetails?.size.toString().plus(" items")
            binding.tvCharDetailsCount.text =
                transList.get(0).masterDTO.grnCharDetails?.size.toString().plus(" items")
            binding.tvAdvanceCount.text =
                transList.get(0).masterDTO.advanceLineItems?.size.toString()
                    .plus(" items")/*.plus("-"+count)*/
            binding.tvVendorCreditCount.text =
                transList.get(0).masterDTO.advanceDetails?.size.toString().plus(" items")
            binding.tvStocksCount.text =
                transList.get(0).masterDTO.stockDetails?.size.toString().plus(" items")
        }
    }
