...
import com.olam.warehouse.login.ui.transaction.reprint.VegaCameroonReprintTypeFragment
...
class TransactionActivity : HomeBaseActivity() {
    ...
    private fun initUI() {
       if(getCurrentKey().split("_")[0].contains("VEGA")) {
            when {
                getCurrentKey().contains("CM",true) && getCurrentKey().contains("COCO",true) ->{
                    displayFragment(VegaCameroonReprintTypeFragment.newInstance(), false)
                }
