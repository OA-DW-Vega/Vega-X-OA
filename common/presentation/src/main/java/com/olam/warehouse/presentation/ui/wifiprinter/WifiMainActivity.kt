...
import com.olam.warehouse.presentation.ui.wifiprinter.Constants.REQUEST_CODE_ASK_PERMISSIONS
...
class WifiMainActivity : AppCompatActivity(), Observer, PrintCompleteService {
    ...
    private var mllPdf: LinearLayout? = null
    ...
    private var mllFileLocation: LinearLayout? = null
    private var mtvFileLink: TextView? = null
    var image: Image? = null

    companion object {
        const val ALL_PERMISSIONS_FILE = 10
        const val ALL_PERMISSIONS_WIFI = 11
        var permissionList = mutableListOf<String>()
        var permissionListWifi = mutableListOf<String>()
        val REQUIRED_PERMISSIONS_FILE =
    ...
    private fun initExtra() {
        ...
        if(intent.hasExtra(IS_SET_DEFAULT_SIZE))
            PreferenceHelper.save(IS_SET_DEFAULT_SIZE, true)
        else
            PreferenceHelper.save(IS_SET_DEFAULT_SIZE, false)
    }
...
//            pdfFile = File(folder, "file name with extension")
            ...
            mllPdf = findViewById<View>(R.id.llPdf) as LinearLayout
            mllFileLocation = findViewById<View>(R.id.llFileLocation) as LinearLayout
            mtvFileLink = findViewById<View>(R.id.tvFileLink) as TextView
            ...
            if(!hasPermissions(this.applicationContext, *REQUIRED_PERMISSIONS_WIFI)){
                ActivityCompat.requestPermissions(this,
                    REQUIRED_PERMISSIONS_WIFI, ALL_PERMISSIONS_WIFI)
            }else{
                displayLocationSettingsRequest1(this)
            }
            ...
            mllBluetoothPrint?.setOnClickListener {
                ...
                if(list.isNotEmpty() && list[0].isNotEmpty()) {
                    ...
                    toast(getString(R.string.print_key_empty))
                }
            }

            mllPdf?.setOnClickListener {
                if(!hasPermissions(this.applicationContext, *REQUIRED_PERMISSIONS_FILE)){
                    ActivityCompat.requestPermissions(this,
                        REQUIRED_PERMISSIONS_FILE, ALL_PERMISSIONS_FILE)
                }else{
                    getFileName()
                }
            }
    ...
    fun getFileName(){
        MaterialDialog(this).show {
            ...
            var fileName = ""
            input(waitForPositiveButton = false, prefill = "receipt", hint = "file name") { dialog, text ->
                val inputField = dialog.getInputField()
                val isValid = text.isNotEmpty()
                fileName = text.toString()
                inputField.error = if (isValid) null else "Enter file name"
                dialog.setActionButtonEnabled(WhichButton.POSITIVE, isValid)
            }
            positiveButton(text = UIUtils.getSpannedText(getString(R.string.confirm), true)) {
                createpdfNew1(if(fileName.isNotEmpty())fileName else "receipt")
            }
    ...
    private fun createpdfNew(): File? {
        ...
        val directory_path = getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS)?.absolutePath + "/receipt/"
        try {
            ...
            val pdfReader= PdfWriter(filePath)
            val pdfDocument= PdfDocument(pdfReader)
            val document = Document(pdfDocument)

            val documentRect = document.pdfDocument.defaultPageSize
            bitmapPrintKeys.forEachIndexed { index, s ->
                try {
                    addImage(document, s, documentRect, false)
                } catch (e: java.lang.Exception) {
                    e.printStackTrace()
                }
                if((index+1)!=bitmapPrintKeys.size) document.pdfDocument.addNewPage()
            }
            document.close()
            return filePath
        } catch (e: Exception) {
    ...
    fun createpdfNew1(fileName: String): File? {
        val directory_path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)?.absolutePath + "/vegax/"
        //val fileName = getcurdateformatForFileName()
        try {
            val file1 = File(directory_path)
            if (!file1.exists()) {
                file1.mkdirs()
            }
//            val targetPdf = directory_path
//            val filePath = File(targetPdf)
            val file2 = File.createTempFile(fileName, ".pdf", file1)
            val pdfReader= PdfWriter(file2)
            val pdfDocument= PdfDocument(pdfReader)
            val document = Document(pdfDocument)
            ...
            bitmapPrintKeys.forEachIndexed { index, s ->
                try {
                    addImage(document, s, documentRect, true)
                } catch (e: java.lang.Exception) {
                    e.printStackTrace()
                }
            ...
            document.close()
            mllFileLocation?.visible()
            mtvFileLink?.text = File(directory_path).path
            return file2
        } catch (e: Exception) {
            e.printStackTrace()
        }
...
//                        Log.d("Martin", "PendingIntent unable to execute request.")
    ...
    private fun initPrintDialog() {
        ...
        mPrintDialog?.setCancelable(false)
    ...
    override fun onResume() {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                registerReceiver(mWifiScanner, IntentFilter(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION), RECEIVER_NOT_EXPORTED)
            }else{
    ...
    private fun addImage(
        ...
        try {
            ...
            var imageDataFactory= ImageDataFactory.create(decodedString)
            image= Image(imageDataFactory)
        ...
        val actualHeight =image!!.imageHeight
        val actualWidth = image!!.imageWidth
        ...
        //image?.scaleAbsolute((PageSize.A4.width - (image?.imageScaledWidth?:0f)) / 2, (PageSize.A4.height - (image?.imageHeight?:0f)) / 2)
    ...
    private fun setEntityLevelVarient(toolBar: Toolbar) {
        when {
            PreferenceHelper.get(
                com.olam.warehouse.presentation.utils.Constants.CURRENT_ORIGIN_KEY,
                ""
            ).contains(
                com.olam.warehouse.presentation.utils.Constants.OFI
            ) -> {
