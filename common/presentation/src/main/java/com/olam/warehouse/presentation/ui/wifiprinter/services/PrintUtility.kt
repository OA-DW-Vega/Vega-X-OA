...
class PrintUtility : Observer {
    ...
    fun printDocument(pdfFile: File?) {
        try {
            ...
            val isSetDefaultSize = PreferenceHelper.get(Constants.IS_SET_DEFAULT_SIZE, false)
            val attribute = PrintAttributes.Builder()
            attribute.setMediaSize(PrintAttributes.MediaSize.ISO_A4) //8*10 inch
            pdfFile?.let { pdf ->
                mCurrentPrintJob = mPrintManager?.print(
                    jobName,
                    PrintServicesAdapter(mActivity, mFragment, pdf), /*if(isSetDefaultSize)attribute.build() else*/
                    attribute.build()
                )
            }
        }catch (e:Exception){
    ...
    fun completePrintJob() {
        mPrintJobs = mPrintManager?.printJobs?: emptyList()
