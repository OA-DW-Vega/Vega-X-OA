...
//            .addInterceptor(AuthInterceptor(context))
    ...
    single(named(Constants.KEYCLOAK_URL_CLIENT)) {
        ...
        val clientBuilder = OkHttpClient.Builder()
            /*.sslSocketFactory(DoAuthInterceptor
                setupCert(context).socketFactory,
                trustAllCerts[0] as X509TrustManager
            )*/
            .addInterceptor(HttpLoggingInterceptor().apply {
    ...
    single(named(Constants.KEYCLOAK_RESET_PASSWORD_URL)) {
        Retrofit.Builder()
            .client(get(named(Constants.KEYCLOAK_URL_CLIENT)))
            .baseUrl("https://digitalauthdev.olamnet.com/auth/admin/realms/vega-manna-dev/users/2a23aba6-3a03-4718-9003-e8bd15d900e7/")
            .addConverterFactory(GsonConverterFactory.create())
            .build()
    }

    single(named(Constants.AZURE_URL)) {
        Retrofit.Builder()
            .client(get())
    ...
    single(named(Constants.REPORT_POWER_BI_URL)) {
        Retrofit.Builder()
            .client(get(named(Constants.DO_CLIENT)))
