...
abstract class VegaCoCoaOffloadDao : BaseDao<VegaCoCoaReceiving>() {
    ...
    @Query("SELECT * FROM VegaCocoaOffloadingBagMaterial where mtnNumber =:mtnNumber and isSyncStatus = 0")
    abstract fun getBagItems(mtnNumber: String): LiveData<List<VegaCoCoaOffloadingBagMaterial>>
    ...
    @Query("DELETE FROM VegaCoCoaOffloadingBagMaterial where mtnNumber = :bagtype")
