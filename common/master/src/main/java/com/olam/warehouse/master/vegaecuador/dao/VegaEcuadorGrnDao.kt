...
abstract class VegaEcuadorGrnDao {
    ...
    abstract fun getSuppliers(selectedPlantId: String): LiveData<List<VegaVendor>>

    @Query("SELECT * FROM VegaQualityParameter WHERE ( :entryObligatory IS NULL OR entryObligatory = :entryObligatory) AND materialCode =:material")
    ...
    @Query("SELECT * FROM VegaQualityParameter WHERE (  vegaMandatory = :entryObligatory) AND materialCode =:material")

    abstract fun getQualityParamsDB1(
        material: String,
        entryObligatory: String?
    ): LiveData<List<VegaQualityParameter>>
