...
abstract class VegaNicaraguaInvoiceDao {
    ...
    @Query("SELECT * FROM VegaReceiving where erdat BETWEEN :startTime AND  :endTime and deleteFlag = 0")
    ...
    @Query("UPDATE VegaNicaraguaInvoiceDetails  SET postdate= :postdate where  tempId =:tmpWbId")
    abstract fun updatePostingDate(tmpWbId: String, postdate: String)

    @Query("SELECT * FROM VegaReceiving")
    abstract fun getReceivingWithAdvanceLineItem(): LiveData<List<VegaNicGrnInvoiceWithAdvanceItems>>

    @Query("SELECT * FROM VegaNicaraguaInvoiceDetails where isReceiptData = 0")
    abstract fun getInvoiceWithAdvanOfflineData(): LiveData<List<VegaNicPtbfInvoiceWithAdvanceItems>>
}
