    ...
    suspend fun saveTrnasMasterData(data: List<TransactionMaster>) {
        data.forEach {
            if (it.key?.split("_")?.get(0)?.contains("DO")!!) {
                if(!it.masterDTO.sapMatProductDTO.isNullOrEmpty())
                    doReceivingDao.saveSapMaterialByProductList(it.masterDTO.sapMatProductDTO!!)
                ...
                it.masterDTO.doDispatchDetails?.forEach { dispatchDetail ->
                    //dispatchDetail.originId=dispatchDetail.originId?:""
                    //dispatchDetail.productId=dispatchDetail.productId?:""
                ...
                it.masterDTO.doTransList?.let {
                    if(it.size>0) {
                        val txnMasterPojo = TransactionListPojo(it ?: emptyList())
