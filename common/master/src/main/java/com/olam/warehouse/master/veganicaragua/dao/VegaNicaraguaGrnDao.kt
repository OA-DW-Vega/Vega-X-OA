...
abstract class VegaNicaraguaGrnDao {
    ...
    @Query("UPDATE VegaReceiving SET deleteFlag = 1 WHERE tmpWbId = :tmpWbId")
    abstract fun updateDeleteFlag(tmpWbId: String)
    ...
    @Query("UPDATE VegaReceiving  SET postdate= :postdate where  tmpWbId =:tmpWbId")
    abstract fun updatePostingDate(tmpWbId: String, postdate: String)

    @Query("SELECT * FROM VegaReceiving")
    abstract fun getReceivingWithLineItemDirect(): List<VegaReceiving>

    @Query("SELECT * FROM VegaNicaraguaInvoiceDetails WHERE isReceiptData = 0")
    abstract fun getInvoiceOfflineData1(): LiveData<List<VegaNicaraguaInvoiceDetails>>

    @Query("SELECT * FROM VegaNicaraguaInvoiceDetails WHERE isReceiptData = 0")
    abstract fun getInvoiceOfflineDataDirect(): List<VegaNicaraguaInvoiceDetails>
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    abstract fun saveFarmerTransData(items: TrackTraceFarmerTransDetails)

    @Query("SELECT * FROM TrackTraceFarmerTransDetails where tmpWbId = :tmpWbId")
    abstract fun getFarmerTransData(tmpWbId: String): TrackTraceFarmerTransDetails
