...
import com.olam.warehouse.master.common.data.work.SessionOutTokenFetchWorker
...
fun getSessionRequestWorker(password: Data): OneTimeWorkRequest {
    return OneTimeWorkRequest.Builder(SessionOutTokenFetchWorker::class.java).addTag(password.toString())
        .setConstraints(constraint).setInputData(password).build()
}
