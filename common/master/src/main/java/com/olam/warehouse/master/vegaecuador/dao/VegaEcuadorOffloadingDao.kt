...
abstract class VegaEcuadorOffloadingDao : BaseDao<VegaReceiving>() {
    ...
    @Query("SELECT * FROM VegaCameroonOffloadingBagMaterial where wbId =:plantId")
...
/*
    ...
    @Query("SELECT * FROM VegaReceiving WHERE weighBridgeId = :wbid")
    abstract fun getSavedReceivingBagItems(wbid: String): LiveData<VegaCameroonOffloadingWithBagItem>
    ...
    @Query("SELECT * FROM VegaMaterial WHERE materialCode = :name OR eudrEquivalentMap= :name")
    abstract fun getProductByName(name:String): LiveData<List<VegaMaterial>>
    ...
    @Query("SELECT * FROM VegaFeatureMaster")
    abstract fun getFeatureMaster(): LiveData<List<VegaFeatureMaster>>
