...
import com.olam.warehouse.vegax.pileindiacoffee.databinding.FragmentIndiaCoffeePileManagementFilterBinding
import com.olam.warehouse.vegax.pileindiacoffee.databinding.ItemVegaIndiaCoffeePileFilterBinding
import com.olam.warehouse.vegax.pileindiacoffee.ui.data.domain.model.CoffeeFilterList
import com.olam.warehouse.vegax.pileindiacoffee.ui.utils.*
import java.util.*

class VegaIndiaCoffeePileFilterFragment : BaseFragment() {

    private lateinit var binding: FragmentIndiaCoffeePileManagementFilterBinding
    override val layoutResourceId = R.layout.fragment_india_coffee_pile_management_filter
    private var callBack: CallBack? = null
    private var fullFilter = ArrayList<String>()
    private var listFliterWhLoc = ArrayList<CoffeeFilterList>()
    private var startRange: String = ""
    private var endRange: String = ""
    private var aboveThirty: Boolean = false

    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaIndiaCoffeePileFilterFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentIndiaCoffeePileManagementFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        for (item in whLocations.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFliterWhLoc.add(CoffeeFilterList(item, true))
                else listFliterWhLoc.add(CoffeeFilterList(item))
            }
        }
    ...
    private fun setUpAdapter(listFliterWhLoc: ArrayList<CoffeeFilterList>) {
        binding.rvWhLocation.setUpAdapter(
            listFliterWhLoc,
            R.layout.item_vega_india_coffee_pile_filter_,
            ItemVegaIndiaCoffeePileFilterBinding::inflate,
            { it, pos, bindItem ->
