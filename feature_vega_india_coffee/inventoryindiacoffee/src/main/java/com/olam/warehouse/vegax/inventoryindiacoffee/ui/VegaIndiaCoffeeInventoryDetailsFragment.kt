...
import com.olam.warehouse.vegax.inventoryindiacoffee.data.domain.model.*
import com.olam.warehouse.vegax.inventoryindiacoffee.databinding.FragmentVegaIndiaCoffeeInventoryListBinding
import com.olam.warehouse.vegax.inventoryindiacoffee.databinding.ItemVegaIndiaCoffeeInventoryDetailsBinding
import com.olam.warehouse.vegax.inventoryindiacoffee.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaIndiaCoffeeInventoryDetailsFragment : BaseFragment() {
    private lateinit var binding: FragmentVegaIndiaCoffeeInventoryListBinding
    private val vm: VegaIndiaCoffeeInventoryViewModel by viewModel()
    override val layoutResourceId = R.layout.fragment_vega_india_coffee_inventory_list
    ...
    companion object {
        fun newInstance(bundle: Bundle) = VegaIndiaCoffeeInventoryDetailsFragment().putArgs {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("inventorysesame/ui/details/VegaIndiaCoffeeInventoryDetailsFragment")
    ...
    private fun updateLotInfo(response: Resource<GenericReqAndResp<List<VegaIndiaCoffeeInventoryStocks>>>) {
        response.let {
            when (it.status) {
    ...
    private fun prepareInventoryList(items: List<VegaIndiaCoffeeInventoryStocks>): ArrayList<InventoryLots> {
        ...
        if (getCurrentKey().split("_")[2].contains(KEY_SESAME)) {
            ...
            items.forEach {
                val stock = InventoryLots(
                        ...
                        materialCode = it.materialCode.toString(),
                        materialName = it.materialName.toString(),
                        kor = it.materialQuality.qualityParams.ZNG_CASHEW_KOR,
                        stockQty = it.weight.toString(),
                        warehouseLocation = WarehouseLocation(warehouse = Warehouse(Plant(CountryDetail()))),
                        inventoryQC = qcList,
                        unitOfMeasure = it.unitOfMeasure
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        setHasOptionsMenu(true)
        binding = FragmentVegaIndiaCoffeeInventoryListBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun setUpAdapter(data: List<InventoryLots>) {
        ...
        var qcList = listOf<InventoryQC>()
        if (lotList.size > 0) binding.tvNoData.gone() else binding.tvNoData.visible()
        binding.rvInventory.setUpAdapter(
            lotList,
            R.layout.item_vega_india_coffee_inventory_details,
            ItemVegaIndiaCoffeeInventoryDetailsBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvLotName.text = it.lotId
//            if (getCurrentKey().split("_")[2].contains(KEY_SESAME)) {
//                tv_weight_data.text = it.stockQty.plus(" ").plus("MT")
//                tv_impurity_value.text = it.admixture
//            } else if (getCurrentKey().split("_")[2].contains(KEY_CASHEW)) {
                qcList = it.inventoryQC
                it.inventoryQC.forEach {
                    if ((it.qcName.contains("ZQM_MOISTURE")) && (!(it.value.equals("0.00  %")))) {
                        bindItem.tvImpurityValue.text = it.value
                    }

                    if ((it.qcName.contains("ZQM_GRADE")) && (!(it.value.equals("0.00  %")))) {
                        bindItem.tvMouldValue.text = it.value
                    }

                }
                bindItem.tvWeightData.text = it.stockQty.plus(" ").plus("KG")

//                tv_impurity.text = "KOR"
//            }
                if (colorCode != 0)
    ...
    fun applyFilter(bundle: Bundle) {
        ...
        lotList.forEach { lot ->
            ...
            if (getCurrentKey().split("_")[2].contains(KEY_SESAME)) {
                ...
                admixtureList.forEach { admixture ->
                    if (lot.kor.isNullOrEmpty()) return@forEach
                    val data = admixture.replace("%", "").replace("<A", "").replace("A", "")
                    when {
                        data.contains("<=") -> {
                            val spdata = data.split("<=")
                            when (spdata[0].trim() == "") {
                                ...
                                false -> {
                                    if (lot.kor.replace(
                                                    ...
                                                    .toDouble() > spdata[0].toDouble() && lot.kor.replace(
                                                    "%",
                                                    ""
                                            ).trim().toDouble() <= spdata[1].toDouble()
                                    ) filterData2.add(
            ...
            lotList.forEach { lotItem ->
                ...
                if ((materialList.size == 0 || (filterData1.map { it.lotId }
                                .contains(lot) && filterData1.map { it.materialCode }
                                .contains(lotItem.materialCode))) && (admixtureList.size == 0 || filterData2.map { it.lotId }
                                .contains(
                                        lot
                                ))
