...
import kotlinx.android.synthetic.main.fragment_vega_india_coffee_inventory_list.*
import kotlinx.android.synthetic.main.item_vega_india_coffee_inventory_details.view.*
...
class VegaIndiaCoffeeInventoryDetailsFragment : BaseFragment() {
    ...
    private fun setUpAdapter(data: List<InventoryLots>) {
        ...
        binding.rvInventory.setUp(lotList, R.layout.item_vega_india_coffee_inventory_details, { it, pos ->
...
//            } else if (getCurrentKey().split("_")[2].contains(KEY_CASHEW)) {
            ...
            it.inventoryQC.forEach{
                if((it.qcName.contains("ZQM_MOISTURE")) && (!(it.value.equals("0.00  %")))) {
                    tv_impurity_value.text = it.value
                }

                if((it.qcName.contains("ZQM_GRADE")) && (!(it.value.equals("0.00  %")))) {
                    tv_mould_value.text = it.value
                }
