...
class VegaIndiaCoffeeFgrnViewModel(
    ...
    private var getLotqualitySource: LiveData<Resource<GenericReqAndResp<List<VegaQualityPreParameter>>>> =
        MutableLiveData()
    private val _getLotQuality =
        MediatorLiveData<Resource<GenericReqAndResp<List<VegaQualityPreParameter>>>>()
    val getLotQuality: LiveData<Resource<GenericReqAndResp<List<VegaQualityPreParameter>>>> get() = _getLotQuality
    ...
    private var fgrnSequenceSource: LiveData<Resource<GenericReqAndResp<NicaraguaUpdateFgrnSequencePost>>> =
        MutableLiveData()
    private val _updateFgrnSequence =
        MediatorLiveData<Resource<GenericReqAndResp<NicaraguaUpdateFgrnSequencePost>>>()
    val updateFgrnSequence: LiveData<Resource<GenericReqAndResp<NicaraguaUpdateFgrnSequencePost>>> get() = _updateFgrnSequence
    ...
    fun getMiscellaneousItems(role: String) = viewModelScope.launch(dispatchers.main) {
    ...
    fun getLotQualityData(batchNo: String, materialId: String) =
        viewModelScope.launch(dispatchers.main) {
            _getLotQuality.removeSource(getLotqualitySource) // We make sure there is only one source of livedata (allowing us properly refresh)
            withContext(dispatchers.io) {
                getLotqualitySource = useCase.getPreSamplingQualityList(batchNo, materialId)
            }
            _getLotQuality.addSource(getLotqualitySource) {
                _getLotQuality.value = it
            }
        }
    ...
    fun updateFgrnSequence(paramPost: NicaraguaUpdateFgrnSequencePost) =
        viewModelScope.launch(dispatchers.main) {
    ...
    fun generateFGRNTicketNumber(): String {
        var tallySequnceId = ""
