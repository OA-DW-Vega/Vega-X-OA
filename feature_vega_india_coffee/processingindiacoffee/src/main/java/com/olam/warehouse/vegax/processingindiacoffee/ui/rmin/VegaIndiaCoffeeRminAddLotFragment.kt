...
import com.olam.warehouse.vegax.processingindiacoffee.databinding.FragmentAddLotRminIndiaCoffeeBinding
import com.olam.warehouse.vegax.processingindiacoffee.databinding.ItemLotInfoBinding
import com.olam.warehouse.vegax.processingindiacoffee.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel

class VegaIndiaCoffeeRminAddLotFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_add_lot_rmin_india_coffee
    private lateinit var binding: FragmentAddLotRminIndiaCoffeeBinding
    ...
    companion object {
        fun newInstance(fromSummary: Boolean, model: VegaCocoaRminProcessing, isPoSelection: Boolean) =
            VegaIndiaCoffeeRminAddLotFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentAddLotRminIndiaCoffeeBinding.inflate(layoutInflater)
        initExtra()
        initUI()
        return binding.root
    ...
    private fun initExtra() {
        ...
        vm.vegaIndiaCoffeeRminWithItems.observe(viewLifecycleOwner, Observer {
            if (it != null) {
    ...
    private fun initUI() {
        ...
        binding.btnAddLots.setOnClickListener { navigateToLists() }
        binding.btnProceed.setOnClickListener {
            if (binding.tvPoQuantity.text.toString().isNullOrEmpty() && isPoSelection == false) {
                Toast.makeText(
                    activity,
                ...
                if (addedLotList.size > 0) {
                    if (validateLotWeight()) {
                        ...
                        callBack?.replaceFragment(
                            if (fromSummary == true) SUMMARY else SHIFT,
                            isMultipleLot,
                            model!!
                        )
                    } else Toast.makeText(
    ...
    private fun setUpAdapter(list: MutableList<VegaCocoaRminLots>) {
        binding.rvList.setUpAdapter(
            list,
            R.layout.item_lot_info,
            ItemLotInfoBinding::inflate,
            { item, pos, bindItem ->
                ...
                bindItem.tvWeightValue.text = weight.plus(" ").plus(item.unitOfMeasure)
                if (isEditableLot) {
                    bindItem.clSelectAllWeight.visibility = View.VISIBLE
                } else {
                    bindItem.etWeight.isEnabled = false
                    item.editedWeight = item.weight
                    bindItem.clSelectAllWeight.visibility = View.GONE
                }
                ...
                bindItem.etWeight.onChange {
                    if (it.isNotEmpty()) {
                        ...
                        bindItem.etWeight.error =
                            context.getString(R.string.empty_weight)/*getString(R.string.less_weight_error)*/
                    }
                }
    ...
    private fun navigateToLists() {
        model?.lotList?.clear()
        lotLis?.let { model?.lotList?.addAll(it) }
        callBack?.replaceFragment(Lot_Lists, isMultipleLot, model!!)
                ...
                ConfigItems.DRYING_PLANT.item -> {
                    ...
                    if (getCurrentKey().split("_")[1].contains("NI")) {
                        if (plantList.contains(plantId)) {
                            binding.btnAddLots.visible()
                            binding.btnAddLot.visibility = View.GONE
