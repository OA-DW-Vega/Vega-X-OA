...
class VegaIndiaCoffeeRminLotListFragment : BaseFragment() {
    ...
    companion object {
        fun newInstance(model: VegaCocoaRminProcessing, isMultipleAdd: Boolean) =
            VegaIndiaCoffeeRminLotListFragment().putArgs {
    ...
    private fun initUI() {
        ...
        { val datList = arrayListOf<String>()
            model?.materialCodes?.distinct()?.toList()?.let { datList.addAll(it) }
            vm.getStockList(datList)
        }
                ...
                Resource.Status.SUCCESS -> {
                    ...
                    when (it.data?.success) {
                        true -> {
                            ...
                            if (getCurrentKey().split("_")[1].contains("NI")) {
                               dispatchLotsList = dispatchLotsList.distinct().toMutableList()
                            }
    ...
    private fun setupAdapter(data1: MutableList<VegaCocoaRminLots>) {
        ...
        if(getCurrentKey().contains("VEGA_NI")){
            if(model?.materialName?.contains("toll",true) == true){
               data= data1.filter { it.vendor == model?.vendorCode } as MutableList<VegaCocoaRminLots>
            }
        }
                ...
                bindItem.llLotItem.setOnClickListener { view ->
                   ...
                   bindItem.ivSelect.isChecked = it.isAdded!!
                    ...
                    if (!isMultipleAdd) {
                        ...
                        val vegaLot= data[pos]
                        vegaLot.isAdded= it.isAdded
                        binding.rvLots.adapter?.notifyItemChanged(pos,vegaLot)
                    }
                    updateWeight()
                }
    ...
    private fun updateWareHouseSpinner() {
        val stageAdapter =
            ArrayAdapter(requireContext(), R.layout.item_vega_india_coffee_processing_rmin_grade, wareHouseList)
