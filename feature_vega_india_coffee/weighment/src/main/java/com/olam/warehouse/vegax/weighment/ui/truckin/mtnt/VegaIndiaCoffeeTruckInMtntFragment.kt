...
import com.olam.warehouse.vegax.weighment.databinding.FragmentVegaIndiaCoffeeMtntTruckinBinding
import com.olam.warehouse.vegax.weighment.databinding.ItemVegaIndiaCoffeeMaterialLayoutBinding
...
class VegaIndiaCoffeeTruckInMtntFragment : BaseFragment(), VegaSingleSelectCommonListener {
    ...
    private lateinit var binding: FragmentVegaIndiaCoffeeMtntTruckinBinding
    override val layoutResourceId = R.layout.fragment_vega_india_coffee_mtnt_truckin
    ...
    companion object {
        fun newInstance(mtntData: VegaMtnt) = VegaIndiaCoffeeTruckInMtntFragment().putArgs {
            putParcelable(MTNT_DATA, mtntData)
        }
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentVegaIndiaCoffeeMtntTruckinBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        if (mtntData.transportVendorCode?.isNotEmpty()!!)
            binding.tvTransVendor.text =
                mtntData.transportVendorCode.plus("-").plus(mtntData.transportVendorName)
...
//        binding.tvDestinationPlantValue.setOnClickListener { showReceivingPlantDialog() }
        ...
        vm.product.observe(viewLifecycleOwner, Observer {
            ...
            materialLists = products as ArrayList<String>
           //val productAdapter = ArrayAdapter(requireContext(), R.layout.item_drop_down, R.id.item, products)
    ...
    private fun showReceivingLocationDialog() {
        ...
        MaterialDialog(requireContext()).show {
            ...
            listItemsSingleChoice(items = location) { _, index, text ->
                ...
                /*mtntData.recStorageLocationCode = purchaseOrder[index].storageLocationCode
                ...
                if (text.contains("-")) {
                    val storage = text.split(" - ")
                    getPurchaseOrderByStorageLocation(storage[0])
                } else
                    getPurchaseOrderByStorageLocation(text.toString())
            }
            positiveButton(text = UIUtils.getSpannedText(getString(R.string.ok), true))
        }
    }
  ...
  /*  private fun getPlantName(plantId: String): String {
   ...
   /* private fun showStoDialog() {
        val location = STONumbers
        MaterialDialog(requireContext()).show {
            title(R.string.receiving_location_popup)
            listItemsSingleChoice(items = location) { _, index, text ->
                getWeight(text.toString())
                binding.tvStoValue.text = text
                val cureentPur = purchaseOrder.filter { it.purchaseDocNum.equals(text) }
    ...
    private fun setUpMaterialAdapter() {
        binding.rvMaterialList.setUpAdapter(
            materialModelList,
            R.layout.item_vega_india_coffee_material_layout,
            ItemVegaIndiaCoffeeMaterialLayoutBinding::inflate,
            { item, pos, bindItem ->
...
//        mtntData.wsGate = WB01
    ...
    private fun showSingleSelectDialog(title: String, currentFlag: String) {
        ...
        when (currentFlag) {
            ...
            VENDOR -> {
                list.addAll(vendorList)
            }
            PRODUCT -> {
                list.addAll(materialLists)
            }
