...
import com.olam.warehouse.vegax.dispatchindiacoffee.databinding.ItemVegaDispatchSummaryDetailsListBinding
...
//        if (dispatchLotsList.size > 1) binding.llBatchNo.visible() else binding.llBatchNo.gone()
    ...
    private fun setUpAdapter(selectedStocks: List<VegaDispatchLots>) {
        ...
        binding.rvDispatchLots.setUpAdapter(
            dispatchLots.asReversed(),
            R.layout.item_vega_dispatch_summary_details_list,
            ItemVegaDispatchSummaryDetailsListBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvLotNo.text = it.batchNumber
                bindItem.tvWeight.text = it.weight.plus(" ").plus(it.unitOfMeasure)
                bindItem.tvDispatchWeight.text =
                    if (!it.editedWeight.equals("")) it.editedWeight.plus(" ")
                        .plus(it.unitOfMeasure) else "0.0".plus(it.unitOfMeasure)
                bindItem.tvLocation.text = it.storageLocationCode
                bindItem.tvKor.text = it.kor
                bindItem.tvOrigin.text = it.region
            }, {
                val item = this
            })
    }
