package com.olam.warehouse.vegax.dispatchindiacoffee.ui.stock
...
import com.olam.warehouse.vegax.dispatchindiacoffee.databinding.ItemVegaLotsBinding
...
//        val v = LayoutInflater.from(parent.context).inflate(R.layout.item_vega_lots, parent, false)
        val v = ItemVegaLotsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return ViewHolder(v)
    }
    ...
    inner class ViewHolder(itemView: ItemVegaLotsBinding) : RecyclerView.ViewHolder(itemView.root) {
        val binding = itemView
        fun bindItems(stock: VegaDispatchLots?, position: Int) {
            stock?.let {
                //itemView.isEnabled = it.isAdded!!
                binding.tvLotNo.text = it.batchNumber
                binding.tvWeight.text = it.weight.plus(" ").plus(it.unitOfMeasure)
                binding.tvKor.text = it.kor
//                binding.tvOrigin.text = it.region
                binding.tvOrigin.text = it.storageLocationCode
                if (it.isProgress!!) binding.llProgressBar.visible() else binding.llProgressBar.gone()
                binding.cbLotSelect.isChecked = it.isChecked!!
                binding.cbLotSelect.setOnCheckedChangeListener { buttonView, isChecked ->
