...
import com.olam.warehouse.vegax.dispatchindiacoffee.databinding.ActivityVagaDispatchLotsBinding
...
class VegaDispatchLotsActivity : HomeBaseActivity() {
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        try {
            ...
            searchView.setBackgroundColor(
                com.olam.warehouse.vegax.dispatchindiacoffee.utils.getColor(
    ...
    private fun initUI() {
        ...
        addedLotsList = intent.getStringArrayListExtra(Constants.SELECTED_STOCKS_ID) as ArrayList<String>
    ...
    private fun updateUI(response: Resource<List<VegaDispatchLots>>) {
        ...
        when (response.status) {
            ...
            Resource.Status.ERROR -> {
                hideLoading()
                UIUtils.showErrorDialog(this, response.error.toString())
            }
            else -> hideLoading()
        }
    }
    ...
    private fun updateParams(data: Resource<GenericReqAndResp<List<VegaDispatchLotQuality>>>) {
        data.let {
            when (it.status) {
                ...
                Resource.Status.ERROR -> {
                    //hideLoading()
                    UIUtils.showErrorDialog(this, "Quality Fetching Failed")
                    // context?.toast()
                }
                else -> {
                }
    ...
    private fun updateStockWeight() {
        ...
        val weightSum = selectedLotsList.sumByDouble { it.weight?.toDouble() ?: 0.0 }
