...
import kotlinx.android.synthetic.main.item_vega_weighbridge_qualty_approve_india_coffee.view.*
        ...
        data.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    hideLoading()
                    val weighBridge =
                        it.data?.data?.filter { it.qcStatus == "D" && it.weighBridgeType == "PROCURE" }
...
//            tvTruckNo.text = it.vehicleNumber
                    ...
                    tvWeight.text = it.netWeight?.replace(" ", "").plus(it.unitsOfMeasure)
...
//            val times = it.erdat?.split('(', ')')
             ...
             mListener?.onFragmentInteraction(fragment)*/
                    ...
                    val wbDetails = weighBridgeList1.filter { it.weighBridgeId == this.weighBridgeId }
