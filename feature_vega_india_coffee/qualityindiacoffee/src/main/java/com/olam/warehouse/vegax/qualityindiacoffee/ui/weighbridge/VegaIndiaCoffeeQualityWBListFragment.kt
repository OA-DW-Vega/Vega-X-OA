package com.olam.warehouse.vegax.qualityindiacoffee.ui.weighbridge
...
import com.olam.warehouse.vegax.qualityindiacoffee.databinding.FragmentVegaIndiaCoffeeQualityWeighBridgeListBinding
...
class VegaIndiaCoffeeQualityWBListFragment : BaseFragment() {

    private var mAdapter = VegaIndiaCoffeeWeighBridgeListAdapter { moveBagdetail(it) }
    private val vm: VegaIndiaCoffeeQualityViewModel by viewModel()
    private lateinit var mListener: OnWeighBridgeListener
    private lateinit var binding: FragmentVegaIndiaCoffeeQualityWeighBridgeListBinding
    override val layoutResourceId = R.layout.fragment_vega_india_coffee_quality_weigh_bridge_list
    ...
    companion object {
        fun newInstance() = VegaIndiaCoffeeQualityWBListFragment().putArgs {
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaIndiaCoffeeQualityWeighBridgeListBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("quality/ui/weighbridge/VegaQualityWBListFragment - $weightmentType")
            .title("Quality").with(tracker)
    }

    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        super.onCreateOptionsMenu(menu, inflater)
        activity?.menuInflater?.inflate(R.menu.search_vega_quality_menu, menu)
                ...
                when (weightmentType) {
                    SUPPLIER -> {
                        weighTypedata = data?.filter { wb -> wb.direction == DIRECTIONIN }
                            ...
                            ?.filter { value -> value.qcStatus.toString() == "R" }
                    }
                    MTNR -> {
                        weighTypedata = data?.filter { wb -> wb.direction == DIRECTIONIN }
