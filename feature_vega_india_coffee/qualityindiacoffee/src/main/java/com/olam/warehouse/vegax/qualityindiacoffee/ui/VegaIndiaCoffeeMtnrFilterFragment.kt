...
import com.olam.warehouse.vegax.qualityindiacoffee.data.domain.model.IndiaCoffeeFilterList
import com.olam.warehouse.vegax.qualityindiacoffee.databinding.FragmentVegaIndiaCoffeeMtnrFilterBinding
import com.olam.warehouse.vegax.qualityindiacoffee.databinding.ItemVegaIndiaCoffeeMtnrInventoryFilterBinding
import com.olam.warehouse.vegax.qualityindiacoffee.utils.*
import java.util.*

class VegaIndiaCoffeeMtnrFilterFragment : BaseFragment() {

    private lateinit var binding: FragmentVegaIndiaCoffeeMtnrFilterBinding
    override val layoutResourceId = R.layout.fragment_vega_india_coffee_mtnr_filter
    private var callBack: CallBack? = null
    private var fullFilter = ArrayList<String>()
    private var listFliterWhLoc = ArrayList<IndiaCoffeeFilterList>()
    private var startRange: String = ""
    private var endRange: String = ""
    private var aboveThirty: Boolean = false

    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaIndiaCoffeeMtnrFilterFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentVegaIndiaCoffeeMtnrFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        for (item in whLocations.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFliterWhLoc.add(IndiaCoffeeFilterList(item, true))
                else listFliterWhLoc.add(IndiaCoffeeFilterList(item))
            }
        }
    ...
    private fun setUpAdapter(listFliterWhLoc: ArrayList<IndiaCoffeeFilterList>) {
        binding.rvWhLocation.setUpAdapter(
            listFliterWhLoc,
            R.layout.item_vega_india_coffee_mtnr_inventory_filter,
            ItemVegaIndiaCoffeeMtnrInventoryFilterBinding::inflate,
            { it, pos, bindItem ->
