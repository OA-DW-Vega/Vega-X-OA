...
import kotlinx.android.synthetic.main.item_vega_india_coffee_quality_summary.view.*
...
class VegaIndiaCoffeeQualitySummaryFragment : BaseFragment() {
    ...
    private fun postQuality(offloadingParamList: MutableList<VegaQualityParameter>) {
        ...
        if (AppUtils.isOnline()) {
            ...
            offloadingData.qualityDetails = offloadingParams as List<VegaQuality>
    ...
    private fun setUpAdapter(data: MutableList<VegaQualityParameter>) {
        ...
        data.let { list = it }
        binding.rvBagDetailSummary.setUp(list, R.layout.item_vega_india_coffee_quality_summary, { it, pos ->
            if(pos%2==0){
                linearRow.setBackgroundColor(getColor(com.olam.warehouse.presentation.R.color.white))
            }else {
