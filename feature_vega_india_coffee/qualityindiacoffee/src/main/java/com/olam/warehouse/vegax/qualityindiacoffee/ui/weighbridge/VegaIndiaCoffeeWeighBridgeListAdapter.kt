...
import com.olam.warehouse.vegax.qualityindiacoffee.databinding.ItemVegaIndiaCoffeeQualityWeighBridgeDetailsBinding
import com.olam.warehouse.vegax.qualityindiacoffee.utils.PROCURE
import com.olam.warehouse.vegax.qualityindiacoffee.utils.SUPPLIER
import com.olam.warehouse.vegax.qualityindiacoffee.utils.WAREHOUSE


class VegaIndiaCoffeeWeighBridgeListAdapter(private val onClick: (VegaQualityWBDetails?) -> Unit) :
    RecyclerView.Adapter<VegaIndiaCoffeeWeighBridgeListAdapter.WeighBridgeViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WeighBridgeViewHolder {
        /*val v =
            LayoutInflater.from(parent.context)
                .inflate(R.layout.item_vega_india_coffee_quality_weigh_bridge_details, parent, false)*/
        val v = ItemVegaIndiaCoffeeQualityWeighBridgeDetailsBinding.inflate(
            LayoutInflater.from(parent.context),
            parent,
            false
        )
...
//        })
    ...
    inner class WeighBridgeViewHolder(itemView: ItemVegaIndiaCoffeeQualityWeighBridgeDetailsBinding) :
        ...
        fun bindItems(weighbridge: VegaQualityWBDetails?) {
            weighbridge?.let {
                ...
                if (it.weighBridgeType == PROCURE) {
                    ...
                    binding.tvSupplierName.text = it.supplierCode.plus("-").plus(it.supplierName)
                } else {
