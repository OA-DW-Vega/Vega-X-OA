...
import com.olam.warehouse.vegax.qualityindiacoffee.databinding.FragmentVegaIndiaCoffeeMtnrQualityAssignLotBinding
import com.olam.warehouse.vegax.qualityindiacoffee.databinding.ItemVegaIndiaCoffeeMtnrLotBinding
import com.olam.warehouse.vegax.qualityindiacoffee.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaIndiaCoffeeMtnrQualityAssignLotFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_vega_india_coffee_mtnr_quality_assign_lot
    private lateinit var binding: FragmentVegaIndiaCoffeeMtnrQualityAssignLotBinding
    ...
    companion object {
        fun newInstance(
            ...
            VegaIndiaCoffeeMtnrQualityAssignLotFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaIndiaCoffeeMtnrQualityAssignLotBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("processingcameroon/ui/fgrn/VegaCoffeeFgrnAssignLotFragment")
            .title("Processing Cocoa")
            .with(tracker)
    }

    private fun initUI() {
        wbDetails = arguments?.getParcelable(FRAG_ITEM) ?: VegaQualityWBDetails()
    ...
    private fun updateConfigItems(configItems: List<VegaConfigDetails>) {
        ...
        defaultStorageLoc.forEach {
            if ((currentMaterial.contains(it.materialCode) && !it.materialCode.isNullOrEmpty() && !isExist && isMaterial) ||
                (it.materialCode.isNullOrEmpty() && !isExist && !isMaterial)) {
...
//                binding.tvBatchDefault.gone()
    ...
    private fun updateStorageLoc(customLocationList: MutableList<VegaCustomStLocation>) {
        customLocationList.let {
            ...
            val stageAdapter =
                ArrayAdapter(requireContext(), R.layout.item_vega_india_coffee_mtnr_grade, storageLoc)
            ...
            binding.spDefaultStorageLoc.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                override fun onNothingSelected(p0: AdapterView<*>?) {/*nothing being selected*/}
                    ...
                    when (it.data?.success) {
                        true -> {
                            ...
                            if (isThirdPartyMaterialDetail) {
                                stockList.addAll(dataValue.filter { it.vendor == wbDetails.supplierCode })
                            } else {
                            ...
                            stockList.forEachIndexed { index, s ->
                                ...
                                when (s.unitOfMeasure) {
                                    ...
                                    "MT" ->
                                        stockList[index].weight = convertMtToKg(stockList[index].weight.toString())
                                }
                            }
    ...
    private fun updateAdapter(stockList: MutableList<VegaCoffeeRminLots>) {
        ...
        binding.rvInventoryLot.setUpAdapter(
            stockList,
            R.layout.item_vega_india_coffee_mtnr_lot,
            ItemVegaIndiaCoffeeMtnrLotBinding::inflate,
            { it, pos, bindItem ->
                bindItem.ivDelete.gone()
                bindItem.tvLotNoValue.text = it.batchNumber
//            tvWeightValue.text = it.weight.plus(" ").plus(it.unitOfMeasure)
                bindItem.tvWeightValue.text = it.weight.plus(" ").plus("KG")
