...
import com.olam.warehouse.vegax.grnindiacoffee.databinding.CutsomDialogBcApproveIndiaCoffeLayoutBinding
import com.olam.warehouse.vegax.grnindiacoffee.databinding.FragmentVegaIndiaCoffeeGrnDetailsBinding
import com.olam.warehouse.vegax.grnindiacoffee.databinding.VegaIndiaCoffeeGrnCutsomDialogLayoutBinding
    ...
    private fun showSingleSelectDialog(currentFlag: String) {
        ...
        when (currentFlag) {
            PTBF -> {
                ...
                val purchaseOrderList1 =
                    purchaseOrderList.filter { it.bsart == PTBF }
        ...
        if(list.size>0){
            MaterialDialog(requireContext()).show {
                ...
                listItemsSingleChoice(items = list) { _, index, text -> var selectedPo= if(text.contains("-")) {
                        text.split("-")[0]
                    }else text

                    binding.tvSelectPO.text = text
                    val item = filterpurchaseOrderList.singleOrNull { it.poId == selectedPo }
...
//        val mDialogView = LayoutInflater.from(activity?.applicationContext).inflate(R.layout.vega_india_coffee_grn_cutsom_dialog_layout, null)
        val mDialogView =
            VegaIndiaCoffeeGrnCutsomDialogLayoutBinding.inflate(LayoutInflater.from(activity?.applicationContext))
        ...
        mDialogView.tvConfirmGrn.setOnClickListener {
            mAlertDialog?.dismiss()
            var selectedPo = ""
            selectedPo = if (binding.tvSelectPO.text.toString().contains("-")) {
                binding.tvSelectPO.text.toString().split("-")[0]
            }else binding.tvSelectPO.text.toString()

            wbDetails.storageLocationCode = binding.tvStorageLocation.text.toString()
            wbDetails.purchaseDocNum = selectedPo
            ...
            var filteredPOList = purchaseOrderList.filter {
...
//        val mDialogView = LayoutInflater.from(activity?.applicationContext).inflate(R.layout.cutsom_dialog_bc_approve_india_coffe_layout, null)
        val mDialogView =
            CutsomDialogBcApproveIndiaCoffeLayoutBinding.inflate(LayoutInflater.from(activity?.applicationContext))
        val mBuilder = AlertDialog.Builder(activity)
            .setView(mDialogView.root)
        mAlertDialog = mBuilder?.show()
        mAlertDialog?.setCancelable(false)
