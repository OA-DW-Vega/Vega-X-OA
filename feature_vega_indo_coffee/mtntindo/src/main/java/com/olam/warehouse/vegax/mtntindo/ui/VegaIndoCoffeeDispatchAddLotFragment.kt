...
import com.olam.warehouse.vegax.mtntindo.databinding.ItemIndoCoffeeDispatchLayoutLotSummaryBinding
...
class VegaIndoCoffeeDispatchAddLotFragment : BaseFragment(),
    ...
    private fun setUpLotsAdapter(list: MutableList<VegaEcuadorDispatchLots>) {
        binding.rvLots.setUpAdapter(
            list,
            R.layout.item_indo_coffee_dispatch_layout_lot_summary,
            ItemIndoCoffeeDispatchLayoutLotSummaryBinding::inflate,
            { item, pos, bindItem ->
                ...
                bindItem.etWeight.onChange {
                    if (it.isNotEmpty()) {
                        val come: Int? =
                            it.toDouble().compareTo(item.grossWeight?.toDouble() ?: 0.0)
                        if (come ?: 0 <= 0) {
