package com.olam.warehouse.vegax.mtntindo.data.repo
 ...
 */
interface VegaIndoCoffeeDispatchRepository {
...
class VegaIndoCoffeeDispatchRepositoryImpl(
    private val api: VegaIndoCoffeeDispatchApi,
    private val dao: VegaEcuadorDispatchDao,
    private val masterDao: MasterDao
) : VegaIndoCoffeeDispatchRepository {
    ...
    override suspend fun getPurchaseOrderLocal(plantId: String): LiveData<List<VegaEcuadorDispatchPurchaseOrders>> {
        return if (plantId.isNotEmpty()) dao.getPurchaseOrders(plantId) else dao.getIndoPurchaseOrders()
    }
    ...
    override suspend fun updateRemarks(remark: String, batchNumber: String) =
        dao.updateRemark(remark, true, batchNumber)
    ...
    override suspend fun getLotDetailsLocal(charge: String): LiveData<List<VegaEcuadorDispatchStocks>> =
        dao.getLotDetail(charge)
