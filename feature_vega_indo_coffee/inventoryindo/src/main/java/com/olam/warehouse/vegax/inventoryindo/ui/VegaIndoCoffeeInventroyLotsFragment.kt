...
import kotlinx.android.synthetic.main.item_indo_coffee_inventory_lot.view.*
...
class VegaIndoCoffeeInventroyLotsFragment : BaseFragment() {
                            ...
                            when {
                                filterData.size > 0 -> {
                                    filterData.forEach { qtyWb ->
                                        newText?.let { text ->
                                            if (qtyWb.batchNumber.contains(text.toUpperCase())) {
    ...
    private fun setUpAdapter(lotList: ArrayList<VegaIndoCoffeeInventoryLots>) {
        ...
        binding.rvLotList.setUp(lotList, R.layout.item_indo_coffee_inventory_lot, { it, pos ->
