    ...
    private fun postQuality(
        ...
        when (weightmentType) {
            PROCURE -> {
                ...
                weighBridgeDetails.qualityDetails = prepareVegaQualityList(qtyParams, mAdapter.getItems())
                weighBridgeDetails.batchNumber = if(batchNo1?.isNotEmpty() == true)batchNo1 ?: "" else weighBridgeDetails.batchNumber.toString()
                ...
                weighBridgeDetails.let {
                    qualityPostList.add(
                        VegaCoffeeLot(
                            ...
                            batchNumber = if(batchNo1?.isNotEmpty() == true)batchNo1 ?: "" else weighBridgeDetails.batchNumber.toString(),
                            ...
                            qualityDetails = prepareVegaQualityList(qtyParams, mAdapter.getItems())
                        )
                    )
                }
                val editPlantDetails = getPlantDetails()
                if (binding.etBatchNo.text.toString().isNotEmpty()) editPlantDetails.plantCode =
                ...
                if (isOnline()) {
                    ...
                    weighBridgeDetails.challan = binding.etBatchNo.text.toString()
...
//            intent.putExtra(AppUtils.TITLE, getString(R.string.quality_success))
            intent.putExtra(AppUtils.TITLE, getString(R.string.quality_completed_successfully))
        else
        ...
        if (!grnNo.isNullOrEmpty())
            ...
            intent.putExtra(
                AppUtils.SUB_TITLE,
                "WB ID: ".plus(weighBridgeDetails.weighBridgeId).plus("\n").plus(getString(R.string.new_lot_id_created).plus(batch))
            )
        } else
            intent.putExtra(
                AppUtils.SUB_TITLE,
