...
import com.olam.warehouse.vegax.offloadingindo.databinding.FragmentIndoCoffeeOffloadAddWeightLayoutBinding
import com.olam.warehouse.vegax.offloadingindo.databinding.ItemIndoCoffeeOffloadAddBagBinding
 ...
 */
class VegaIndoCoffeeMtnrWeighScalePalletFragment : BaseFragment() {

    override val layoutResourceId: Int = R.layout.fragment_indo_coffee_offload_add_weight_layout
    private lateinit var binding: FragmentIndoCoffeeOffloadAddWeightLayoutBinding
    ...
    private var offloadReceive: VegaCoffeeReceiving? = null

    companion object {
        fun newInstance(item: VegaCoffeeReceiveLots) = VegaIndoCoffeeMtnrWeighScalePalletFragment().putArgs {
            putParcelable("MODEL_BUNDLE", item)
        }

        fun newInstance(item: VegaCoffeeReceiveLots, receive: VegaCoffeeReceiving) =
            VegaIndoCoffeeMtnrWeighScalePalletFragment().putArgs {
                putParcelable("MODEL_BUNDLE", item)
                putParcelable("receive", receive)
            }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentIndoCoffeeOffloadAddWeightLayoutBinding.inflate(inflater)
        initUI()
        return binding.root
    }

    private fun initUI() {
        defaultLot = arguments?.getParcelable<VegaCoffeeReceiveLots>("MODEL_BUNDLE") ?: VegaCoffeeReceiveLots()
        offloadReceive = arguments?.getParcelable<VegaCoffeeReceiving>("receive")
        ...
        if (offloadReceive != null) vm.getBagItems(offloadReceive?.tempWBId.toString(), "") else vm.getBagItems(
            defaultLot.tempWBId.toString(),
            defaultLot.batch
        )
        ...
        updateTotalWeights()
        if (offloadReceive == null) {
            binding.tvTitle.text = getString(R.string.mtnr_weighment)
        } else {
            binding.tvTitle.text = getString(R.string.supplier_weighment)
    ...
    /*private fun updatePalletUI(response: Resource<GenericReqAndResp<List<VegaCoffeeWeighScalePallet>>>?) {
    ...
    private fun moveBagAddWeight(bagMaterial: VegaCoffeeOffloadingBagMaterial) {
        ...
        bundle.putParcelable(Constants.BAG_MATERIAL, prepareCoffeeBagMaterial(bagMaterial))
        ...
        bundle.putBoolean("isLotHide", offloadReceive != null)
    ...
    private fun moveToSummary() {
        ...
        if (offloadReceive == null) {
            ...
            val weight = binding.tvNetWtValue.text.toString()
            offloadReceive?.netWeight = weight.split(" ")[0]
            offloadReceive?.unitsOfMeasure = "KG"
            offloadReceive?.grossWeight = binding.tvStockValue.text.toString()
            offloadReceive?.bagType = if (bagList.size > 0) bagList[0].bagType else ""
            offloadReceive?.bagCount = bagList.sumBy { it.bagCount.toInt() }.toString()
            offloadReceive?.bagWeight = bagList.sumByDouble { it.tareWeight?.toDouble() ?: 0.0 }.toString()
            callBack?.replaceFragment(MTNR_WEIGHSCALE_SUMMARY, offloadReceive!!)
        }
    }
    ...
    fun updateBagWeight(bagMaterial: VegaCoffeeOffloadingBagMaterial) {
        ...
        //if (tareWeightCalculation <= 3 || (tareWeightCalculation <= 3 && bagMaterial.bagType.isEmpty() && bagMaterial.noOfPallet?.isEmpty() == true)) {
            if (!isExistValue) {
                ...
                bagMaterial.tempWBId = if (offloadReceive != null) offloadReceive?.tempWBId else defaultLot.tempWBId
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCoffeeOffloadingBagMaterial>) {
        ...
        binding.rvWeight.setUpAdapter(
            bagList1.asReversed(),
            R.layout.item_indo_coffee_offload_add_bag,
            ItemIndoCoffeeOffloadAddBagBinding::inflate,
            { it, pos, bindItem ->
                if (pos % 2 == 0) {
                    bindItem.clBagHead.setBackgroundResource(com.olam.warehouse.presentation.R.drawable.rectangle_grey_border)
                } else {
                    bindItem.clBagHead.setBackgroundResource(R.drawable.shape_indo_coffee_offload_rect_light_grey1)
                }
