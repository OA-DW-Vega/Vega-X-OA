...
import kotlinx.android.synthetic.main.item_vega_indo_grn_transaction.view.*
...
//    private var priceEdit = false
    ...
    private fun setupAdapter(itemList: List<VegaGrnWeighBridgeId>) {
        ...
        binding.rvTransaction.setUp(itemList as ArrayList, R.layout.item_vega_indo_grn_transaction, { it, pos ->
            if (it.status == 4) {
                tvOffloadTempId.text = getString(R.string.grn)
                tvOffloadTempIdValue.text = it.grnNumber
            } else {
                tvOffloadTempId.text = getString(R.string.weigh_bridge_id)
                tvOffloadTempIdValue.text =
                        if (it.weighBridgeId?.isNotEmpty() == true) it.weighBridgeId else it.wbTempId
            }
            tvVendorValue.text = if (it.supplierName?.isNotEmpty() == true) it.supplierName else it.transportVendorCode
            tvMaterialValue.text = it.materialName
            tvBatchNoValue.text = it.storageLocationCode
            tvProcureTypeValue.text = it.purchaseType
            tvErrorValue.text = /*if (it.syncStarted == 1) getString(R.string.sync_already_triggered) else*/
                    it.message
            ...
            if (it.message.isNullOrEmpty() && it.status != 4) clErrorStatus.gone() else clErrorStatus.visible()
