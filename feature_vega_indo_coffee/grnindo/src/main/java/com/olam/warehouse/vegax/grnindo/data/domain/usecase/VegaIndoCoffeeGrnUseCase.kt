...
class VegaIndoCoffeeGrnUseCase(
    ...
    suspend fun getQualityParams(materialId: String): LiveData<List<VegaQualityParamsWithQualitative>> = repository.getQualityParams(materialId)
    suspend fun getWeighBridgeIdDetail(wbid: String) =
        repository.getWeighBridgeIdDetail(wbid)
    suspend fun uploadGrnPrint(req: VegaUploadPrintRequest) = repository.uploadGrnPrint(req)
