...
class VegaIndoCoffeeGrnViewModel(
    ...
    private var uploadGrnPrintSource: LiveData<Resource<GenericReqAndResp<VegaUploadPrintResponse>>> =
        MutableLiveData()
    private var _uploadGrnPrint = MediatorLiveData<Resource<GenericReqAndResp<VegaUploadPrintResponse>>>()
    val uploadGrnPrint: LiveData<Resource<GenericReqAndResp<VegaUploadPrintResponse>>> get() = _uploadGrnPrint
    ...
    fun uploadGrnPrint(req : VegaUploadPrintRequest) = viewModelScope.launch(dispatchers.main) {
        _uploadGrnPrint.removeSource(uploadGrnPrintSource) // We make sure there is only one source of livedata (allowing us properly refresh)
        withContext(dispatchers.io) {
            uploadGrnPrintSource = useCase.uploadGrnPrint(req)
        }
        _uploadGrnPrint.addSource(uploadGrnPrintSource) {
            _uploadGrnPrint.value = it
        }
    }
}
