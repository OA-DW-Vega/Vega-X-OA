...
import com.olam.warehouse.vegax.exportsalesindo.databinding.ItemIndoSalesSyncStatusProgressBarBinding
import com.olam.warehouse.vegax.exportsalesindo.databinding.ItemVegaIndoExportSalesTransactionBinding
...
//    private var priceEdit = false
    ...
    private fun initUI() {
        ...
        vm.exportSalesLocal.observe(viewLifecycleOwner, Observer {
            if (it != null) {
                updateUI(it)
            }
        })
    ...
    private fun setupAdapter(itemList: List<VegaIndoCoffeeExportSalesOrder>) {
        ...
        binding.rvTransaction.setUpAdapter(
            ...
            ItemVegaIndoExportSalesTransactionBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvOffloadTempIdValue.text = it.tmpId
                bindItem.tvMaterialValue.text = it.materialName
                bindItem.tvBatchNoValue.text = it.saleOrderId
                bindItem.tvProcureTypeValue.text = it.openQuantity.plus(" ").plus(it.unitOfMeasure)
                ...
                if (it.syncStatusMsg.isNullOrEmpty() && it.status != 4) bindItem.clErrorStatus.gone() else bindItem.clErrorStatus.visible()
                ...
                bindItem.rvSynProgress.setUpAdapter(
                    ...
                    ItemIndoSalesSyncStatusProgressBarBinding::inflate,
                    { it, pos, bindItem ->
