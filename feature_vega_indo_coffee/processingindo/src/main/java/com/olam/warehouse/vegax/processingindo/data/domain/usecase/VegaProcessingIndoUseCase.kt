package com.olam.warehouse.vegax.processingindo.data.domain.usecase
...
class VegaProcessingIndoUseCase(private val repository: VegaProcessingIndoRepository) {
    suspend fun getGrades() = repository.getGrades()
    suspend fun getStages() = repository.getStages()
    suspend fun getBom(bomPostReq: VegaProcessingIndoRminBomPost) =
    ...
    suspend fun postRminDetails(model: VegaProcessingIndoRminProcessingPost) = repository.postRminDetails(model)
    ...
    suspend fun postCreatePo(bomPostReq: VegaProcessingIndoCreatePoReq) = repository.postCreatePo(bomPostReq)
