package com.olam.warehouse.vegax.processingindo.data.repo
...
interface VegaProcessingIndoRepository {
    suspend fun getGrades(): LiveData<List<VegaMaterial>>
    suspend fun getStages(): LiveData<List<VegaProcessingStage>>
    suspend fun getBom(bomPostReq: VegaProcessingIndoRminBomPost): LiveData<Resource<GenericReqAndResp<VegaProcessingRminBom>>>

    suspend fun getStocks(material: String): LiveData<Resource<GenericReqAndResp<List<VegaCocoaRminLots>>>>
    suspend fun postCreatePo(bomPostReq: VegaProcessingIndoCreatePoReq): LiveData<Resource<GenericReqAndResp<VegaProcessingRminPo>>>
    ...
    suspend fun postRminDetails(model: VegaProcessingIndoRminProcessingPost): LiveData<Resource<GenericReqAndResp<List<VegaProcessingIndoRminResponse>>>>
...
class VegaProcessingIndoRepositoryImpl(
    private val api: VegaProcessingIndoApi,
    private val dao: VegaCocoaRMinDao,
    private val masterDao: MasterDao
) : VegaProcessingIndoRepository {
    val currentKey = PreferenceHelper.get(Constants.CURRENT_KEY, "")
    override suspend fun getGrades() = dao.getProducts()
    override suspend fun getStages() = dao.getProcessingStage()
    override suspend fun getBom(bomPostReq: VegaProcessingIndoRminBomPost): LiveData<Resource<GenericReqAndResp<VegaProcessingRminBom>>> {
    ...
    override suspend fun postCreatePo(bomPostReq: VegaProcessingIndoCreatePoReq): LiveData<Resource<GenericReqAndResp<VegaProcessingRminPo>>> {
        ...
        return object : NetworkOnlyBoundResource<GenericReqAndResp<List<VegaCocoaFgrnItems>>>() {
            override suspend fun createCall(): GenericReqAndResp<List<VegaCocoaFgrnItems>> =
                api.fetchFgrnPoDetailsList(
                    VegaIndoProcessingOrderReq(getCurrentKey(),
                    auart,
                    cfgNo,
                    stageFevor,
                    getPlantDetails())
    ...
    override suspend fun getPoGrades(poNo: String,
        ...
        return object : NetworkOnlyBoundResource<GenericReqAndResp<List<VegaCocoaFgrnItemsGrades>>>() {
            override suspend fun createCall(): GenericReqAndResp<List<VegaCocoaFgrnItemsGrades>> =
                api.getFgrnGrades(VegaProcessingIndoProcessOrderDetailsPostReq(getCurrentKey(), getPlantDetails(), poNo, rmin))
        }.build().asLiveData()
    }
    ...
    override suspend fun saveLots(
        ...
        dao.insertRminModelNew(model)
        dao.saveLotsNew(dispatchLotsList)
    }

    override suspend fun saveRminProcess(model: VegaCocoaRminProcessing) =
        dao.insertRminModelNew(model)
    ...
    override suspend fun postRminDetails(model: VegaProcessingIndoRminProcessingPost): LiveData<Resource<GenericReqAndResp<List<VegaProcessingIndoRminResponse>>>> {
        return object : NetworkOnlyBoundResource<GenericReqAndResp<List<VegaProcessingIndoRminResponse>>>() {
            override suspend fun createCall(): GenericReqAndResp<List<VegaProcessingIndoRminResponse>> =
