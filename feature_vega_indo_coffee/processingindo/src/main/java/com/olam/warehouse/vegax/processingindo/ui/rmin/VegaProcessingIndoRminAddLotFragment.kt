...
import com.olam.warehouse.vegax.processingindo.databinding.FragmentProcessingIndoAddLotRminBinding
import com.olam.warehouse.vegax.processingindo.databinding.ItemProcessingIndoLotInfoBinding
import com.olam.warehouse.vegax.processingindo.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel

class VegaProcessingIndoRminAddLotFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_processing_indo_add_lot_rmin
    private lateinit var binding: FragmentProcessingIndoAddLotRminBinding
    ...
    companion object {
        fun newInstance(fromSummary: Boolean, model: VegaCocoaRminProcessing) =
            VegaProcessingIndoRminAddLotFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentProcessingIndoAddLotRminBinding.inflate(layoutInflater)
        initExtra()
        initUI()
        return binding.root
    }
...
//        vm.getLotList(model?.cgfNo ?: "", model?.poNumber ?: "", model?.bom ?: "", model?.materialName ?: "")
    ...
    private fun setUpAdapter(list: MutableList<VegaCocoaRminLots>) {
        binding.rvList.setUpAdapter(
            list,
            R.layout.item_processing_indo_lot_info,
            ItemProcessingIndoLotInfoBinding::inflate,
            { item, pos, bindItem ->
                ...
                if (isEditableLot) {
                    ...
                    bindItem.etWeight.isEnabled = true
    ...
    private fun updateWeightToProcess() {
        if (!isEditableLot) calculateWtp(vm.lotList.value?:ArrayList<VegaCocoaRminLots>())
        else binding.cvCard.tvProceesValue.text =
            addedLotList.sumOf { if (it.editedWeight.isNullOrEmpty()) 0.0 else it.editedWeight?.toDouble()!! }
