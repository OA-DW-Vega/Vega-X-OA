...
import kotlinx.android.synthetic.main.item_processing_indo_bom_layout.view.*
...
class VegaProcessingIndoRminPoBomSelectFragment : BaseFragment() {
    ...
    private fun updateDetails(poList: List<VegaFgrnProcessingOrder>) {
        ...
        if (poList.size == 1) {
            ...
            binding.rvPoList.setUp(data as MutableList, R.layout.item_processing_indo_bom_layout, { item, pos ->
                if (pos % 2 == 0) {
                ...
                tvInputGradeValue.text = item.netWeight.plus(" ").plus(item.unitsOfMeasure.toString())
    ...
    private fun updateBomList(boms: List<VegaProcessingRminBoms>?) {
        ...
        if (boms?.size == 1) {
            ...
            binding.rvPoList.setUp(bomList, R.layout.item_processing_indo_bom_layout, { it, pos ->
                if (pos % 2 == 0) {
