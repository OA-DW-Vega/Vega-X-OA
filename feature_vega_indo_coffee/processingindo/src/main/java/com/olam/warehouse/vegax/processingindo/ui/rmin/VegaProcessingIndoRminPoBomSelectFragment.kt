...
import com.olam.warehouse.vegax.processingindo.data.domain.model.VegaProcessingIndoRminBomPost
import com.olam.warehouse.vegax.processingindo.databinding.FragmentProcessingIndoBomSelectLayoutBinding
import com.olam.warehouse.vegax.processingindo.databinding.ItemProcessingIndoBomLayoutBinding
...
class VegaProcessingIndoRminPoBomSelectFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_processing_indo_bom_select_layout
    private lateinit var binding: FragmentProcessingIndoBomSelectLayoutBinding
    ...
    companion object {
        fun newInstance(title: String, isPo: Boolean, model: VegaCocoaRminProcessing) =
            VegaProcessingIndoRminPoBomSelectFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentProcessingIndoBomSelectLayoutBinding.inflate(layoutInflater)
        initExtra()
        initUI()
        return binding.root
    }
                    ...
                    when (it.data?.success) {
                        true -> {
                            response.data?.data?.let {
                                ...
                                it.forEach { item ->
                                   /* val rmindata =
                                        item.rmin?.filter { rmin -> model?.materialCodes?.contains(if(rmin.materialCode?.length==18)
                                            rmin.materialCode?.removeRange(0,6).toString()
                                        else rmin.materialCode?.toString() ) == true }
    ...
    private fun updateDetails(poList: List<VegaFgrnProcessingOrder>) {
        ...
        if (poList.size == 1) {
            ...
            binding.rvPoList.setUpAdapter(
                data as MutableList,
                R.layout.item_processing_indo_bom_layout,
                ItemProcessingIndoBomLayoutBinding::inflate,
                { item, pos, bindItem ->
                    if (pos % 2 == 0) {
