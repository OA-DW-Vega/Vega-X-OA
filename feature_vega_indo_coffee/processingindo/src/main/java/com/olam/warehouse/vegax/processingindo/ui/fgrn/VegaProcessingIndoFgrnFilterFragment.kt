...
import com.olam.warehouse.vegax.processingindo.data.domain.model.vegaProcessingIndoFilterList
import com.olam.warehouse.vegax.processingindo.databinding.FragmentVegaProcessingIndoFgrnFilterBinding
import com.olam.warehouse.vegax.processingindo.databinding.ItemVegaProcessingIndoFgrnInventoryFilterBinding
import com.olam.warehouse.vegax.processingindo.utils.*
import java.util.*

/**
 * Created by Baskaran Kannan on 6/22/2020.
 */
class VegaProcessingIndoFgrnFilterFragment : BaseFragment() {

    private lateinit var binding: FragmentVegaProcessingIndoFgrnFilterBinding
    override val layoutResourceId = R.layout.fragment_vega_processing_indo_fgrn_filter
    private var callBack: CallBack? = null
    private var fullFilter = ArrayList<String>()
    private var listFliterWhLoc = ArrayList<vegaProcessingIndoFilterList>()
    private var startRange: String = ""
    private var endRange: String = ""
    private var aboveThirty: Boolean = false

    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaProcessingIndoFgrnFilterFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentVegaProcessingIndoFgrnFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        for (item in whLocations.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFliterWhLoc.add(vegaProcessingIndoFilterList(item, true))
                else listFliterWhLoc.add(vegaProcessingIndoFilterList(item))
            }
        }
    ...
    private fun setUpAdapter(listFliterWhLoc: ArrayList<vegaProcessingIndoFilterList>) {
        binding.rvWhLocation.setUpAdapter(
            listFliterWhLoc,
            R.layout.item_vega_processing_indo_fgrn_inventory_filter,
            ItemVegaProcessingIndoFgrnInventoryFilterBinding::inflate,
            { it, pos, bindItem ->
