...
import com.olam.warehouse.vegax.processingindo.databinding.FragmentVegaProcessingIndoFgrnPoDetailsBinding
import com.olam.warehouse.vegax.processingindo.databinding.ItemVegaProcessingIndoFgrnPoDetailsBinding
import com.olam.warehouse.vegax.processingindo.utils.FRAG_GRADES
import com.olam.warehouse.vegax.processingindo.utils.FRAG_PENDING
import com.olam.warehouse.vegax.processingindo.utils.getColor
import com.olam.warehouse.vegax.processingindo.utils.weightToProcess
 ...
 */
class VegaProcessingIndoFgrnPODetailsFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_vega_processing_indo_fgrn_po_details
    private lateinit var binding: FragmentVegaProcessingIndoFgrnPoDetailsBinding
    ...
    companion object {
        fun newInstance() = VegaProcessingIndoFgrnPODetailsFragment().putArgs {
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaProcessingIndoFgrnPoDetailsBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun moveToNext(offlineFgrnList: List<VegaCocoaFgrnItems>) {
        when (getCurrentFragment()) {
            is VegaProcessingIndoFgrnPODetailsFragment -> when (offlineFgrnList.size > 0) {
            ...
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, position: Int, p3: Long) {
                stage.forEachIndexed { index, vegaProcessingStage ->
                    if (stageListData[position] == vegaProcessingStage.processName) {
                        ...
                        currentPo.processName = vegaProcessingStage.processName
    ...
    private fun updateAdapter(poList: MutableList<VegaCocoaFgrnItems>) {
        ...
        binding.rvPoList.setUpAdapter(
            poList, R.layout.item_vega_processing_indo_fgrn_po_details,
            ItemVegaProcessingIndoFgrnPoDetailsBinding::inflate,
            { item, pos, bindItem ->
                ...
                val weight =
                    DecimalFormat("#.##", DecimalFormatSymbols(Locale.ENGLISH)).format(
                        weightToProcess
                    )
                        .toString().replace(",", ".")
                ...
                currentPo.processName = currentPo1.processName
