...
import kotlinx.android.synthetic.main.item_vega_processing_indo_fgrn_po_details.view.*
...
class VegaProcessingIndoFgrnPODetailsFragment : BaseFragment() {
    ...
    private fun updateAdapter(poList: MutableList<VegaCocoaFgrnItems>) {
        ...
        binding.tvToatalNo.text = getString(R.string.total_pos_0).plus(" ").plus(poList.size)
        binding.rvPoList.setUp(poList, R.layout.item_vega_processing_indo_fgrn_po_details, { item, pos ->
            ...
            val weight =
                DecimalFormat("#.##", DecimalFormatSymbols(Locale.ENGLISH)).format(weightToProcess)
                    .toString().replace(",", ".")
