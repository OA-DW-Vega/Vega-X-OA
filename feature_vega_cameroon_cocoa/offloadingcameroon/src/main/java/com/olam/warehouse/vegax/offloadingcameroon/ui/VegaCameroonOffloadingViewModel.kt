...
class VegaCameroonOffloadingViewModel(
    ...
    private var receivingBagSource: LiveData<VegaCameroonOffloadingWithBagItem> = mutableLiveDataOf()
    private val _receivingBag = MediatorLiveData<VegaCameroonOffloadingWithBagItem>()
    val receivingBag: LiveData<VegaCameroonOffloadingWithBagItem> get() = _receivingBag
    ...
    fun getSavedReceivingBagItems(wbid: String) =
        viewModelScope.launch(dispatchers.main) {
            _receivingBag.removeSource(receivingBagSource) // We make sure there is only one source of livedata (allowing us properly refresh)
            withContext(dispatchers.io) {
                receivingBagSource = useCase.getSavedReceivingBagItems(wbid)
            }
            _receivingBag.addSource(receivingBagSource) {
                _receivingBag.value = it
            }
        }
    ...
    fun getFeatureMaster() = viewModelScope.launch(dispatchers.main) {
        ...
        withContext(dispatchers.io) {
            featureMasterSource = useCase.getFeatureMaster()
        }
