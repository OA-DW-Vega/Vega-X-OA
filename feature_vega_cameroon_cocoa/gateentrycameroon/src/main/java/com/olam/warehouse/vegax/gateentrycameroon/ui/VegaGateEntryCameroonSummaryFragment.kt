...
//        settingTrackTracevalue()
    ...
    private fun postGateEntry() {
        ...
        if (AppUtils.isOnline()) {
            vm.postGateEntryData(VegaGateEntryCameroonPost(getCurrentKey(), plantDetails, postData,sourceLotId = gateEntryData.sourceLotId,
                eudrComplaint = gateEntryData.eudrStatus,
                ttFarmerList = gateEntryData.ttFarmerList))
        }
    }
   ...
   /* private fun settingTrackTracevalue(){
        *//*indirect flow*//*
        if(gateEntryData.sourceLotId.isNotEmpty()){
            binding.llSourceLot.visible()
            binding.llEudrStatus.visible()
            binding.tvSourceLotId.setText(gateEntryData.sourceLotId)
            if(gateEntryData.eudrStatus){
                binding.tvEudrStatus.setText(Constants.COMPLAINT)
            } else {
                binding.tvEudrStatus.setText(Constants.NON_COMPLAINT)
            }
        }
        *//*direct flow*//*
//        if(gateEntryData.ttFarmerList.size>0){
            binding.llEudrStatus.visible()
            if(gateEntryData.eudrStatus){
                binding.tvEudrStatus.setText(Constants.COMPLAINT)
            } else {
