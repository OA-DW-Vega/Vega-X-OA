...
// Update the button according to DSE Values
    ...
    private fun updateButton(
        ...
        var request1= convertLongToTime(request.toLong())
        val startTime = request1.split("T")
        val t =if(startTime.size>0) startTime[1].split(".") else startTime
        val temp =if(startTime.isNotEmpty()) t[0].split(":") else t
        ...
        if (response?.trim() == "null") {
            ...
            if (totalDays1 == totalDays2) {
                ...
                diff = TimeUnit.MILLISECONDS.toHours(days + duration)
                 ...
                 response.let {
                     val response1= response?.let { convertLongToTime(it.toLong()) }
                     val startTime1 = response1?.split("T")
                     val t1 = if((startTime1?.size ?: 0) > 0)startTime1!![1].split(".") else startTime1
                     val temp1 = if(t1?.isNotEmpty() == true) t1[0].split(":") else t1

                     /*val y = response?.split("+")
                     val endTime = y?.get(0)?.split("T")
                     val time2 = endTime?.get(1)?.split(".")
                     val temp2 = time2?.get(0)?.split(":")*/
                     val twoHours= if(temp1?.isNotEmpty() == true) temp1[0].toLong() else 0L
                     val twoMins= if((temp1?.size ?: 0) > 0) (temp1?.get(1) ?:"0").toLong() else 0L
                     val twoSecs= if((temp1?.size ?: 0) > 1) (temp1?.get(2)?:"0").toLong() else 0L

                     val time2hours = TimeUnit.HOURS.toMillis(twoHours)
                     val time2mins = TimeUnit.MINUTES.toMillis(twoMins)
                     val time2secs = TimeUnit.SECONDS.toMillis(twoSecs)
                     val totalTime2 = time2hours + time2mins + time2secs

                     val endDate = startTime1?.get(0)?.split("-")
                     val date2days = TimeUnit.DAYS.toMillis((endDate?.get(2)?:"0").toLong())
                     val totalDays2 = date2days
