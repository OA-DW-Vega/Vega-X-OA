...
import com.olam.warehouse.vegax.ppqcameroon.databinding.ItemVegaCameroonPpqQualitySummaryParamsBinding
    ...
    private fun setUpAdapter(data: List<VegaCameroonPpqInspectionLotParams>) {
        data.let {
            ...
            binding.rvApproveQuality.setUpAdapter(
                ...
                ItemVegaCameroonPpqQualitySummaryParamsBinding::inflate,
                { item, pos, bindItem ->
                    i++
                    if (i % 2 == 0) {
                    ...
                    bindItem.tvQualityNameApprove.text = item?.charDesc
                    if (item?.qualitative?.size!! > 0) {
                        bindItem.tvUnitApprove.text = item.MeanValue
