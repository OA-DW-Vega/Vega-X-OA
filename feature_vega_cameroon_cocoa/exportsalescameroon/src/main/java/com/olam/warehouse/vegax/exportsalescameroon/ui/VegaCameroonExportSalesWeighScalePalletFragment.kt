...
import com.olam.warehouse.vegax.exportsalescameroon.data.domain.model.VegaCameroonExportSalesWeighScalePallet
import com.olam.warehouse.vegax.exportsalescameroon.databinding.FragmentVegaCameroonExportSalesWeighScalePalletBinding
import com.olam.warehouse.vegax.exportsalescameroon.databinding.ItemVegaCameroonExportSalesAddBagBinding
import com.olam.warehouse.vegax.exportsalescameroon.utils.FRAG_ADD_BAG_WEIGHT
import com.olam.warehouse.vegax.exportsalescameroon.utils.MODEL_BUNDLE
import com.olam.warehouse.vegax.exportsalescameroon.utils.UPDATE_WEIGHT
...
class VegaCameroonExportSalesWeighScalePalletFragment : BaseFragment() {

    override val layoutResourceId: Int
        get() = R.layout.fragment_vega_cameroon_export_sales_weigh_scale_pallet

    private lateinit var binding: FragmentVegaCameroonExportSalesWeighScalePalletBinding
    private var callBack: VegaCameroonExportSalesReplaceFragmentCallback? = null
    ...
    companion object {
        fun newInstance(item: VegaCoffeeExportSalesLots) =
            VegaCameroonExportSalesWeighScalePalletFragment().putArgs {
                putParcelable(MODEL_BUNDLE, item)
            }
    }
    interface VegaCameroonExportSalesReplaceFragmentCallback {
        fun replaceFragment(
    ...
    override fun onCreateView(
        ...
        binding = FragmentVegaCameroonExportSalesWeighScalePalletBinding.inflate(inflater)
        initUI()
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track()
            .screen("exportsalescameroon/ui/VegaCameroonExportSalesWeighScalePalletFragment")
    ...
    override fun onAttach(context: Context) {
        ...
        callBack = context as? VegaCameroonExportSalesReplaceFragmentCallback
    }
    ...
    private fun initUI() {
        ...
        defaultLot =
            arguments?.getParcelable<VegaCoffeeExportSalesLots>(MODEL_BUNDLE) as VegaCoffeeExportSalesLots
...
//        vm.getBagItems(defaultLot.batchNumber, defaultLot.materialCode)
        vm.getSalesPalletBag(defaultLot.batchNumber)
        vm.salesBagWithMaterial.observe(viewLifecycleOwner, Observer { updateBagItems(it) })
    ...
    private fun updatePalletUI(response: Resource<GenericReqAndResp<List<VegaCameroonExportSalesWeighScalePallet>>>?) {
        when (response?.status) {
            Resource.Status.SUCCESS -> {
                ...
                if (response.data?.data?.size!! > 0) {
                    ...
                    palletWeight = response.data?.data?.get(0)?.totalPalletWeight ?: "0"
                    Bundle().apply {
                        val isEdit =
    ...
    private fun savePalletDetails(){
        defaultLot.palletWeight= palletWeight
        defaultLot.palletAvg= palletAvg
        defaultLot.palletCount= palletCount
        vm.saveExportLot(defaultLot)
    }
    private fun moveToSummary() {
        ...
        callBack?.replaceFragment(UPDATE_WEIGHT, VegaCameroonExportSalesWSBagModel(binding.tvNetWtValue.text.toString(), true))
    ...
    private fun updateBagItems(bagSalesItems: VegaCameroonExportSalesWithBagItems) {
        bagSalesItems.let {
            val bagItems= bagSalesItems.bagMaterialList
            val palleDetails= bagSalesItems.salesLots
            palletWeight= palleDetails.palletWeight?:"0"
            palletAvg= palleDetails.palletAvg?:"0"
            palletCount= palleDetails.palletCount?:"0"
    ...
    //                }
            ...
            if(palletWeight.isEmpty() ||  palletWeight.equals("0.0") || palletWeight.equals("0")){
                vm.getPalletInfo(defaultLot.batchNumber, defaultLot.materialCode)
...
//    }
    ...
    fun updateBagWeight(bagMaterial: VegaCocoaSweepingBagMaterial) {
        ...
        if (bagList.size == 1 && !palletCount.equals("0")) bagList[0].startTime = DateUtils.getCurrentTimeInMills().toString()
       if (palletCount.isNotEmpty()) if (bagList.size == palletCount.toInt()) bagList[palletCount.toInt() - 1].endTime = DateUtils.getCurrentTimeInMills().toString()
        ...
        if (palletCount.isNotEmpty() ) if (bagList.size == palletCount.toInt() && !palletCount.equals("0")) binding.btProceed.setBackgroundColor(
            getColor(com.olam.warehouse.presentation.R.color.colorPrimaryOfi)
        )
        else binding.btProceed.setBackgroundColor(getColor(com.olam.warehouse.presentation.R.color.grey))
        updateTotalWeights()
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCocoaSweepingBagMaterial>) {
        ...
        binding.rvWeight.setUpAdapter(
            bagList1.asReversed(),
            R.layout.item_vega_cameroon_export_sales_add_bag,
            ItemVegaCameroonExportSalesAddBagBinding::inflate,
            { it, pos, bindItem ->
                if (pos % 2 == 0) {
                ...
                val avgAvlue = if (it.palletAverage?.isNotEmpty() == true)
                    it.palletAverage?.toDouble()?.plus(it.bagCount.toInt().times(it.tareWeight?.toDouble()!!)) else 0.0
...
//            else enableProceed(bagList.size > 0)
    ...
    private fun updateTotalWeights() {
        ...
        if(palletWeight!="" && palletCount!="" && palletWeight!="0" && palletCount!="0"){
            palletAvg = palletWeight.toDouble().div(palletCount.toInt()).formatThreeDigits().replace(",", "")
        }else palletAvg="0"

        for (item in bagList) {
            grossWeight = grossWeight.plus(item.grossWeight.toDouble())
            if(palletAvg.isNotEmpty())
            tareWeight = tareWeight.plus(item.tareWeight?.toDouble()?.times(item.bagCount.toDouble())!!).plus(palletAvg.toDouble())
            else  tareWeight = 0.0
        }
