...
import com.olam.warehouse.vegax.processingcameroon.data.domain.model.CameroonFilterList
import com.olam.warehouse.vegax.processingcameroon.databinding.FragmentVegaCameroonFgrnFilterBinding
import com.olam.warehouse.vegax.processingcameroon.databinding.ItemVegaCameroonFgrnInventoryFilterBinding
import com.olam.warehouse.vegax.processingcameroon.utils.*
import java.util.*

class VegaCameroonFgrnFilterFragment : BaseFragment() {

    private lateinit var binding: FragmentVegaCameroonFgrnFilterBinding
    override val layoutResourceId = R.layout.fragment_vega_cameroon_fgrn_filter
    private var callBack: CallBack? = null
    private var fullFilter = ArrayList<String>()
    private var listFliterWhLoc = ArrayList<CameroonFilterList>()
    private var startRange: String = ""
    private var endRange: String = ""
    private var aboveThirty: Boolean = false

    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaCameroonFgrnFilterFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentVegaCameroonFgrnFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        for (item in whLocations.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFliterWhLoc.add(CameroonFilterList(item, true))
                else listFliterWhLoc.add(CameroonFilterList(item))
            }
        }
    ...
    private fun setUpAdapter(listFliterWhLoc: ArrayList<CameroonFilterList>) {
        binding.rvWhLocation.setUpAdapter(
            listFliterWhLoc,
            R.layout.item_vega_cameroon_fgrn_inventory_filter,
            ItemVegaCameroonFgrnInventoryFilterBinding::inflate,
            { it, pos, bindItem ->
