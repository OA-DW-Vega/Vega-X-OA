...
class VegaCameroonRMINAddLotFragment : BaseFragment(), CameroonRminItemRemoveListener,
    ...
    companion object {
        fun newInstance(fromSummary: Boolean, model: VegaCoffeeRminProcessing) =
            VegaCameroonRMINAddLotFragment().putArgs {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("processingcameroon/ui/rmin/VegaCameroonRMINAddLotFragment")
    ...
    private fun moveToShiftSelect() {
        callBack?.replaceFragment(if (fromSummary == true) SUMMARY else SHIFT, isMultipleLot, model!!, "", "",VegaCoffeeRminLots())
    }
                    ...
                    response.data?.data?.let { it1 ->
                        it1.forEachIndexed { index, s ->
                            when (s.unitOfMeasure) {
                                ...
                                "MT" ->
                                    it1[index].weight = convertMtToKg(it1[index].weight.toString())
                            }
                        }
                        when (it1.size == 1) {
    ...
    fun updateAddWeight(weight: String, batchNo: String?, palletWeight: String, palletCount: String,isSave:Boolean) {
        if(!weight.isNullOrEmpty() && isSave) {
            val split = weight.split(" ")

            if (adapter?.getItemList()?.isNullOrEmpty() == false) {
                adapter?.getItemList()?.forEachIndexed { index, vegaCoffeeRminLots ->
                    if (vegaCoffeeRminLots.batchNumber.equals(batchNo))
                        adapter?.updateAddWeight(index, split[0], split[1])
                }
            }
