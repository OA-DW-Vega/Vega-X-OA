...
import kotlinx.android.synthetic.main.item_cameroon_weighscale_lot_card_layout.view.*
...
class VegaCameroonMtntWeighScaleAddLotFragment : BaseFragment() {
    ...
    override fun onResume() {
        super.onResume()
        val tvWhValue = activity!!.findViewById<View>(R.id.tvWhValue)
        val tvSendingWHValue = activity!!.findViewById<View>(R.id.tvSendingWHValue)
    ...
    private fun setUpMaterialAdapter() {
        binding.rvMaterialList.setUp(vm.materialModelList, R.layout.item_cameroon_material_layout, { it, pos ->
            ...
            when(it.uom){
                ...
                "MT" ->
                    tvStoWeightValue.text =
                        convertMtToKg(it.soWeight.toString()).toDouble().formatThreeDigits().replace(",", "").plus(" ")
                            .plus("KG")
            }
    ...
    private fun setUpAdapter(list: ArrayList<VegaCocoaDispatchLots>) {
        val lots = list
        binding.clLotSummary.rvList.setUp(lots, R.layout.item_cameroon_weighscale_lot_card_layout, { item, pos ->
            ...
            when(item.unitOfMeasure){
                ...
                "MT" ->
                    tvScaleWeightValue.text =
                        convertMtToKg(item.weight.toString()).toDouble().formatThreeDigits().plus(" ").plus("KG")
            }
