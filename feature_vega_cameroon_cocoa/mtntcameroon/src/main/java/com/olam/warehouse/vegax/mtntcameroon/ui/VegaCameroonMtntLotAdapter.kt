...
import kotlinx.android.synthetic.main.item_cameroon_lot_summary.view.*

class VegaCameroonMtntLotAdapter(
    var data: ArrayList<VegaCocoaDispatchLots>,
    var isEdit: Boolean = false,
    var isClose: Boolean = false,
    var listenerCameroon: ItemRemoveListenerCameroon
) :
    RecyclerView.Adapter<VegaCameroonMtntLotAdapter.LotViewHolder>() {

    class LotViewHolder(bind: ItemCameroonLotSummaryBinding) : RecyclerView.ViewHolder(bind.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LotViewHolder {
        val viewHolder =
            ItemCameroonLotSummaryBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: LotViewHolder, position: Int) {
        ...
        holder.itemView.ivClose.setImageDrawable(holder.itemView.context.getDrawable(if (isClose) R.drawable.ic_cameroon_close_black else R.drawable.ic_cameroon_edit_gray))
    ...
    private fun showConformationDialog(position: Int, view: View) {
        MaterialDialog(view.context).show {
            ...
            UIUtils.getMetirialCustomView(
                ...
                {
                    listenerCameroon.itemRemoved(data[position])
