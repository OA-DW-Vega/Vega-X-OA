...
import com.olam.warehouse.vegax.mtntcameroon.databinding.FragmentCameroonMtntWeighscaleAddPalletBinding
import com.olam.warehouse.vegax.mtntcameroon.databinding.ItemVegaCameroonMtntAddBagBinding
...
import com.olam.warehouse.vegax.mtntcameroon.utils.FRAG_ADD_BAG_WEIGHT
import com.olam.warehouse.vegax.mtntcameroon.utils.MODEL_BUNDLE
import com.olam.warehouse.vegax.mtntcameroon.utils.UPDATE_WEIGHT
...
class VegaCameroonMtntWeighScalePalletFragment : BaseFragment() {

    override val layoutResourceId: Int =
        R.layout.fragment_cameroon_mtnt_weighscale_add_pallet
    private lateinit var binding: FragmentCameroonMtntWeighscaleAddPalletBinding
    ...
    companion object {
        fun newInstance(item: VegaCocoaDispatchLots) = VegaCameroonMtntWeighScalePalletFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentCameroonMtntWeighscaleAddPalletBinding.inflate(inflater)
        initUI()
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track()
            .screen("mtntcameroon/ui/weighscale/VegaCameroonMtntWeighScalePalletFragment")
    ...
    private fun initUI() {
        ...
        defaultLot =
            arguments?.getParcelable<VegaCocoaDispatchLots>(MODEL_BUNDLE) as VegaCocoaDispatchLots
        binding.tvTitle.text =
            getString(com.olam.warehouse.login.R.string.mtnt).plus(" - ")
                .plus(getString(R.string.weigh_scale))
        vm.configItems.observe(viewLifecycleOwner, Observer { updateConfigItems(it) })
        vm.getConfigItems(UserRoles.SALES.role)

        vm.getMtntPalletBag(defaultLot.batchNumber)
        vm.mtntBagWithMaterial.observe(viewLifecycleOwner, Observer { updateBagMtntItems(it) })
    ...
    private fun updatePalletUI(response: Resource<GenericReqAndResp<List<VegaCameroonWeighScalePallet>>>?) {
    ...
    fun onBackPressed() {
        var model = true
        if (palletCount.isNotEmpty() && palletCount.toInt() > 0) {
        ...
        callBack?.replaceFragment(UPDATE_WEIGHT, VegaCameroonWSBagModel(weight, model))
    }

    private fun moveToSummary() {
        defaultLot.palletWeight= palletWeight
        defaultLot.palletCount= palletCount
        vm.savePalletLot(defaultLot)

        activity?.onBackPressed()
        callBack?.replaceFragment(UPDATE_WEIGHT, VegaCameroonWSBagModel(binding.tvNetWtValue.text.toString(), true))
    }
    ...
    private fun updateBagMtntItems(bagItems: VegaCameroonMtntWithBagItems) {
        bagItems.let {
            var mtntBagList= bagItems.bagMaterialList
            palletWeight= bagItems.dispatchLots.palletWeight?:""
            palletCount= bagItems.dispatchLots.palletCount?:""
            bagList.clear()
            bagList.addAll(mtntBagList)
...
//    }
    ...
    fun updateBagWeight(bagMaterial: VegaCocoaSweepingBagMaterial) {
        ...
        if(palletCount.isNotEmpty() && !palletCount.equals("0")) {
            if (bagList.size == palletCount.toInt())
                bagList[palletCount.toInt() - 1].endTime = DateUtils.getCurrentTimeInMills().toString()
        }
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCocoaSweepingBagMaterial>) {
        ...
        binding.rvWeight.setUpAdapter(
            bagList1.asReversed(),
            R.layout.item_vega_cameroon_mtnt_add_bag,
            ItemVegaCameroonMtntAddBagBinding::inflate,
            { it, pos, bindItem ->
                if (pos % 2 == 0) {
                    bindItem.clBagHead.setBackgroundResource(com.olam.warehouse.presentation.R.drawable.rectangle_grey_border)
                } else {
                    bindItem.clBagHead.setBackgroundResource(R.drawable.shape_cameroon_rect_light_grey1)
                }
    ...
    fun updatePalletDetails(noOfPallet: String, palletWeight: String) {
        if (noOfPallet.isNotEmpty() && palletWeight.isNotEmpty() && (!palletWeight.equals("0") && !palletWeight.equals("0.0"))) {
            ...
            if (palletCount.isNotEmpty() && !palletCount.equals("0")) enableProceed(true)
            else enableProceed(bagList.size > 0)
