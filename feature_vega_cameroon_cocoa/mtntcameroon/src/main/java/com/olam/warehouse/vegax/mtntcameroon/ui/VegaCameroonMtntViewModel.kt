...
class VegaCameroonMtntViewModel(private val useCase: VegaCameroonDispatchUseCase, private val dispatchers: AppDispatchers) :
    ...
    private var mtntBagWithMaterialSource: LiveData<VegaCameroonMtntWithBagItems> =
        MutableLiveData()
    private val _mtntBagWithMaterial = MediatorLiveData<VegaCameroonMtntWithBagItems>()

    val mtntBagWithMaterial: LiveData<VegaCameroonMtntWithBagItems> get() = _mtntBagWithMaterial
    ...
    fun savePalletLot(dispatchLots: VegaCocoaDispatchLots) = viewModelScope.launch {
        withContext(dispatchers.io) {
            useCase.savePalletLot(dispatchLots)
        }
    }

    fun getMtntPalletBag(batchNo: String) = viewModelScope.launch(dispatchers.main) {
        _mtntBagWithMaterial.removeSource(mtntBagWithMaterialSource)
        withContext(dispatchers.io) {
            mtntBagWithMaterialSource = useCase.getMtntPalletBagDetails(batchNo)
        }
        _mtntBagWithMaterial.addSource(mtntBagWithMaterialSource) {
            _mtntBagWithMaterial.value = it
        }
    }
