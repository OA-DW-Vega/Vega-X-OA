...
class VegaCameroonMtntConsignmentFragment : BaseFragment(),
        ...
        vm.custonLocation.observe(viewLifecycleOwner, Observer {
            ...
            if (sendingWareHouseList.size == 1) {
                sendingWareHouseList.forEach {
                    ...
                    vm.dispatchWh.recPlantName = binding.tvSendingWHValue.text.trim().toString()
                }
            }
        })
    ...
    private fun validateProceed() =
        if (binding.tvWhValue.text.isNotEmpty() && binding.tvstoValue.text.isNotEmpty() && binding.tvDriverNameValue.text!!.isNotEmpty()
                ...
                when (item.uom) {
                    "KG" ->
                        bindItem.tvStoWeightValue.text =
                            item.soWeight?.toDouble()?.formatThreeDigits()?.replace(",", "")
                                .plus(" ")
                                .plus(item.uom)
                    "MT" ->
                        bindItem.tvStoWeightValue.text =
                            convertMtToKg(item.soWeight.toString()).toDouble().formatThreeDigits()
                                .replace(",", "")
                                .plus(" ")
                                .plus("KG")
                }
