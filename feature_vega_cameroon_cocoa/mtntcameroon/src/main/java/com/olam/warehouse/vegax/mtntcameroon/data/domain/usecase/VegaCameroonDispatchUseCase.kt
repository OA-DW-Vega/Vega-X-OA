package com.olam.warehouse.vegax.mtntcameroon.data.domain.usecase
...
class VegaCameroonDispatchUseCase(private val repository: VegaCameroonMtntRepository) {
    ...
    suspend fun postDeliveryDetail(vegaDeliveryPost: VegaCameroonDeliveryPost) =
    ...
    suspend fun postWeighScaleDeliveryDetail(vegaDeliveryPost: VegaCameroonDeliveryPost) =
    ...
    suspend fun savePalletLot(model:VegaCocoaDispatchLots) = repository.savePalletLot(model)

    suspend fun getMtntPalletBagDetails(batchNumber: String)= repository.getMtntPalletBagDetails(batchNumber)
}
