...
import com.olam.warehouse.vegax.inventorycameroon.data.domain.model.FilterList
import com.olam.warehouse.vegax.inventorycameroon.databinding.FragmentVegaCameroonInventoryFilterBinding
import com.olam.warehouse.vegax.inventorycameroon.utils.*
import kotlinx.android.synthetic.main.item_vega_cameroon_inventory_filter.view.*
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

/**
 * Created by Keerthi Santhanam on 7/10/2020.
 */
class VegaCameroonInventoryFilterFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_vega_cameroon_inventory_filter
    private lateinit var binding: FragmentVegaCameroonInventoryFilterBinding
    ...
    private var listFilterWaste = ArrayList<FilterList>()
    ...
    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaCameroonInventoryFilterFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentVegaCameroonInventoryFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("inventorycameroon/ui/VegaCameroonInventoryFilterFragment")
    ...
    private fun initUI() {
        ...
        val impurityList = bundle?.getStringArrayList(FILTER_SMOKY) ?: ArrayList()
        ...
        //Waste Adapter
        for (item in wasteList.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFilterWaste.add(FilterList(item, true))
                else listFilterWaste.add(FilterList(item))
            }
        }
        setUpWasteAdapter(listFilterWaste)
    ...
    private fun applyFilter() {
        ...
        val selectedWaste = listFilterWaste.filter { item -> item.isSelected }.map { it.value } as ArrayList
        val bundle = Bundle()
        bundle.putStringArrayList(FILTER_MATERIAL, selectedMaterial)
        bundle.putStringArrayList(FILTER_HUMIDITY, selectedHumidity)
        bundle.putStringArrayList(FILTER_SMOKY, selectedImpurity)
        bundle.putStringArrayList(FILTER_MOULD, selectedMould)
        bundle.putStringArrayList(FILTER_WASTE, selectedWaste)
    ...
    private fun updateCheckedAll() {
        ...
        binding.cbHumidity.setOnCheckedChangeListener { buttonView, isChecked ->
            if (isChecked) {
            ...
            binding.rvHumidity.adapter?.notifyDataSetChanged()
        }
        ...
        binding.cbWaste.setOnCheckedChangeListener { buttonView, isChecked ->
            if (isChecked) {
                listFilterWaste.forEach { it.isSelected = true }
            } else {
                listFilterWaste.forEach { it.isSelected = false }
            }
            binding.rvWaste.adapter?.notifyDataSetChanged()
        }

    }

    private fun setUpMaterialAdapter(listFilterMaterial: ArrayList<FilterList>) {
        binding.rvMaterial.setUp(listFilterMaterial, R.layout.item_vega_cameroon_inventory_filter, { it, pos ->
    ...
    private fun setUpHumidityAdapter(listFilterHumidity: ArrayList<FilterList>) {
        binding.rvHumidity.setUp(listFilterHumidity, R.layout.item_vega_cameroon_inventory_filter, { it, pos ->
    ...
    private fun setUpImpurityAdapter(listFilterImpurity: ArrayList<FilterList>) {
        binding.rvTotalImpurity.setUp(listFilterImpurity, R.layout.item_vega_cameroon_inventory_filter, { it, pos ->
    ...
    private fun setUpMouldAdapter(listFilterMould: ArrayList<FilterList>) {
        binding.rvMould.setUp(listFilterMould, R.layout.item_vega_cameroon_inventory_filter, { it, pos ->
    ...
    private fun setUpWasteAdapter(listFilterWaste: ArrayList<FilterList>) {
        binding.rvWaste.setUp(listFilterWaste, R.layout.item_vega_cameroon_inventory_filter, { it, pos ->

            tvInventoryFilter.text = it.value
            tvInventoryFilter.setOnClickListener { view ->
                it.isSelected = !it.isSelected
                listFilterWaste[pos].isSelected = it.isSelected
                binding.rvWaste.adapter?.notifyDataSetChanged()
            }
            ViewCompat.setBackground(
                tvInventoryFilter,
