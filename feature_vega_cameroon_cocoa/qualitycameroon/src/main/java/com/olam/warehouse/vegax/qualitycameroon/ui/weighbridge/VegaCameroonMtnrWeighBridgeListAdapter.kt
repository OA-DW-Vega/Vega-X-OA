...
import com.olam.warehouse.vegax.qualitycameroon.databinding.ItemVegaCameroonMtnrQualityWeighBridgeDetailsBinding
...
class VegaCameroonMtnrWeighBridgeListAdapter(private val onClick: (VegaQualityWBDetails?) -> Unit) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WeighBridgeViewHolder {
        ...
        val v = ItemVegaCameroonMtnrQualityWeighBridgeDetailsBinding.inflate(
...
//        })
    ...
    inner class WeighBridgeViewHolder(itemView: ItemVegaCameroonMtnrQualityWeighBridgeDetailsBinding) :
                ...
                when (it.unitsOfMeasure) {
                    "KG" -> {
                        binding.tvWeight.text =
                            it.netWeight.toString().toDouble().formatThreeDigits()
                                .plus(it.unitsOfMeasure.toString())
                    }
                    "MT" -> {
                        binding.tvWeight.text = convertMtToKg(it.netWeight.toString()).toDouble()
                            .formatThreeDigits()
                            .plus(" KG")
                    }
                }
