...
import kotlinx.android.synthetic.main.item_vega_cameroon_quality_weigh_bridge_details.view.*
        ...
        fun bindItems(weighbridge: VegaQualityWBDetails?) {
            weighbridge?.let {
                if (it.weighBridgeType == PROCURE) {
                    itemView.tvdifference.text = "GRN Status"
                    if (!it.netWeight.equals("0.000"))
                        itemView.tvSupplierName.text = "Complete"
                    else
                        itemView.tvSupplierName.text = "Pending"
