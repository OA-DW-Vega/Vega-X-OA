...
import com.olam.warehouse.vegax.qualitycameroon.databinding.ItemVegaCameroonLotCardLayoutBinding
...
class VegaCameroonQualityMtnrWBLotListFragment : BaseFragment() {
    ...
    private fun setAdapter(mQualityLotList1: MutableList<VegaCoffeeLot>) {
        ...
        binding.rvLots.setUpAdapter(
            mQualityLotList1,
            R.layout.item_vega_cameroon_lot_card_layout,
            ItemVegaCameroonLotCardLayoutBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvScaleLotValue.text = it.batchNumber
                when (it.unitsOfMeasure) {
                    "KG" -> {
                        bindItem.tvScaleWeightValue.text =
                            it.netWeight?.toDouble()?.formatThreeDigits().plus(" ")
