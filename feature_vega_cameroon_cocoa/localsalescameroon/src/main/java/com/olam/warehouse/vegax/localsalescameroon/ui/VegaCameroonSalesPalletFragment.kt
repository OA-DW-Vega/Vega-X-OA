...
import com.olam.warehouse.vegax.localsalescameroon.databinding.FragmentSalesCameroonAddPalletBinding
import com.olam.warehouse.vegax.localsalescameroon.databinding.ItemCameroonSalesPalletBagInfoBinding
import com.olam.warehouse.vegax.localsalescameroon.utils.ADD_WEIGHT
import com.olam.warehouse.vegax.localsalescameroon.utils.LOT_DETAIL
import com.olam.warehouse.vegax.localsalescameroon.utils.getColor
import com.olam.warehouse.vegax.localsalescameroon.utils.prepareCocoaBagMaterial
 ...
 */
class VegaCameroonSalesPalletFragment : BaseFragment() {

    override val layoutResourceId: Int = R.layout.fragment_sales_cameroon_add_pallet
    private lateinit var binding: FragmentSalesCameroonAddPalletBinding
    ...
    companion object {
        fun newInstance(item: VegaCoffeeSalesLots) = VegaCameroonSalesPalletFragment().putArgs {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("localsalescameroon/ui/VegaCameroonSalesPalletFragment")
    ...
    override fun onCreateView(
        ...
        binding = FragmentSalesCameroonAddPalletBinding.inflate(inflater)
        initUI()
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        defaultLot =
            arguments?.getParcelable<VegaCoffeeSalesLots>(LOT_DETAIL) as VegaCoffeeSalesLots
        binding.tvTitle.text =
            getString(com.olam.warehouse.login.R.string.local_sales).plus(" - ")
                .plus(getString(R.string.weighscale))

        // vm.bagItems.observe(viewLifecycleOwner, Observer { updateBagItems(it) })
        // vm.getBagItems(defaultLot.batchNumber, defaultLot.materialCode, defaultLot.salesTempId)
        vm.getSalesPalletBag(defaultLot.batchNumber)
        vm.salesBagWithMaterial.observe(viewLifecycleOwner, Observer {
            updateBagItems(it)
        })
    ...
    private fun updatePalletUI(response: Resource<GenericReqAndResp<List<VegaCameroonSalesPallet>>>?) {
                ...
                if (response.data?.data?.size!! > 0) {
                    ...
                    palletWeight =
                        if (response.data?.data?.get(0)?.totalPalletWeight == "0.0") "0" else response.data?.data?.get(0)?.totalPalletWeight
                            ?: "0"
                    palletAvg =
                        if (!palletCount.equals("0")) palletWeight.toDouble().div(palletCount.toInt())
                            .formatThreeDigits()
                            .replace(
                                ",",
                                ""
    ...
    private fun updateBagItems(bagSalesItems: VegaCameroonSalesWithBagItems) {
        bagSalesItems.let {
            val bagItems = bagSalesItems.bagMaterialList
            palletWeight = bagSalesItems.salesLots.palletWeight ?: ""
            palletCount = bagSalesItems.salesLots.palletCount ?: ""
            palletAvg = bagSalesItems.salesLots.palletAvg ?: ""
            ...
            if (bagItems.size > 0) {
                ...
                else enableSave(bagList.size > 0)
                if (palletCount.isNotEmpty()) {
                    if (bagList.size == palletCount.toInt() || palletCount.equals("0")) {
...
//    }
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCoffeeSalesBagMaterial>) {
        ...
        binding.rvWeight.setUpAdapter(
            bagList1.asReversed(),
            R.layout.item_cameroon_sales_pallet_bag_info,
            ItemCameroonSalesPalletBagInfoBinding::inflate,
            { it, pos, bindItem ->
                if (pos % 2 == 0) {
    ...
    fun updatePalletDetails(noOfPallet: String, palletWeight: String) {
        if (noOfPallet.isNotEmpty() && palletWeight.isNotEmpty() && !palletWeight.equals("0")) {
            ...
            palletAvg =
                palletWeight.toDouble().div(noOfPallet.toInt()).formatThreeDigits().replace(",", "")
