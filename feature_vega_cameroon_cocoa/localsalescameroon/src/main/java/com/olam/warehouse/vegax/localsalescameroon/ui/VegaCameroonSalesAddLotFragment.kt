...
import kotlinx.android.synthetic.main.item_cameroon_sales_lot.view.*
import kotlinx.android.synthetic.main.item_cameroon_sales_material_list.view.*
import kotlinx.android.synthetic.main.item_cameroon_sales_material_list.view.tvMaterial
...
class VegaCameroonSalesAddLotFragment : BaseFragment(), CameroonRecyclerViewItemClickListener {
        ...
        response.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    hideLoading()
                    salesOrderList = it.data?.data as MutableList<VegaCameroonSalesOrderModel>
                    soNumbers.clear()
    ...
    private fun showSingleSelectDialog(isWh: Boolean, title: String) {
        customDialog =
            CustomCameroonSingleSelectDialog(title, isWh, soNumbers, activity!!, this)
        customDialog?.show()
            ...
            when (lot.salesType) {
                SALES_TYPE_WEIGHSCALE -> {
                    ...
                    when(lot.unitOfMeasure){
                        ...
                        "MT" -> {
                            tvScaleDispatchValue.text =
                                bagList.sumByDouble { it.netWeight.toDouble() }.formatThreeDigits()
                                    .plus(" ").plus("KG")
                            tvScaleWeightValue.text =
                                convertMtToKg(lot.weight.toString()).toDouble().formatThreeDigits()
                                    .plus(" ").plus("KG")
                        }
                    }
