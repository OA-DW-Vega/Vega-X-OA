...
class VegaGhanaMtnrQualityWBListFragment : BaseFragment() {

    private var mAdapter = VegaGhanaMtnrWeighBridgeListAdapter { moveBagdetail(it) }
    ...
    companion object {
        fun newInstance() = VegaGhanaMtnrQualityWBListFragment().putArgs {
        }
    ...
    private fun updateUIWithOnlineData(response: Resource<GenericReqAndResp<List<VegaQualityWBDetails>>>) {
        when (response.status) {
            Resource.Status.SUCCESS -> {
                ...
                weighTypedata?.forEach {
                    it.netWeight = it.netWeight?.toDouble()?.formatThreeDigits()
                }
    ...
    private fun updateUI(response: List<VegaQualityWBDetails>?) {
        ...
        response?.let { qualityDetail ->
            if (qualityDetail.isNotEmpty()) {
                val offlineData =
                    qualityDetail.filter { value -> value.isNotWBID }.filter { it.wbTempId.contains("TMP") }
