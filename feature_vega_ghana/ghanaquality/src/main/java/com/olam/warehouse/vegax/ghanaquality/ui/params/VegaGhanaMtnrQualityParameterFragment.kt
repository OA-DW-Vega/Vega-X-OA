...
import com.olam.warehouse.vegax.ghanaquality.ui.params.VegaGhanaMtnrQualityParamsAdapter
...
class VegaGhanaMtnrQualityParameterFragment : BaseFragment() {

    private var offlineData: List<VegaGhanaLotQualityDetails> =
        ArrayList<VegaGhanaLotQualityDetails>()
    ...
    private var mAdapter = VegaGhanaMtnrQualityParamsAdapter { enableProceedBtn(it) }
    ...
    companion object {
        fun newInstance() = VegaGhanaMtnrQualityParameterFragment().putArgs {}
    }
    ...
    private fun initExtra() {
        ...
        //getQualityParams to getMtnrQualityParams
        if (isData!!)
            materialNo?.let {
                    ...
                    if (!isData!!) {
                        it.forEach { item ->
                            if (item.qualityParameter.vegaMandatory.equals("X") || item.qualityParameter.entryObligatory.equals(
                                ...
                                {
                                    if( (!item.qualityParameter.nameChar.equals("GH_CASHEW_AVG_BAG_WT")) && (!item.qualityParameter.nameChar.equals("DO_TRANSACTION_NO")) && (!item.qualityParameter.nameChar.equals("LOBM_UDCODE")) )
                                     value.add(item)
                            }
            ...
            UIUtils.getMetirialCustomView(
                ...
                getString(com.olam.warehouse.presentation.R.string.cancel),
                {
                 ...
                 mAdapter.getItems().forEach { qualityParameterList.add(it?.qualityParameter) }*/
                    ...
                    quality.materialCode = materialNo.toString()
...
//                    it.materialCode = "000000".plus(it.materialCode)
    ...
    private fun saveOfflinePostRequest(qualityPostList: ArrayList<VegaCoffeeLot>) {
        ...
        vm.saveVegaQualityWeightWBDetails(qualityPostList[0].weighBridgeId.toString(),batchNo.toString(),qualityPostList[0].netWeight.toString())
...
//            vm.saveMtnrLotQualityDetails(batchNo,it.materialCode,it.qualityParameterValue)
