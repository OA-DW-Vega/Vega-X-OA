package com.olam.warehouse.vegax.ghanaquality.ui.params
...
import com.olam.warehouse.login.ui.quickpinaccess.createpin.pinlockview.ResourceUtils.Companion.getDrawable
...
import com.olam.warehouse.vegax.ghanaquality.databinding.FragmentVegaGhanaQualityParamsBinding
 ...
 */
class VegaGhanaQualityParameterFragment : BaseFragment() {
    ...
    private var mAdapter = VegaGhanaQualityParamsAdapter { enableProceedBtn(it) }
    ...
    companion object {
        fun newInstance() = VegaGhanaQualityParameterFragment().putArgs {}
    }

    private lateinit var binding: FragmentVegaGhanaQualityParamsBinding
    override val layoutResourceId = R.layout.fragment_vega_ghana_quality_params
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentVegaGhanaQualityParamsBinding.inflate(layoutInflater)
    ...
    private fun initUI() {
        ...
        binding.btnReject.setOnClickListener {
            weighBridgeDetails.finalApproval = FNREJECT
    ...
    private fun initExtra() {
        ...
        if (bagCount != "0") {
            averageBagWeight = weighBridgeDetails.grossWeight?.toDouble()!! / bagCount?.toDouble()!!
        }
    ...
    private fun proceedToPost(finalApproval: String, msg: Int) {
        ...
        if (isValueNeed) {
            qualityParameterList.add(usageDecision)
        ...
        params?.let {
            when (it.isNotEmpty()) {
                true -> {
                    ...
                    if (!isData!!) {
                        it.forEach { item ->
                            if (item.qualityParameter.vegaMandatory.equals("X") || item.qualityParameter.entryObligatory.equals(
                                ...
                                if (!item.qualityParameter.nameChar.equals("GH_CASHEW_AVG_BAG_WT"))
                                    value.add(item)
                            }
                    ...
                    binding.tvBagName.text = "Avg Bag Wt."
                    if (bagCount != "0") {
                        binding.edBagWtValue.isEnabled = false
                        binding.edBagWtValue.isFocusable = false
                        binding.edBagWtValue.setText(averageBagWeight.formatThreeDigits())
                    } else {
                        binding.edBagWtValue.isEnabled = true
                        binding.edBagWtValue.isFocusable = true
                        binding.llEdit.background =
                            getDrawable(context, com.olam.warehouse.presentation.R.drawable.do_custom_edit_text)
                    }
                }
        ...
        MaterialDialog(requireContext()).show {
            ...
            UIUtils.getMetirialCustomView(
                ...
                getString(com.olam.warehouse.presentation.R.string.cancel),
                {
                    ...
                    quality.qualityParameterValue = binding.edBagWtValue.text.toString()
