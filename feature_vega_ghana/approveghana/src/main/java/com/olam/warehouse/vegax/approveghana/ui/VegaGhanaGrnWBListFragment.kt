...
import com.olam.warehouse.vegax.approveghana.databinding.FragmentVegaGhanaGrnWeighbridgeListBinding
import com.olam.warehouse.vegax.approveghana.databinding.ItemVegaGhanaWeighbridgeGrnBinding
 ...
 */
class VegaGhanaGrnWBListFragment : BaseFragment() {
    ...
    private lateinit var binding: FragmentVegaGhanaGrnWeighbridgeListBinding
    private var callBack: CallBack? = null
    override val layoutResourceId = R.layout.fragment_vega_ghana_grn_weighbridge_list
    private var offlineDataList = mutableListOf<VegaGrnWeighBridgeId>()
    private var transactionId = ""

    companion object {
        fun newInstance(transId: String) = VegaGhanaGrnWBListFragment().putArgs {
            putString(TRANSACTIONID, transId)
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaGhanaGrnWeighbridgeListBinding.inflate(layoutInflater)
        return binding.root
    }
...
//            binding.llQualityOffline.visible()
    ...
    private fun setUpAdapter(data: List<VegaGrnWeighBridgeId>?) {
        var data1 = data
        if(transactionId.isNotEmpty() && data?.any { it.weighBridgeId?.contains(transactionId.takeLast(8), true) == true } == true) {
            data1 = data.filter { it.weighBridgeId?.contains(transactionId.takeLast(8), true) == true }
        }
        val weighBridgeList1 = data1 as MutableList<VegaGrnWeighBridgeId>
        var count = 0
        binding.rvWeighBridgeId.setUpAdapter(
            weighBridgeList1.asReversed(),
            R.layout.item_vega_ghana_weighbridge_grn,
            ItemVegaGhanaWeighbridgeGrnBinding::inflate,
            { it, pos, bindItem ->
                it.unitPrice = it.unitPrice.toString().replace("\\s".toRegex(), "")
                bindItem.tvProcurementType.text = it.materialName
