...
class VegaGhanaGrnDetailsFragment : BaseFragment() {
    ...
    private var isQualityApproved: Boolean = false
    ...
    companion object {
        fun newInstance(grnData: VegaGrnWeighBridgeId, isQualityApproved: Boolean) =
            VegaGhanaGrnDetailsFragment().putArgs {
                putParcelable(GRN_DATA, grnData)
                putBoolean(QUALITY_DATA, isQualityApproved)
            }
    }
    ...
    private fun initUI() {
        ...
        isQualityApproved = arguments?.getBoolean(QUALITY_DATA, false)!!
        ...
        if (isQualityApproved) {
            binding.llItemBottomView.visible()
...
//            binding.etPrice.isEnabled = false
        ...
        if (PreferenceHelper.get(Constants.USER_ROLES, "").toString().contains("ROLE_GRN")) {
            binding.llPriceDetails.gone()
...
//        binding.btQualityDetails.isEnabled = false
        ...
        binding.btnReject.setOnClickListener {
            if (wbDetails.purchaseDocNum.isNullOrEmpty() && !wbDetails.purchaseDocNum.equals("0.00")) {
    ...
    private fun updateConfigItems(configItems: List<VegaConfigDetails>) {
        ...
        defaultStorageLoc.forEach {
            if (currentMaterial.contains(it.materialCode) && !it.materialCode.isEmpty() && !isExist && isMaterial && it.applicable?.contains(
                    "Y"
                )!!
            ) {
        ...
        when (response?.status) {
            Resource.Status.SUCCESS -> {
                ...
                response.data?.data?.let {
                    binding.tvUnithead.text = it.department
                    unitHead= it.department.toString()
                }
            ...
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    ...
                    if (it.data?.data?.grnNumber.isNullOrEmpty()) {
                        moveToFailurePage(it.data?.data?.message.toString(), it.data?.data?.weighBridgeId.toString())
                    } else
                        moveToSuccessPage(
            ...
            storageLocation?.forEach { item ->
                if (item.procureLocationCode.contains(wbDetails.storageLocationCode.toString(), true)) {
                    binding.tvStorageLocation.text =
                        wbDetails.storageLocationCode.plus("-")
                            .plus(item.procureLocationName.toString())
                }
            }
        })
        ...
        MaterialDialog(requireContext()).show {
            ...
            UIUtils.getMetirialCustomView(
                ...
                getString(com.olam.warehouse.presentation.R.string.cancel),
                {
                    ...
                    wbDetails.finalApproval = FNREJECT
//                wbDetails.storageLocationCode = binding.tvStorageLocation.text.toString()
                    val wbData = preparePostGrnData(wbDetails, admixtureValue, qualityDetails,unitHead)
                    if (isOnline()) {
                        vm.postGrn(
                            VegaGhanaGrnPost(
                                key = getCurrentKey(),
        ...
        MaterialDialog(requireContext()).show {
            ...
            UIUtils.getMetirialCustomView(
                ...
                getString(com.olam.warehouse.presentation.R.string.cancel),
                {
                    ...
                    wbDetails.finalApproval = FNQUALITY
//                wbDetails.storageLocationCode = binding.tvStorageLocation.text.toString()
                    val wbData = preparePostGrnData(wbDetails, admixtureValue, qualityDetails,unitHead)
                    if (isOnline()) {
                        vm.postGrn(
...
//        intent.putExtra("fromsesamegrn", true)
    ...
    private fun moveToFailurePage(messg: String, wbId: String) {
        val intent = Intent(requireContext(), SuccessActivity::class.java)
        if (isOnline()) {
            intent.putExtra(AppUtils.TITLE, messg)
        } else
            intent.putExtra(AppUtils.TITLE, getString(R.string.grn_success_offline))
        intent.putExtra(AppUtils.SUB_TITLE, "WB ID : ".plus(wbId))
        intent.putExtra(AppUtils.MSG, messg)
        startActivity(intent)
        requireActivity().finish()
    }
}
