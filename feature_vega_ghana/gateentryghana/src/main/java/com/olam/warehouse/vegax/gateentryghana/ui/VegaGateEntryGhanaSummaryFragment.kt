...
import com.olam.warehouse.vegax.gateentryghana.databinding.FragmentGateEntryGhanaSummaryBinding
import com.olam.warehouse.vegax.gateentryghana.utils.GATE_ENTRY_DATA
 ...
 */
class VegaGateEntryGhanaSummaryFragment : BaseFragment() {
    ...
    private lateinit var binding: FragmentGateEntryGhanaSummaryBinding
    override val layoutResourceId = R.layout.fragment_gate_entry_ghana_summary

    companion object {
        fun newInstance(gateEntryData: VegaGateEntry) =
            VegaGateEntryGhanaSummaryFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentGateEntryGhanaSummaryBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun updateUI(data: Resource<GenericReqAndResp<VegaGateEntryGhanaResponse>>?) {
        data?.let {
            when (it.status) {
    ...
    private fun postGateEntry() {
        ...
        if (AppUtils.isOnline()) {
            vm.postGateEntryData(VegaGateEntryGhanaPost(getCurrentKey(), getPlantDetails(), postData))
        }
    }
