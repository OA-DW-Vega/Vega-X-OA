...
import com.olam.warehouse.master.vegaghana.entity.VegaGhanaCashewFGRNHistoryTranxResponse
import com.olam.warehouse.master.vegaghana.entity.VegaGhanaCashewGRNHistoryTransactions
import com.olam.warehouse.master.vegaghana.entity.VegaGhanaCashewGRNHistoryTranxResponse
...
import com.olam.wharhouse.vegax.transhistoryghanacashew.databinding.FragmentHistoryTransactionsGhanaCashewFgrnListBinding
import com.olam.wharhouse.vegax.transhistoryghanacashew.databinding.ItemHistoryTransactionsGhanaCashewFgrnBinding
...
import com.olam.wharhouse.vegax.transhistoryghanacashew.utils.HISTORY_TRANSACTIONS_LOT_FGRN
import com.olam.wharhouse.vegax.transhistoryghanacashew.utils.HISTORY_TRANSACTIONS_LOT_GRN
...
class VegaHistoryGhanaCashewFgrnListFragment : BaseFragment() {

    override val layoutResourceId: Int = R.layout.fragment_history_transactions_ghana_cashew_fgrn_list
    private lateinit var binding: FragmentHistoryTransactionsGhanaCashewFgrnListBinding
...
by viewModel()
    ...
    private var history = mutableListOf<VegaGhanaCashewFGRNHistoryTransactions>()
    private var filteredHistory = mutableListOf<VegaGhanaCashewFGRNHistoryTransactions>()
    private var historyData = VegaGhanaCashewFGRNHistoryTransactions()
    private val mSearchList = mutableListOf<VegaGhanaCashewFGRNHistoryTransactions>()
    private var vegaWbIds = listOf<VegaGhanaCashewFGRNHistoryTransactions>()
    private var vegaWbsortedIds = listOf<VegaGhanaCashewFGRNHistoryTransactions>()
    private var sortList = VegaGhanaCashewFGRNHistoryTransactions()
    ...
    companion object {
        fun newInstance(
            historyTransactions: VegaGhanaCashewFGRNHistoryTransactions
        ) = VegaHistoryGhanaCashewFgrnListFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentHistoryTransactionsGhanaCashewFgrnListBinding.inflate(inflater)
        return binding.root
    }
    ...
    private fun updateUIWithOnlineData(response: Resource<GenericReqAndResp<VegaGhanaCashewFGRNHistoryTranxResponse>>) {
        response.let {
            when (it.status) {
                    ...
                    response.data?.data?.fgrnTxn.let { it1 ->
                        if (it1?.isNotEmpty() == true) {
                            ...
                            if (vegaWbsortedIds.size>0) {
                                ...
                                vegaWbIds =
                                    vegaWbsortedIds.sortedWith(compareBy({ it.quantity.toString() })) as MutableList<VegaGhanaCashewFGRNHistoryTransactions>
                                ...
                                history = vegaWbIds as MutableList<VegaGhanaCashewFGRNHistoryTransactions>
                            //getWarehouseList()
    ...
    /* private fun getWarehouseList() {
         if (history.isNotEmpty()) {
             wareHouseList.clear()
             wareHouseList.addAll(history.listOfField(VegaGhanaCashewFGRNHistoryTransactions::storageLocationCode).toSet())
    ...
    private fun setUpAdapter(data: List<VegaGhanaCashewFGRNHistoryTransactions>) {
        val history1 = data as MutableList<VegaGhanaCashewFGRNHistoryTransactions>
        binding.rvLots.setUpAdapter(
            history1,
            R.layout.item_history_transactions_ghana_cashew_fgrn,
            ItemHistoryTransactionsGhanaCashewFgrnBinding::inflate,
            { it1, pos, bindItem ->
