 ...
 */
class VegaGhanaOffloadingViewModel(
    private val useCase: VegaGhanaOffloadingUseCase,
    ...
    fun getSavedBagItems() =
        viewModelScope.launch(dispatchers.main) {
            ...
            withContext(dispatchers.io) {
                bagSourceSaved = useCase.getSavedBagItems()
            }
    ...
    fun updateSyncedMtnrDeletedItem() = viewModelScope.launch(dispatchers.main) {
        withContext(dispatchers.io) {
            useCase.updateSyncedMtnrDeletedItem()
        }
    }
    ...
    fun deleteStatus(mtnNumber: String) = viewModelScope.launch {
        withContext(dispatchers.io) {
            useCase.deleteStatus(mtnNumber)
        }
    }
    ...
    fun deleteMtnrQuality(wbId: String) = viewModelScope.launch {
        withContext(dispatchers.io) {
            useCase.deleteMtnrQuality(wbId)
        }
    }
    ...
    fun updateOBD(mtnNumber: String, flag: Boolean) =
        viewModelScope.launch {
            withContext(dispatchers.io) {
                useCase.updateOBD(mtnNumber, flag)
            }
        }
