...
class VegaGhanaMtnrWeighScaleSummaryFragment : BaseFragment() {
    ...
    companion object {
        fun newInstance(data: VegaCoffeeReceiving) =
            VegaGhanaMtnrWeighScaleSummaryFragment()
                .putArgs {
    ...
    private fun moveToOfflineSuccessPage() {
        ...
        batchList.forEach { item ->
            ...
            vegaCoffeeReceivingData.bagCount =
                bagItems.sumBy { tar -> tar.bagCount.toInt() }.toString().trim()
        }
        vm.saveMtnrReceivingLots(vegaCoffeeReceivingData, VegaCoffeeReceiveLots())
    ...
    private fun prepareDeliveryList(): List<VegaGhanaOffloadingDeliveryDetail> {
        ...
        for (item in batchList) {
            ...
            deliveryDetail.transitLoss = ((item.weight?.toDouble()?:0.0).minus(item.editedWeight?.trim()?.toDouble() ?: 0.0)).toString()
