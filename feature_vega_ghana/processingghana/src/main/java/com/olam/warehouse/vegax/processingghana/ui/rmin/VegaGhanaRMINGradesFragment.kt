...
import kotlinx.android.synthetic.main.item_vega_ghana_fgrn_grades.view.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaGhanaRMINGradesFragment : BaseFragment() {
    ...
    private var poDetailsList = mutableListOf<VegaCoffeeFgrnItemsGrades>()
    ...
    companion object {
        fun newInstance(model: VegaCoffeeRminProcessing, id: String) = VegaGhanaRMINGradesFragment().putArgs {
    ...
    private fun initUI() {
        ...
        if (isOnline()) {
            ...
            vm.getProcessOrderDetails(fgrnItem.poNumber)
        }
        vm.processOrderDetailsItemLocal.observe(viewLifecycleOwner, Observer {
            var tempList = it.filter { it.bwart == "261" }
            tempList.forEach {
    ...
    private fun validateGrade() {
        when (gradeList.any { it.isGradeChecked!! }) {
            true -> {
                ...
                callBack?.replaceFragment(ADDLOT, false, fgrnItem, false)
            }
    ...
    private fun updateAdapter(gradeList: MutableList<VegaCoffeeFgrnItemsGrades>) {
        ...
        binding.rvGrades.setUp(gradeList, R.layout.item_vega_ghana_fgrn_grades, { item, pos ->
