...
import com.olam.warehouse.vegax.processingghana.databinding.ItemProcessingGhanaBomLayoutBinding
...
class VegaProcessingGhnaRminPoBomSelectFragment : BaseFragment() {
    ...
    private fun initExtra() {
        ...
        atSourceVal = model?.atSourceVal.toString()
    }
            ...
            if (AppUtils.isOnline()) {
                ...
                vm.processOrderItemLocal.observe(viewLifecycleOwner, Observer {
                    customLocationList.forEach { item ->
                        var tempList =
                                it.filter { item.procureLocationCode == it.storageLocationCode!! }.filter { it.materialCode.contains(model?.materialCode.toString()) == true }
                        tempList.forEach {
                    ...
                    when (it.data?.success) {
                        true -> {
                            response.data?.data?.let {
                                var poList1 = ArrayList<VegaFgrnProcessingOrder>()
                                it.forEach { item ->
                                    val rmindata =
                                ...
                                customLocationList.forEach { item ->
                                    val tempList = poList1.filter { item.procureLocationCode == it.storageLocationCode!! }
    ...
    private fun updateDetails(poList: List<VegaFgrnProcessingOrder>) {
        ...
        if (poList.size == 1) {
            ...
            modelcoffee?.poNumber = poList[0].processOrderNo
            modelcoffee?.baseMaterialCode = poList[0].materialCode
            modelcoffee?.baseMaterialName = poList[0].materialName
            modelcoffee?.stage = model?.stage!!
            modelcoffee?.vendor = poList[0].vendor
            modelcoffee?.atSourceVal = atSourceVal
    ...
    private fun updateBomList(boms: List<VegaProcessingRminBoms>?) {
        ...
        if (boms?.size == 1) {
            ...
            modelcoffee?.atSourceVal = atSourceVal
            if (AppUtils.isOnline())
                modelcoffee?.materialCode = bomList1[0].baseMaterialCode.toString()
            ...
            binding.rvPoList.setUpAdapter(
                bomList,
                R.layout.item_processing_ghana_bom_layout,
                ItemProcessingGhanaBomLayoutBinding::inflate,
                { it, pos, bindItem ->
                    if (pos % 2 == 0) {
