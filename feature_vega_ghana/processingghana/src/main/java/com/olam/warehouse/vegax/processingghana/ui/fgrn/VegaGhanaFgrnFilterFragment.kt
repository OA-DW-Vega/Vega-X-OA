...
import com.olam.warehouse.vegax.processingghana.data.domain.model.GhanaFilterList
import com.olam.warehouse.vegax.processingghana.databinding.FragmentVegaGhanaFgrnFilterBinding
import com.olam.warehouse.vegax.processingghana.databinding.ItemVegaGhanaFgrnInventoryFilterBinding
import com.olam.warehouse.vegax.processingghana.utils.*
import java.util.*

class VegaGhanaFgrnFilterFragment : BaseFragment() {

    private lateinit var binding: FragmentVegaGhanaFgrnFilterBinding
    override val layoutResourceId = R.layout.fragment_vega_ghana_fgrn_filter
    private var callBack: CallBack? = null
    private var fullFilter = ArrayList<String>()
    private var listFliterWhLoc = ArrayList<GhanaFilterList>()
    private var startRange: String = ""
    private var endRange: String = ""
    private var aboveThirty: Boolean = false

    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaGhanaFgrnFilterFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentVegaGhanaFgrnFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        for (item in whLocations.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFliterWhLoc.add(GhanaFilterList(item, true))
                else listFliterWhLoc.add(GhanaFilterList(item))
            }
        }
    ...
    private fun setUpAdapter(listFliterWhLoc: ArrayList<GhanaFilterList>) {
        binding.rvWhLocation.setUpAdapter(
            listFliterWhLoc,
            R.layout.item_vega_ghana_fgrn_inventory_filter,
            ItemVegaGhanaFgrnInventoryFilterBinding::inflate,
            { it, pos, bindItem ->
