...
//            binding.icLotCard.tvWeightValue.text = weight
    ...
    private fun validateGrades() {
        val netWeight = if(binding.etWeight.text?.isNotEmpty() == true)binding.etWeight.text.toString().toDouble() else 0.0
        val poQuantity = if(binding.tvAvailablePoQtyValue.text?.isNotEmpty() == true)binding.tvAvailablePoQtyValue.text.toString().toDouble() else 0.0
...
//                binding.rvWeight.adapter?.notifyDataSetChanged()
    ...
    fun updateBagWeight(bagMaterial: VegaCoffeeFgrnGradesMatrialWeights) {
        ...
        if (!isExistValue) {
            ...
            bagList[pos].netWeight = binding.etWeight.text.toString()
            bagList[pos].bagType = binding.tvSelectType.text.toString()
            bagList[pos].bagCount = binding.etNoBags.text.toString()
