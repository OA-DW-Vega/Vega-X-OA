...
import com.olam.warehouse.vegax.processingghana.databinding.ItemVegaGhanaFgrnLotBinding
...
//        var batchidlogic = position1.plus(position3).plus(position4).plus(position5).plus(productTypeCode.takeLast(1))
    ...
    private fun updateAdapter(stockList: MutableList<VegaCoffeeRminLots>) {
        ...
        binding.rvInventoryLot.setUpAdapter(
            stockList,
            R.layout.item_vega_ghana_fgrn_lot,
            ItemVegaGhanaFgrnLotBinding::inflate,
            { it, pos, bindItem ->
                bindItem.ivDelete.gone()
                bindItem.tvLotNoValue.text = it.batchNumber
                bindItem.tvWeightValue.text = it.weight.plus(" ").plus("MT")
