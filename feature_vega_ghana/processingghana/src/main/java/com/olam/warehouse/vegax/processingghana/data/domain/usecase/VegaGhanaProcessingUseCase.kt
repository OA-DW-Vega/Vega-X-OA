...
class VegaGhanaProcessingUseCase(val repository: VegaGhanaProcessingRepository) {
    ...
    suspend fun getMaterialtypelist() = repository.getMaterialtypelist()
    ...
    suspend fun getAllFGRNItems(fgrnItem: VegaCoffeeFgrnItems) = repository.getAllFGRNItems(fgrnItem)
    ...
    suspend fun getProcessOrders(type: String): LiveData<List<VegaGhanaProcessingOrder>> =
            repository.getProcessOrders(type)
