...
class VegaGhanaFgrnViewModel(
    ...
    fun getAllFGRNItems(fgrnItem: VegaCoffeeFgrnItems) = viewModelScope.launch(dispatchers.main) {
        _bagList.removeSource(bagListSource)
        withContext(dispatchers.io) {
            bagListSource = useCase.getAllFGRNItems(fgrnItem)
        }
    ...
    fun getProcessOrders() = viewModelScope.launch(dispatchers.main) {
        ...
        withContext(dispatchers.io) {
            processOrderSource = useCase.getProcessOrders("OP")
        }
