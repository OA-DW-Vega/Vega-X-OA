...
import com.olam.warehouse.vegax.processingghana.databinding.FragmentGhanaRminAddLotBinding
import com.olam.warehouse.vegax.processingghana.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel

class VegaGhanaRMINAddLotFragment : BaseFragment(), GhanaRminItemRemoveListener,
    GhanaRminAddWeightListener {

    override val layoutResourceId = R.layout.fragment_ghana_rmin_add_lot
    private lateinit var binding: FragmentGhanaRminAddLotBinding
    ...
    private var adapter: VegaGhanaRminLotAdapter? = null
    ...
    companion object {
        fun newInstance(
            ...
            VegaGhanaRMINAddLotFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentGhanaRminAddLotBinding.inflate(layoutInflater)
        initExtra()
        initAdapter()
        initUI()
        return binding.root
    ...
    private fun initUI() {
        ...
        if (isPoSelection!!)
            binding.cvCard.tvMaterialValue.text = model?.baseMaterialName
        else
        ...
        if (model?.poNumber?.isNotEmpty()!!) {
            ...
            binding.cvCard.tvPoNoValue.gone()
            binding.cvCard.tvPoNo.gone()
        }

        binding.btnAddLot.setOnClickListener {
            if (
                currentMaterial.isNullOrEmpty() && !AppUtils.isOnline() && isPoSelection!!) {
                currentMaterial = model!!.materialCode
                navigateToLotList()
            } else
                if (currentMaterial.isNullOrEmpty()) {
                    currentMaterial = model!!.materialCode
                    navigateToLotList()
                } else
                    navigateToLotList()
                    ...
                    if (validateLotWeight()) {
                        ...
                        if (!isPoSelection!!) {
                                ...
                                model?.weightToProcess =
                                    binding.cvCard.tvProceesValue.text.toString()
                                // model?.p = binding.tvPoQuantity.text.toString()
                            }
...
//        if (model?.poNumber?.isNotEmpty()!!) vm.getPoGrades(model?.poNumber.toString(), isRmin)
    ...
    private fun moveToShiftSelect() {
        callBack?.replaceSummaryFragment(SUMMARY, isMultipleLot, model!!, isPoSelection!!)
    }
    ...
    private fun validateEmptyWeight(): Boolean {
        val emptyWeight = vm.lotList.filter { it.editedWeight.isNullOrEmpty() || it.editedWeight?.toDouble()==0.0}
    ...
    override fun calculateWtp() {
        weightToProcess = 0.0
        vm.lotList.forEach {
            if(it.editedWeight.isNullOrEmpty()){
                weightToProcess = 0.0
            }
            else{
        ...
        binding.cvCard.tvProceesValue.text =
            weightToProcess.toString().toDouble().formatThreeDigits().plus(" ")
                .plus(if (vm.lotList.size > 0) "MT" else "MT")
    }
    ...
    override fun updateAddWeightCal(weight: String) {2
        var come : Int?
        val split = weight.split(" ")
        adapter?.updateAddWeight(addWeightPosition, split[0])
...
//        vm.lotList[addWeightPosition].weightToDispatchUOM = split[1]
        ...
        vm.lotList[addWeightPosition].isLowerWeight = come <= 0
