...
//        if (model?.poNumber?.isNotEmpty()!!) vm.getPoGrades(model?.poNumber.toString(), isRmin)
    ...
    private fun moveToShiftSelect() {
        callBack?.replaceFragment(if (fromSummary == true) SUMMARY else SUMMARY, isMultipleLot, model!!, "", "")
    }
    ...
    private fun validateEmptyWeight(): Boolean {
        val emptyWeight = vm.lotList.filter { it.editedWeight.equals("0.0") || it.editedWeight.isNullOrEmpty() }
