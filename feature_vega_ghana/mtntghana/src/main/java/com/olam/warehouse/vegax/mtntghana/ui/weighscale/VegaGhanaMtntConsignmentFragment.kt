...
//        vm.getPurchaseOrder("")
    ...
    private fun updateConfigItems(configItems: List<VegaConfigDetails>?) {
        configItems?.forEach {
            when (it.process) {
                ...
                ConfigItems.DEFAULT_TRANSPORT_VENDOR.item -> {
                    var vendorcode: List<String> = it.value.toString().split(":")
                    binding.tvVendorValue.text = vendorcode[0]
                    binding.tvVendorNumberValue.text = vendorcode[1]
                    vendorvalue = vendorcode[1].toString()
                }
            }
        }
    ...
    private fun prepareTruckWb() {
        ...
        vm.dispatchWh.fromVendorCode = vendorvalue
...
//                purchaseOrder.map { it.warehouseId.plus("-").plus(getMTNTPlantName(it.warehouseId ?: "")) }.toSet()
            ...
            val location = purchaseOrder
                .map {
                    it.warehouseId.plus("-").plus(getMTNTPlantName(it.warehouseId ?: "")).plus("-")
                        .plus(it.storageLocationCode ?: "").plus("-").plus(it.storageLocationName ?: "")
                }.toSet()
    ...
    private fun getPurchaseOrderByStorageLocation(warehouseId: String) {
        ...
        val orderList = purchaseOrder.filter { it.warehouseId == warehouseId.trim() }
            .filter { it.storageLocationCode == selectedDestinationWH }
