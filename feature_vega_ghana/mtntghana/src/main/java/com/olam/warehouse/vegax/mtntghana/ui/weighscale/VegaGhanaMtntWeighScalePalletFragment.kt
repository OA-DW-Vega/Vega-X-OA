...
import com.olam.warehouse.vegax.mtntghana.data.domain.model.VegaGhanaMtntWeighScalePallet
import com.olam.warehouse.vegax.mtntghana.databinding.FragmentGhanaMtntWeighscaleAddPalletBinding
...
import com.olam.warehouse.vegax.mtntghana.utils.FRAG_ADD_BAG_WEIGHT
import com.olam.warehouse.vegax.mtntghana.utils.MODEL_BUNDLE
import com.olam.warehouse.vegax.mtntghana.utils.UPDATE_WEIGHT
import com.olam.warehouse.vegax.mtntghana.utils.getColor
import kotlinx.android.synthetic.main.item_vega_ghana_mtnt_add_bag.view.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import kotlin.random.Random

class VegaGhanaMtntWeighScalePalletFragment : BaseFragment() {

    override val layoutResourceId: Int =
        R.layout.fragment_ghana_mtnt_weighscale_add_pallet
    private lateinit var binding: FragmentGhanaMtntWeighscaleAddPalletBinding
    private var callBack: VegaGhanaReplaceFragmentCallback? = null
    private val vm: VegaGhanaMtntViewModel by viewModel()
    private var defaultLot = VegaGhanaCocoaDispatchLots()
    ...
    companion object {
        fun newInstance(item: VegaGhanaCocoaDispatchLots) = VegaGhanaMtntWeighScalePalletFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentGhanaMtntWeighscaleAddPalletBinding.inflate(inflater)
        initUI()
        return binding.root
    }

    private fun initUI() {
        defaultLot = arguments?.getParcelable<VegaGhanaCocoaDispatchLots>(MODEL_BUNDLE) as VegaGhanaCocoaDispatchLots
        ...
        if (AppUtils.isOnline())
            vm.getBagItems(defaultLot.batchNumber, defaultLot.materialCode, defaultLot.weighBridgeId)
        else
            vm.getBagItems(defaultLot.batchNumber, defaultLot.materialCode, defaultLot.weighBridgeId)
...
//        vm.pallet.observe(viewLifecycleOwner, Observer { updatePalletUI(it) })
    ...
    private fun updatePalletUI(response: Resource<GenericReqAndResp<List<VegaGhanaMtntWeighScalePallet>>>?) {
    ...
    fun updateBagWeight(bagMaterial: VegaCocoaSweepingBagMaterial) {
        ...
        bagMaterial.weighBridgeId = defaultLot.weighBridgeId
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCocoaSweepingBagMaterial>) {
        ...
        binding.rvWeight.setUp(bagList1.asReversed(), R.layout.item_vega_ghana_mtnt_add_bag, { it, pos ->
            if (pos % 2 == 0) {
                clBagHead.setBackgroundResource(com.olam.warehouse.presentation.R.drawable.rectangle_grey_border)
            } else {
                clBagHead.setBackgroundResource(R.drawable.shape_ghana_rect_light_grey1)
            }
            tvSno.text = pos.plus(1).toString()
            tvBag.text = it.bagCount
            tvGrossWeight.text = it.grossWeight.toDouble().formatThreeDigits().plus(" ").plus("MT")
            val avgAvlue = it.palletAverage?.toDouble()?.plus(it.bagCount.toInt().times(it.tareWeight?.toDouble()!!))
            tvTarWeight.text = avgAvlue?.formatThreeDigits().plus(" ").plus("MT")
            tvNetWeight.text =
                it.grossWeight.toDouble().minus(avgAvlue!!).formatThreeDigits().plus(" ").plus("MT")
