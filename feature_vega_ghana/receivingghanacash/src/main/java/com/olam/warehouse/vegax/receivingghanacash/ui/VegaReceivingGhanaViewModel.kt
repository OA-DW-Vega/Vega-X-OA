...
import com.olam.warehouse.vegax.receivingghanacash.utils.getLineItemFromReceiving
 ...
 */
class VegaReceivingGhanaViewModel(private val useCase: VegaReceivingGhanaReceivingUseCase, private val dispatchers: AppDispatchers) :
    BaseViewModel() {

    private var receiveSource: LiveData<Resource<GenericReqAndResp<VegaReceivingGhanaResponse>>> = MutableLiveData()
    ...
    private val _receive = MediatorLiveData<Resource<GenericReqAndResp<VegaReceivingGhanaResponse>>>()
    ...
    val receive: LiveData<Resource<GenericReqAndResp<VegaReceivingGhanaResponse>>> get() = _receive
    ...
    fun postReceivingData(
        receivingData: VegaReceivingGhanaPost
    ...
    fun postReceivingMtnData(receivingData: VegaReceivingGhanaPost) = viewModelScope.launch(dispatchers.main) {
        _receive.removeSource(receiveSource) // We make sure there is only one source of livedata (allowing us properly refresh)
        withContext(dispatchers.io) {
