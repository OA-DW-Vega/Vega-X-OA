...
class VegaGhanaInventoryViewModel(
    ...
    private var reportsource: LiveData<Resource<GenericReqAndResp<List<VegaGhanaInventoryReportResponse>>>> = MutableLiveData()
    private val _reports = MediatorLiveData<Resource<GenericReqAndResp<List<VegaGhanaInventoryReportResponse>>>>()
    val inventoryreport: LiveData<Resource<GenericReqAndResp<List<VegaGhanaInventoryReportResponse>>>> get() = _reports
    ...
    fun getInventoryReports(parampost: VegaGhanaInventoryPost) = viewModelScope.launch(dispatchers.main) {
        _reports.removeSource(reportsource) // We make sure there is only one source of livedata (allowing us properly refresh)
        withContext(dispatchers.io) {
            reportsource = useCase.getInventoryReports(parampost)
        }
        _reports.addSource(reportsource) {
            _reports.value = it
        }
    }
