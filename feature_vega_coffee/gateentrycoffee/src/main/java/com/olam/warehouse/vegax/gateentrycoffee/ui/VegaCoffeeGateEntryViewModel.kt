...
class VegaCoffeeGateEntryViewModel(
    private val useCase: VegaCoffeeGateEntryUseCase,
    ...
    private var gateEntrySource: LiveData<Resource<GenericReqAndResp<VegaGateEntryResponse>>> = MutableLiveData()
    private val _gateEntry = MediatorLiveData<Resource<GenericReqAndResp<VegaGateEntryResponse>>>()
    val gateEntry: LiveData<Resource<GenericReqAndResp<VegaGateEntryResponse>>> get() = _gateEntry
    ...
    private var gateLocalEntrySource: LiveData<VegaGateEntry> = MutableLiveData()
    private val _gateEntryLocal = MediatorLiveData<VegaGateEntry>()
    val gateEntryLocal: LiveData<VegaGateEntry> get() = _gateEntryLocal

    fun getWaitingTruckList(isWeighscale: Boolean) = viewModelScope.launch(dispatchers.main) {
        ...
        withContext(dispatchers.io) {
            gateEntryTruckResource = useCase.getWaitingTrucks(isWeighscale)
        }
    ...
    fun postGateEntryData(gateEntryPost: VegaGateEntryPost, isWB: Boolean) = viewModelScope.launch(dispatchers.main) {
        _gateEntry.removeSource(gateEntrySource) // We make sure there is only one source of livedata (allowing us properly refresh)
        withContext(dispatchers.io) {
            gateEntrySource = useCase.postGateEntryData(gateEntryPost, isWB)
        }
    ...
    fun saveNewGateEntryData(gateEntry: VegaGateEntry) = viewModelScope.launch {
        withContext(dispatchers.io) {
            useCase.saveGateEntry(gateEntry)
        }
    }
