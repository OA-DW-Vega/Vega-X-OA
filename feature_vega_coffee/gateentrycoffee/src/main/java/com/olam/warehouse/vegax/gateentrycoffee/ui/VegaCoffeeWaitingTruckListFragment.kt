...
import com.olam.warehouse.vegax.gateentrycoffee.databinding.FragmentVegaCoffeeWaitingTruckListBinding
import com.olam.warehouse.vegax.gateentrycoffee.utils.*
import kotlinx.android.synthetic.main.item_vega_coffee_waiting_truck_list.view.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper


class VegaCoffeeWaitingTruckListFragment : BaseFragment() {
    private lateinit var binding: FragmentVegaCoffeeWaitingTruckListBinding
    ...
    override val layoutResourceId = R.layout.fragment_vega_coffee_waiting_truck_list
    private val vm: VegaCoffeeGateEntryViewModel by viewModel()
    private var vegaWbIds = listOf<VegaGateEntry>()


    companion object {
        fun newInstance(gateEntryData: VegaGateEntry) = VegaCoffeeWaitingTruckListFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaCoffeeWaitingTruckListBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("gateentrycoffee/ui/VegaCoffeeWaitingTruckListFragment - $wType").title("Gate Entry")
            .with(tracker)
    }
    ...
    private fun initUI() {
        ...
        if (isOnline()) vm.getWaitingTruckList(gateEntryData.imageString.equals(WEIGHSCALE))
    ...
    private fun setUpAdapter(data: List<VegaGateEntry>) {
        ...
        binding.rvWeighbridge.setUp(gateEntry1.asReversed(), R.layout.item_vega_coffee_waiting_truck_list, { it, pos ->
            ...
            tvWeight.text = it.grossWeight.plus(" ").plus(it.unitsOfMeasure)
        ...
        when (response.status) {
            Resource.Status.SUCCESS -> {
                ...
                response.data?.let { it1 ->
                    if (it1.isNotEmpty()) {
                        vegaWbIds = listOf()
                        val filterList = ArrayList<VegaGateEntry>()
                        if (gateEntryData.imageString.equals(WEIGHSCALE)) {
                            filterList.addAll(it1.filter { it.weighMethod == "WS" })
                        } else {
                            filterList.addAll(it1.filter { it.weighMethod == "WB" })
                        }
