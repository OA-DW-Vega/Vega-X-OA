...
class VegaCoffeeOffloadingViewModel(
    ...
    fun generateMtnrSequnceNumber(): String {
        var mtnrSequnceId = ""
        ...
        if (PreferenceHelper.get(Constants.CROP_FINANCIAL_YEAR, "").isEmpty()) {
            ...
            year = PreferenceHelper.get(Constants.CROP_FINANCIAL_YEAR, "")
        }
...
//            (currentyear+1).toString().substring(2) else currentyear.toString().substring(2)
        ...
        val mtnrSequence = PreferenceHelper.get(Constants.MTNR_SEQUENCE, "")
        mtnrSequnceId = plantId.plus(year).plus(mtnrSequence)

        return mtnrSequnceId
    }
