...
import com.olam.warehouse.vegax.processingcoffee.databinding.FragmentRminAddLotBinding
import com.olam.warehouse.vegax.processingcoffee.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaCoffeeRMINAddLotFragment : BaseFragment(), CoffeeRminItemRemoveListener,
    CoffeeRminAddWeightListener {

    override val layoutResourceId = R.layout.fragment_rmin_add_lot
    private lateinit var binding: FragmentRminAddLotBinding
    ...
    private var adapter: VegaCoffeeRminLotAdapter? = null
    ...
    companion object {
        fun newInstance(fromSummary: Boolean, model: VegaCoffeeRminProcessing) =
            VegaCoffeeRMINAddLotFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentRminAddLotBinding.inflate(layoutInflater)
        initExtra()
        initAdapter()
        initUI()
        return binding.root
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("processingcoffee/ui/rmin/VegaCoffeeRMINAddLotFragment")
            .title("IVC/Coffee/Processing/RMIN Add Lot")
            .with(tracker)
    }
    ...
    private fun initUI() {
        ...
        //binding.cvCard.tvProceesValue.text = gradeList.get(selectedItemPosition).weightToProcess
        when {
    ...
    private fun clearAllValues() {
        binding.cvCard.tvMaterialValue.text = gradeList[selectedItemPosition].materialName
        //binding.cvCard.tvProceesValue.text = gradeList[selectedItemPosition].weightToProcess
        adapter?.clear()
        vm.lotList.clear()
    ...
    private fun updateLotFromLocalDB(list: ArrayList<VegaCoffeeRminLots>) {
        if (list.isNotEmpty()) {
            vm.lotList.clear()
            adapter?.clear()
            //list.forEach { if (it.materialCode == currentMaterial) vm.lotList.add(it) }
