...
class VegaCoffeeRminViewModel(
    ...
    private var postRminSource: LiveData<Resource<GenericReqAndResp<List<VegaCoffeeProcessingRminResponse>>>> =
        MutableLiveData()
    private val _postRmin = MediatorLiveData<Resource<GenericReqAndResp<List<VegaCoffeeProcessingRminResponse>>>>()
    val postRmin: LiveData<Resource<GenericReqAndResp<List<VegaCoffeeProcessingRminResponse>>>> get() = _postRmin
    ...
    fun postRmin(poReq: VegaCoffeeProcessingFgrnPost) = viewModelScope.launch(dispatchers.main) {
