...
import com.olam.warehouse.vegax.processingcoffee.databinding.ItemVegaCoffeeFgrnLotBinding
...
class VegaCoffeeFgrnAssignLotFragment : BaseFragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("processingcoffee/ui/fgrn/VegaCoffeeFgrnAssignLotFragment").title("IVC/Coffee/Processing/FGRN Assign Lot")
            .with(tracker)
    }
                ...
                when (it.status) {
                    Resource.Status.SUCCESS -> {
                        when (it.data?.success) {
                            true -> {
                                showErrorDialogWithFAQLink(requireContext(),getString(R.string.batch_doesnt_exists) )
                            }
...
//            vm.getConfigItems(UserRoles.PROCESSING.role)
    ...
    private fun updateAdapter(stockList: MutableList<VegaCoffeeRminLots>) {
        ...
        binding.rvInventoryLot.setUpAdapter(
            stockList,
            R.layout.item_vega_coffee_fgrn_lot,
            ItemVegaCoffeeFgrnLotBinding::inflate,
            { it, pos, bindItem ->
