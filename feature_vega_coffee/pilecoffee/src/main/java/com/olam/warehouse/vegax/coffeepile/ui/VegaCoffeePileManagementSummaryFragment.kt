...
import kotlinx.android.synthetic.main.item_pile_management_summary.view.*
    ...
    private fun setupAdapter(data: MutableList<VegaCocoaDispatchLots>) {
        ...
        binding.tvProceesValue.text =
            if (pileSelection.weight.isNullOrEmpty() || pileSelection.weight.equals("0.0")) weightTransfer().plus(" ")
                .plus(data[0].unitOfMeasure) else pileSelection.weight.plus(" ").plus(data[0].unitOfMeasure)
        binding.rvLotList.setUp(data, R.layout.item_pile_management_summary, { it, pos ->
