...
import com.olam.warehouse.vegax.coffeepile.databinding.FragmentPileManagementBinding
import com.olam.warehouse.vegax.coffeepile.databinding.ItemPileManagementBinding
...
import com.olam.warehouse.vegax.coffeepile.ui.utils.PILE_MANAGEMENT_EDIT
...
class VegaCoffeePileManagementFragment : BaseFragment(), VegaSingleSelectListener {

    override val layoutResourceId = R.layout.fragment_pile_management
    private lateinit var binding: FragmentPileManagementBinding
    ...
    companion object {
        ...
        ) = VegaCoffeePileManagementFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentPileManagementBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("pilemanagement/ui//VegaCoffeePileManagementFragment").title("IVC/Coffee/Pile Management/Pile")
            .with(tracker)
        initUI()
    }
...
{
   if(!fromVendor.vendorCode.toString().isNullOrEmpty())
   {
   ...
   {
       Toast.makeText(context, "Please select the Valid vendor", Toast.LENGTH_SHORT).show()
   }
}
            else
{
    ...
    private fun setUpAdapter(list: ArrayList<VegaCocoaDispatchLots>) {
        ...
        binding.rvLots.setUpAdapter(
            lots,
            R.layout.item_pile_management,
            ItemPileManagementBinding::inflate,
            { item, pos, bindItem ->
                ...
                bindItem.etWeight.onChange {
                    if (it.isNotEmpty()) {
                        ...
                        if (come ?: 0 <= 0) {
                            ...
                            vm.lots.forEach { item1 ->
                                if (item1.batchNumber.equals(item.batchNumber) && item1.materialCode.equals(
                                        item.materialCode
                                    )
                                ) item.editedWeight =
                                    editVal
