...
import com.olam.warehouse.vegax.salescoffee.databinding.FragmentCoffeeSalesAddLotBinding
import com.olam.warehouse.vegax.salescoffee.utils.*
import kotlinx.android.synthetic.main.item_coffee_sales_lot.view.*
import kotlinx.android.synthetic.main.item_coffee_sales_material_list.view.*
import kotlinx.android.synthetic.main.item_coffee_sales_material_list.view.tvMaterial
...
class VegaCoffeeSalesAddLotFragment : BaseFragment(), RecyclerViewItemClickListener {
    ...
    override val layoutResourceId: Int = R.layout.fragment_coffee_sales_add_lot
    private lateinit var binding: FragmentCoffeeSalesAddLotBinding

    companion object {
        fun newInstance(dispatch: VegaCoffeeSalesOrder, dispatchType: String) =
            VegaCoffeeSalesAddLotFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentCoffeeSalesAddLotBinding.inflate(inflater)
        initExtra()
        initUI()
        /*listener = this*/
        return binding.root
    ...
    private fun moveToSummary(remark: String) {
        ...
        getCurrentDate("END")
        val duration = vm.salesOrder.endTime?.toLong()?.minus(vm.salesOrder.startTime?.toLong() ?: 0)
                ...
                SALES_TYPE_WEIGHSCALE -> {
                    ...
                    if (bagList.size > 0) {
                        ...
                        if (lot.unitOfMeasure.equals("MT"))
                            editedWt = bagList.sumByDouble { it.netWeight.toDouble() }.div(1000).formatThreeDigits()
                        else
                            editedWt = bagList.sumByDouble { it.netWeight.toDouble() }.formatThreeDigits()
                        lot.editedWeight = editedWt
                    } else lot.editedWeight = "0"
                    flSalesLot.visible()
                    flAnticipatedSales.gone()
                    ...
                    tv_add_weight.setOnClickListener { view ->
                    ...
                    ivScaleClose.setOnClickListener { view -> showConformationDialog(lot, view) }
                    cbScaleStorageLoss.isChecked = lot.endLotFlag ?: false
                    clEndLot.setOnClickListener { view ->
