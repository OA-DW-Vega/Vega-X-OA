package com.olam.warehouse.vegax.qualitycoffee.ui.weighbridge
...
import com.olam.warehouse.vegax.qualitycoffee.databinding.FragmentVegaCoffeeQualityWeighBridgeListBinding
...
class VegaCoffeeQualityWBListFragment : BaseFragment() {

    private var mAdapter = VegaCoffeeWeighBridgeListAdapter { moveBagdetail(it) }
    private val vm: VegaCoffeeQualityViewModel by viewModel()
    private lateinit var mListener: OnWeighBridgeListener
    private lateinit var binding: FragmentVegaCoffeeQualityWeighBridgeListBinding
    override val layoutResourceId = R.layout.fragment_vega_coffee_quality_weigh_bridge_list
    ...
    companion object {
        fun newInstance() = VegaCoffeeQualityWBListFragment().putArgs {
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaCoffeeQualityWeighBridgeListBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("quality/ui/weighbridge/VegaCoffeeQualityWBListFragment - $weightmentType")
            .title("IVC/Coffee/Quality/WBList").with(tracker)
    }

    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        super.onCreateOptionsMenu(menu, inflater)
        activity?.menuInflater?.inflate(R.menu.search_vega_quality_coffee_menu, menu)
                    ...
                    MTNR -> {
                        ...
                        if (getCurrentKey().split("_")[1].contains("NI")) {
                            if(weighBridgeId.isNotEmpty())
                                weighTypedata = weighTypedata?.filter { value -> value.weighBridgeId == weighBridgeId }
                            else if(deliveryID.isNotEmpty())
                                weighTypedata = weighTypedata?.filter { value -> value.delivery == deliveryID }
                        }
                    }
                }
    ...
    private fun updateUI(response: List<VegaQualityWBDetails>?) {
        response?.let { qualityDetail ->
            if (qualityDetail.isNotEmpty()) {
                val vegaWbIds = qualityDetail.filter { wb -> !wb.challan.isNullOrEmpty() }
                mListener.setQualityWBList(vegaWbIds)
                mQualityWBList.clear()
                mQualityWBList = vegaWbIds as MutableList<VegaQualityWBDetails>
                mAdapter.addItems(vegaWbIds)
                binding.tvNoData.gone()
                binding.rvWeighbridge.visible()
            } else {
                binding.tvNoData.visible()
