package com.olam.warehouse.vegax.qualitycoffee.ui.lot
...
import com.olam.warehouse.vegax.qualitycoffee.databinding.FragmentVegaCoffeeWeighbridgeLotBinding
import com.olam.warehouse.vegax.qualitycoffee.databinding.ItemVegaCoffeeLotCardLayoutBinding
...
class VegaCoffeeQualityWBLotListFragment : BaseFragment() {

    private val vm: VegaCoffeeQualityViewModel by viewModel()
    private lateinit var mListener: OnLotListener
    private lateinit var binding: FragmentVegaCoffeeWeighbridgeLotBinding
    override val layoutResourceId = R.layout.fragment_vega_coffee_weighbridge_lot
    ...
    companion object {
        fun newInstance() = VegaCoffeeQualityWBLotListFragment().putArgs {
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaCoffeeWeighbridgeLotBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("quality/ui/weighbridge/VegaCoffeeQualityWBListFragment - $weightmentType")
            .title("IVC/Coffee/Quality/WBLot List").with(tracker)
    }
    ...
    private fun moveBagdetail(wbDetails: VegaCoffeeLot?) {
        //mListener.onLotClick(wbDetails, copiedWbid, copiedMaterial)
    }
...
//        val data = mQualityLotList1.filter { it.qcStatus.isNullOrEmpty() }.toMutableList()
        ...
        if (mQualityLotList1.size > 0) {
            if(!flag) countOfLot = mQualityLotList1.size
            //println("nnnLotCount Wb  $countOfLot $flag")
            binding.tvNoLots.gone()
            binding.rvLots.visible()
        } else {
            binding.rvLots.gone()
        ...
        binding.rvLots.setUpAdapter(
            mQualityLotList1,
            R.layout.item_vega_coffee_lot_card_layout,
            ItemVegaCoffeeLotCardLayoutBinding::inflate,
            { it, pos, bindItem ->
