...
class VegaCoffeeQualityActivity : HomeBaseActivity(), VegaCoffeeQualityWBListFragment.OnWeighBridgeListener, VegaCoffeeQualityWBLotListFragment.OnLotListener,
    VegaCoffeeQualityParameterFragment.Callback,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (getCurrentKey().split("_")[1].contains("NI")) {
            grade = intent.getStringExtra("gradeData").toString()
            certificate = intent.getStringExtra("certificateData").toString()
            receivingData = intent.getParcelableExtra("receivingData")!!
            receivingData.certificate = certificate
            receivingData.gradeDesc = grade
            weighBridgeId = intent.getStringExtra("mtnr_wbid").toString()
            deliveryID = receivingData.delivery.toString()
    ...
    private fun backNaviagation() {
        val fragment = this.supportFragmentManager.findFragmentById(R.id.flQuality)
        when(fragment) {
            is VegaCoffeeQualityWBListFragment -> {
                if(getCurrentKey().split("_")[1].contains("NI") && mQualityWBList.isNotEmpty()) {
                    toast(getString(R.string.complete_quality_valid))
                }else
                    super.onBackPressed()
            }
            else -> {
