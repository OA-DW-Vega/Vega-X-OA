...
import com.olam.warehouse.vegax.qualitycoffee.databinding.ItemVegaCoffeeQualityWeighBridgeDetailsBinding
import com.olam.warehouse.vegax.qualitycoffee.utils.PROCURE
import com.olam.warehouse.vegax.qualitycoffee.utils.SUPPLIER

/**
 * Created by Baskaran Kannan on 12/27/2019.
 */
class VegaCoffeeWeighBridgeListAdapter(private val onClick: (VegaQualityWBDetails?) -> Unit) :
    RecyclerView.Adapter<VegaCoffeeWeighBridgeListAdapter.WeighBridgeViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WeighBridgeViewHolder {
        /*val v =
            LayoutInflater.from(parent.context)
                .inflate(R.layout.item_vega_coffee_quality_weigh_bridge_details, parent, false)*/
        val v = ItemVegaCoffeeQualityWeighBridgeDetailsBinding.inflate(
            LayoutInflater.from(parent.context),
            parent,
            false
        )
...
//        })
    ...
    inner class WeighBridgeViewHolder(itemView: ItemVegaCoffeeQualityWeighBridgeDetailsBinding) :
        RecyclerView.ViewHolder(itemView.root) {
        val binding = itemView
        fun bindItems(weighbridge: VegaQualityWBDetails?) {
            weighbridge?.let {
                if (getCurrentKey().split("_")[1].contains("NI")) {
                    binding.materialLabel.visibility = View.GONE
                }
