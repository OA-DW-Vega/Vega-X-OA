...
import kotlinx.android.synthetic.main.item_coffee_tp_lot_with_add_weight.view.*
...
class VegaCoffeeThirdPartySalesAddLotFragment : BaseFragment(), VegaSingleSelectListener {
    ...
    private fun setUpAdapter(list: ArrayList<VegaCocoaDispatchLots>) {
        val lots = list
        binding.rvLots.setUp(lots, R.layout.item_coffee_tp_lot_with_add_weight, { item, pos ->
            ...
            if (getCurrentKey().split("_")[1].contains("NI")) {
                tvEditedWeight.text = item.weight?.toDouble()
