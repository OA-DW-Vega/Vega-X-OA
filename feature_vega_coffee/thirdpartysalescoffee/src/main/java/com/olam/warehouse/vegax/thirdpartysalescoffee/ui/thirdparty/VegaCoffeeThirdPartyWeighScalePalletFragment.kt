...
import com.olam.warehouse.vegax.thirdpartysalescoffee.databinding.FragmentCoffeeThirdpartyWeighscaleAddPalletBinding
import com.olam.warehouse.vegax.thirdpartysalescoffee.databinding.ItemVegaCoffeeThirdpartyAddBagBinding
...
import com.olam.warehouse.vegax.thirdpartysalescoffee.utils.FRAG_ADD_BAG_WEIGHT
import com.olam.warehouse.vegax.thirdpartysalescoffee.utils.UPDATE_WEIGHT
...
class VegaCoffeeThirdPartyWeighScalePalletFragment : BaseFragment() {

    override val layoutResourceId: Int =
        R.layout.fragment_coffee_thirdparty_weighscale_add_pallet
    private lateinit var binding: FragmentCoffeeThirdpartyWeighscaleAddPalletBinding
    ...
    companion object {
        fun newInstance(item: VegaCocoaDispatchLots) = VegaCoffeeThirdPartyWeighScalePalletFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentCoffeeThirdpartyWeighscaleAddPalletBinding.inflate(inflater)
        initUI()
        return binding.root
    }
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("pilemanagement/ui//VegaCoffeePileManagementFragment").title("IVC/Coffee/Third Party Sales/WeighScale Pallet")
            .with(tracker)
    }
    private fun initUI() {
        defaultLot = arguments?.getParcelable<VegaCocoaDispatchLots>("MODEL_BUNDLE") as VegaCocoaDispatchLots
        binding.tvTitle.text =
            getString(com.olam.warehouse.login.R.string.third_party_sales).plus(" - ")
    ...
    fun onBackPressed() {
        ...
        callBack?.replaceFragment(UPDATE_WEIGHT, VegaCoffeeWSBagModel(weight, model))
    }

    private fun moveToSummary() {
        activity?.onBackPressed()
        callBack?.replaceFragment(UPDATE_WEIGHT, VegaCoffeeWSBagModel(binding.tvNetWtValue.text.toString(), true))
    }
    ...
    private fun updateBagItems(bagItems: List<VegaCocoaSweepingBagMaterial>?) {
        bagItems?.let {
            ...
            if (bagItems.isNotEmpty()) {
                ...
                if (getCurrentKey().split("_")[1].contains("NI")) updateTotalWeights()
                else {
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCocoaSweepingBagMaterial>) {
        ...
        binding.rvWeight.setUpAdapter(
            bagList1.asReversed(),
            R.layout.item_vega_coffee_thirdparty_add_bag,
            ItemVegaCoffeeThirdpartyAddBagBinding::inflate,
            { it, pos, bindItem ->
                if (pos % 2 == 0) {
                    bindItem.clBagHead.setBackgroundResource(com.olam.warehouse.presentation.R.drawable.rectangle_grey_border)
                } else {
                    bindItem.clBagHead.setBackgroundResource(R.drawable.shape_coffee_tp_rect_light_grey1)
                }
