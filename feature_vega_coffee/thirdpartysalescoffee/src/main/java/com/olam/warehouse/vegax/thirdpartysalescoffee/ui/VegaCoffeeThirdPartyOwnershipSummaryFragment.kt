...
import com.olam.warehouse.master.common.utils.covertToDouble
...
class VegaCoffeeThirdPartyOwnershipSummaryFragment : BaseFragment() {
    ...
    private fun setUpAdapter(list: ArrayList<VegaCocoaDispatchLots>) {
        binding.rvLotList.setUp(list, R.layout.item_third_party_add_lot, { item, pos ->
            ...
            if (getCurrentKey().split("_")[1].contains("NI") && model.transferType.equals(
                ...
                tfd.visibility = View.GONE
                tvEditedWeight.visibility = View.GONE
                ...
                tvStLocation.text = editedWeigh.plus(" ")
                ...
                tvStLoss.text = lossvalue.plus(" ").plus(item.unitOfMeasure)
                tvStLoss.visible()
            } else {
                val editedWeight =
                    if (item.editedWeight.isNullOrEmpty()) "0.0" else item.editedWeight.toString()
                        .toDouble()
                        .formatThreeDigits().plus(" ").plus(item.weightToDispatchUOM)
                tvEditedWeight.text = editedWeight.plus(" ")
                tvEditedWeight.visibility = View.VISIBLE
                etWeight.visibility = View.GONE
                cbSelectAll.visibility = View.GONE
                tvSelectAll.visibility = View.GONE
            }
            ivClose.setImageDrawable(ivClose.context.getDrawable(R.drawable.ic_coffee_tp_edit_gray))
    ...
    private fun postDelivery() {
        ...
        if (model.transferType.equals(TP_TO_TP, true))
            ...
            vm.postSalesTruckInData(VegaReceivingPost(getCurrentKey(), getPlantDetails(), prepareWeighbridgePostData()))
        } else
