...
import com.olam.warehouse.vegax.offloadinggrnghanacocoa.databinding.FragmentVegaGhanaCocoaMtnrWeighBridgeListBinding
...
class VegaGhanaCocoaMtnrWBListFragment : BaseFragment() {

    private var mAdapter = VegaGhanaCocoaMtnrWBListAdapter { moveBagdetail(it) }
    private val vm: VegaGhanaCocoaOffloadingViewModel by viewModel()
    private lateinit var mListener: OnWeighBridgeListener
    private lateinit var binding: FragmentVegaGhanaCocoaMtnrWeighBridgeListBinding
    override val layoutResourceId = R.layout.fragment_vega_ghana_cocoa_mtnr_weigh_bridge_list
...
//        fun onQualityOfflineClick()
    ...
    companion object {
        fun newInstance() = VegaGhanaCocoaMtnrWBListFragment().putArgs {
        }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaGhanaCocoaMtnrWeighBridgeListBinding.inflate(layoutInflater)
        return binding.root
    }
                    ...
                    newText.let {
                        if (newText?.isEmpty() == true) {
                            ...
                            mQualityWBList.forEach { qtyWb ->
                                newText?.let { text ->
                                    if (qtyWb.vehicleNumber?.contains(text) == true || qtyWb.delivery?.contains(
                                            text
                                        ) == true
                                    ) {
    ...
    private fun updateUIWithOnlineData(response: Resource<GenericReqAndResp<List<VegaQualityWBDetails>>>) {
        when (response.status) {
            Resource.Status.SUCCESS -> {
                ...
                weighTypedata = data
//                    ?.filter { wb -> wb.direction == DIRECTIONIN }
...
//                }
            ...
            Resource.Status.ERROR -> {
                showErrorDialogWithFAQLink(requireContext(), "Error")
                hideLoading()
            }
        }
    }
