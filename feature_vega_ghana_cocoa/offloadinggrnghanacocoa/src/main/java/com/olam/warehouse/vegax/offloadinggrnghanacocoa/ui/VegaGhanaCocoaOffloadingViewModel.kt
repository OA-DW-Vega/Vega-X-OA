...
class VegaGhanaCocoaOffloadingViewModel(
    ...
    private var liveStocksSource: LiveData<Resource<GenericReqAndResp<List<VegaCocoaRminLots>>>> =
        MutableLiveData()
    private val _liveStocks =
        MediatorLiveData<Resource<GenericReqAndResp<List<VegaCocoaRminLots>>>>()
    val liveStocks: LiveData<Resource<GenericReqAndResp<List<VegaCocoaRminLots>>>> get() = _liveStocks
    ...
    private var bagSourceSaved: LiveData<List<VegaEcuadorOffloadingBagMaterial>> =
        mutableLiveDataOf()
    ...
    var lotListSelected = VegaGRNDWLotManualModel()
    val weighBridgeWithLots = MutableLiveData<VegaGhanaCocoaMtntWithLots>()

    private var validateLotSource : LiveData<Resource<GenericReqAndResp<VegaGRNDWLotManualModel>>> = mutableLiveDataOf()
    val _validateLot =  MediatorLiveData<Resource<GenericReqAndResp<VegaGRNDWLotManualModel>>>()
    var validateLot : LiveData<Resource<GenericReqAndResp<VegaGRNDWLotManualModel>>>  = _validateLot


    val loader = MutableLiveData<Boolean>()

   private var lotListForDWSSource : LiveData<Resource<GenericReqAndResp<List<VegaGRNDWLotManualModel>>>> = mutableLiveDataOf()
    val _lotListForDWS =  MediatorLiveData<Resource<GenericReqAndResp<List<VegaGRNDWLotManualModel>>>>()
   var lotListForDWS : LiveData<Resource<GenericReqAndResp<List<VegaGRNDWLotManualModel>>>>  = _lotListForDWS
    ...
    fun getLotListForDWSInventory(vendorSAPCode:String,key:String) = viewModelScope.launch(dispatchers.main)
    {
        loader.postValue(true)
        _lotListForDWS.removeSource(lotListForDWSSource)
            withContext(dispatchers.io) {
                lotListForDWSSource = useCaseCocoa.getDWSLotFromInventory(vendorSAPCode, key)
            }
        _lotListForDWS.addSource(lotListForDWSSource) {
            _lotListForDWS.value = it
        }
    }
    ...
    fun validateLot(plantId: String,batchNumber: String,key:String) =
        viewModelScope.launch(dispatchers.main) {
        loader.postValue(true)
        _validateLot.removeSource(validateLotSource)
        withContext(dispatchers.io) {
            validateLotSource = useCaseCocoa.validateLot(batchNumber,key)
        }
        _validateLot.addSource(validateLotSource) {
            _validateLot.value = it
        }

    }


    fun saveSelectedLot(selectedLot:VegaGRNDWLotManualModel)=viewModelScope.launch(Dispatchers.Main)
    {
        withContext(dispatchers.io)
        {
            lotListSelected = selectedLot
        }
    }
    ...
    fun getMtntWithLots(wbId: String) = viewModelScope.launch(dispatchers.main) {
        withContext(dispatchers.io) {
            weighBridgeWithLots.postValue(useCaseCocoa.getDwWithLots(wbId))
        }
    }
   ...
   /* fun getMaterialStorageLocation() = viewModelScope.launch(dispatchers.main) {
    ...
    fun fetchStocks(material: String) = viewModelScope.launch(dispatchers.main) {
        ...
        withContext(dispatchers.io) {
            stocksSource = useCaseCocoa.getStocks(material, false)
        }
    ...
    fun getLiveStocks(material: String) = viewModelScope.launch(dispatchers.main) {
        _liveStocks.removeSource(liveStocksSource)
        withContext(dispatchers.io) {
            liveStocksSource = useCaseCocoa.getStocks(material, true)
        }
        _liveStocks.addSource(liveStocksSource) {
            _liveStocks.value = it
        }
    }
    ...
    fun getFarmerList(){
        viewModelScope.launch(dispatchers.main) {
            ...
            withContext(dispatchers.io) {
                farmerListSource = useCaseCocoa.getFarmerList()
            }
    ...
    fun getFeatureMaster() = viewModelScope.launch(dispatchers.main) {
        ...
        withContext(dispatchers.io) {
            featureMasterSource = useCaseCocoa.getFeatureMaster()
        }
