...
//        vm.offloadingPost.observe(viewLifecycleOwner, Observer { updateUI(it) })
    ...
    private fun initExtra() {
        ...
        binding.tvTruckNoValue.text = wbDetails.bagCount
...
//        vm.getOBDDetails(vegaCoffeeReceivingData.delivery)
    ...
    private fun prepareDeliveryList(): List<VegaGhanaOffloadingDeliveryDetail> {
        ...
        for (item in rejectList) {
            ...
            deliveryDetail.batchNumber = /*generateRejectBatchId()*/wbDetails.batchNumber
        ...
        if(acceptedBagCount.toLong() > 0) {
            ...
            acceptDeliveryDetail.batchNumber = /*generateAcceptBatchId()*/wbDetails.batchNumber.toString()
    ...
    private fun moveToSuccessPage(data: VegaMtntResponse) {
        ...
        intent.putExtra(
            ...
            getString(R.string.wb_id_is, data.wbId).plus("\n")
                .plus(getString(R.string.mtnr_number, data.mtnNumber).plus("\n")
                    .plus(getString(R.string.grn_number1, data.grnNumber)))
        )
//        intent.putExtra(
