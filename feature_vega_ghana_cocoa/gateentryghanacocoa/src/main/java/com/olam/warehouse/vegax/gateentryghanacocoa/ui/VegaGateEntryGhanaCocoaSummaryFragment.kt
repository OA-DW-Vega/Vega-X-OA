...
import com.olam.warehouse.vegax.gateentryghanacocoa.databinding.FragmentGateEntryGhanaCocoaSummaryBinding
import com.olam.warehouse.vegax.gateentryghanacocoa.utils.GATE_ENTRY_DATA
...
class VegaGateEntryGhanaCocoaSummaryFragment : BaseFragment() {
    ...
    private lateinit var binding: FragmentGateEntryGhanaCocoaSummaryBinding
    override val layoutResourceId = R.layout.fragment_gate_entry_ghana_cocoa_summary

    companion object {
        fun newInstance(gateEntryData: VegaGateEntry) =
            VegaGateEntryGhanaCocoaSummaryFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentGateEntryGhanaCocoaSummaryBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("gateentry/ui/VegaGateEntryGhanaSummaryFragment").title("Gate Entry").with(tracker)
        initUI()
    }
    ...
    private fun initUI() {
        ...
        binding.tvTransportVendor.text =
            gateEntryData.transportVendorCode.plus("-").plus(gateEntryData.transportVendorName)
        binding.tvTruckId.text = gateEntryData.truckId
        ...
        binding.tvOBDNumber.text = gateEntryData.delivery
        binding.tvNoOfBags.text = gateEntryData.bagCount
        binding.tvProductText.text =
        ...
        binding.tvWeight.text = gateEntryData.bagCount
        ...
        vm.gateEntryCocoa.observe(viewLifecycleOwner, Observer { updateUI(it) })
    }
    ...
    private fun updateUI(data: Resource<GenericReqAndResp<VegaGateEntryGhanaCocoaResponse>>?) {
        data?.let {
            when (it.status) {
    ...
    private fun postGateEntry() {
        ...
        if (AppUtils.isOnline()) {
            vm.postGateEntryData(VegaGateEntryGhanaCocoaPost(getCurrentKey(), getPlantDetails(), postData))
        }
    }
    ...
    private fun moveToSuccessPage(wbId: String?) {
        ...
        intent.putExtra(AppUtils.TITLE, getString(R.string.success_truck_reporting))
