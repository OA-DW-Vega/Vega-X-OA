...
class VegaGhanaCocoaCocoaMtntWeighScaleSummaryFragment : BaseFragment(), VegaGhanaCocoaMtntLotRemoveListener {
    ...
    companion object {
        fun newInstance(data: VegaCocoaDispatchWB) = VegaGhanaCocoaCocoaMtntWeighScaleSummaryFragment().putArgs {
    ...
    private fun initUi() {
        ...
        binding.tvEudrStatusValue.text = summaryObj?.eudrStatus
...
//        intent.putExtra(AppUtils.SUB_TITLE, "Temp ID :".plus(dispatchLotsList[0].weighBridgeId))
        intent.putExtra(AppUtils.SUB_TITLE, "Temp ID :".plus(vm.dispatchWh.weighBridgeId))
    ...
    private fun prepareDeliveryList(): List<VegaGhanaMtntDeliveryDetail> {
        ...
        var bagItems =
            bagList.filter { it.batchNumber == dispatch?.batchNumber && it.baseMaterial == dispatch.materialCode }
        deliveryDetail.batchNumber = dispatch?.batchNumber
        deliveryDetail.batchNumber = ""
        deliveryDetail.materialCode = dispatch?.materialCode?.trim()
        ...
        deliveryDetail.departurePoint = departurePointCodeValue[0].trim()
//            deliveryDetail.plantId = summaryObj?.recPlantId
        ...
        deliveryDetail.purchaseQuantity = summaryObj?.purchaseQuantity
            ...
            if (bagItems.isNotEmpty()) {
                ...
                bag.unitsOfMeasure = dispatch?.unitsOfMeasure.toString()
                val bagSort = arrayListOf<VegaCocoaSweepingBagMaterial>()
                bagSort.add(bag)
                bagItems = bagSort
                deliveryDetail.bagList = bagItems
...
//                    "WeighScaleId".plus(" " + it.weighBridgeId).plus(" ").plus(message.plus(it.msgList[0]).plus("\n"))
    ...
    private fun moveToSuccessPage(deliveryId: String?, docNo: List<String>) {
       ...
       // if (AppUtils.isOnline())
            intent.putExtra(
            AppUtils.TITLE,
            getString(R.string.success_mtnt)
        )
        intent?.putExtra(AppUtils.EUDR_STATUS, if(summaryObj?.eudrStatus?.equals(Constants.EUDR_QP_VALUE, true) == true)"1" else "0" )
...
//        )
        val finalCloseStock =
            if (!summaryObj?.purchaseQuantity.isNullOrEmpty() && !summaryObj?.closingStockCount.isNullOrEmpty()) (summaryObj?.closingStockCount?.toInt())?.minus(
                summaryObj?.purchaseQuantity?.toInt() ?: 0
            ) else 0
        ...
        if (!documentNo.isEmpty())
            intent.putExtra(
                ...
                ).plus("\n").plus(getString(R.string.closingStock1)).plus(finalCloseStock)
            )
        else
            intent.putExtra(
                AppUtils.SUB_TITLE,
