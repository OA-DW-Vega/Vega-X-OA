...
import com.olam.warehouse.vegax.mtntghanacocoa.data.domain.model.ValidateNumber
...
class VegaGhanaCocoaCocoaMtntConsignmentFragment : BaseFragment(),
    VegaGhanaCocoaSingleSelectListener, VegaSingleSelectCommonListener {
    ...
    private var model = mutableListOf<VegaCocoaDispatchWB>()
    ...
    private var storageLocation1 = ArrayList<String>()
    private var totalEnteredBags: Double? = 0.0
    private var localDbBags: Double? = 0.0
...
//        vm.getStorageLocationDetail()
        ...
        vm.weighBridgeWithLotsSourceNew.observe(viewLifecycleOwner, androidx.lifecycle.Observer {
            if (it != null) {
                model.clear()
                it.forEach { item ->
                    model.add(item.dispatch)
                }
            }
        })
        vm.getWeighBridgeStockCount()
        ...
        binding.tvnoofbagsValue.onChange { text ->
            when {
                text.isEmpty() -> {
                    ...
                    if (binding.tvproductValue.text.toString().isNotEmpty()) {
                        if (AppUtils.isOnline()) {
                            updateStockDetails()
                        } else {
                            //var materialCode = (binding.tvproductValue.text.toString()).split("-")
                            // vm.getWeighBridgeStockCount(materialCode[0], selectedSendingWH, loggedInPlantId)
...
//                binding.tvSendingWHValue.setCompoundDrawablesWithIntrinsicBounds(0, 0,R.drawable.ic_add_icon, 0)
        ...
        binding.tvproductValue.setOnClickListener {
            showSingleSelectDialog1(getString(R.string.select_material), AppUtils.MATERIAL)
        }
...
//        vm.getPurchaseOrder("")
        ...
        vm.truck.observe(viewLifecycleOwner, Observer {
        ...
        vm.custonLocation.observe(viewLifecycleOwner, Observer {
            ...
            if (sendingWareHouseList.size >= 1) {
                sendingWHList.clear()
                sendingWareHouseList.forEachIndexed { index, vegaCustomStLocation ->

                    sendingWHList.add(
                        sendingWareHouseList[index].procureLocationCode.plus(" - ").plus(
                            sendingWareHouseList[index].procureLocationName
                        )
                    )
                    if (index == 0) {
                        ...
                        if (data.size > 0) {
                            var materialData =
                                productList.filter { ((MATERIAL_CODE.plus(it.materialCode)).equals(data[0].materialCode)) || ((MATERIAL_CODE.plus(it.eudrEquivalentMap)).equals(data[0].materialCode)) }
                            if (materialData.size > 0) {
                                if(isCompliantMaterial){
                                ...
                                binding.tvproductValue.text = materialData[0].materialCode.plus("-")
                                    .plus(materialData[0].materialName)
                                if (AppUtils.isOnline()) {
                                    var materialCode = binding.tvproductValue.text.toString().split("-")
                                    vm.fetchStocks(materialCode[0].trim())
                                } else {
        ...
        jsonData.forEach {
            if (it.contains(JSON_RECEVING_LOCATION_LIST)) {
                ...
                receiveLocation.forEach { it1 ->
                    try {
                        allPlants().singleOrNull { it1.split(":")[0].contains(it.plantId) }?.apply {
                            storageLocation1.add(
                                plantName.plus(" - ")
...
//        seasonList.add(TruckManagementSeasonResponse("2","2020-10-01T00:00:00.000+0000","2021-09-15T00:00:00.000+0000","GH06","1"))
    ...
    private fun validateProceed() {
        if (binding.tvWhValue.text.isNotEmpty() && binding.tvDeparturePointValue.text.isNotEmpty() && binding.tvDriverName.text.isNotEmpty()
            ...
            if (binding.tvnoofbagsValue.text.toString().equals("0")) {
                showSnack(getString(R.string.number_of_bags_zero))
            } else if ((((binding.tvnoofbagsValue.text.toString()).toDouble()) + (localDbBags)!!) > ((binding.tvClosingStockValue.text.toString()).toDouble())) {
                ...
                if(AppUtils.isOnline()) {
                    ...
                    val sdWayBillNo = binding.tvSDWaybillNoValue.text.toString()
                    val wbType = "STO"
                    vm.validateReceiptNumber(plantId, sdWayBillNo, wbType)
                }else{
                    prepareTruckWb()
                    vm.saveWeighBridgeDetails()
                    materialModelList.forEach {
    ...
    private fun prepareTruckWb() {
        ...
        //vm.dispatchWh.fromVendorCode=binding.tvTransportVendorValue.text.toString().split("-")[1].trim()
        ...
        vm.dispatchWh.destinationWH = destinationWarehouse[0].trim()
        vm.dispatchWh.destinationWHName = destinationWarehouse[1].trim()
        ...
        vm.dispatchWh.eudrStatus = if(isCompliantMaterial) Constants.EUDR_QP_VALUE else Constants.ATTR_UNKNOWN_QP_VALUE
    ...
    private fun showSingleSelectDialog1(title: String, currentFalg: String) {
        val list = ArrayList<String>()

        when(currentFalg) {
            AppUtils.MATERIAL -> {
                val dataval = materialStorageLocationList.filter {
                    it.storageLocationCode.equals(selectedSendingWH)
                }.map { it.materialCode }
                if (dataval.size > 0) {
                    var materialData = emptyList<VegaMaterial>()
                    if(isCompliantMaterial) {
                        materialData =
                            productList.filter { dataval.contains(MATERIAL_CODE.plus(it.materialCode)) && it.complainceFlag.equals(Constants.COMPLAINT) }
                    } else {
                        materialData =
                            productList.filter { dataval.contains(MATERIAL_CODE.plus(it.materialCode)) && it.complainceFlag.equals(Constants.NON_COMPLAINT)}
                    }
                    val listMaterial = materialData.map { it.materialCode.plus(" - ").plus(it.materialName) }
                    list.clear()
                    list.addAll(listMaterial)
                }
            }
        }
        customDialog =
    ...
    override fun clickOnItem(data: String, currentFlag: String) {
        ...
        when(currentFlag){
            AppUtils.MATERIAL -> {
                val materialValue = data.split("-")
                binding.tvproductValue.setText(data)
                vm.dispatchWh.materialCode = MATERIAL_CODE.plus(materialValue[0].trim())
                vm.dispatchWh.materialName = materialValue[1]
                if (AppUtils.isOnline()) {
                    var materialCode = binding.tvproductValue.text.toString().split("-")
                    vm.fetchStocks(materialCode[0].trim())
    ...
    private fun enableProceed() {
        val enable =
            (binding.tvWhValue.text.isNotEmpty() && binding.tvDeparturePointValue.text.isNotEmpty() && binding.tvDriverName.text.isNotEmpty()
                ...
                .isNotEmpty() && binding.tvLicenseNumberValue.text.toString()
                .isNotEmpty() && binding.tvnoofbagsValue.text.toString().isNotEmpty()
                    )
        if (enable) {
...
//            binding.tvstoValue.text = data
            ...
            sendingWareHouseList.forEachIndexed { index, vegaCustomStLocation ->
                ...
                if (sendingWareHouseList.get(index).procureLocationCode.equals(data.split(" - ")[0])) {

                    selectedSendingWH = sendingWareHouseList[index].procureLocationCode
                    loggedInPlantId = (sendingWareHouseList[index].plant).toString()
                    ...
                    if (dataval.size > 0) {
                        var materialData =
                            productList.filter { ((MATERIAL_CODE.plus(it.materialCode)).equals(dataval[0].materialCode)) || ((MATERIAL_CODE.plus(it.eudrEquivalentMap)).equals(dataval[0].materialCode))}
                        if (materialData.size > 0) {
                            if(isCompliantMaterial){
                                materialData = materialData.filter { it.complainceFlag.equals(Constants.COMPLAINT) }
                            } else {
        ...
        if (AppUtils.isOnline()) {
            var stockValue =
                stocksList.filter { it.storageLocationCode.equals(selectedSendingWH) }
                    ...
                    .filter { it.materialCode.equals(MATERIAL_CODE.plus(materialCode[0].trim())) }
            totalWeight = (stockValue.sumOf { it.weight?.toDouble() ?: 0.0 })
...
//        val location = purchaseOrder.map {it.warehouseId.plus("-").plus(getMTNTPlantName(it.warehouseId ?: "")) }.toSet()
    ...
    private fun updateofflineStockDetails() {
        ...
        var databaseCount =
            model.filter { it.storageLocationCode.equals(selectedSendingWH) }
                .filter {
                    it.plantId.equals(loggedInPlantId)
                }.filter { it.materialCode.equals(MATERIAL_CODE.plus(materialCode[0].trim())) }
        var weight: Double? = 0.0
        var totalWeight: Double? = 0.0
        //var enteredBags: Double? = 0.0
        totalEnteredBags = 0.0
        localDbBags = 0.0
        /*stockValue.forEach { it1 ->
            totalWeight = ((weight!!) + (it1.weight?.toDouble()!!))
        }*/
        ...
        if (databaseCount.size > 0) {
            databaseCount.forEach { item ->
                if(item.netWeight?.isEmpty() == true) item.netWeight="0.0"
                totalEnteredBags = ((totalEnteredBags)!! + (item.netWeight.toString().toDouble()))

                if(item.purchaseQuantity?.isEmpty() == true) item.purchaseQuantity="0.0"
                localDbBags = ((localDbBags)!! + (item.purchaseQuantity.toString().toDouble()))
            }
            totalEnteredBags = ((totalEnteredBags)!! + ((calculateWeight()).toDouble()))
        } else {
            totalEnteredBags = ((calculateWeight()).toDouble())
        }
    ...
    private fun calculateWeight(): String {
        ...
        var uom = ((uomDetails.filter { ((it.materialCode).equals(materialValue[0].trim())) }).filter {
    ...
    private fun isStockAvailable(count: Int) {
        vm.dispatchWh.closingStockCount=count.toString()
        when {
            count < 0 -> {
