   ...
   /* private fun setUpAdapter(list: List<VegaGhanaCocoaDispatchLots>) {
    ...
    private fun prepareDeliveryList(): List<VegaGhanaMtntDeliveryDetail> {
        ...
        dispatchLotsList.forEach { item ->
            ...
            //var bagItems = bagList.filter { item.batchNumber == deliveryDetail.batchNumber}
            ...
            //deliveryDetail.recStorageLocationCode = summaryObj?.destinationWHName
            ...
            deliveryDetail.purchaseQuantity = item.editedWeight
            deliveryDetail.grossWeight = item.editedWeight
            deliveryDetail.netWeight = item.editedWeight
            /*deliveryDetail.netWeight = summaryObj?.purchaseQuantity?.toDouble()?.times(62.5).toString()
            deliveryDetail.grossWeight = summaryObj?.purchaseQuantity?.toDouble()?.times(62.5).toString()*/
            /*deliveryDetail.netWeight = it.editedWeight!!.toDouble().times(62.5).toString()
            deliveryDetail.grossWeight = it.editedWeight!!.toDouble().times(62.5).toString()*/
            ...
            //deliveryDetail.purchaseQuantity = it.editedWeight
        ...
        bagItems.forEach { item1 ->
            item1.bagCount = item.editedWeight.toString()
            val palletAvg =
            ...
            grossWeight = item.editedWeight!!.toDouble()
            netWeight = item.editedWeight!!.toDouble()

            if(grossWeight.toString().trim().isNullOrEmpty()
                && netWeight.toString().trim().isNullOrEmpty()){
                deliveryDetail.grossWeight ="0"
                deliveryDetail.netWeight = "0"
            }else {
                deliveryDetail.grossWeight =grossWeight.toString().trim()
                deliveryDetail.netWeight = netWeight.toString().trim()
            }
            ...
            //item1.bagCount = it.editedWeight.toString()
        }
        if (bagItems.isNotEmpty()) {
            ...
            deliveryDetail.nohu1 = bagItems.sumBy { tar -> tar.bagCount.toDouble().roundToInt() }.toString().trim()
            bagItems.forEach {
                deliveryDetail.huno = it.unitsOfMeasure
                deliveryDetail.huwt = bagTareWeight.toString().trim()
                deliveryDetail.huno2 = "MT"
                deliveryDetail.huwt2 = it.palletAverage
                deliveryDetail.nohu2 = it.noOfPallet
                if(grossWeight.toString().trim().isNullOrEmpty()
                    && netWeight.toString().trim().isNullOrEmpty()){
                    deliveryDetail.grossWeight ="0"
                    deliveryDetail.netWeight = "0"
            ...
            bag.bagCount = item.editedWeight.toString()
            if (bagTypeList.size == 1) {
                bag.bagMaterialCode = bagTypeList[0].bagMaterialCode.trim()
                bag.tareWeight = bagTypeList[0].tareWeight.toString().trim()
                bag.bagType = bagTypeList[0].bagType.trim()
            } else {
                bagTypeList.forEach { item1 ->
                    bag.bagMaterialCode = item1.bagMaterialCode.trim()
                    bag.tareWeight = item1.tareWeight.toString().trim()
                    bag.bagType = item1.bagType.trim()
                }
            }
            var totalweight: Double =
                (calculateWeight(dispatch?.materialCode.toString())).toDouble()
            bag.grossWeight = (totalweight * (bag.bagCount.toDouble())).toString()
            bag.netWeight = (totalweight * (bag.bagCount.toDouble())).toString()
            bag.noOfPallet = "0"
            bag.palletAverage = "0"
            bag.palletWeight = "0"
            bag.unitsOfMeasure = "KG"

            var bagSort = arrayListOf<VegaCocoaSweepingBagMaterial>()
            bagSort.add(bag)
            deliveryDetail.bagList = bagSort
        }
