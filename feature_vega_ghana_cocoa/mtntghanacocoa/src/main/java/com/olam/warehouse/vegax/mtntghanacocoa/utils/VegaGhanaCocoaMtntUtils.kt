package com.olam.warehouse.vegax.mtntghanacocoa.utils
...
const val JSON_TEXT_UPDATE = "TEXT_UPDATE"
const val BAG = "BAG"
const val MATERIAL_CODE = "000000"
const val UNIT_ZERO = "0.0"
const val UNIT_KG = "MT"
const val VEGA_MODULE="VEGA_MODULE"
const val DWS_MODULE="DWS_MODULE"
...
fun getMTNTPlantName(plantId: String, isPlantID: Boolean): String {

    val plants = Gson().fromJson<List<Plant>>(PreferenceHelper.get(Constants.MTNT_PLANT_LIST, ""))
    val plant = plants.singleOrNull { it.plantId == plantId.trim() }
    val plantName = plants.singleOrNull { it.plantName == plantId.trim() }

    return if (isPlantID) plant?.plantName ?: "" else plantName?.plantId ?: ""
}
