...
class VegaGhanaCocoaCocoaMtntWeighBridgeAddLotFragment : BaseFragment(), GhanaCocoaMTNTItemRemoveListener {
    ...
    private fun showRemarkDialog() {
        ...
        vm.lotList.forEach { it1 ->
            var calculateWeight: Double = calculateWeight(it1.materialCode).toDouble()
            totalWeight = ((totalWeight!!) + (it1.weight?.toDouble()!!).div(calculateWeight))
        }


        vm.dispatchWh.purchaseQuantity = totalWeight?.formatThreeDigits()
