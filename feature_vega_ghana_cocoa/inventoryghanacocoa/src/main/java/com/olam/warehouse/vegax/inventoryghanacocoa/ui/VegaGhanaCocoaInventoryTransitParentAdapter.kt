...
import kotlinx.android.synthetic.main.item_vega_ghana_cocoa_inventory_child.view.*
import kotlinx.android.synthetic.main.item_vega_ghana_cocoa_inventory_parent.view.*
import kotlinx.android.synthetic.main.item_vega_ghana_cocoa_material_details.view.*
       ...
       // var filteredPlantData = data.filter { it.supplyingPlantId.equals(items[position].plant) }
        ...
        holder.binding.expandLayout.setOnExpandListener(object :
            ...
            override fun onExpand(expanded: Boolean) {
                ...
                if (items[position].isExpanded == true) {
                    items[position].isExpanded?.let { holder.itemView.expand_layout.setExpand(it) }
                }
            }
        })
        ...
        holder.itemView.rv_child.adapter =
            VegaGhanaInventoryTransitChildAdapter(storagecodes,storageNames,filteredMaterialList,(filteredMaterialData.distinct()), items[position],storageLocation[position],listener, colorCode, materialList)
    }
...
class VegaGhanaInventoryTransitChildAdapter(
    ...
    override fun onBindViewHolder(holder: ChildViewHolder, position: Int) {
        ...
        holder.itemView.rv_material.setUp(finalMaterialList, R.layout.item_vega_ghana_cocoa_material_details, { it, pos ->
            tvWeightValue.text = it.weight.formatThreeDigits().plus(" ").plus(it.uom)
        ...
        holder.itemView.cvLotDetail.setOnClickListener { listener.navigateTransitToDetails(items[position], code) }
