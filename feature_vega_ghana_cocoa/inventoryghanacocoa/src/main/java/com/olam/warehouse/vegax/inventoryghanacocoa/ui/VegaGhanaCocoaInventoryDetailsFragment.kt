...
import kotlinx.android.synthetic.main.fragment_vega_ghana_cocoa_inventory_list.*
import kotlinx.android.synthetic.main.item_vega_ghana_cocoa_inventory_details.view.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaGhanaCocoaInventoryDetailsFragment : BaseFragment() {
    ...
    private fun setUpAdapter(data: List<InventoryLots>) {
        ...
        binding.rvInventory.setUp(
            ...
            { it, pos ->
                tv_admixture_value.text = DateUtils.getFormatedDate(it.grnDate)
                tv_lot_name.text = it.lotId
                var totalweight: Double = calculateWeight(it.materialCode).toDouble()
                tv_weight_data.text =
                    ((it.stockQty.toDouble()).div(totalweight)).formatThreeDigits().plus(" ")
                        .plus(BAGS)
                if (colorCode != 0)
