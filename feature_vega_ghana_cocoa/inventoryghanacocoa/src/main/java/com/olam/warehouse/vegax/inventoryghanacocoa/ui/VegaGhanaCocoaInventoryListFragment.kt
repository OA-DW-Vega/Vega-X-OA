 ...
 */
class VegaGhanaCocoaInventoryListFragment : BaseFragment(), VegaGhanaCocoaInventoryNavigateToDetailsListener {
    ...
    companion object {
        fun newInstance() = VegaGhanaCocoaInventoryListFragment().putArgs {
        }
    }
    ...
    private fun setAdapter(data: List<VegaInventoryWarehouseModel>) {
        ...
        binding.rvInventory.adapter =
            VegaGhanaCocoaInventoryParentAdapter(data, materialList, uomDetails, this)
    }
    ...
    /* private fun calculateWeight(): String {
         var netWeight = ""
         var uom = ((uomDetails.filter { (it.materialCode).equals(receivingData.materialCode)) }).filter { it.fromUom.equals(BAG) }).single()
         netWeight = ((uom.value1?.toInt()?.toDouble()?.div(uom.value2?.toInt()?.toDouble()!!))).toString()
         return netWeight
     }*/
}
