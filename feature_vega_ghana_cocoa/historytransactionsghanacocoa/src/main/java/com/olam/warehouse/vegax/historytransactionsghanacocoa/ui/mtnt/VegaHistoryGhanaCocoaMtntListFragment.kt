...
import com.olam.warehouse.vegax.historytransactionsghanacocoa.databinding.FragmentHistoryTransactionsGhanaCocoaMtntListBinding
import com.olam.warehouse.vegax.historytransactionsghanacocoa.databinding.ItemHistoryTransactionsGhanaCocoaMtntBinding
...
import com.olam.warehouse.vegax.historytransactionsghanacocoa.utils.HISTORY_TRANSACTIONS_LOT_MTNT
import com.olam.warehouse.vegax.historytransactionsghanacocoa.utils.allPlants
...
class VegaHistoryGhanaCocoaMtntListFragment : BaseFragment() {

    override val layoutResourceId: Int = R.layout.fragment_history_transactions_ghana_cocoa_mtnt_list
    private lateinit var binding: FragmentHistoryTransactionsGhanaCocoaMtntListBinding
    ...
    private var history = mutableListOf<VegaGhanaCocoaMtntHistoryTransactions>()
    private var filteredHistory = mutableListOf<VegaGhanaCocoaMtntHistoryTransactions>()
    private var historyData = VegaGhanaCocoaMtntHistoryTransactions()
    private val mSearchList = mutableListOf<VegaGhanaCocoaMtntHistoryTransactions>()
    private var vegaWbIds = listOf<VegaGhanaCocoaMtntHistoryTransactions>()
    private var vegaWbsortedIds = listOf<VegaGhanaCocoaMtntHistoryTransactions>()
    ...
    companion object {
        fun newInstance(
            historyTransactions: VegaGhanaCocoaMtntHistoryTransactions
        ) = VegaHistoryGhanaCocoaMtntListFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentHistoryTransactionsGhanaCocoaMtntListBinding.inflate(inflater)
        return binding.root
    }
    ...
    private fun updateUIWithOnlineData(response: Resource<GenericReqAndResp<VegaGhanaCocoaMtntHistoryTranxResponse>>) {
        response.let {
            when (it.status) {
                    ...
                    response.data?.data?.mtnTxnDetails.let { it1 ->
                        if (it1?.isNotEmpty() == true) {
                          //  history.clear()
                            ...
                            {
                                vegaWbIds = vegaWbsortedIds.sortedWith(compareBy({ it.deliveryNumber })) as MutableList<VegaGhanaCocoaMtntHistoryTransactions>
                            ...
                            history = vegaWbIds as MutableList<VegaGhanaCocoaMtntHistoryTransactions>
    ...
    private fun setUpAdapter(data: List<VegaGhanaCocoaMtntHistoryTransactions>?) {
        val history1 = data as MutableList<VegaGhanaCocoaMtntHistoryTransactions>
        binding.rvLots.setUpAdapter(
            history1,
            R.layout.item_history_transactions_ghana_cocoa_mtnt,
            ItemHistoryTransactionsGhanaCocoaMtntBinding::inflate,
            { it1, pos, bindItem ->
                if(it1.transporterName?.isNotEmpty() == true) {
                    val vendorNameList =
                ...
                bindItem.tvQuantityValue.text = it1.quantity
