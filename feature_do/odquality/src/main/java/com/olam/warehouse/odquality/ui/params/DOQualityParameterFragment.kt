...
class DOQualityParameterFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        binding.btnParamsProceed.setOnClickListener {
            ...
            val totalQuantity =
                validDataList.sumOf { it?.qualityParameter?.qualityParameterValue?.toInt() ?: 0 }
    ...
    private fun initExtra() {
        arguments?.let {
            ...
            sourceLotId= it.getString(SOURCE_LOT_ID)
        }
        ...
        if (currentKey.startsWith("DO_ID")) {
            if (qcStatus == "R") {
                binding.btnParamsProceed.visibility = View.VISIBLE
            } else {
                binding.btnParamsProceed.visibility = View.GONE
            }
            binding.btnParamsSave.visibility = View.VISIBLE
        } else {
            binding.btnParamsSave.visibility = View.GONE
            binding.btnParamsProceed.visibility = View.VISIBLE
        }
                    ...
                    when (savedResponse.status) {
                        Resource.Status.SUCCESS -> {
                            when (savedResponse.data?.success) {
                                ...
                                } else -> showErrorDialogWithFAQLink(requireContext(),savedResponse.data?.message.toString())
                                //activity?.toast("Error ${savedResponse.data?.message}")
                            }
                        }
                        Resource.Status.ERROR -> {
                            showErrorDialogWithFAQLink(requireContext(),savedResponse.error.toString())
    ...
    private fun updateUI(params: List<DOQualityParamsWithQualitative>?, savedQualityData: DOQualitySavedResponse? = null) {
        ...
        params?.let {
            ...
            when (it.isNotEmpty()) {
                true -> mAdapter.addItems(sortByListOfItems(it), tarWeight, netWeight, grossWeight, challanNo, avgBagWeight, unitsOfMeasure, savedQualityData, sourceLotId, materialObj.complainceFlag)
                else -> {
