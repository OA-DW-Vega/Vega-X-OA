...
import com.olam.warehouse.odreceiving.databinding.ActivityScannerBltBinding
...
@RuntimePermissions
class ScannerActivityBlt : HomeBaseActivity(), SurfaceHolder.Callback, View.OnClickListener, Detector.Processor<Barcode> {
    ...
    private lateinit var binding: ActivityScannerBltBinding
    override val layoutResourceId = R.layout.activity_scanner_blt

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        showCameraWithPermissionCheck()
        val tracker: Tracker? = App.getTracker()
        TrackHelper.track().screen("odreceiving/ui/blt/ScannerActivityBlt").title("OD/Receiving")
            .with(tracker)
        binding = ActivityScannerBltBinding.inflate(layoutInflater)
    ...
    class ScannerActivityBltShowCameraPermissionRequest(target: ScannerActivityBlt): PermissionRequest {
        private val weakTarget: WeakReference<ScannerActivityBlt> = WeakReference(target)
    ...
    private fun initUI() {
        ...
        binding.btnSave.setOnClickListener {
            ...
            if (text.isEmpty() || text.isBlank()) {
                toast(getString(R.string.enter_qr_code))
            } else {
                val intent = Intent()
                intent.putExtra(SCANNED_ID, text)
