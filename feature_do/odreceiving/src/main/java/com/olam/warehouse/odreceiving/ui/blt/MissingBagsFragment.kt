...
import com.olam.warehouse.odreceiving.databinding.BagMissingListItemBinding
...
//        binding.tvMissedBagValue.text = "Missed Bags : $bagMissedCount"
        binding.rvMissingBags.setUpAdapter(
            bags,
            R.layout.bag_missing_list_item,
            BagMissingListItemBinding::inflate,
            { bagItem, pos, bindingItem ->
                bindingItem.tvMissQr.text = bagItem.bagQrCode.toString()

                if (bagItem.bagMissed) {
                    bindingItem.btnMarkMissingBag.text =
                        requireContext().getString(R.string.bag_already_miss)
                    bindingItem.btnMarkMissingBag.isEnabled = false
                    bindingItem.btnMarkMissingBag.setBackgroundColor(Color.DKGRAY)
                } else {
                    bindingItem.btnMarkMissingBag.text = getString(R.string.mark_missing)
                    bindingItem.btnMarkMissingBag.isEnabled = true
                }

                bindingItem.btnMarkMissingBag.setOnClickListener { view ->
                    if (bindingItem.btnMarkMissingBag.text == getString(R.string.mark_missing)) {
...
//        binding.tvMissedBagValue.text = "Missed Bags : $bagMissedCount"
    ...
    private fun showAlert(
        bag: Bag,
        button: Button? = null) {
        val builder = AlertDialog.Builder(this.requireActivity()).apply {
            setMessage(
                getString(R.string.mark_as_missing).plus("\n")
