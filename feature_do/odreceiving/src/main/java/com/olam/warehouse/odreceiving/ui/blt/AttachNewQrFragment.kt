...
import kotlinx.android.synthetic.main.attach_new_qr_list_item.view.*
    ...
    private fun initUI() {
        ...
        binding.rvAttachNewQr.setUp(bags, R.layout.attach_new_qr_list_item, { bagItem, pos ->
            ...
            if (bagItem.isQrCodeReplaced) {
                oldQrCode.text = getString(R.string.old_qr_code)
                newQrCode.visibility = View.VISIBLE
                tvNewMissQr.visibility = View.VISIBLE
                tvNewMissQr.text = bagItem.newQrCode.toString()
                btnAttachQrCode.isEnabled = false
                btnAttachQrCode?.setBackgroundColor(Color.DKGRAY)
            } else {
                oldQrCode.text = getString(R.string.qr_code)
                newQrCode.visibility = View.GONE
                tvNewMissQr.visibility = View.GONE
                btnAttachQrCode.isEnabled = true
            }
