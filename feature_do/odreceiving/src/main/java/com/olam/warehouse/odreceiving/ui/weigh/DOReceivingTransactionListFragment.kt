...
import com.olam.warehouse.odreceiving.databinding.ItemDoReceivingTransactionDetailsBinding
...
class DOReceivingTransactionListFragment : BaseFragment() {
    ...
    private var onlineTransList = mutableListOf<DOTransactionDetail>()
    private var offlineTransList = mutableListOf<DOTxnDetail>()
...
//            vm.getTransactionsOffline()
        ...
        vm.product.observe(viewLifecycleOwner, Observer { materials ->
            if(onlineTransList.isNotEmpty()) {
                    materialsList.clear()
                    materialsList.addAll(materials)
                    checkMaterialMatch(materials, onlineTransList)
            } else
            checkMaterialMatchOffline(materials, offlineTransList)
        })
    }
    ...
    private fun setUpTransListOffline(data: List<DOTxnDetail>) {
        offlineTransList= data.toMutableList()
        vm.getProducts()
    }
    ...
    private fun setUpTransList(data: List<DOTransactionDetail>) {
        onlineTransList=data.toMutableList()
        vm.getProducts()
    }
    ...
    private fun setUpAdapter(data: List<DOTransactionDetail>? = null, dataOffline: List<DOTxnDetail>? = null) {
        if (data != null) {
            ...
            binding.rvTransaction.setUpAdapter(
                ...
                R.layout.item_do_receiving_transaction_details,
                ItemDoReceivingTransactionDetailsBinding::inflate,
                { it, pos, bindingItem ->
                    bindingItem.tvTransactionId.text = it.lotTransactionId
                    bindingItem.tvProduct.text = it.materialName
                    bindingItem.tvSupplier.text = it.supplierName
                    bindingItem.tvSupplierId.text = it.displaySupplierId
                    bindingItem.tvCreatedDate.text = it.creationDate
                    bindingItem.tvWeight.text = it.netweight
                    bindingItem.tvUom.text = it.weightUOM
                    bindingItem.tvSourceLotId.text = it.sourceLotId
                    if(it?.eudrComplaint == true){
                        bindingItem.tvEudrFlag.text = Constants.COMPLAINT
                    } else {
                        bindingItem.tvEudrFlag.text = Constants.NON_COMPLAINT
                    }
                },
                {
                    if (isOnline()) {
...
//        doTxnDetail.transactionDetails = mutableListOf(TransactionDetail(transactionDetail.noOfBags, transactionDetail.lotTransactionId, bagList))
    ...
    private fun moveToWeighPage(data: DOTransactionDetail?) {
        ...
        receivingData.sourceLotId = data.sourceLotId
        receivingData.eudrComplaint = data.eudrComplaint
