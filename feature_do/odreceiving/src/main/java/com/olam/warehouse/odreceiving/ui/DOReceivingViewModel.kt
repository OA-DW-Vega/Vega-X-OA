...
//    val transaction: LiveData<Resource<GenericReqAndResp<DOTransactionDetail>>> get() = _transaction
    ...
    private var sapMaterialByProductListSource: LiveData<Resource<GenericReqAndResp<List<DOSapMaterialList>>>> = MutableLiveData()
    private val _materialByProduct=MediatorLiveData<Resource<GenericReqAndResp<List<DOSapMaterialList>>>>()
    val materialByProductList:LiveData<Resource<GenericReqAndResp<List<DOSapMaterialList>>>> get()= _materialByProduct

    private var sapMaterialOfflineByProductListSource: LiveData<List<DOSapMaterialList>> = MutableLiveData()
    private val _offlineMaterialByProduct= MediatorLiveData<List<DOSapMaterialList>>()
    val offlineMaterialByProductList:LiveData<List<DOSapMaterialList>> get()= _offlineMaterialByProduct
    ...
    fun getMaterialByProducts()= viewModelScope.launch(dispatchers.main){
        _materialByProduct.removeSource(sapMaterialByProductListSource)
        withContext(dispatchers.io){
            sapMaterialByProductListSource=useCase.getSapMaterialByProduct()
        }
        _materialByProduct.addSource(sapMaterialByProductListSource){
            _materialByProduct.value=it
        }
    }

    fun getOfflineMaterialByProduct()= viewModelScope.launch(dispatchers.main){
        _offlineMaterialByProduct.removeSource(sapMaterialOfflineByProductListSource)
        withContext(dispatchers.io){
            sapMaterialOfflineByProductListSource=useCase.getOfflineODSapMaterialByProduct()
        }
        _offlineMaterialByProduct.addSource(sapMaterialOfflineByProductListSource){
            _offlineMaterialByProduct.value=it
        }
    }
