...
import kotlinx.android.synthetic.main.item_do_receiving_summary_mtnr.*
...
class DOReceivingSummaryMtnrFragment : BaseFragment() {
    ...
    private fun setUpRecyclerView() {
        if (postData.isNotEmpty()) {
            ...
            binding.rvSummaryMtn.setUp(lotIds, R.layout.item_do_receiving_summary_mtnr, { it, pos ->
                val rec = map[it]
                tvLotNo.text = it
                tvBag.text = rec?.sumBy { _rec -> _rec.bagCount?.toInt() ?: 0 }.toString()
                tvPallet.isGone = isWeighScale
                if (!isWeighScale) {
                    tvPallet.text = rec?.sumBy { _rec -> _rec.palletCount?.toInt() ?: 0 }.toString()
                }
                tvWeight.text = rec?.sumByDouble { _rec -> _rec.netWeight }?.format().toString()
            })
        }
    }
