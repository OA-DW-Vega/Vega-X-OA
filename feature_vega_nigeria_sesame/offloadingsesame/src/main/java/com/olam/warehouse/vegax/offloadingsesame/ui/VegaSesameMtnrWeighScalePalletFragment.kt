...
import com.olam.warehouse.vegax.offloadingsesame.databinding.FragmentVegaSesameOffloadAddWeightLayoutBinding
import com.olam.warehouse.vegax.offloadingsesame.databinding.ItemVegaSesameOffloadAddBagBinding
import com.olam.warehouse.vegax.offloadingsesame.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import kotlin.random.Random

class VegaSesameMtnrWeighScalePalletFragment : BaseFragment() {

    override val layoutResourceId: Int =
        R.layout.fragment_vega_sesame_offload_add_weight_layout
    private lateinit var binding: FragmentVegaSesameOffloadAddWeightLayoutBinding
    ...
    companion object {
        fun newInstance(item: VegaCoffeeReceiveLots) = VegaSesameMtnrWeighScalePalletFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentVegaSesameOffloadAddWeightLayoutBinding.inflate(inflater)
        initUI()
        return binding.root
    }
    ...
    /*private fun updatePalletUI(response: Resource<GenericReqAndResp<List<VegaSesameWeighScalePallet>>>?) {
    ...
    fun onBackPressed() {
        val weight =
            if (binding.tvNetWtValue.text.toString().length < 3) ZERO_VALUE.plus(" ").plus(UNIT_KG) else binding.tvNetWtValue.text.toString()
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCoffeeOffloadingBagMaterial>) {
        ...
        binding.rvWeight.setUpAdapter(
            bagList1.asReversed(),
            R.layout.item_vega_sesame_offload_add_bag,
            ItemVegaSesameOffloadAddBagBinding::inflate,
            { it, pos, bindItem ->
                if (pos % 2 == 0) {
                    bindItem.clBagHead.setBackgroundResource(com.olam.warehouse.presentation.R.drawable.rectangle_grey_border)
                } else {
                    bindItem.clBagHead.setBackgroundResource(R.drawable.shape_sesame_offload_rect_light_grey1)
                }
