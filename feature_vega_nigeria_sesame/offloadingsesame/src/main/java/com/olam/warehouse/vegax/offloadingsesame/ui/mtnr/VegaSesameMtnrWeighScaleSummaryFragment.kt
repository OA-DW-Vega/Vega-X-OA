...
import kotlinx.android.synthetic.main.offloading_custom_edit_text_sesame.view.*
...
class VegaSesameMtnrWeighScaleSummaryFragment : BaseFragment() {
    ...
    private fun setUpAdapter(list: List<VegaCoffeeReceiveLots>) {
        ...
        binding.rvLots.setUp(lots, R.layout.item_sesame_mtnr_weighscale_lot_card_layout, { item, pos ->
            ...
            val editedWeight = if (receivingKg.isNullOrEmpty()) "0.0" else receivingKg.toString().toDouble().formatThreeDigits()
            ...
            tvTotalWeightLossValue.text = weightLoss?.formatThreeDigits()?.plus(" ")?.plus("KG")
            ivEdit.visible()
