...
import com.olam.warehouse.vegax.offloadingsesame.databinding.FragmentVegaOffloadingSesameSupplierSummaryBinding
import com.olam.warehouse.vegax.offloadingsesame.ui.VegaSesameOffloadingViewModel
import com.olam.warehouse.vegax.offloadingsesame.utils.OFFLOADING_DATA
import com.olam.warehouse.vegax.offloadingsesame.utils.OFFLOADING_POST_BAG_DATA
import com.olam.warehouse.vegax.offloadingsesame.utils.OFFLOADING_POST_DATA
import com.olam.warehouse.vegax.offloadingsesame.utils.getTmpId
 ...
 */
class VegaSesameOffloadingSupplierSummaryFragment : BaseFragment() {
    override val layoutResourceId: Int = R.layout.fragment_vega_offloading_sesame_supplier_summary
    ...
    private lateinit var binding: FragmentVegaOffloadingSesameSupplierSummaryBinding
    ...
    companion object {
        ...
        ) = VegaSesameOffloadingSupplierSummaryFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentVegaOffloadingSesameSupplierSummaryBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        binding.tvGrossValue.text = receivingData.grossWeight.plus(" ").plus("KG")
        binding.tvNoBagsValue.text = receivingData.bagCount
        binding.tvBagsTareValue.text = receivingData.tareWeight.plus(" ").plus("KG")
        binding.tvNetWeightValue.text = receivingData.netWeight.plus(" ").plus("KG")
            ...
            if(count!=0&& DateUtils.isFirstDaysOfMonth(context!!, count))
            {
                UIUtils.showPostingDateDialog(context!!, count, object : UIUtils.DialogClick {
                    override fun onPositive(date: String) {
                        var postingDate= DateUtils.oneFormatToOtherFormat(date,"dd-MMM-yyyy","yyyyMMdd")
                        mReceiving.forEach {  vegaReceiving ->
                            vegaReceiving.postingDate=postingDate
                        }
