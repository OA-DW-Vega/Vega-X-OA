...
import com.olam.warehouse.vegax.grnsesame.databinding.FragmentVegaNigeriaSesameGrnDetailsBinding
import com.olam.warehouse.vegax.grnsesame.utils.*
 ...
 */
class VegaNigeriaSesameGrnDetailsFragment : BaseFragment() {
    ...
    private var approveQualityList = ArrayList<VegaGRNSesameQuality>()
    private var storageLocation: List<VegaCustomStLocation>? = null

    private lateinit var binding: FragmentVegaNigeriaSesameGrnDetailsBinding
    override val layoutResourceId = R.layout.fragment_vega_nigeria_sesame_grn_details
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentVegaNigeriaSesameGrnDetailsBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    companion object {
        fun newInstance(grnData: VegaGrnWeighBridgeId) = VegaNigeriaSesameGrnDetailsFragment().putArgs {
    ...
    private fun initUI() {
        context?.let {
            getActionBtnChangedView(binding.grnDetailsTitle, it, false)
    ...
    private fun updateUI(data: Resource<GenericReqAndResp<List<VegaGRNSesameQuality>>>) {
        data.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    hideLoading()
                    it.data?.data?.let { it1 ->
                        if (it1.isNotEmpty()) approveQualityList = it1 as ArrayList<VegaGRNSesameQuality>
                    ...
                    if (isOnline()) {
                        vm.postGrn(
                            VegaSesameGrnPost(
                                ...
                                lotQualityDetails= approveQualityList,
                                wbDetailsList= wbDetails,
    ...
    private fun moveToSuccessPage(grn: String,wbId: String,batchNo: String,encodedImageContent:String?) {
