 ...
 */
class VegaNigeriaSesameGrnViewModel(
    private val useCase: VegaNigeriaSesameGrnUseCase,
    ...
    private var qualitySource: LiveData<Resource<GenericReqAndResp<List<VegaGRNSesameQuality>>>> =
        MutableLiveData()
    private val _qualityDetails =
        MediatorLiveData<Resource<GenericReqAndResp<List<VegaGRNSesameQuality>>>>()
    val qualityDetails: LiveData<Resource<GenericReqAndResp<List<VegaGRNSesameQuality>>>> get() = _qualityDetails
    ...
    fun postGrn(grnPost: VegaSesameGrnPost) = viewModelScope.launch(dispatchers.main) {
