...
import com.olam.warehouse.vegax.pilesesame.databinding.ItemPileManagmentSelectionSesameBinding
...
//                isNewLotCreate = false
    ...
    private fun removeCheckedPos(item:Int, list: MutableList<VegaCocoaDispatchLots>) {
...
//    }
    ...
    private fun updateAdapter(stockList: MutableList<VegaCocoaDispatchLots>) {
        ...
        if ((getCurrentKey().split("_")[1].contains("NG")) /*&& (getCurrentKey().split("_")[2] == "SESA")*/) {
            binding.rvInventoryLot.setUpAdapter(
                stockList,
                R.layout.item_pile_managment_selection_sesame,
                ItemPileManagmentSelectionSesameBinding::inflate,
                { it, pos, bindItem ->
                    bindItem.tvBagStore.gone()
                    bindItem.tvStoreLocation.gone()
            ...
            binding.rvInventoryLot.setUpAdapter(
                ...
                ItemPileManagmentSelectionSesameBinding::inflate,
                { it, pos, bindItem ->
                    bindItem.tvBagStore.visible()
                    bindItem.tvStoreLocation.visible()
                    bindItem.tvPileNo.text = it.batchNumber
                    bindItem.tvWeight.text = it.weight.plus(" ").plus(it.unitOfMeasure)
                    ...
                    bindItem.llLotItem.setOnClickListener { view ->
                        ...
                        bindItem.checkbox.isChecked = it.isAdded
                        if (it.isAdded!!)
                            removeCheckedPos(pos, stockList)

                        binding.cbCreateNewLot.isChecked = false
                        isNewLotCreate = false
                        if (it.isAdded) pileSelection = it
                    }
                    enableDisableBtn(stockList.any { it.isAdded })
                })
