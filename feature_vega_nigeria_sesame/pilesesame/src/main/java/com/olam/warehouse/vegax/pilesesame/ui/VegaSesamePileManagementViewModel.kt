...
class VegaSesamePileManagementViewModel(
    ...
    private var lotQualitySource: LiveData<Resource<GenericReqAndResp<List<VegaDispatchLotQuality>>>> =
        MutableLiveData()
    private val _lotQualityDetails = MediatorLiveData<Resource<GenericReqAndResp<List<VegaDispatchLotQuality>>>>()
    val lotQuality: LiveData<Resource<GenericReqAndResp<List<VegaDispatchLotQuality>>>> get() = _lotQualityDetails
    ...
    fun getLotQualityParams( charge:String, material: String) =
        viewModelScope.launch(dispatchers.main) {
            _lotQualityDetails.removeSource(lotQualitySource)
            withContext(dispatchers.io) {
                lotQualitySource = useCase.getLotQualityParams(charge, material)
            }
            _lotQualityDetails.addSource(lotQualitySource) {
                _lotQualityDetails.value = it
            }
        }
   ...
   /* private var ticketSequenceSource: LiveData<Resource<GenericReqAndResp<NicaraguaUpdatePileSequence>>> =
        MutableLiveData()
    private val _ticketSequence =
        MediatorLiveData<Resource<GenericReqAndResp<NicaraguaUpdatePileSequence>>>()
    val ticketSequence: LiveData<Resource<GenericReqAndResp<NicaraguaUpdatePileSequence>>> get() = _ticketSequence*/
    ...
    private var wsWeightedAvgSource: LiveData<Resource<GenericReqAndResp<VegaNigeriaSesameWeightedAverageResponse>>> =
        MutableLiveData()
    private val _wsWeightedAvgPost =
        MediatorLiveData<Resource<GenericReqAndResp<VegaNigeriaSesameWeightedAverageResponse>>>()
    val wsWeightedAvgPost: LiveData<Resource<GenericReqAndResp<VegaNigeriaSesameWeightedAverageResponse>>> get() = _wsWeightedAvgPost

    fun postWeightedAverage(vegaWeightedAvgPost: VegaNigeriaSesameWeightedAveragePost) {
        viewModelScope.launch(dispatchers.main) {
   ...
   /* fun updateTicketSequence(paramPost: NicaraguaUpdatePileSequence) =
        viewModelScope.launch(dispatchers.main) {
            _ticketSequence.removeSource(ticketSequenceSource) // We make sure there is only one source of livedata (allowing us properly refresh)
            withContext(dispatchers.io) {
                ticketSequenceSource = useCase.updatePileSequence(paramPost)
            }
            _ticketSequence.addSource(ticketSequenceSource) {
                _ticketSequence.value = it
            }
        }*/
...
*/
        val cropyear = PreferenceHelper.get(Constants.CROP_YEAR, "").last()
        pileSequnceId = position1.plus(cropyear).plus(pileSequence)
    ...
    fun generatePileTicketNumberUsingFGRNSeq(): String {
        var ticketNumber = ""
        val tallySequence = PreferenceHelper.get(Constants.FGRN_TALLY_SEQUENCE, "")
        val cropyear = PreferenceHelper.get(Constants.CROP_YEAR, "")
        ticketNumber = cropyear.plus("-").plus(tallySequence)

        return ticketNumber
    }

    fun generatePileTicketNumberUsingTallySeq():String{
        var ticketNumber = ""
