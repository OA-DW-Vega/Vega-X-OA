...
class VegaNigeriaSesameMtntWeighScaleSummaryFragment : BaseFragment(),
    ...
    private fun setUpMaterialAdapter() {
        binding.header.rvMaterialList.setUp(vm.materialModelList, R.layout.item_nigeria_sesame_material_layout, { it, pos ->
        ...
        if(count!=0&& DateUtils.isFirstDaysOfMonth(context!!, count))
        {
            UIUtils.showPostingDateDialog(context!!, count, object : UIUtils.DialogClick {
                override fun onPositive(date: String) {
                    var postingDate= DateUtils.oneFormatToOtherFormat(date,"dd-MMM-yyyy","yyyy-MM-dd'T'HH:mm")
