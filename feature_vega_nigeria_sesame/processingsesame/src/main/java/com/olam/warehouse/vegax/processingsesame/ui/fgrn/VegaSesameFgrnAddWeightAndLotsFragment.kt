...
import com.olam.warehouse.vegax.processingsesame.databinding.FragmentVegaSesameFgrnAddWeightAndLotsBinding
import com.olam.warehouse.vegax.processingsesame.databinding.ItemVegaSesameFgrnAddBagBinding
...
class VegaSesameFgrnAddWeightAndLotsFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_vega_sesame_fgrn_add_weight_and_lots
    private lateinit var binding: FragmentVegaSesameFgrnAddWeightAndLotsBinding
    ...
    private var secondMaterialStorageLocation: String? = ""
    ...
    companion object {
        fun newInstance(
            ...
            isEdit: Boolean,
            vegaStage: VegaProcessingStage
        ) = VegaSesameFgrnAddWeightAndLotsFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaSesameFgrnAddWeightAndLotsBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        gradeList =
            Gson().fromJson<List<VegaCoffeeFgrnItemsGrades>>(arguments?.getString(FRAG_ID) ?: "")
    ...
    private fun updateLotCard(fgrnGrades: VegaCoffeeFgrnItemsGrades) {
        ...
        if (fgrnGrades.isDefaultLot!!) {
            ...
            if (fgrnGrades.batchNumber.isNotEmpty() && !weight.equals("0.0")) {
                ...
                binding.icAssignLotCard.tvWeightValue.text = weight.plus(" ").plus("KG")
            } else {
                enableDisableAssign(true)
                binding.flAssignLot.gone()
            }
    ...
    private fun validateGrades() {
        val netWeight = binding.clNet.tvNetWeightValue.text.toString().split(" ")[0].toDouble()
        ...
        if (netWeight <= poQuantity) {
            if (netWeight <= processWeight) {
                if (currentGrade.xchpf.toString() == "X") {
                    ...
                    if (currentGrade.lotStorageLocationCode.isNullOrEmpty()) {
                        currentGrade.lotStorageLocationCode = secondMaterialStorageLocation
                        vm.saveFgrnGrade(currentGrade)
                    }
                    validateLot()
                }
            } else {
                activity?.toast("Total FGRN weight cannot be greater than Weight in Processing")
            }
        } else {
    ...
    private fun validateLot() {
        if ((currentGrade.isDefaultLot!! && !currentGrade.eligibeWeight.equals("0.0")) ||
            ...
            if (currentGrade.eligibeWeight.isNullOrEmpty())
                currentGrade.eligibeWeight = "0.0"
    ...
    private fun moveToSifftSelection() {
        ...
        if (isEdit)
            ...
            /*fgrnItem.gradeList?.forEach {
                if(it.xchpf != "X"){
                    it.lotStorageLocationCode = secondMaterialStorageLocation
                    vm.saveFgrnGrade(it)
                }
            }*/
            callBack?.replaceFgrnFragment(FRAG_SHIFT, fgrnItem, "", isThirdPartyMaterial, "", vegaStage)
    ...
    private fun updateGradesWithBagItems(items: VegaCoffeeFgrnGradesWithBagItems?) {
        ...
        items.let {
            ...
            if (it.fgrnGrades.xchpf.toString() == "X") {
                binding.clAssignLot.visibility = View.VISIBLE
            } else {
                binding.clAssignLot.visibility = View.GONE
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCoffeeFgrnGradesMatrialWeights>) {
        ...
        binding.rvWeight.setUpAdapter(
            bagList1.asReversed(), R.layout.item_vega_sesame_fgrn_add_bag,
            ItemVegaSesameFgrnAddBagBinding::inflate,
            { it, pos, bindItem ->
                if (pos % 2 == 0) {
    ...
    private fun enableSaveBtn(flag: Boolean) {
        when (flag) {
            true -> {
                ...
                binding.btSave.setTextColor(
                    ContextCompat.getColor(
                        requireContext(),
                        com.olam.warehouse.presentation.R.color.text_black
                    )
                )
                ViewCompat.setBackground(
                    binding.btSave,
                    ContextCompat.getDrawable(
                        binding.btSave.context,
                        com.olam.warehouse.presentation.R.drawable.rectangle_black_border
                    )
                )
            }
            false -> {
                ...
                ViewCompat.setBackground(
                    ...
                    ContextCompat.getDrawable(
                        binding.btSave.context,
                        com.olam.warehouse.presentation.R.drawable.rectangle_grey_border_4dp
                    )
                )
            }
        }
    ...
    fun updateLotDetails(bundle: Bundle) {
        ...
        secondMaterialStorageLocation = currentGrade.lotStorageLocationCode
        vm.saveFgrnGrade(currentGrade)
    }
