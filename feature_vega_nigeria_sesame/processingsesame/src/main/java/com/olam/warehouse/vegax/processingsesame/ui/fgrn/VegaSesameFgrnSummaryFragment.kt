...
import kotlinx.android.synthetic.main.item_vega_sesame_bag_consump_summary.view.*
import kotlinx.android.synthetic.main.item_vega_sesame_fgrn_summary.view.*
...
//        intent.putExtra(AppUtils.SUB_TITLE, "Process Order ".plus(po).plus("\n").plus("Batch Number ").plus(data.toString().replace("[", "").replace("]", "")))
    ...
    private fun updateAdapter(gradeItems: List<VegaCoffeeFgrnGradesWithBagItems>) {
        binding.rvFgrnSummary.setUp(gradeItems as MutableList, R.layout.item_vega_sesame_fgrn_summary, { it, pos ->
    ...
    private fun updateBagConsumptionAdapter(
        ...
        binding.rvBagConsumption.setUp(bagConsumpList, R.layout.item_vega_sesame_bag_consump_summary, { item, pos ->
