...
import com.olam.warehouse.vegax.processingsesame.data.domain.model.VegaSesameProcessType
import com.olam.warehouse.vegax.processingsesame.data.domain.model.VegaSesameShiftProcessTypeModel
import com.olam.warehouse.vegax.processingsesame.databinding.FragmentVegaSesameFgrnAssignLotBinding
import com.olam.warehouse.vegax.processingsesame.databinding.ItemVegaSesameFgrnLotBinding
...
class VegaSesameFgrnAssignLotFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_vega_sesame_fgrn_assign_lot
    private lateinit var binding: FragmentVegaSesameFgrnAssignLotBinding
    ...
    companion object {
        fun newInstance(
            ...
            VegaSesameFgrnAssignLotFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaSesameFgrnAssignLotBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun updateProcessType(miscellaneous: List<VegaCocoaMiscellaneous>) {
        ...
        val gson = Gson()
        var processTypeList = mutableListOf<VegaSesameProcessType>()
        jsonData.forEach {
            if (it.contains(JSON_PROCESS_TYPE_LIST)) {
                val processTypeList = gson.fromJson(it, VegaSesameShiftProcessTypeModel::class.java)
    ...
    private fun updateStorageLoc(customLocationList: MutableList<VegaCustomStLocation>) {
        customLocationList.let {
            ...
            val stageAdapter =
                ArrayAdapter(requireContext(), R.layout.item_vega_sesame_processing_rmin_grade, storageLoc)
                    ...
                    when (it.data?.success) {
                        true -> {
                            ...
                            stockList.forEachIndexed { index, s ->
                                stockList[index].weight = convertMtToKg(
                                    stockList[index].weight.toString(),
                                    stockList[index].unitOfMeasure.toString()
                                )
                            }
    ...
    private fun updateAdapter(stockList: MutableList<VegaCoffeeRminLots>) {
        ...
        binding.rvInventoryLot.setUpAdapter(
            stockList,
            R.layout.item_vega_sesame_fgrn_lot,
            ItemVegaSesameFgrnLotBinding::inflate,
            { it, pos, bindItem ->
