...
import kotlinx.android.synthetic.main.fragment_vega_nigeria_sesame_inventory_list.*
import kotlinx.android.synthetic.main.item_vega_nigeria_sesame_inventory_details.view.*
...
class VegaNigeriaSesameInventoryDetailsFragment : BaseFragment() {
    ...
    private fun setUpAdapter(data: List<InventoryLots>) {
        ...
        binding.rvInventory.setUp(lotList, R.layout.item_vega_nigeria_sesame_inventory_details, { it, pos ->
            tv_lot_name.text = it.lotId
            if (getCurrentKey().split("_")[2].contains(KEY_SESAME)) {
                tv_weight_data.text = it.stockQty.plus(" ").plus("MT")
                tv_admixture_value.text = it.admixture
            } else if (getCurrentKey().split("_")[2].contains(KEY_CASHEW)) {
                tv_weight_data.text = it.stockQty.plus(" ").plus("KG")
                tv_admixture_value.text = it.kor
