...
import com.olam.warehouse.vegax.inventorysesame.data.domain.model.*
import com.olam.warehouse.vegax.inventorysesame.databinding.FragmentVegaNigeriaSesameInventoryListBinding
import com.olam.warehouse.vegax.inventorysesame.databinding.ItemVegaNigeriaSesameInventoryDetailsBinding
import com.olam.warehouse.vegax.inventorysesame.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaNigeriaSesameInventoryDetailsFragment : BaseFragment() {
    private lateinit var binding: FragmentVegaNigeriaSesameInventoryListBinding
    private val vm: VegaNigeriaSesameInventoryViewModel by viewModel()
    override val layoutResourceId = R.layout.fragment_vega_nigeria_sesame_inventory_list
    ...
    companion object {
        fun newInstance(bundle: Bundle) = VegaNigeriaSesameInventoryDetailsFragment().putArgs {
    ...
    private fun updateLotInfo(response: Resource<GenericReqAndResp<List<VegaSesameInventoryStocks>>>) {
        response.let {
            when (it.status) {
    ...
    private fun prepareInventoryList(items: List<VegaSesameInventoryStocks>): ArrayList<InventoryLots> {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        setHasOptionsMenu(true)
        binding = FragmentVegaNigeriaSesameInventoryListBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun setUpAdapter(data: List<InventoryLots>) {
        ...
        binding.rvInventory.setUpAdapter(
            lotList,
            R.layout.item_vega_nigeria_sesame_inventory_details,
            ItemVegaNigeriaSesameInventoryDetailsBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvLotName.text = it.lotId
                if (getCurrentKey().split("_")[2].contains(KEY_SESAME)) {
                    bindItem.tvWeightData.text = it.stockQty.plus(" ").plus("MT")
                    bindItem.tvAdmixtureValue.text = it.admixture
                } else if (getCurrentKey().split("_")[2].contains(KEY_CASHEW)) {
                    bindItem.tvWeightData.text = it.stockQty.plus(" ").plus("KG")
                    bindItem.tvAdmixtureValue.text = it.kor
                    bindItem.tvImpurity.text = "KOR"
                }
                if (colorCode != 0)
