...
object ModuleDependency {
    ...
    const val NIGERIAWEIGHMENT = ":feature_vega_nigeria:nigeriaweighment"
...
//    const val GHANAQUALITY = ":feature_vega_ghana_cashew:qualityghana"
    ...
    const val HISTORYTRANSACTIONSGHANACOCOA = ":feature_vega_ghana_cocoa:historytransactionsghanacocoa"
//    const val CREATEMAPAR = ":feature_vega_ar:createmapar"
    const val NIGERIABAGISSUE = ":feature_vega_nigeria:bagissuenigeriacocoa"
    const val ECUADORPROCESSING = ":feature_vega_ecuador:processingecuador"
    const val ECUADORPILE = ":feature_vega_ecuador:pileecuador"
    const val ECUADOREXPORTSALES = ":feature_vega_ecuador:exportsalesecuador"
    const val ECUADORLOCALSALES = ":feature_vega_ecuador:localsalesecuador"
    ...
    const val GHANACASHEWTRANSHISTORY = ":feature_vega_ghana:transhistoryghanacashew"
    ...
    fun getDynamicFeatureModules() = getAllModules()
        .filter { it.startsWith(FEATURE_CASHIEW) || it.startsWith(FEATURE_CAMEROON_COCOA) || it.startsWith(FEATURE_COCOA)|| it.startsWith(FEATURE_ECUDOR)|| it.startsWith(FEATURE_NICARAGUA) }
        .toSet()

    fun getDynamicFeatureModulesForGHNGCOTT() = getAllModules()
        .filter { it.startsWith(FEATURE_GHANA_COCOA) || it.startsWith(FEATURE_GHANA_CASHEW) || it.startsWith(FEATURE_NIGERIA_SESAME)|| it.startsWith(FEATURE_NIGERIA_COCOA)|| it.startsWith(FEATURE_PORT_WAREHOUSE) || it.startsWith(COFFEEPPQ)||it.startsWith(COMMON_TRANSACTION_HISTORY)|| it.startsWith(FEATURE_OD)}
        .toSet()
    ...
    fun getNicaraguaDynamicFeatureModules() = getAllModules()
        .filter {
            ...
            ) || it.startsWith(SPLITLOT) || it.startsWith(REPORTS)
        }.toSet()
    ...
    fun getCocoaDynamicFeatureModules() = getAllModules()
        .filter { it.startsWith(FEATURE_COCOA) || it.startsWith(INDOQUALITY) || it.startsWith(
            INDOGRN) || it.startsWith(INDIACOFFEEQUALITYAPPROVE) }
        .toSet()
    ...
    fun getCashewDynamicFeatureModules() = getAllModules()
        .filter { it.startsWith(FEATURE_CASHIEW)  || it.startsWith(NIGERIABAGISSUE)   }
        .toSet()
    ...
    fun getNigeriaCocoaDynamicFeatureModules() = getAllModules()
        .filter { it.startsWith(FEATURE_NIGERIA_COCOA)||it.startsWith(COFFEEPPQ)|| it.startsWith(QUALITYOFANYLOT)
                || it.startsWith(COMMON_TRANSACTION_HISTORY) }
        .toSet()

    fun getCameroonDynamicFeatureModules() = getAllModules()
        .filter { it.startsWith(FEATURE_CAMEROON_COCOA) || it.startsWith(STOCK_RECON) }
        .toSet()
    ...
    fun getGhanaCashewDynamicFeatureModules() = getAllModules()
            .filter {
                it.startsWith(FEATURE_GHANA_CASHEW) || it.startsWith(FEATURE_OD)|| it.startsWith(QUALITYOFANYLOT) || it.startsWith(DUMMYQUALITY)
                        || it.startsWith(COMMON_SECRET_ID) || it.startsWith(REPORTS) || it.startsWith(NOTIFY_CONFIG)
            }
            .toSet()
    ...
    fun getIndoCoffeeAndODDynamicFeatureModules() = getAllModules()
            .filter { it.startsWith(FEATURE_INDO_COFFEE) || it.startsWith(FEATURE_OD)|| it.startsWith(REPORTS) }
            .toSet()
}
