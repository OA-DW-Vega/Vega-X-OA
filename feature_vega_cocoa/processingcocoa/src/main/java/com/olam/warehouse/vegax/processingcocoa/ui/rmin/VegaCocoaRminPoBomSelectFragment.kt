...
import com.olam.warehouse.vegax.processingcocoa.databinding.FragmentPoBomSelectLayoutBinding
import com.olam.warehouse.vegax.processingcocoa.databinding.ItemBomLayoutBinding
...
class VegaCocoaRminPoBomSelectFragment : BaseFragment() {
    ...
    companion object {
        fun newInstance(title: String, isPo: Boolean, model: VegaCocoaRminProcessing) =
            VegaCocoaRminPoBomSelectFragment().putArgs {
                    ...
                    when (it.data?.success) {
                        true -> {
                            response.data?.data?.let {
                                poList.clear()
                                it.forEach { item ->
                                    println("modelMater:" + model?.materialCode.toString())
                                    val rmindata =
