package com.olam.warehouse.vegax.processingcocoa.utils
...
fun prepareFgrnPostRequest(
    ...
    lotDetails: List<VegaCocoaFgrnGradesWithBagItems>?, isRoundOff: Boolean, isIndexweighmenttype: Boolean,sapMaterialList: ArrayList<VegaMaterial> = ArrayList<VegaMaterial>()
    ...
    poReq.weighmentType = poOrderDetails?.weighmentType
    ...
    lotDetails?.forEach { it ->
        ...
        when (it.fgrnGrades.meins) {
            "MT" -> {
               // if(it.fgrnGrades.isRoundOff!!)
                ...
                //else netWeight = (grossWeight.minus(tareWeight)).div(1000)
            }
            "KG" -> netWeight = grossWeight.minus(tareWeight)
            else ->  netWeight = grossWeight.minus(tareWeight)
        }
        processLot.meterStartWeight = poOrderDetails?.meterStartWeight
        processLot.meterEndWeight = poOrderDetails?.meterEndWeight
        ...
        processLot.operatorName = poOrderDetails?.operatorName
        processLot.shiftType = poOrderDetails?.shiftSelection
...
): VegaCocoaProcessingCreatePoReq {
    val poReq = VegaCocoaProcessingCreatePoReq()
