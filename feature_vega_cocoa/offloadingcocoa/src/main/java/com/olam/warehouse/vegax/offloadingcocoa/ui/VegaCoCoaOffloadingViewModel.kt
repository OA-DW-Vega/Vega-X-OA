...
class VegaCoCoaOffloadingViewModel(
    ...
    private var weightBridgeSource: LiveData<Resource<GenericReqAndResp<List<VegaCoCoaReceiving>>>> = MutableLiveData()
    ...
    private var _weighBridge = MediatorLiveData<Resource<GenericReqAndResp<List<VegaCoCoaReceiving>>>>()
    ...
    val weighBridge: LiveData<Resource<GenericReqAndResp<List<VegaCoCoaReceiving>>>> get() = _weighBridge
    ...
    private var offloadingSupplierPostSource: LiveData<Resource<GenericReqAndResp<VegaMtntResponse>>> =
        MutableLiveData()
    private val _offloadingSupplierPost = MediatorLiveData<Resource<GenericReqAndResp<VegaMtntResponse>>>()
    val offloadingSupplierPost: LiveData<Resource<GenericReqAndResp<VegaMtntResponse>>> get() = _offloadingSupplierPost
    ...
    fun postOffloadingSupplierDetail(vegaOffloadingPost: VegaCocoOffloadingSupplierPostRequest) =
        viewModelScope.launch(dispatchers.main) {
            _offloadingSupplierPost.removeSource(offloadingSupplierPostSource) // We make sure there is only one source of livedata (allowing us properly refresh)
            withContext(dispatchers.io) {
                offloadingSupplierPostSource = useCase.postOffloadingSupplierDetail(vegaOffloadingPost)
            }
            _offloadingSupplierPost.addSource(offloadingSupplierPostSource) {
                _offloadingSupplierPost.value = it
            }
        }

    fun getWeighBridgeData(): LiveData<Resource<GenericReqAndResp<List<VegaCoCoaReceiving>>>> {
    ...
    fun getBagItems(batchNumber: String?, mtnNumber: String, weighBridgeId: String) = viewModelScope.launch(dispatchers.main) {
        _bagItems.removeSource(bagSource) // We make sure there is only one source of livedata (allowing us properly refresh)
        withContext(dispatchers.io) {
            bagSource = useCase.getBagItems(batchNumber, mtnNumber, weighBridgeId)
        }
