...
import kotlinx.android.synthetic.main.item_vega_cocoa_offload_trasaction_layout.view.*
import kotlinx.android.synthetic.main.item_vega_cocoa_offload_virtual_pending_list.view.*
...
class VegaCocoaMtnrTransactionFragment : BaseFragment() {
    ...
    private fun setupAdapter(itemList: List<VegaCoCoaReceivingMtnrWithLots>) {
        ...
        binding.rvTransaction.setUp(
            ...
            { it, pos ->
                tvObdNoValue.text = it.receiving.delivery
                tvDateValue.text = it.receiving.erdat
                tvMaterialValue.text = it.receiving.materialName
                tvTruckNoValue.text = it.receiving.vehicleNumber
                tvWeightValue.text = it.receiving.netWeight
                tvTempGrnValue.text = it.receiving.tempGrnNumber
                if (it.receiving.transitLossDocNo.isNullOrEmpty()) {
                    tvTransitLoss.visibility = View.GONE
                    tvTransitLossValue.visibility = View.GONE
                } else
                    tvTransitLossValue.text = it.receiving.transitLossDocNo
                tvSendingWHValue.text =
                    it.receiving.storageLocationCode.plus(" - ").plus(it.receiving.storageLocationName)
                clMain.setOnClickListener { moveToDetails(itemList[pos].receiving) }
            })
    }
    ...
    private fun setupPendingAdapter(itemList: List<VegaCoCoaReceivingMtnrWithLots>) {
        ...
        binding.rvTransaction.setUp(
            ...
            { it, pos ->
                tvObdNoValueP.text = it.receiving.delivery
                tvVendorValueP.text = abs(it.receiving.tempGrnNumber!!.toLong()).toString()
                tvDateValueP.text = it.receiving.erdat
                tvMaterialValueP.text = it.receiving.materialName
                tvSendingWHValuePend.text = it.receiving.supplierName
                tvWeightValueP.text = it.receiving.netWeight
                tvError.text = it.receiving.syncStatusMsg
                ...
                tvShowMore.setOnClickListener { view -> showErrorDialog(it.receiving.syncStatusMsg.toString()) }
                tvSendingWHValueP.text =
                    it.receiving.storageLocationCode.plus(" - ").plus(it.receiving.storageLocationName)
                clMainP.setOnClickListener { moveToDetails(itemList[pos].receiving) }
                ivCloseP.setOnClickListener {
