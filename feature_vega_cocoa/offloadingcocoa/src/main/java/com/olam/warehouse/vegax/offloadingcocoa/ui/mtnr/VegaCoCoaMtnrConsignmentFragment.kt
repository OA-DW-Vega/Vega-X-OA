...
import com.olam.warehouse.vegax.offloadingcocoa.databinding.ItemMtnrLotCardLayoutBinding
...
class VegaCoCoaMtnrConsignmentFragment : BaseFragment(), VegaSingleSelectListener {
    ...
    private fun setUpAdapter(list: ArrayList<VegaCoCoaReceiveLots>) {
        ...
        if (editLotId.isNotEmpty()) {
            ...
            lineItems =
                list.filter { it.batch.equals(editLotId) } as MutableList<VegaCoCoaReceiveLots>
        } else {
        ...
        binding.rvList.setUpAdapter(
            lineItems,
            R.layout.item_mtnr_lot_card_layout,
            ItemMtnrLotCardLayoutBinding::inflate,
            { item, pos, bindItem ->
                ...
                val editedWeight =
                    if (item.editedWeight.isNullOrEmpty()) "0.0" else item.editedWeight.toString()
                        .toDouble()
                        .formatThreeDigits()
                bindItem.tvScaleDispatchValue.setText(editedWeight, TextView.BufferType.EDITABLE)
                bindItem.tvDispatchUOMValue.text = item.uom
                bindItem.ivEdit.visibility = View.GONE
                bindItem.tvVendorName.visibility =
                    if (isThirdPartyMaterial(item.materialNumber)) View.VISIBLE else View.GONE
                bindItem.tvVendorNameValue.visibility =
                    if (isThirdPartyMaterial(item.materialNumber)) View.VISIBLE else View.GONE
                bindItem.tvVendorNameValue.text = vendorName
                bindItem.cbEndLot.isChecked = vm.vegaCoCoaReceivingData.endLot ?: false
                ...
                bindItem.tvAddWeight.setOnClickListener {
                    ...
                    vm.vegaCoCoaReceivingData.imagePath = imageFilePath
                    vm.vegaCoCoaReceivingData.vehicleNumber = binding.tvTruckNoValue.text.toString()
                    vm.vegaCoCoaReceivingData.truckDriverName =
                        binding.tvDriverNameValue.text.toString()
                    vm.vegaCoCoaReceivingData.contactNumber =
                        binding.tvDriverNoValue.text.toString()
                    vm.vegaCoCoaReceivingData.wayBillNo =
                        binding.tvWaybillNumberValue.text.toString()
