    ...
    private fun updateSalesOrder(response: Resource<GenericReqAndResp<List<VegaCocoaSalesOrderModel>>>) {
        response.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    hideLoading()
                    //salesOrderList = it.data?.data?.filter { productList.map { it.materialCode }.contains(if(it.salesOrderList.isNotEmpty())it.salesOrderList.get(0).materialNumber?.takeLast(12) else "" )  } as MutableList<VegaCocoaSalesOrderModel>
                    response.data?.data?.let { it1 ->
                        val data =  it1.filter { productList.map { it.materialCode }.contains(it.materialNumber?.takeLast(12) )  } as MutableList<VegaCocoaSalesOrderModel>
                        salesOrderList = if (vm.dispatchWh.materialCode.isNullOrEmpty())
                            data as MutableList<VegaCocoaSalesOrderModel>
                        else
                            data.filter { it.materialNumber == vm.dispatchWh.materialCode } as MutableList<VegaCocoaSalesOrderModel>
