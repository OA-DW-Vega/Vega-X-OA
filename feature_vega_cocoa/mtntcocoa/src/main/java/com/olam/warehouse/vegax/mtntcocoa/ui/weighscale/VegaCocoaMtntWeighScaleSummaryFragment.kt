...
import com.olam.warehouse.vegax.mtntcocoa.data.domain.model.VegaCocoaMtntDeliveryDetail
import com.olam.warehouse.vegax.mtntcocoa.data.domain.model.VegaCocoaWeighscaleDeliveryPost
import com.olam.warehouse.vegax.mtntcocoa.databinding.FragmentCocoaMtntWsSummaryBinding
import com.olam.warehouse.vegax.mtntcocoa.databinding.ItemCocoaLotSummaryBinding
import com.olam.warehouse.vegax.mtntcocoa.databinding.ItemMaterialLayoutBinding
import com.olam.warehouse.vegax.mtntcocoa.ui.CallBack
import com.olam.warehouse.vegax.mtntcocoa.ui.ItemRemoveListener
import com.olam.warehouse.vegax.mtntcocoa.ui.VegaCocoaMtntViewModel
import com.olam.warehouse.vegax.mtntcocoa.utils.UNIT_KG
import com.olam.warehouse.vegax.mtntcocoa.utils.UNIT_MT
import com.olam.warehouse.vegax.mtntcocoa.utils.WEIGHSCALE
...
class VegaCocoaMtntWeighScaleSummaryFragment : BaseFragment(), ItemRemoveListener {

    private var dispatchLotsList = mutableListOf<VegaCocoaDispatchLots>()
    override val layoutResourceId: Int = R.layout.fragment_cocoa_mtnt_ws_summary
    private lateinit var binding: FragmentCocoaMtntWsSummaryBinding
    ...
    private lateinit var vegaCocoaDeliveryPost: VegaCocoaWeighscaleDeliveryPost
    ...
    var isRoundOff = false

    companion object {
        fun newInstance(data: VegaCocoaDispatchWB) = VegaCocoaMtntWeighScaleSummaryFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentCocoaMtntWsSummaryBinding.inflate(inflater)
        initExtra()
        initUi()
                ...
                ConfigItems.MTNT_WEIGHSCALE.item -> {
                    when {
                        it.applicable?.contains("Y")!! -> {
                            binding.header.tvTruckNoValue.visibility = View.GONE
                            binding.header.tvTruckNo.visibility = View.GONE
                            binding.header.tvDriverName.visibility = View.GONE
                            binding.header.tvDriverNameValue.visibility = View.GONE
                            binding.header.tvContact.visibility = View.GONE
                            binding.header.tvContactValue.visibility = View.GONE
                        }
                    }
                }
            }
    ...
    private fun setUpAdapter(list: List<VegaCocoaDispatchLots>) {
        ...
        binding.rvLots.setUpAdapter(
            lots,
            R.layout.item_cocoa_lot_summary,
            ItemCocoaLotSummaryBinding::inflate,
            { item, pos, bindItem ->
                bindItem.tvLotId.text = item.batchNumber
                bindItem.tvStLocation.text = item.storageLocationCode
                bindItem.tvWeightValue.text =
                    if(item.weight?.isNotEmpty() == true)item.weight?.toDouble()?.formatThreeDigits()?.plus(" ")?.plus(item.unitOfMeasure) else ""
                bindItem.tvGradeValue.text = item.materialName
                bindItem.tvEditedWeight.text =
                    if(item.editedWeight?.isNotEmpty() == true)item.editedWeight?.toDouble()?.formatThreeDigits().toString().plus(" ").plus("KG") else ""
                bindItem.tvUnit.visibility = View.GONE
                bindItem.tvEditedWeight.visibility = View.VISIBLE
                bindItem.ivClose.setImageDrawable(bindItem.ivClose.context.getDrawable(R.drawable.ic_edit_gray))
    ...
    private fun postDelivery() {
        vegaCocoaDeliveryPost = VegaCocoaWeighscaleDeliveryPost(
            getCurrentKey(),
            getPlantDetails(),
            "",
            ...
            prepareDeliveryList(), WEIGHSCALE, salesFlag = false,
            operatorName = summaryObj?.operatorName
        )
        vm.postWeighScaleDeliveryDetails(vegaCocoaDeliveryPost)
    }


    private fun prepareDeliveryList(): List<VegaCocoaMtntDeliveryDetail> {
        val list = ArrayList<VegaCocoaMtntDeliveryDetail>()
        val year: Int = Calendar.getInstance().get(Calendar.YEAR)
        for (item in dispatchLotsList) {
            val deliveryDetail = VegaCocoaMtntDeliveryDetail()
            ...
            bagItems.forEach { item1 ->
                ...
                if(item1.isRoundOff.toString().equals("true"))
                {
//                    bagTareWeight = bagTareWeight.roundToInt().toDouble()
                    isRoundOff = true
                }
...
//                deliveryDetail.huwt = bagTareWeight.toString().trim()
                /*Incorrect tare weight display in MTNT-weigscale due to round off every bag issue: fixed*/
                if (isRoundOff)
    ...
    private fun updateUI(response: Resource<GenericReqAndResp<List<VegaCocoaMtntDeliveryDetail>>>) {
    ...
    private fun prepareSuccessMessage(data: List<VegaCocoaMtntDeliveryDetail>?): String {
        ...
        if (!data.isNullOrEmpty()) {
            ...
            data.forEach {
              ...
              if(!it.encodedImageContent.isNullOrEmpty())
                encodedImageContent= it.encodedImageContent.toString()
            }
            return message
        }
        return ""
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track()
            .screen("/mtntcocoa/ui/weighscale/VegaCocoaMtntWeighScaleSummaryFragment")
