...
class VegaCocoaMtntWeighscaleConsignmentFragment : BaseFragment(),
    ...
    private var selectedmaterialModel: ArrayList<VegaCoffeePurchaseOrderMaterialModel> = ArrayList()
    private var selectedmaterialList: ArrayList<String> = ArrayList()
    private var productList = emptyList<VegaMaterial>()
    private var selectitem: Boolean = false
                ...
                ConfigItems.MTNT_WEIGHSCALE.item -> {
                    when {
                        it.applicable?.contains("Y")!! -> {
                            binding.tvTruckNo.visibility = View.GONE
                            binding.tvTruckNoValue.visibility = View.GONE
                            binding.tvDriverName.visibility = View.GONE
                            binding.tvDriverNameValue.visibility = View.GONE
                            binding.tvDriverNo.visibility = View.GONE
                            binding.tvDriverNoValue.visibility = View.GONE
    ...
    private fun validateProceed() =
        ...
        if (!selectedmaterialList.isNullOrEmpty() && selectedmaterialList.size > 0) {

            if (((binding.tvWhValue.text.isNotEmpty() && binding.tvstoValue.text.isNotEmpty())
                        || binding.tvstoValue1.text.isNotEmpty()) && binding.etOperatorName.text!!.isNotEmpty()
            ) {
                ...
                if (selectedmaterialList.size > 0) {
                    selectedmaterialModel.clear()
                    for (item in materialModelList) {
                        if (selectedmaterialList.contains(item.materialCode)) {
                            val materialModel = VegaCoffeePurchaseOrderMaterialModel()
                            materialModel.materialCode = item.materialCode
                            materialModel.materialName = item.materialName
                            materialModel.soNumber = item.soNumber
                            materialModel.soWeight = item.soWeight
                            materialModel.uom = item.uom
                            materialModel.purchaseOrderNum = item.purchaseOrderNum
                            materialModel.weighBridgeId = item.weighBridgeId
                            materialModel.purchaseOrderDesc = item.purchaseOrderDesc
                            selectedmaterialModel.add(materialModel)
                        }
                    }

                    /*materialModelList.filter { selectedmaterialList.contains(it.materialCode) }
                    println("=================after filter materialModelList========> ${materialModelList.size}")
                    selectedmaterialModel.clear()
                    selectedmaterialModel.addAll(materialModelList)
                    println("=================after selectedmaterialModel ========> ${selectedmaterialModel.size}")*/
                }
                saveMaterialDetails(selectedmaterialModel)
                callback.replaceFragment(WEIGHSCALE_ADD_LOT, vm.dispatchWh)
            } else showSnack(getString(R.string.mandatory))
        } else showSnack(getString(R.string.chooseMaterialtype))
    ...
    private fun enableProceed() {
        val enable =
            (((binding.tvWhValue.text.isNotEmpty() && binding.tvstoValue.text.isNotEmpty())
                    || binding.tvstoValue1.text.isNotEmpty())
                    && binding.etOperatorName.text!!.isNotEmpty()
                    || selectedmaterialList.size > 0 && !selectedmaterialList.isNullOrEmpty())
        if (enable) {
    ...
    private fun setUpMaterialAdapter() {
        binding.rvMaterialList.setUpAdapter(
            ...
            { item, pos, bindItem ->
                ...
                bindItem.llItemlay.setOnClickListener {
                    if (bindItem.ivSelect.isChecked) {
                        bindItem.ivSelect.isChecked = false
                        selectedmaterialList.remove(item.materialCode)
                        vm.deleteMaterialCodeData(item.materialCode)
                    } else {
                        bindItem.ivSelect.isChecked = true
                        selectedmaterialList.add(item.materialCode)
                    }
