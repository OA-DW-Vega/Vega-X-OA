...
import com.olam.warehouse.vegax.mtntcocoa.databinding.FragmentCocoaMtntWsAddLotLayoutBinding
...
class VegaCocoaMtntWeighScaleAddLotFragment : BaseFragment() {
    override val layoutResourceId = R.layout.fragment_cocoa_mtnt_ws_add_lot_layout
    private lateinit var binding: FragmentCocoaMtntWsAddLotLayoutBinding
    ...
    companion object {
        fun newInstance(model: VegaCocoaDispatchWB) =
            VegaCocoaMtntWeighScaleAddLotFragment()
                .putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentCocoaMtntWsAddLotLayoutBinding.inflate(layoutInflater)
        initExtra()
        initUI()
        return binding.root
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("/mtntcocoa/ui/weighscale/VegaCocoaMtntWeighScaleAddLotFragment")
    ...
    private fun initExtra() {
        ...
        dispatchWbFromTruckList =
            arguments?.getParcelable<VegaCocoaDispatchWB>(MODEL_BUNDLE) as VegaCocoaDispatchWB
        vm.weighBridgeWithLots.observe(viewLifecycleOwner, Observer {
            if (it != null) {
                vm.dispatchWh = it.dispatch
                vm.lotList.clear()
                vm.lotList.addAll(it.lineItems.filter { it.processOrderNo.equals(vm.dispatchWh.purchaseDocNum) })
    ...
    private fun initUI() {
        ...
        binding.tvDestValue.text =
            model?.plantId.plus("-").plus(model?.plantName)
        ...
        binding.clLotSummary.btAdd.setOnClickListener {
            vm.validateLotWs(binding.clLotSummary.etEnterContainer.text.toString())
        }
    ...
    fun updateAddWeight(weight: VegaCocoaWSBagModel) {
    ...
    fun updateLotList(list: ArrayList<VegaCocoaDispatchLots>) {
        ...
        for (item in removedLots) {
            vm.removeLotFromListWs(item.batchNumber)
        }
        addedNew.forEach { it.weighBridgeId = model?.weighBridgeId.toString() }
        vm.addLoTInDBWs(addedNew)
    ...
    private fun setUpMaterialAdapter() {
        binding.rvMaterialList.setUp(vm.materialModelList, R.layout.item_material_layout, { it, pos ->
    ...
    private fun validateProceed() {
        if (vm.lotList.isNotEmpty()) {
            if (validateEmptyWeight()) {
                if (validateLotWeight()) {
                    if (validatePalletCount())
                        showRemarkDialog()
                    else UIUtils.showErrorDialog(requireContext(), getString(R.string.pallet_not_matched).plus(" ").plus(palletCountNotMatchBatches))
                        /*Toast.makeText(
                        activity,
                        getString(R.string.pallet_not_matched).plus(" ").plus(palletCountNotMatchBatches),
                        Toast.LENGTH_SHORT
                    ).show()*/
                } else UIUtils.showErrorDialog(requireContext(),getString(R.string.lot_more_weight).plus(" - ").plus(moreWeightBatches))
                    /*Toast.makeText(
                    activity,
                    getString(R.string.lot_more_weight).plus(" - ").plus(moreWeightBatches),
                    Toast.LENGTH_SHORT
                ).show()*/
            } else UIUtils.showErrorDialog(requireContext(), getString(R.string.zero_less_weight_error))
                /*Toast.makeText(
                activity,
            ...
            UIUtils.showErrorDialog(requireContext(), getString(R.string.please_add_lot))
            /*Toast.makeText(
                activity,
    ...
    private fun setUpAdapter(list: ArrayList<VegaCocoaDispatchLots>) {
        ...
        binding.clLotSummary.rvList.setUp(
            ...
            { item, pos ->
                ...
                item.processOrderNo = vm.dispatchWh.purchaseDocNum
                ...
                if (item.unitOfMeasure == "MT") {
                    if (!item.editedWeight.isNullOrEmpty()) {
                        var convertededitedweight = item.editedWeight.toString().toDouble()
                        var lotweight = convertMtToKg(item.weight.toString()).toDouble()
                        item.isLowerWeight = lotweight.toInt() > convertededitedweight.toInt()
                    }
                }
                ...
                tv_add_weight.setOnClickListener {
                    ...
                    callback.replaceFragment(WEIGHSCALE_ADD_WEIGHT, item)
                }
    ...
    private fun showConformationDialog(position: Int, view: View) {
        MaterialDialog(view.context).show {
            ...
            UIUtils.getMetirialCustomView(
                ...
                {
                    vm.removeLotFromListWs(vm.lotList[position].batchNumber)
    ...
    private fun itemRemoved(pos: Int) {
        vm.removeLotFromListWs(vm.lotList[pos].batchNumber)
    ...
    private fun showRemarkDialog() {
        ...
        showDialog(getString(R.string.end_load_msg), object : DialogClick {
            override fun onPositive(remark: String) {
                if (remark.isEmpty()) UIUtils.showErrorDialog(requireContext(), getString(com.olam.warehouse.presentation.R.string.enter_remark))
                    /*Toast.makeText(
                        activity,
