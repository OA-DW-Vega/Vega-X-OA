...
class VegaCocoaMtntWeighScaleAddLotFragment : BaseFragment() {
    ...
    private fun initExtra() {
        ...
        vm.weighBridgeWithLots.observe(viewLifecycleOwner, Observer {
            if (it != null) {
                ...
                binding.rvMaterialList.removeAllViewsInLayout()
            ...
            when (it.process) {
                ConfigItems.LOT_SELECTION_MULTI.item -> {
                    when {
                        ...
                        {   isMultipleLot = true
                            binding.tvTruckIDNo.visibility = View.GONE
                            binding.tvTruckValue.visibility = View.GONE
                        }
    ...
    private fun validateProceed() {
        if (vm.lotList.isNotEmpty()) {
            ...
            } else showErrorDialogWithFAQLink(requireContext(), getString(R.string.zero_less_weight_error))
            /*Toast.makeText(
            activity,
            ...
            showErrorDialogWithFAQLink(requireContext(), getString(R.string.please_add_lot))
            /*Toast.makeText(
                activity,
            ...
            { item, pos, bindItem ->
                ...
                if (item.unitOfMeasure == "MT") {
                    if (!item.editedWeight.isNullOrEmpty()) {
                        ...
                        item.isLowerWeight = lotweight >= convertededitedweight
                    }
                }
    ...
    private fun showRemarkDialog() {
        ...
        showDialog(getString(R.string.end_load_msg), object : DialogClick {
            override fun onPositive(remark: String) {
                if (remark.isEmpty()) showErrorDialogWithFAQLink(
                    requireContext(),
