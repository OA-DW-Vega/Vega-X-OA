...
import kotlinx.android.synthetic.main.selecting_incoming_mtn_row_item.view.*
import kotlinx.android.synthetic.main.selection_incoming_mtn_offline_activity.*
...
class PortIncomingMtnSelectionActivityOffline : HomeBaseActivity() {
    ...
    private fun setUpAdpater(mlist: MutableList<PortMtn>) {
        incomingRecyclerView.setUp(mlist, R.layout.selecting_incoming_mtn_row_item, { it, pos ->
            tvMtnNum.text = it.mtnNumber
            tvSupplyPlantIdd.text = it.suplierPlantId
            tvTruckNumberr.text = it.truckNumber
            tvGradess.text = it.grade
            rbMtnn.isChecked = it.isChecked
            rbMtnn.setOnClickListener {
                changeSelection(pos)
            }
    ...
    private fun changeSelection(position: Int) {
        ...
        onGradeCheck()
        incomingRecyclerView.adapter?.notifyDataSetChanged()
    }
