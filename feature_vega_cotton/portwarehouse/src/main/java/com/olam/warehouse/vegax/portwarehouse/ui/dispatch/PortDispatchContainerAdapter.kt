...
import com.olam.warehouse.vegax.portwarehouse.databinding.ItemDisptachContainerBinding
...
class PortDispatchContainerAdapter constructor(val mListener: OnContainerRemoveListener) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder {
        ...
        val v =
            ItemDisptachContainerBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    class ItemViewHolder(
        private val iv: ItemDisptachContainerBinding,
        ...
        fun bindItems(data: ContainerWithBales?) {
            data?.container?.let { it ->
                ...
                iv.llParent.setBackgroundColor(
                ...
                if (it.isSelected == true) {
                    iv.ivCheckBox.visibility = View.VISIBLE
                } else {
                    iv.ivCheckBox.visibility = View.GONE
                }
                iv.btnStatus.setOnClickListener { moveToNext(data, dispatchType) }
                iv.ivClose.setOnClickListener { mListener.onDelete(data) }
                iv.llParent.setOnLongClickListener { v ->
                    if (iv.ivCheckBox.visibility == View.GONE) {
                        iv.ivCheckBox.visibility = View.VISIBLE
        ...
        private fun moveToNext(data: Container, isDirect: Boolean) {
            when (data.status?.let { it1 -> ContainerStatus.from(it1) }) {
                ...
                ContainerStatus.Completed -> {
                    val intent = Intent(iv.root.context, PortDispatchBreakSealActivity::class.java)
                    ...
                    iv.root.context.startActivity(intent)
                }
                else -> return
            }
        }
