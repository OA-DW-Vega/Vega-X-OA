...
import com.olam.warehouse.vegax.ginningwarehouse.databinding.FragmentGinningDispatchOfflineBinding
import com.olam.warehouse.vegax.ginningwarehouse.ui.utils.*
import com.olam.warehouse.vegax.ginningwarehouse.ui.work.dispatchPostWorker
...
class GinningOfflineDispatchFragment : BaseFragment() {
    ...
    private lateinit var binding: FragmentGinningDispatchOfflineBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentGinningDispatchOfflineBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    fun initUI() {
        binding.rvDeliverySummary.layoutManager = LinearLayoutManager(this.context)
        binding.rvDeliverySummary.adapter = mAdapter

        binding.tvConfirmDispatch.setOnClickListener {
            val btnText = binding.tvConfirmDispatch.text
    ...
    private fun showConfirmDialog() {
        syncBtn = binding.tvConfirmDispatch.text.toString()
    ...
    private fun enableProceedBtn(online: Boolean) {
        ...
        if (online) {
            binding.tvConfirmDispatch.isEnabled = true
            ViewCompat.setBackgroundTintList(
                binding.tvConfirmDispatch,
                context?.let {
            ...
            binding.tvConfirmDispatch.isEnabled = false
            ViewCompat.setBackgroundTintList(
                binding.tvConfirmDispatch,
                context?.let {
                    ContextCompat.getColorStateList(
    ...
    fun changeBtn(offlineList: ArrayList<VegaCottonGinningDispatchDelivery>) {
        ...
        if (offlineList.size == count.size) binding.tvConfirmDispatch.text = getString(R.string.ok)
        else binding.tvConfirmDispatch.text = getString(R.string.resync)
    }
}
