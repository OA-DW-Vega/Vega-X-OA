...
class GinningDispatchViewModel(
    ...
    private var fetchBalesSource: LiveData<List<Bale>> =
            MutableLiveData()
    private val _fetchBales = MediatorLiveData<List<Bale>>()
    val fetchBales: LiveData<List<Bale>> get() = _fetchBales

    fun fetchBaleDetails() = viewModelScope.launch(dispatchers.main) {
        _fetchBales.removeSource(fetchBalesSource) // We make sure there is only one source of livedata (allowing us properly refresh)
        withContext(dispatchers.io) {
            fetchBalesSource = useCase.fetchBaleDetails()
        }
        _fetchBales.addSource(fetchBalesSource) {
            _fetchBales.value = it
        }
    }
