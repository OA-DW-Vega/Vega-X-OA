...
import com.olam.warehouse.vegax.ginningwarehouse.databinding.ActivityGinningInprogressBinding
import com.olam.warehouse.vegax.ginningwarehouse.ui.data.model.enums.LotStatus
import com.olam.warehouse.vegax.ginningwarehouse.ui.di.injectGinningInprogressFeature
...
class GinningInprogressActivity : HomeBaseActivity() {
    ...
    private lateinit var binding: ActivityGinningInprogressBinding
    override val layoutResourceId = R.layout.activity_ginning_inprogress

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityGinningInprogressBinding.inflate(layoutInflater)
    ...
    private fun initUI() {
        ...
        binding.etBale.onChange {
            val baleId = it.trim()
            binding.btnAdd.isEnabled = baleId.length == 10
            val color =
                if (binding.btnAdd.isEnabled) getColorFromId(com.olam.warehouse.presentation.R.color.colorPrimaryOfi) else getColorFromId(
                    R.color.grey
                )
            binding.btnAdd.setBackgroundColor(color)
        }

        binding.btnAdd.setOnClickListener { validateBaleId(binding.etBale.text.toString()) }

        binding.btnProceed.setOnClickListener { moveToGinningConfirmPage() }
        ...
        vm.validateBale.observe(this, Observer {it1->
            if(it1==null)showErrorDialogWithFAQLink(this, "Something went wrong, please try again later")
            it1?.let{
                when (it.status) {
    ...
    private fun updateUI(data: GinningInprogress) {
        ...
        mLotDetail = data
        binding.tvLotNo.text = lotNumber
        data.bales?.let {
