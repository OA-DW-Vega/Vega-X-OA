...
import com.olam.warehouse.vegax.ginningwarehouse.databinding.FragmentGinningDispatchConfirmBinding
...
import com.olam.warehouse.vegax.ginningwarehouse.ui.utils.isOnline
...
class GinningDispatchConfirmFragment : BaseFragment() {
    ...
    private lateinit var binding: FragmentGinningDispatchConfirmBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentGinningDispatchConfirmBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    fun initUI() {
        ...
        vm.deliveryWithBales.observe(viewLifecycleOwner, androidx.lifecycle.Observer { balesData ->
            if (balesData != null && balesData.bales.size > 0) {
                if (balesData.bales != null) {
                    ...
                    totalNetWeight =
                        balesData.bales.sumByDouble { it.netWeight ?: 0.0 }.formatTwoDigits()
                    totalGrossWeight =
                    ...
                    binding.txtOrdernumber.text = deliveryNo
                    binding.txtBalescount.text = balesData.bales.size.toString()
                    ...
                    binding.txtTotalweight.text = totalNetWeight.plus(" ").plus("KG")
                    binding.txtNetcount.text = totalNetWeight.plus(" ").plus("KG")
                    binding.txtTarecount.text = totalTarWeight.plus(" ").plus("KG")
                    binding.txtGrosscount.text = totalGrossWeight.plus(" ").plus("KG")
        ...
        binding.btnConfirm.setOnClickListener {
            deliveryDtoVegaCotton?.let { it1 ->
                showConfirmDialog(it1)
            }
        }
    ...
    private fun updateUI(response: DispatchPostResponse?) {
        if (response?.success!=null && response.success) {
            ...
            showErrorDialogWithFAQLink(requireContext(), response?.message.toString())
