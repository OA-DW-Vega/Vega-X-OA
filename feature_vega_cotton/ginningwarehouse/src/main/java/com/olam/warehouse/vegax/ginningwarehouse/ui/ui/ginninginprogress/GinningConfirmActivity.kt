...
import com.olam.warehouse.ginning.utils.LOT_DETAIL
...
import kotlinx.android.synthetic.main.activity_ginning_confirm.*
...
class GinningConfirmActivity : HomeBaseActivity() {
    ...
    private fun initUI() {
        ...
        mLotDetail?.let {
            tvLotNo.text = it.lotNumber
            tvNoOfBales.text = it.bales?.size.toString()
            tvWeight.text = it.bales?.sumByDouble { it.grossWeight ?: 0.0 }?.format().plus("KG")
            tvDate.text = getReadableCurrentDate()
