...
import kotlinx.android.synthetic.main.fragment_ginning_dispatch_add_bale.*
import kotlinx.android.synthetic.main.item_ginning_dispatch_grades.view.*
...
class GinningDispatchAddBaleFragment : BaseFragment() {
    ...
    fun initUI() {
        ...
        spn_ordernumber.setSelection(vm.lastSelectedPosition)
        spn_ordernumber.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
        ...
        txt_date.text = DateUtils.fromMillisToTimeString(getCurrentTimeInMills())
        tvEmptyContent.text = getString(R.string.currently_no_order_to_dispatch)
        btn_scantodispatch.setOnClickListener { moveToScannerActivity() }
        btn_proceed.setOnClickListener { moveToConfirmPage() }
        llDispatchSummary.setOnClickListener {
                    ...
                    Resource.Status.SUCCESS -> {
                        ...
                        it.data?.let {
                            ...
                            if (it.data !== null && it.data.baleID.isNotEmpty()) {
                                ...
                                UIUtils.showErrorDialog(requireContext(), it.message)
                            }
                        }
        ...
        vm.deliveryWithGrades.observe(viewLifecycleOwner, androidx.lifecycle.Observer {
            if (it != null && it.grades != null) {
                ...
                if (it.grades.size > 0) {
                    txt_dispatchgradesnot.gone()
                    rvDispatchGrades.visible()
                } else {
                    txt_dispatchgradesnot.visible()
                    rvDispatchGrades.gone()
                }
        ...
        vm.deliveryDetailsOfflineVegaCotton.observe(viewLifecycleOwner, androidx.lifecycle.Observer {
            ...
            if (offlineList.size > 0) llDispatchSummary.visible() else llDispatchSummary.gone()
    ...
    private fun updateSpinnerUI() {
        ...
        if (deliveryNo?.isNotEmpty()!!) {
            ...
            spn_ordernumber.setSelection(postion + 1)
        }
    }
    ...
    private fun getDeliveryDetails(dispatchDeliveryVegaCotton: VegaCottonGinningDispatchDelivery) {
        ...
        txt_containerNumber.text = containerNo
        txt_truckNumber.text = truckNo
        txt_weight.text = dispatchDeliveryVegaCotton.maxWeightAllowed.toString().plus(" ").plus("KG")
        ...
        btn_add.setOnClickListener {
            validateBaleId(edit_baleID.text.toString())
        }
        edit_baleID.onChange { changeButttonColor(it) }
    }

    private fun setUpGradeAdapter(grades: List<Grade>) {
        rvDispatchGrades.setUp(
            grades as MutableList<Grade>,
            R.layout.item_ginning_dispatch_grades,
            { it, pos ->
                tvGradeName.text = it.grade
                tvGradeWeight.text = it.weight.plus(" ").plus("KG")
            })
    }
    ...
    private fun changeButttonColor(value: String) {
        when (value.length) {
            10 -> {
                ...
                ViewCompat.setBackgroundTintList(btn_add,
    ...
    private fun moveToOfflineBale(baleId: String) {
        ...
        if (baleData != null) {
            if (baleData.baleID.isEmpty()) UIUtils.showErrorDialog(
                requireContext(),
                getString(R.string.bale_not_available)
            )
            else if (!grades.contains(baleData.grade)) UIUtils.showErrorDialog(
                requireContext(),
...
//                edit_baleID.text.clear()
            ...
            UIUtils.showErrorDialog(requireContext(), getString(R.string.bale_not_available))
//            edit_baleID.text.clear()
        }
...
//        edit_baleID.text.clear()
        if (baleHashMap.size > 0) recycler_linearlayout.visible() else recycler_linearlayout.gone()
