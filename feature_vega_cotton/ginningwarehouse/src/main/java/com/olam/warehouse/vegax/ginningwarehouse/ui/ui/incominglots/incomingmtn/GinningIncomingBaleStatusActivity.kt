...
import com.olam.warehouse.ginning.utils.BALE
import com.olam.warehouse.ginning.utils.PLANT
...
import kotlinx.android.synthetic.main.activity_ginning_incoming_bale_status.*
...
class GinningIncomingBaleStatusActivity : HomeBaseActivity() {
    ...
    private fun initUI() {
        ...
        tvPlant.text = plantName
        llDamaged.setOnClickListener {
        ...
        llGood.setOnClickListener {
            ...
            enableUpdateButton()
            llDamageTypeContainer.visibility = View.INVISIBLE
    ...
    private fun updateUI() {
        bale?.let {
            tvReceivedFrom.text = it.mtnNumber
            tvBaleGrade.text = it.grade

            if (it.delyNetQty != null && it.delyNetQty?.isNotEmpty() == true) {
                tvBaleWeight.text = it.delyNetQty + " " + it.unitOfMeasurement
            }else {
                tvBaleWeight.text = it.netWeight + " " + it.unitOfMeasurement
            }

            tvBaleNumber.text = it.baleId
    ...
    private fun showDamageType(type: String) {
        llDamageTypeContainer.visibility = View.VISIBLE
        tvDamageType.text = type
    }
    ...
    private fun enableUpdateButton(){
        btnUpdateStatus.setBackgroundColor(
