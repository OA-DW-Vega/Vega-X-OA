...
import com.olam.warehouse.vegax.containermanagementnigeria.databinding.FragmentVegaNigeriaContainerInventoryListBinding
...
import com.olam.warehouse.vegax.containermanagementnigeria.utils.getColor
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaNigeriaContainerInventoryListFragment : BaseFragment() {

    private lateinit var binding: FragmentVegaNigeriaContainerInventoryListBinding
    private val vm: VegaNigeriaContainerManagementViewModel by viewModel()
    override val layoutResourceId = R.layout.fragment_vega_nigeria_container_inventory_list
    ...
    private var mAdapter = VegaNigeriaContainerInventoryAdapter {
        if (it != null) {
    ...
    companion object {
        fun newInstance() = VegaNigeriaContainerInventoryListFragment().apply {}
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaNigeriaContainerInventoryListBinding.inflate(layoutInflater)
        return binding.root
    }
...
//        initExtra()
        ...
        TrackHelper.track().screen("containermanagement/ui/containerInventory/VegaNigeriaContainerInventoryListFragment")
            .title("Vega_Nigeria/Containermanagement").with(tracker)
    }

    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        super.onCreateOptionsMenu(menu, inflater)
        activity?.menuInflater?.inflate(R.menu.search_vega_nigeria_container_menu, menu)
        try {
    ...
    private fun initUI(){
        ...
        binding.tvSort.setOnClickListener {
         ...
         /*   if (containerInventoryList.isNotEmpty()) {
                containerInventoryList.let { offloading = it as MutableList<ContainerInventory> }
                offloading = offloading.asReversed()
                containerInventoryList = offloading as ArrayList<ContainerInventory>
//                setUpAdapter(containerInventoryList)
                mAdapter.addItems(containerInventoryList)
...
//        binding.tvTotalCount.setOnClickListener { totalCountOfLots() }
    ...
    fun showFilterDialog() {
        ...
        val dialogView: View = inflater.inflate(R.layout.item_vega_nigeria_container_filter, null)
       ...
       /* if(selectedFilter?.size ?:0 >0){
            selectedFilter?.forEach {
                when(it){
                    "New" ->{
                        backgroundColor(tvCompleted,true)
                    }
                    "Completed" ->{
                        backgroundColor(tvCompleted,true)
                    }
...
//        var selectedFilter: ArrayList<String>? = null
//        val dialogView: View = inflater.inflate(R.layout.item_vega_Nigeria_container_filter, null)
//        val tvCompleted = dialogView.findViewById<TextView>(R.id.tvCompleted)
//        val tvNewContainer = dialogView.findViewById<TextView>(R.id.tvNewContainer)
//        val cbMaterial = dialogView.findViewById<CheckBox>(R.id.cbMaterial)
//        val btFilter: Button = dialogView.findViewById(R.id.btFilter)
    ...
    private fun sortByWeight(list: List<ContainerInventory>) {
        ...
        if (isDescends) {
//            sortedList = list.sortedByDescending { wareHouse -> wareHouse.containerWeight.toFloat() }
            sortedList = list
        } else {
//            sortedList = list.sortedBy { wareHouse -> wareHouse.containerWeight.toFloat() }
            sortedList = list.reversed()
        }
        mAdapter.addItems(sortedList)
    }

    private fun updateUIWithOnlineData(data: Resource<GenericReqAndResp<VegaNigeriaContainerInventoryModel>>) {
        data.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    ...
                    if (it.data?.success == false) {
                        ...
                        containerInventoryList = it.data?.data?.containerDTOs?.filter {
                            !it.status.equals("Deleted") && !it.status.equals("Completed")
