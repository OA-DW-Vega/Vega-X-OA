...
import com.olam.warehouse.vegax.grnnigeria.databinding.ItemVegaNigeriaWeighbridgeGrnBinding
    ...
    private fun updateUIWithLocalData(data: List<VegaGrnWeighBridgeId>?) {
        ...
        if (weighBridge?.size!! > 0) {
            weighBridge.forEach {wb->
                if (wb.unitsOfMeasure.equals("KG",true)){
                wb.netWeight = if (wb.netWeight.isNotEmpty()) convertKgToMT(wb.netWeight) else "0.0"
                wb.grossWeight = if (wb.grossWeight?.isNotEmpty() == true) convertKgToMT(wb?.grossWeight?:"0.0") else "0.0"
                wb.unitsOfMeasure = "MT"
                }
            }
                    ...
                    if (weighBridge?.size!! > 0) {
                        ...
                        weighBridge.forEach { wb ->
                            if (!offlineDataList.map { it.weighBridgeId }
                                ...
                                if (wb.unitsOfMeasure.equals("KG",true)) {
                                    wb.netWeight = if (wb.netWeight.isNotEmpty()) convertKgToMT(wb.netWeight) else "0.0"
                                    wb.grossWeight = if (wb.grossWeight?.isNotEmpty() == true) convertKgToMT(
                                        wb?.grossWeight ?: "0.0"
                                    ) else "0.0"
                                    wb.unitsOfMeasure = "MT"
                                }
...
//                    binding.tvDate.text = sdf.format(cal.time)
    ...
    private fun setUpAdapter(data: List<VegaGrnWeighBridgeId>?) {
        ...
        binding.rvWeighBridgeId.setUpAdapter(
            ...
            ItemVegaNigeriaWeighbridgeGrnBinding::inflate,
            { it, pos, bindItem ->
...
//                    if (it.purchaseDocNum.isNullOrEmpty()) getString(R.string.spot_purchase) else getString(R.string.fixed_purchase)
                ...
                println("Roshna =>" + bindItem.tvLotNo.text)
                // vm.getQualityDetails(it.batchNumber.toString(), it.materialCode.toString())
                bindItem.tvQualityDetails.setOnClickListener {
                    wbDetails.batchNumber = bindItem.tvLotNo.text.toString()
                    vm.getQualityDetails(bindItem.tvLotNo.text.toString(), materialCode.toString())
                    //  callBack?.replaceQualityFragment(GRN_QUALITY_DETAILS, it, approveQualityList)
                }

                bindItem.cbLotID.setOnCheckedChangeListener { buttonView, isChecked ->
                    if (isChecked) {
            ...
            {
                val compliance= vegaMaterialList.singleOrNull { it.materialCode.takeLast(12).contains(this.materialCode?.takeLast(12).toString()) }?.complainceFlag?:""
                val item = this
                item.complianceFlag = compliance
