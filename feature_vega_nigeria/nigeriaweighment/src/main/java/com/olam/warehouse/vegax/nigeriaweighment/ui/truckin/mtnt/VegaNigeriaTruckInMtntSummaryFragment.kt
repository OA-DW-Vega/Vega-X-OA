...
import com.olam.warehouse.vegax.nigeriaweighment.databinding.FragmentVegaNigeriaTruckinMtntSummaryBinding
import com.olam.warehouse.vegax.nigeriaweighment.ui.VegaNigeriaMtntViewModel
import com.olam.warehouse.vegax.nigeriaweighment.utils.DIRECTIONIN
import com.olam.warehouse.vegax.nigeriaweighment.utils.MTNT_DATA
import com.olam.warehouse.vegax.nigeriaweighment.utils.getTmpId
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaNigeriaTruckInMtntSummaryFragment : BaseFragment() {
    ...
    private lateinit var binding: FragmentVegaNigeriaTruckinMtntSummaryBinding
    override val layoutResourceId = R.layout.fragment_vega_nigeria_truckin_mtnt_summary
    private var workFlowData: WorkflowFields? = null


    companion object {
        fun newInstance(mtntData: VegaMtnt) = VegaNigeriaTruckInMtntSummaryFragment().putArgs {
            putParcelable(MTNT_DATA, mtntData)
        }
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentVegaNigeriaTruckinMtntSummaryBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        workFlowData = getCurrentWorkflowDetails(getPlantDetails().plantId, "26")
        ...
        vm.nigeriaMtnt.observe(viewLifecycleOwner, Observer { updateUI(it) })

    }

    private fun updateUI(data: Resource<GenericReqAndResp<VegaNigeriaMtntResponse>>?) {
        data?.let {
            when (it.status) {
    ...
    private fun showConfirmDialog() {
        MaterialDialog(requireContext()).show {
            ...
            UIUtils.getMetirialCustomView(this, getString(R.string.confirm), getString(R.string.cancel), {
                postReceiving()
            }, { dismiss() })
        }
    }
    ...
    private fun postReceiving() {
        ...
        if (AppUtils.isOnline()) {
            vm.postMtntData(VegaNigeriaMtntPost(
                key = getCurrentKey(),
                plant = getPlantDetails(),
                weighDetails = mMtntList,
