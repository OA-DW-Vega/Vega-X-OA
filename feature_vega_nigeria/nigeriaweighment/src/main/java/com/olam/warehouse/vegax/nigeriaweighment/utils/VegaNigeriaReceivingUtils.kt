package com.olam.warehouse.vegax.nigeriaweighment.utils
...
fun getLineItemFromReceivingLineItemReceiving(postData: MutableList<VegaReceivingLineItem>): ArrayList<VegaQualityWBDetails> {
    val items = ArrayList<VegaQualityWBDetails>()
    postData.forEach {
        val item = VegaQualityWBDetails()
        ...
        //item.posnr = it.posnr
        item.unitsOfMeasure = it.unitsOfMeasure
        item.plant = it.plantId
        //item.wsGate = it.wsGate
        item.weighBridgeType = it.weighBridgeType
        item.storageLocation = it.location
        //item.tareWeight = it.tareWeight
        item.supplierName = it.supplierName
        //item.isSynced = it.isSynced
        //item.status = it.status
