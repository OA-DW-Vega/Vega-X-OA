...
import com.olam.warehouse.vegax.inventorynigeria.data.domain.model.*
import com.olam.warehouse.vegax.inventorynigeria.databinding.FragmentVegaNigeriaCocoaInventoryListBinding
import com.olam.warehouse.vegax.inventorynigeria.databinding.ItemVegaNigeriaCocoaInventoryDetailsBinding
import com.olam.warehouse.vegax.inventorynigeria.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaNigeriaCocoaInventoryDetailsFragment : BaseFragment() {
    ...
    companion object {
        fun newInstance(bundle: Bundle) = VegaNigeriaCocoaInventoryDetailsFragment().putArgs {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track()
            .screen("inventorysesame/ui/details/VegaNigeriaSesameInventoryDetailsFragment")
            .title("Nigeria_Cocoa/Inventory").with(tracker)
        initUI()
    }
    ...
    private fun initUI() {
        ...
        if (lotId.isNotEmpty()) {
            ...
            vm.qualityParamDetails.observe(viewLifecycleOwner, Observer { updateLotInfo(it) })
    ...
    private fun updateLotInfo(response: Resource<GenericReqAndResp<List<VegaCocoaInventoryStocks>>>) {
        response.let {
            when (it.status) {
    ...
    private fun prepareInventoryList(items: List<VegaCocoaInventoryStocks>): ArrayList<InventoryLots> {
    ...
    private fun setUpAdapter(data: List<InventoryLots>) {
        ...
        binding.rvInventory.setUpAdapter(
            lotList,
            R.layout.item_vega_nigeria_cocoa_inventory_details,
            ItemVegaNigeriaCocoaInventoryDetailsBinding::inflate,
            { it, pos, bindItem ->
            ...
            lotList.forEach { lotItem ->
                ...
                if ((materialList.size == 0 || (filterData1.map { it.lotId }
                    ...
                    && (impurityList.size == 0 || filterData3.map { it.lotId }
                        .contains(lot)) && (mouldList.size == 0 || filterData4.map { it.lotId }
                        .contains(
                            lot
                        ))
                )
