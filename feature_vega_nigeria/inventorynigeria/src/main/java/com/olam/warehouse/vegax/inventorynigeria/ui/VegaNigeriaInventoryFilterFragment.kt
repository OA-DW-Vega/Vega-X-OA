...
import com.olam.warehouse.vegax.inventorynigeria.data.domain.model.FilterList
import com.olam.warehouse.vegax.inventorynigeria.databinding.FragmentVegaNigeriaInventoryFilterBinding
import com.olam.warehouse.vegax.inventorynigeria.utils.*
import kotlinx.android.synthetic.main.fragment_vega_nigeria_inventory_filter.*
import kotlinx.android.synthetic.main.item_vega_nigeria_inventory_filter.view.*
 ...
 */
class VegaNigeriaInventoryFilterFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_vega_nigeria_inventory_filter
    private lateinit var binding: FragmentVegaNigeriaInventoryFilterBinding
    ...
    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaNigeriaInventoryFilterFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentVegaNigeriaInventoryFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("inventorynigeria/ui/VegaNigeriaInventoryFilterFragment").title("Inventory Cocoa")
            .with(tracker)
        initUI()
    }
...
/*        val minMoisture = tvMinFilter.text.toString().toFloat()
        val maxMoisture = tvMaxFilter.text.toString().toFloat()
        val selectedMoisture = ArrayList<Float>()

            if(tvMinFilter.text.isNotEmpty() && tvMinFilter.text > 0 ){
                if(tvMaxFilter.text.isNotEmpty() && minMoisture < maxMoisture)
                selectedMoisture.add(tvMinFilter.text.toString().toFloat())
                else
                    
        }else
        {
            selectedMoisture.add(0f)
        }

        if(tvMaxFilter.text.isNotEmpty() && tvMaxFilter.text > 0 ){
            selectedMoisture.add(tvMaxFilter.text.toString().toFloat())
        }else
        {
            selectedMoisture.add(0f)
        }*/
        //val selectedMoisture = listFliterMoist.map

        //val selectedDiscBeanWgt = listFliterMoist.map { it.value } as ArrayList
//        val selectedMould = listFliterMoist.map { it.value } as ArrayList
//        val selectedMoisture = listFliterMoist.map { it.value } as ArrayList
//        val selectedAdmixture = listFliterMoist.map { it.value } as ArrayList
...
//        bundle.putStringArrayList(FILTER_BEAN, selectedDiscBeanWgt)
//        bundle.putStringArrayList(FILTER_MOIST, selectedMould)
//        bundle.putStringArrayList(FILTER_FFA, selectedMoisture)
//        bundle.putStringArrayList(FILTER_FAT, selectedAdmixture)
...
//        bundle.putStringArrayList(FILTER_FAT, selectedFat)
    ...
    private fun setUpMaterialAdapter(listFliterMaterial: ArrayList<FilterList>) {
        binding.rvMaterial.setUp(listFliterMaterial, R.layout.item_vega_nigeria_inventory_filter, { it, pos ->
    ...
    private fun setUpLocationAdapter(listFliterLocation: ArrayList<FilterList>) {
        binding.rvLocation.setUp(listFliterLocation, R.layout.item_vega_nigeria_inventory_filter, { it, pos ->
   ...
   /* private fun setUpBeanAdapter(listFliterBean: ArrayList<FilterList>) {
        binding.rvBeanCount.setUp(listFliterBean, R.layout.item_vega_nigeria_inventory_filter, { it, pos ->
    ...
    private fun setUpMoistAdapter(listFliterMoist: ArrayList<FilterList>) {
        binding.rvMoisture.setUp(listFliterMoist, R.layout.item_vega_nigeria_inventory_filter, { it, pos ->
    ...
    private fun setUpFFAAdapter(listFliterFfa: ArrayList<FilterList>) {
        binding.rvFFA.setUp(listFliterFfa, R.layout.item_vega_nigeria_inventory_filter, { it, pos ->
    ...
    private fun setUpFATAdapter(listFliterFat: ArrayList<FilterList>) {
        binding.rvFAT.setUp(listFliterFat, R.layout.item_vega_nigeria_inventory_filter, { it, pos ->
