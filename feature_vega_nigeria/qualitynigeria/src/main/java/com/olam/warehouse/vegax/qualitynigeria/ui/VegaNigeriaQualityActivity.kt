...
class VegaNigeriaQualityActivity : HomeBaseActivity(),
    ...
    private var juteBagDetails = arrayListOf<VegaQualityWBDetails>()
    ...
    private fun initUI() {
        workFlowData = getCurrentWorkflowDetails(com.olam.warehouse.master.common.utils.getPlantDetails().plantId, "9")
    ...
    override fun onWeighBridgeClick(
        ...
        this.juteBagDetails = juteBagDetails as ArrayList<VegaQualityWBDetails>
        isParamValue = false
    ...
    override fun onParamsProceedQuality(
        ...
        wbId: String?,
        batchNo: String,
        finalApproval: String, storageLocationCode: String, complainceFlag: String?
    ) {
        this.complainceFlag= complainceFlag.toString()
        ...
        *//*if (AppUtils.isOnline()) {
        ...
        if (AppUtils.isOnline()) {
            @Suppress("UNCHECKED_CAST")
            wbDetails?.qualityDetails = prepareVegaQualityList(
                qtyNigeriaParams,
                arrayListOf()
            )
            wbDetails?.batchNumber =
                if (batchNo.isNotEmpty()) batchNo else com.olam.warehouse.master.common.utils.getPlantDetails().plantId
            wbDetails?.finalApproval = finalApproval
            wbDetails?.plant = mSelectedPlantId
            qualityPostList.clear()
            wbDetails?.let { qualityPostList.add(it) }
//            juteBagDetails?.let { if(juteBagDetails.size>0){qualityPostList.addAll(it)} }
            plantList = if(getMultiPlantList().isNotEmpty())getMultiPlantList() as MutableList<Plant> else mutableListOf<Plant>()
            var plantDetails = getPlantDetails(mSelectedPlantId).singleOrNull()


            if(getCurrentKey().contains("VEGA_NG") && getCurrentKey().contains("CASH")){
                val bundle = Bundle().apply {
                    putParcelableArrayList(APPROVE_QUALITY_DATA, qualityParameter)
                    putParcelableArrayList(APPROVE_JUTE_BAG_DETAILS, juteBagDetails)
    ...
    private fun moveToSuccessPage(currentWbid: String?, charg: String?, grnNo: String?) {
        ...
        intent.putExtra(AppUtils.EUDR_STATUS, if(complainceFlag.equals(Constants.COMPLAINT)) "1" else "0" )
        if (AppUtils.isOnline()) {
            if (finalApprovalStatus == FNQUALITY || finalApprovalStatus == FNQUALITY_THRESHOLD || finalApprovalStatus == "D" || finalApprovalStatus == "Q") {
                if(grnNo.isNullOrEmpty()){
                    intent.putExtra(AppUtils.TITLE, getString(R.string.quality_success))
                    intent.putExtra(
                        AppUtils.SUB_TITLE,
                        getString(R.string.weigh_bridge_id_text).plus(currentWbid).plus("\n")
                            .plus(getString(R.string.batch_no).plus(" :").plus(charg))
                    )

                } else {
                    /*for mtnr, we need to show quality and grn successful msg here*/
                    intent.putExtra(AppUtils.TITLE, getString(R.string.quality_grn_success))
                    intent.putExtra(
                        AppUtils.SUB_TITLE,
                        getString(R.string.weigh_bridge_id_text).plus(currentWbid).plus("\n")
                            .plus(getString(R.string.batch_no).plus(" : ").plus(charg).plus("\n").plus(getString(R.string.grn_no)).plus(" : ").plus(grnNo))
                    )
                }

            } else {
                intent.putExtra(AppUtils.TITLE, getString(R.string.quality_reject))
                intent.putExtra(AppUtils.SUB_TITLE, getString(R.string.weigh_bridge_id_text).plus(currentWbid))
            }
        } else {
            ...
            else{ intent.putExtra(AppUtils.TITLE, getString(R.string.quality_reject_offline))}
    ...
    override fun onParamsProceed(
        ...
        if (AppUtils.isOnline()) {
            ...
            qualityPostList.clear()
            wbDetails?.let { qualityPostList.add(it) }
            juteBagDetails.let { if(juteBagDetails.size>0){ juteBagDetails.get(0).batchNumber = batchNo
                qualityPostList.addAll(it)} }
            ...
            saveData(qualityParameter, wbId)
            moveToSuccessPage(wbId, batchNo, "")
        }
    }
