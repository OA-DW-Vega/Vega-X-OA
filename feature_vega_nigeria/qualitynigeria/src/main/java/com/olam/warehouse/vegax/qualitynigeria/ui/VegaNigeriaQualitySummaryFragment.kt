...
import kotlinx.android.synthetic.main.item_vega_nigeria_quality_summary_params.view.*
    ...
    private fun initUI() {
        ...
        var qualityCode = getQualityCode(
            (if (beanWtInGram?.replace("%", "")?.trim()
                    .isNullOrEmpty()
            ) 0.0 else (beanWtInGram?.replace("%", "")?.trim())?.toDouble()) ?: 0.0,
            (if (mouldValue?.replace("%", "")?.trim()
                    .isNullOrEmpty()
            ) 0.0 else (mouldValue?.replace("%", "")?.trim())?.toDouble()) ?: 0.0,
            (if (addMixture?.replace("%", "")?.trim()
                    .isNullOrEmpty()
            ) 0.0 else (addMixture?.replace("%", "")?.trim())?.toDouble()) ?: 0.0
        )
        var thresholdLimit = getQualityThresholdLimit(
            (if (beanCount?.replace("%", "")?.trim()
                    .isNullOrEmpty()
            ) 0.0 else (beanCount?.replace("%", "")?.trim())?.toDouble()) ?: 0.0,
            (if (moisture?.replace("%", "")?.trim()
                    .isNullOrEmpty()
            ) 0.0 else (moisture?.replace("%", "")?.trim())?.toDouble()) ?: 0.0,
            (if (mouldValue?.replace("%", "")?.trim()
                    .isNullOrEmpty()
            ) 0.0 else (mouldValue?.replace("%", "")?.trim())?.toDouble()) ?: 0.0,
            (if (slaty?.replace("%", "")?.trim().isNullOrEmpty()) 0.0 else (slaty?.replace("%", "")
                ?.trim())?.toDouble()) ?: 0.0,
            (if (addMixture?.replace("%", "")?.trim()
                    .isNullOrEmpty()
            ) 0.0 else (addMixture?.replace("%", "")?.trim())?.toDouble()) ?: 0.0
