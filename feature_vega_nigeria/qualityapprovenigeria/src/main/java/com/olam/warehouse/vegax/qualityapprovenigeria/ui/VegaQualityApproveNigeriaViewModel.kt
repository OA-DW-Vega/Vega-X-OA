...
class VegaQualityApproveNigeriaViewModel(
    ...
    private var grnSourceNew: LiveData<Resource<GenericReqAndResp<VegaEcuadorGrnResponse>>> =
        MutableLiveData()
    private val _grnNew = MediatorLiveData<Resource<GenericReqAndResp<VegaEcuadorGrnResponse>>>()
    val grnNew: LiveData<Resource<GenericReqAndResp<VegaEcuadorGrnResponse>>> get() = _grnNew

    private var gateEntryGrntSource: LiveData<Resource<GenericReqAndResp<List<VegaNigeriaGateEntryPostData>>>> =
            MutableLiveData()
    private val _gateEntrygrnt =
            MediatorLiveData<Resource<GenericReqAndResp<List<VegaNigeriaGateEntryPostData>>>>()
    val gateEntryGrnt: LiveData<Resource<GenericReqAndResp<List<VegaNigeriaGateEntryPostData>>>> get() = _gateEntrygrnt
    ...
    fun getMtntWeightDetails(wbid: String,weighMethod: String) = viewModelScope.launch(dispatchers.main) {
        _mtntWeight.removeSource(mtntWeightSource)
        withContext(dispatchers.io) {
            mtntWeightSource = useCase.getMtntWeightDetails(wbid,weighMethod)
        }
    ...
    fun postGrn(grnPost: VegaNigeriaGrnPost) = viewModelScope.launch(dispatchers.main) {
        _grnNew.removeSource(grnSourceNew)
        withContext(dispatchers.io) {
            grnSourceNew = useCase.postGrn(grnPost)
        }
        _grnNew.addSource(grnSourceNew) {
            _grnNew.value = it
        }
    }

    fun openGrntDetails(role: String, materialCodes: String, vendorCodes: String, plantId: String) =
            viewModelScope.launch(dispatchers.main) {
                _gateEntrygrnt.removeSource(gateEntryGrntSource) // We make sure there is only one source of livedata (allowing us properly refresh)
                withContext(dispatchers.io) {
                    gateEntryGrntSource =
                            useCase.openGrntDetails(role, materialCodes, vendorCodes, plantId)
                }
                _gateEntrygrnt.addSource(gateEntryGrntSource) {
                    _gateEntrygrnt.value = it
                }
            }
