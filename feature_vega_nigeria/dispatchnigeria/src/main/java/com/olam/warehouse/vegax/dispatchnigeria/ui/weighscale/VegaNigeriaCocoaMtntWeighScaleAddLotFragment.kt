...
import kotlinx.android.synthetic.main.item_nigeria_cocoa_weighscale_lot_card_layout.view.*
...
class VegaNigeriaCocoaMtntWeighScaleAddLotFragment : BaseFragment() {
            ...
            Resource.Status.SUCCESS -> {
                ...
                response.data?.data?.let { it1 ->
                    if (it1.weightedAvg.isNullOrEmpty()) {
                        UIUtils.showErrorDialog(requireContext(), "Weighted Average value is Empty")
                    } else {
    ...
    private fun setUpMaterialAdapter() {
        binding.rvMaterialList.setUp(vm.materialModelList, R.layout.item_nigeria_cocoa_material_layout, { it, pos ->
