...
class VegaNigeriaSecretIdViewModel(
    ...
    fun getWbWeightDetails(wbid: String,wbType: String) = viewModelScope.launch(dispatchers.main) {
        _wbWeight.removeSource(wbWeightSource)
        withContext(dispatchers.io) {
            wbWeightSource = useCase.getWbWeightDetails(wbid,wbType)
        }
    ...
    fun getfetchWBListforMultiPlants(
        ...
        withContext(dispatchers.io) {
            wbMultiPlantsesource =
                useCase.getfetchWBListforMultiPlants(isMTNT, startDate, endDate, plantList,wbtype)
        }
