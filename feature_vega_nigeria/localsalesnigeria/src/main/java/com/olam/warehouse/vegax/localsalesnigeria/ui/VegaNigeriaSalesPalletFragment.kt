...
import com.olam.warehouse.vegax.localsalesnigeria.databinding.FragmentSalesNigeriaAddPalletBinding
import com.olam.warehouse.vegax.localsalesnigeria.utils.ADD_WEIGHT
import com.olam.warehouse.vegax.localsalesnigeria.utils.LOT_DETAIL
import com.olam.warehouse.vegax.localsalesnigeria.utils.getColor
import com.olam.warehouse.vegax.localsalesnigeria.utils.prepareCocoaBagMaterial
import kotlinx.android.synthetic.main.item_nigeria_sales_pallet_bag_info.view.*
 ...
 */
class VegaNigeriaSalesPalletFragment : BaseFragment() {

    override val layoutResourceId: Int = R.layout.fragment_sales_nigeria_add_pallet
    private lateinit var binding: FragmentSalesNigeriaAddPalletBinding
    ...
    companion object {
        fun newInstance(item: VegaCoffeeSalesLots) = VegaNigeriaSalesPalletFragment().putArgs {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("localsalesNigeria/ui/VegaNigeriaSalesPalletFragment")
            .title("Vega_Nigeria/Local Sales").with(tracker)
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentSalesNigeriaAddPalletBinding.inflate(inflater)
        initUI()
        return binding.root
    }
    ...
    private fun updatePalletUI(response: Resource<GenericReqAndResp<List<VegaNigeriaSalesPallet>>>?) {
...
//        callBack?.replaceFragment(UPDATE_WEIGHT, binding.tvNetWtValue.text.toString())
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCoffeeSalesBagMaterial>) {
        ...
        binding.rvWeight.setUp(bagList1.asReversed(), R.layout.item_nigeria_sales_pallet_bag_info, { it, pos ->
            if (pos % 2 == 0) {
    ...
    private fun enableSave(enable: Boolean) {
        ...
        when (enable) {
            true -> ViewCompat.setBackgroundTintList(
                binding.btSave,
                ContextCompat.getColorStateList(activity!!, com.olam.warehouse.presentation.R.color.blue_light)
            )
