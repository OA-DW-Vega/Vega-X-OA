...
import kotlinx.android.synthetic.main.item_nigeria_sales_lot.view.*
import kotlinx.android.synthetic.main.item_nigeria_sales_material_list.view.*
...
class VegaNigeriaSalesAddLotFragment : BaseFragment(), NigeriaRecyclerViewItemClickListener {
        ...
        response.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    hideLoading()
                    salesOrderList = it.data?.data as MutableList<VegaNigeriaSalesOrderModel>
                    soNumbers.clear()
    ...
    private fun showSingleSelectDialog(isWh: Boolean, title: String) {
        customDialog =
            CustomNigeriaSingleSelectDialog(title, isWh, soNumbers, activity!!, this)
        customDialog?.show()
    ...
    private fun setUpMaterialAdapter(selectedSales: List<materialList>) {
        ...
        binding.rvMaterialList.setUp(
            ...
            { it, pos ->
                tvMaterialNew.text = it.materialDesc
                tvSoWeight.text =
                    it.openQuantity?.toDouble()?.formatThreeDigits().plus(" ").plus(it.meins)
                when (it.meins) {
                    "KG" -> {
                        tvSoWeight.text = it.openQuantity?.toDouble()?.formatThreeDigits().plus(" ")
                            .plus(it.meins)
                    }
                    "MT" -> {
