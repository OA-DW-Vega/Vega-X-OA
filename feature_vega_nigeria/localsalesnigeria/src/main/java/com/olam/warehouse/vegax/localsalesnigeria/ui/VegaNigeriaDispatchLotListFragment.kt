...
import com.olam.warehouse.vegax.localsalesnigeria.databinding.FragmentNigeriaSalesLotListBinding
...
import com.olam.warehouse.vegax.localsalesnigeria.utils.MODEL_BUNDLE
import com.olam.warehouse.vegax.localsalesnigeria.utils.getColor
import com.olam.warehouse.vegax.localsalesnigeria.utils.listOfField
 ...
 */
class VegaNigeriaDispatchLotListFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_nigeria_sales_lot_list
    private lateinit var binding: FragmentNigeriaSalesLotListBinding
    ...
    companion object {
        fun newInstance(model: VegaCoffeeSalesOrder, materialList: ArrayList<String>, isMultipleAdd: Boolean = true) =
            VegaNigeriaDispatchLotListFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        setHasOptionsMenu(true)
        binding = FragmentNigeriaSalesLotListBinding.inflate(layoutInflater)
        initExtra()
        initUI()
        return binding.root
    }
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("localsalesNigeria/ui/VegaNigeriaDispatchLotListFragment")
...
//        setupAdapter(dispatchLotsList)
    ...
    private fun setupAdapter(data: MutableList<VegaCoffeeSalesLots>) {
        ...
        binding.rvLots.setUpAdapter(
            data,
            R.layout.item_nigeria_sales_lot_list_detail,
            ItemNigeriaSalesLotListDetailBinding::inflate,
            { it, pos, bindItem ->
...
//            tvWeightValue.text = it.weight?.toDouble()?.formatThreeDigits().plus(" ").plus(it.unitOfMeasure)
                when (it.unitOfMeasure) {
                    ...
                    "MT" -> {
                        bindItem.tvWeightValue.text =
                            (it.weight.toString()).toDouble().formatThreeDigits().plus(" ")
                                .plus("MT")
                    }
                }
    ...
    private fun updateWareHouseSpinner() {
        val stageAdapter = ArrayAdapter(requireContext(), R.layout.item_vega_nigeria_dispatch_wh, wareHouseList)
