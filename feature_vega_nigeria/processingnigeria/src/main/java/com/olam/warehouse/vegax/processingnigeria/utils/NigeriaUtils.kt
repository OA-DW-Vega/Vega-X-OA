...
//            processLot.netWeight = convertKgToMT(netWeight.toString())
            ...
            processLot.huno2 = "MT"
            processLot.nohu1 =
                it.lotBagItems?.sumBy { tar -> tar.bagCount.toInt() }.toString().trim()
            ...
            if (processLot.unitsOfMeasure.equals("MT")) {
                processLot.netWeight = convertKgToMT(netWeight.toString())
                processLot.menge = convertKgToMT(netWeight.toString())
                processLot.menge = grade.weight
                processLot.grossWeight = convertKgToMT(grossWeight.toString().trim())
                processLot.huwt2 = convertKgToMT(it.lotBagItems!![0].palletAverage)
                processLot.huwt = convertKgToMT((bagTareWeight.toString()).trim())
            } else {
    ...
    lotDetails.forEach {
        ...
        if (processLot.unitsOfMeasure.equals("MT")) {
            processLot.netWeight =
                if (it.editedWeight?.isNotEmpty()!!) convertKgToMT(it.editedWeight) else convertKgToMT(
                    it.weight
                )
            processLot.menge =
                if (it.editedWeight?.isNotEmpty()!!) convertKgToMT(it.editedWeight) else convertKgToMT(
