...
import com.olam.warehouse.vegax.processingnigeria.data.domain.model.NigeriaFilterList
import com.olam.warehouse.vegax.processingnigeria.databinding.FragmentVegaNigeriaFgrnFilterBinding
import com.olam.warehouse.vegax.processingnigeria.databinding.ItemVegaNigeriaFgrnInventoryFilterBinding
import com.olam.warehouse.vegax.processingnigeria.utils.*
import java.util.*

class VegaNigeriaFgrnFilterFragment : BaseFragment() {

    private lateinit var binding: FragmentVegaNigeriaFgrnFilterBinding
    override val layoutResourceId = R.layout.fragment_vega_nigeria_fgrn_filter
    ...
    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaNigeriaFgrnFilterFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentVegaNigeriaFgrnFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        for (item in whLocations.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFliterWhLoc.add(NigeriaFilterList(item, true))
    ...
    private fun setUpAdapter(listFliterWhLoc: ArrayList<NigeriaFilterList>) {
        binding.rvWhLocation.setUpAdapter(
            listFliterWhLoc,
            R.layout.item_vega_nigeria_fgrn_inventory_filter,
            ItemVegaNigeriaFgrnInventoryFilterBinding::inflate,
            { it, pos, bindItem ->
