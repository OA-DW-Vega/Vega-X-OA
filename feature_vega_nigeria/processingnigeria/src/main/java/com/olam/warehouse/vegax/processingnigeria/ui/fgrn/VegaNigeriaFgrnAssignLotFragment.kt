...
import com.olam.warehouse.vegax.processingnigeria.data.domain.model.VegaNigeriaCustomerModel
import com.olam.warehouse.vegax.processingnigeria.data.domain.model.VegaNigeriaRMINShiftModel
import com.olam.warehouse.vegax.processingnigeria.databinding.FragmentVegaNigeriaFgrnAssignLotBinding
import com.olam.warehouse.vegax.processingnigeria.databinding.ItemVegaNigeriaFgrnLotBinding
...
class VegaNigeriaFgrnAssignLotFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_vega_nigeria_fgrn_assign_lot
    private lateinit var binding: FragmentVegaNigeriaFgrnAssignLotBinding
    ...
    private var customerList = VegaNigeriaCustomerModel()
    ...
    companion object {
        fun newInstance(
            ...
            VegaNigeriaFgrnAssignLotFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaNigeriaFgrnAssignLotBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("processingnigeria/ui/fgrn/VegaCoffeeFgrnAssignLotFragment").title("Processing Cocoa")
            .with(tracker)
    }
...
//                    binding.tvInventory.setBackgroundResource(com.olam.warehouse.presentation.R.drawable.ic_arrow_up)
        ...
        binding.etLotId.onChange { text ->
            when {
                ...
                text.isNotEmpty() -> {
                    if (text.length <= 10) enableDisableBtn(true) else enableDisableBtn(false)
...
//                    }
        ...
        binding.flBayNo.setOnClickListener { showBayNumberDialog() }
        binding.flCountry.setOnClickListener { showCustomerListDialog() }
...
//            }
        ...
        vm.miscellaneousItems.observe(viewLifecycleOwner, Observer { updateCountryListDetails(it) })
...
//        binding.etLotId.filters = arrayOf(InputFilter.AllCaps())
    ...
    private fun updateCountryListDetails(miscellaneous: List<VegaCocoaMiscellaneous>) {
        miscellaneous.forEach { item -> jsonData.add(item.json ?: "") }
        val gson = Gson()
        jsonData.forEach {
            if (it.contains(JSON_CUSTOMER_LIST_DETAILS)) {
                customerList = gson.fromJson(it, VegaNigeriaCustomerModel::class.java)
            }
        }
    }
...
//        bundle.putString(LOT_ID, binding.etLotId.text.toString())
        if(binding.clCreateLot.isVisible) {
            bundle.putString(
                LOT_ID,
                binding.spCountry.text.toString().split("-")[1].plus(binding.spBayNo.text.toString())
            )
            bundle.putBoolean(CREATE_NEW_LOT, isNewLotCreate)
        }else {
            bundle.putParcelableArrayList(
                STOCK_LIST,
                stockList.filter { it.isChecked!! } as ArrayList<VegaCoffeeRminLots>)
        }
...
//                binding.tvBatchDefault.gone()
    ...
    private fun updateStorageLoc(customLocationList: MutableList<VegaCustomStLocation>) {
        customLocationList.let {
            ...
            val stageAdapter =
                ArrayAdapter(requireContext(), R.layout.item_vega_nigeria_processing_rmin_grade, storageLoc)
    ...
    private fun updateAdapter(stockList: MutableList<VegaCoffeeRminLots>) {
        ...
        binding.rvInventoryLot.setUpAdapter(
            stockList,
            R.layout.item_vega_nigeria_fgrn_lot,
            ItemVegaNigeriaFgrnLotBinding::inflate,
            { it, pos, bindItem ->
                bindItem.cbLotId.isChecked = it.isChecked == true
...
//            tvWeightValue.text = it.weight.plus(" ").plus(it.unitOfMeasure)
    ...
    private fun showBayNumberDialog() {
        val range = 1..11
        val seriesSeq = arrayListOf<String>()
        for(num in 1..11){
            if(num < 10) {
                seriesSeq.add("0" + num.toString())
            }else{
                seriesSeq.add(num.toString())
            }
        }
        MaterialDialog(requireContext()).show {
            title(R.string.select_bayno)
            listItemsSingleChoice(items = seriesSeq) { _, index, text ->
                binding.spBayNo.text = text
                enableAssignLotButton()
            }
    ...
    private fun showCustomerListDialog() {
        val seriesSeq = arrayListOf<String>()
        if(customerList.CUSTOMER.isNotEmpty()) {
            customerList.CUSTOMER.forEach { seriesSeq.add(it.key.toString().plus("-").plus(it.value)) }
        }
        MaterialDialog(requireContext()).show {
            title(R.string.select_customer)
            listItemsSingleChoice(items = seriesSeq) { _, index, text ->
                binding.spCountry.text = text
                enableAssignLotButton()
            }
    ...
    private fun enableAssignLotButton(){
        if(binding.clCreateLot.isVisible && binding.spCountry.text.isNotEmpty() && binding.spBayNo.text.isNotEmpty()
            && !storageLocation.contains("--") && storageLocation.isNotEmpty()){
            enableDisableBtn(true)
        }else{
            enableDisableBtn(false)
        }
