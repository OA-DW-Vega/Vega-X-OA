...
import com.olam.warehouse.vegax.processingnigeria.databinding.ItemSubstagesBomLayoutBinding
...
class VegaNigeriaCocoaRminPoBomSubStagesSelectFragment : BaseFragment() {
    ...
    private fun updateBomSubStagesList(boms: List<VegaNigeriaProcessingSubStageDetails>?) {
        ...
        if (boms?.size == 1) {
            ...
            binding.rvPoList.setUpAdapter(
                bomList,
                R.layout.item_substages_bom_layout,
                ItemSubstagesBomLayoutBinding::inflate,
                { it, pos, bindItem ->
                    if (pos % 2 == 0) {
                    ...
                    bindItem.tvBom.text = context.getString(R.string.sub_processing_stage)
                    bindItem.tvInputGrade.text = context.getString(R.string.material_code)
                    bindItem.tvBomValue.text = it.versionId.plus(" - ").plus(it.processingStage)
                    bindItem.tvInputGradeValue.text = it.materialCode
                    bindItem.tvOutputValue.text = it.versionId
