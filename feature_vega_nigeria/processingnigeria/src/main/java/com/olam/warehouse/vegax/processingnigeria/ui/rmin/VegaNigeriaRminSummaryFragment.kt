...
import com.olam.warehouse.vegax.processingnigeria.databinding.ItemNigeriaSummaryLotBinding
...
class VegaNigeriaRminSummaryFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        if (model?.weightToProcess.toString().contains("MT")) {
            binding.cvCard.tvProceesValue.text =
                (model?.weightToProcess.toString().split(" ")[0]).plus(" ").plus("MT")
        } else {
        ...
        if (model?.poNumber?.isNotEmpty()!!) {
            ...
            binding.cvCard.tvPoNoValue.text = model?.poQuantity.plus(" MT")
        }
    ...
    private fun setupAdapter() {
        ...
        binding.rvLotList.setUpAdapter(
            list,
            R.layout.item_nigeria_summary_lot,
            ItemNigeriaSummaryLotBinding::inflate,
            { it, pos, bindItem ->
                ...
                bindItem.tvScaleWeightValue.text =
                    weight?.formatThreeDigits().plus(" ").plus(it.unitOfMeasure)
...
//            tvTotalWeightLossValue.text = totalLoss?.formatThreeDigits().plus(" ").plus(it.unitOfMeasure)
                ...
                if (it.isEndLot == true) {
                    bindItem.tvTotalWeightLossValue.text =
                        totalLoss?.formatThreeDigits().plus(" ").plus(it.unitOfMeasure)
                } else
    ...
    private fun showConfirmDialog() {
        MaterialDialog(requireContext()).show {
            ...
            UIUtils.getMetirialCustomView(
                ...
                {
                    if (model?.poNumber?.isNotEmpty()!!) {
                        val postReq = prepareRminPostRequest(model, fgrnItemWithGrades, workFlowData!!)
                        vm.postRmin(postReq)
                    } else {
