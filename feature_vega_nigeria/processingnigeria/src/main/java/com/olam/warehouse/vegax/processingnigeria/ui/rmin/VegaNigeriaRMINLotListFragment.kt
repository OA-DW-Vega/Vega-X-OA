...
import kotlinx.android.synthetic.main.item_nigeria_inventory_lot.view.*
...
class VegaNigeriaRMINLotListFragment : BaseFragment() {
                        ...
                        true -> {
                            ...
                            dataValue.forEachIndexed { index, s ->
                                when (s.unitOfMeasure) {
                                    ...
                                    "MT" ->
                                        dataValue[index].weight = convertMtToKg(dataValue[index].weight.toString())
                                }
                            }
    ...
    private fun setupAdapter(data: MutableList<VegaCoffeeRminLots>) {
        ...
        binding.rvLots.setUp(data, R.layout.item_nigeria_inventory_lot, { it, pos ->
            tvLotId.text = it.batchNumber
            tvGradeValue.text = it.materialName
            tvStLocationValue.text = it.storageLocationCode
            tvWeightValue.text = it.weight?.toDouble()?.formatThreeDigits().plus(" ").plus("KG")
