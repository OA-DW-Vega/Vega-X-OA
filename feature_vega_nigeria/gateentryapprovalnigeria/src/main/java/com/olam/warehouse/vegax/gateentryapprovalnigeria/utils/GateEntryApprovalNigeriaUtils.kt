...
fun prepareGateEntryApprovalPost(gateEntryData: MutableList<VegaGateEntryDetails>): List<VegaGateEntry> {
    ...
    gateEntryData.forEach { it1 ->
        ...
        item.item = it1.item
        item.materialCode = it1.materialCode.materialCode
        item.materialName = it1.materialCode.materialName
        item.plantId = it1.plant.plantId
        item.storageLocationCode = it1.storageLocationCode
        item.supplierCode = it1.vendorCode.vendorCode
        item.supplierName = it1.vendorCode.vendorName
        item.truckDriverName = it1.driverNumber
        item.unitsOfMeasure = it1.materialCode.unitsOfMeasure.toString()
