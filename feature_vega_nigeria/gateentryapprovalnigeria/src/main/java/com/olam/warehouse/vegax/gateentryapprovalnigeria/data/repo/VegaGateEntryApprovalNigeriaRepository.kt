...
override suspend fun getStorageLocation(code: String) = dao.getStorageLocation(code)
    ...
    override suspend fun getDMSUploadedImages(
        ...
        return object :
            NetworkOnlyBoundResource<GenericReqAndResp<VegaDMSImageResponse>>() {
            override suspend fun createCall(): GenericReqAndResp<VegaDMSImageResponse> =
                api.getDMSUploadedImages(currentKey, werks, wbId, year)
        }.build().asLiveData()
    }
