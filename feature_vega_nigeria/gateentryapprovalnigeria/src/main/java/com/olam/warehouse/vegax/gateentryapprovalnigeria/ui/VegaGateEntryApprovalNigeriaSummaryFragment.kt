...
class VegaGateEntryApprovalNigeriaSummaryFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        binding.tvProduct.text = gateEntryData.materialCode.materialName.plus("-")
            .plus(gateEntryData.materialCode.materialName)
        binding.tvSupplier.text =
            gateEntryData.vendorCode.vendorCode.plus("-").plus(gateEntryData.vendorCode.vendorName)
        ...
        if (gateEntryData.updatedAt.isNullOrEmpty()) {
            ...
            binding.tvDate.text = DateUtils.fromMillisToTimeString(
                DateUtils.getTimeStampinMilliseconds(
                    (gateEntryData.updatedAt.toString().split("T").get(0))
                )
            )
        }
        ...
        binding.btViewImages.setOnClickListener {
            if (gateEntryData.wbid.isNullOrEmpty()) {
                ...
                vm.getDMSUploadedImages(gateEntryData.wbid.toString(), gateEntryData.plant.plantId)
            }
        }
    ...
    private fun postGateEntry() {
        ...
        val wbData = prepareGateEntryApprovalPost(postData)
