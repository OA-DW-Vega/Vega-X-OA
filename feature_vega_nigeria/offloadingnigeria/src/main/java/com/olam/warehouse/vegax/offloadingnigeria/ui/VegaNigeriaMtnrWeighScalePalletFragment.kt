...
import com.olam.warehouse.vegax.offloadingnigeria.databinding.FragmentVegaNigeriaOffloadAddWeightLayoutBinding
import com.olam.warehouse.vegax.offloadingnigeria.databinding.ItemVegaNigeriaOffloadAddBagBinding
import com.olam.warehouse.vegax.offloadingnigeria.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import kotlin.random.Random

class VegaNigeriaMtnrWeighScalePalletFragment : BaseFragment() {

    override val layoutResourceId: Int =
        R.layout.fragment_vega_nigeria_offload_add_weight_layout
    private lateinit var binding: FragmentVegaNigeriaOffloadAddWeightLayoutBinding
    ...
    companion object {
        fun newInstance(item: VegaCoffeeReceiveLots) =
            VegaNigeriaMtnrWeighScalePalletFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentVegaNigeriaOffloadAddWeightLayoutBinding.inflate(inflater)
        initUI()
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        vm.getBagItemsNew(defaultLot.batch, defaultLot.mtnNumber)
        ...
        updateTotalWeights()
        //vm.getPalletInfoWs(defaultLot.batch, defaultLot.materialNumber)
                ...
                if (response.data?.data?.size!! > 0) {
                    ...
                    Bundle().apply {
                        ...
                        putBoolean(
                            ...
                            response.data?.data?.get(0)?.totalPalletWeight.isNullOrEmpty() || response.data?.data?.get(
                                0
                            )?.totalPalletWeight == "0.0"
                        )
    ...
    private fun moveBagAddWeight(bagMaterial: VegaCoffeeOffloadingBagMaterial) {
        ...
        bundle.putParcelable(Constants.BAG_MATERIAL, prepareNigeriaBagMaterial(bagMaterial))
        bundle.putString(
            Constants.TITLE,
            getString(com.olam.warehouse.presentation.R.string.dispatch_weight_entry)
        )
        bundle.putString(
            Constants.LOT_LABEL,
            getString(com.olam.warehouse.presentation.R.string.lot_no)
        )
        callBack?.replaceFragment(FRAG_ADD_BAG_WEIGHT_NEW, bundle)
    }

    fun onBackPressed() {
        val weight =
            if (binding.tvNetWtValue.text.toString().length < 3) ZERO_VALUE.plus(" ")
                .plus(UNIT_KG) else binding.tvNetWtValue.text.toString()
        ...
        bagItems?.let {
            ...
            if (bagItems.isNotEmpty()) {
                ...
                if (bagList.size == palletCount.toInt() || palletCount.equals("0")) binding.btProceed.setBackgroundColor(
                    getColor(com.olam.warehouse.presentation.R.color.colorPrimaryHeadOfi)
                )
    ...
    fun updateBagWeight(bagMaterial: VegaCoffeeOffloadingBagMaterial) {
        ...
        if (tareWeightCalculation <= 3 || (tareWeightCalculation <= 3 && bagMaterial.bagType.isEmpty() && bagMaterial.noOfPallet?.isEmpty() == true)) {
            ...
            bagList.forEach { material ->
                ...
                material.netWeight =
                    tar?.let { material.grossWeight.toDouble().minus(it).toString() }.toString()
                material.unitsOfMeasure =
                    if (material.unitsOfMeasure?.isEmpty()!!) "KG" else material.unitsOfMeasure
                material.message = getString(R.string.stored_locally)
                material.mtnNumber = defaultLot.mtnNumber
                vm.saveBagDetailsNew(material)
            }
            enableProceed(true)
    ...
    private fun setUpAdapter(bagList: ArrayList<VegaCoffeeOffloadingBagMaterial>) {
        ...
        binding.rvWeight.setUpAdapter(
            bagList1.asReversed(),
            R.layout.item_vega_nigeria_offload_add_bag,
            ItemVegaNigeriaOffloadAddBagBinding::inflate,
            { it, pos, bindItem ->
                if (pos % 2 == 0) {
                    bindItem.clBagHead.setBackgroundResource(com.olam.warehouse.presentation.R.drawable.rectangle_grey_border)
                } else {
                    bindItem.clBagHead.setBackgroundResource(R.drawable.shape_nigeria_offload_rect_light_grey1)
                }
    ...
    fun clearTotalWeights() {
        binding.tvStockValue.text = ZERO_VALUE.plus(" ").plus(UNIT_KG)
        binding.tvStorageValue.text = ZERO_VALUE.plus(" ").plus(UNIT_KG)
        binding.tvNetWtValue.text = ZERO_VALUE.plus(" ").plus(UNIT_KG)
    }
        ...
        MaterialDialog(requireActivity()).show {
            ...
            UIUtils.getMetirialCustomView(
                ...
                getString(com.olam.warehouse.presentation.R.string.cancel),
                {
                    ...
                    setUpAdapter(bagList)
                    vm.deleteBagDetailsNew(position.id)
                    if (bagList.size == 0) {
                        clearTotalWeights()
                        enableProceed(false)
