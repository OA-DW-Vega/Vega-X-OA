...
const val UNIT_EA = "EA"
const val TT_PROCURE_TYPE = "tt_procure_type"
...
fun convertWeights(weight: String, packingMaterialUom: String?, currentUom: String): String {
    if (packingMaterialUom == currentUom) {
        return weight
    } else {
        if (packingMaterialUom == UNIT_KG && currentUom == UNIT_MT) {
            return convertKgToMT(weight)
        } else if (packingMaterialUom == UNIT_MT && currentUom == UNIT_KG) return convertMtToKg(weight)
        else return weight
    }
    return weight
}
