...
//            ), false
//        )
        ttProcurementType = intent.getStringExtra(Constants.PROCUREMENT_TYPE)?:""
            ...
            SUPPLIER -> {
                ...
                displayFragment(
                    VegaNigeriaOffloadingWaitingTruckListFragment.newInstance(
                        gateEntryData,ttProcurementType
                    ), true
                )
            }
//            PARAMS_LIST_FRAG -> displayFragment(VegaNigeriaOffloadingSupplierFragment.newInstance(data as Bundle), false)
        ...
        when (moveFrag) {
            OFFLOADING_SUMMARY_FRAG -> displayFragment(
                VegaNigeriaOffloadingSupplierSummaryFragment.newInstance(
                    receivingData,
                    mReceiving as ArrayList<VegaReceiving>,
                    bagList,palletCount,palletGrossweight,palletAvergaeCount
                ), true
            )
        }
    }
    ...
    override fun replaceFragment(paramsListFrag: String, item: VegaOffloadingTrucks, procurementType: String?) {
        when (paramsListFrag) {
