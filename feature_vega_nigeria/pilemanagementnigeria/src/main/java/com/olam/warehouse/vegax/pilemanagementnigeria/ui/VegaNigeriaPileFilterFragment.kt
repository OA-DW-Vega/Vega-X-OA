...
import com.olam.warehouse.vegax.pilemanagementnigeria.data.domain.model.NigeriaFilterList
import com.olam.warehouse.vegax.pilemanagementnigeria.databinding.FragmentPileManagementFilterNigeriaBinding
import com.olam.warehouse.vegax.pilemanagementnigeria.databinding.ItemVegaNigeriaPileFilterBinding
import com.olam.warehouse.vegax.pilemanagementnigeria.utils.*
import java.util.*

class VegaNigeriaPileFilterFragment : BaseFragment() {

    private lateinit var binding: FragmentPileManagementFilterNigeriaBinding
    override val layoutResourceId = R.layout.fragment_pile_management_filter_nigeria
    private var callBack: CallBack? = null
    private var fullFilter = ArrayList<String>()
    private var listFliterWhLoc = ArrayList<NigeriaFilterList>()
    ...
    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) =
            VegaNigeriaPileFilterFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentPileManagementFilterNigeriaBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        for (item in whLocations.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFliterWhLoc.add(
                    NigeriaFilterList(item, true)
                )
                else listFliterWhLoc.add(NigeriaFilterList(item))
            }
        }
    ...
    private fun setUpAdapter(listFliterWhLoc: ArrayList<NigeriaFilterList>) {
        binding.rvWhLocation.setUpAdapter(
            listFliterWhLoc,
            R.layout.item_vega_nigeria_pile_filter_,
            ItemVegaNigeriaPileFilterBinding::inflate,
            { it, pos, bindItem ->
    ...
    private fun applyFilter() {
        val selectedStoLoc =
            listFliterWhLoc.filter { item -> item.isSelected }.map { it.value } as ArrayList
        val bundle = Bundle()
