...
import com.olam.warehouse.vegax.exportsalesnigeria.databinding.FragmentExportSalesNigeriaAddLotBinding
import com.olam.warehouse.vegax.exportsalesnigeria.databinding.ItemExportNigeriaSalesAddLotBinding
import com.olam.warehouse.vegax.exportsalesnigeria.utils.ADD_WEIGHT
import com.olam.warehouse.vegax.exportsalesnigeria.utils.CONTAINER_DATA
import com.olam.warehouse.vegax.exportsalesnigeria.utils.INVENTORY_FRAG
 ...
 */
class VegaNigeriaExportAddLotFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_export_sales_nigeria_add_lot
    private val vm: VegaNigeriaExportSalesViewModel by viewModel()
    private var callback: CallBack? = null
    private lateinit var binding: FragmentExportSalesNigeriaAddLotBinding
    ...
    companion object {
        fun newInstance(container: VegaCoffeeExportSalesContainer, materialList: ArrayList<String>) =
            VegaNigeriaExportAddLotFragment().putArgs {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("exportsalesNigeria/ui/VegaNigeriaExportAddLotFragment").title("Vega_Nigeria/Export Sales")
            .with(tracker)
        initExtra()
        initUI()
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentExportSalesNigeriaAddLotBinding.inflate(layoutInflater)
...
//            showConfirmSaveDialog()
    ...
    private fun updateLotAdapter(lots: List<VegaCoffeeExportSalesLots>) {
        ...
        //Removed sort by Descending
        // lotList.sortedByDescending { it.slPostion }.toMutableList(),
        binding.rvLots.setUpAdapter(
            lotList.toMutableList(),
            R.layout.item_export_nigeria_sales_add_lot,
            ItemExportNigeriaSalesAddLotBinding::inflate,
            { it, pos, bindItem ->
                val lot = it
...
//                tvWeightValue.text = lot.weight?.toDouble()?.formatThreeDigits().plus(" ").plus(lot.unitOfMeasure)
                bindItem.tvWeightValue.text =
                    lotWeightValue.toDouble().formatThreeDigits().plus(" ").plus("MT")
                val editedWeight =
    ...
    fun updateAddWeight(weight: VegaNigeriaExportSalesWSBagModel) {
        ...
        when(vm.lotList[addWeightPosition].unitOfMeasure){
            ...
            "MT" -> {
                come = split[0].toDouble().compareTo(
                    (vm.lotList[addWeightPosition].weight.toString())
                        .toDouble()
                )
            }
        }
