...
import com.olam.warehouse.vegax.exportsalesnigeria.databinding.FragmentExportNigeriaSummaryBinding
import com.olam.warehouse.vegax.exportsalesnigeria.databinding.ItemExportNigeriaSummaryBinding
 ...
 */
class VegaNigeriaExportSummaryFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_export_nigeria_summary
    private val vm: VegaNigeriaExportSalesViewModel by viewModel()
    private var callback: CallBack? = null
    private lateinit var binding: FragmentExportNigeriaSummaryBinding
    ...
    private var textdetailList = ArrayList<VegaNigeriaExportSalesTextDetail>()
    private var mergedBatchNumbers: List<String?> = ArrayList<String>()
    private var assignLotList = ArrayList<VegaNigeriaExportSalesAssignLot>()
    ...
    companion object {
        ...
        ) = VegaNigeriaExportSummaryFragment().putArgs {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("exportsalesNigeria/ui/VegaNigeriaExportSummaryFragment").title("Vega_Nigeria/Export Sales")
            .with(tracker)
        initExtra()
        initUI()
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentExportNigeriaSummaryBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initExtra() {
        ...
        textdetailList =
            arguments?.getParcelableArrayList<VegaNigeriaExportSalesTextDetail>(TEXTDETAIL_LIST)
                ?: ArrayList<VegaNigeriaExportSalesTextDetail>()
    ...
    private fun updateContainerAdapter(data: VegaCoffeeExportOTWithContainer) {
        ...
        binding.rvLots.setUpAdapter(
            containerList,
            R.layout.item_export_nigeria_summary,
            ItemExportNigeriaSummaryBinding::inflate,
            { it, pos, bindItem ->
                ...
                bindItem.tvWeightValue.text = totalWeight.toString().plus(" ").plus("MT")
    ...
    private fun updateHeaderValue() {
        ...
        vm.containerList.forEach {
            ...
            it.lots.forEach { item ->
                ...
                val totallotsWeight = item.editedWeight?.trim() ?: "0.0"
...
//        binding.tvTotalWeight.text = totalWeight.formatThreeDigits().plus(" ").plus(materialUOM)
        binding.tvTotalWeight.text = totalWeight.formatThreeDigits().plus(" ").plus("MT")
        binding.totaldeliveryqtyvalue.text = totalWeight.formatThreeDigits().plus(" ").plus("MT")
        ...
        if(!isEmptyMergedId)
        {
            binding.tvAssignLot.text = "BATCH IDs: ".plus(
                mergedids.distinct().toString().replace("[", "").replace("]", "")
            )
        }
    }
    ...
    private fun postDelivery() {
        vm.postDeliveryDetail(
            VegaNigeriaExportSalesPostRequest(
                key = getCurrentKey(),
    ...
    private fun prepareDeliveryList(): List<VegaNigeriaExportSalesDeliveryDetail> {
        val list = ArrayList<VegaNigeriaExportSalesDeliveryDetail>()
        val year: Int = Calendar.getInstance().get(Calendar.YEAR)
        vm.containerList.forEach { con ->
            con.lots.forEach { item ->
                val deliveryDetail = VegaNigeriaExportSalesDeliveryDetail()
                ...
                deliveryDetail.recStorageLocationCode = item.receivingStorageLocation // Added for Storage Location field
...
//                    materialUOM, item.unitOfMeasure ?: ""
//                )
                deliveryDetail.netWeight = (item.editedWeight.toString())
...
//                deliveryDetail.recStorageLocationCode = item.storageLocationCode
    ...
    private fun updateUI(response: Resource<GenericReqAndResp<VegaNigeriaExportSalesPostRequest>>) {
        when (response.status) {
            Resource.Status.SUCCESS -> {
                ...
                if (response.data?.success!!) {
                    moveToSuccessPage(data,response.data?.message)
                } else {
                    response.data?.message?.let { showErrorDialogWithFAQLink(requireContext(), it) }
//                    updateAssignLotText(mergedBatchNumbers,assignLotList,selectedStorageLocation)
                }
            }
    ...
    private fun moveToSuccessPage(
        data: List<VegaNigeriaExportSalesDeliveryDetail>?,
        message: String?
    ) {
...
//            customMessage.add("OBD Number ".plus(it.delivery).plus(" created Successfully."))
            //.plus(it.mergedBatchNumber)
        }
...
//        vm.saveLotDetails(vm.containerList as ArrayList<VegaCoffeeExportSalesLots>)
    }
