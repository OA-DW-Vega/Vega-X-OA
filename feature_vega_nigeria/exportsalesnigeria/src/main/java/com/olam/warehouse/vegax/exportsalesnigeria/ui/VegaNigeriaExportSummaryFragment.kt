...
import kotlinx.android.synthetic.main.fragment_export_nigeria_summary.*
import kotlinx.android.synthetic.main.item_export_nigeria_summary.view.*
    ...
    private fun updateContainerAdapter(data: VegaCoffeeExportOTWithContainer) {
        ...
        binding.rvLots.setUp(
            ...
            { it, pos ->
                ...
                tvWeightValue.text = totalWeight.toString().plus(" ").plus("KG")
...
//        binding.tvTotalWeight.text = totalWeight.formatThreeDigits().plus(" ").plus(materialUOM)
        ...
        if(!isEmptyMergedId)
        {
            tvAssignLot.text = "LOT IDs: ".plus(mergedids.distinct().toString().replace("[","").replace("]",""))
        }
    }
...
//        intent.putExtra(AppUtils.SUB_TITLE, deveryIds.toString().replace("[", "").replace("]", "").replace(",", "\n"))
    ...
    fun updateAssignLotText(
        ...
        assignLotList = list
        tvAssignLot.text = "LOT IDs: ".plus(mergedLotIds.toString().replace("[", "").replace("]", ""))
