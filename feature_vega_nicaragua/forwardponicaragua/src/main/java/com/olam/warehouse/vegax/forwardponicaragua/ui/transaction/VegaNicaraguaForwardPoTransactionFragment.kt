...
import com.olam.warehouse.vegax.forwardponicaragua.databinding.ItemVegaNicaraguaForwardPoTransactionBinding
...
import com.olam.warehouse.vegax.forwardponicaragua.utils.FORWARD_PO_CREATION_PREVIEW_FRAG
...
class VegaNicaraguaForwardPoTransactionFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        vm.forwardPOPriceTransList.observe(viewLifecycleOwner, androidx.lifecycle.Observer {
            priceDetails =
                ArrayList(prepareForwardPoPriceDetails(ArrayList(it.filter { it.differential!!.isNotEmpty() })))
            FilteredPriceDetails =
                ArrayList(prepareForwardPoPriceDetails(ArrayList(it.filter { it.differential!!.isEmpty() })))
            var data = Bundle()
        ...
        binding.tvSync.setOnClickListener {
            if (DateUtils.isFirstDayOfMonth(requireContext(), count)) {
                if (forWardPOList.filter { it.syncStatus == false }
    ...
    private fun setupAdapter(itemList: MutableList<VegaNicaraguaForwardPODetails>) {
        ...
        binding.rvTransaction.setUpAdapter(
            itemList,
            R.layout.item_vega_nicaragua_forward_po_transaction,
            ItemVegaNicaraguaForwardPoTransactionBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvPOTempIdValue.text =
                ...
                bindItem.tvGradeValue.text = it.grade + "-" + it.qualityGradeDesc
                bindItem.tvErrorValue.text = it.syncStatusMsg
                bindItem.tvWeightValue.text = it.netWeight + " KG(S)"
                    ...
                    when (workInfo.state) {
                        WorkInfo.State.SUCCEEDED -> {
                            ...
                            binding.textViewCount.text =
                                forWardPoList1.size.minus(syncCount).toString()
                            binding.progressBar.progress = forWardPoList1.size.minus(syncCount)
                            if (syncCount != forWardPoList1.size) startSync(
                                forWardPoList1,
                                syncCount
                            )
