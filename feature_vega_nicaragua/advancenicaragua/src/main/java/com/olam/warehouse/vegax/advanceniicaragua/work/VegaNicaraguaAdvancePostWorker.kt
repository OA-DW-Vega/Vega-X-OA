    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            if(!postCopy.equals(post)) {
                ...
                if (response.isSuccessful) {
                    ...
                    if (respData != null) {
                        ...
                        if ((respData.cashJournalDocumentNumber.isNullOrEmpty()) || respData.accountingDocNumber.isNullOrEmpty()) {
                            if (respData.errorMessage.contains("Check whether document has already been entered under number")) {
                                val splits = respData.errorMessage.trim().split(" ")
                                var accountingDocNumber = ""
                                if (splits.size > 10) accountingDocNumber = splits[10]
                                postData.syncStatusMsg = respData.cashJournalMessage
                                postData.documentNumber = accountingDocNumber
                                postData.accountingNumber = accountingDocNumber
                                postData.erdat = DateUtils.getCurrentTimeInMills().toString()
                                postData.syncStatus = true
                                dao.saveAdvanceDetailsData(postData)
                                Result.success(workDataOf(ADVANCE_OUTPUT_DATA to accountingDocNumber))
                            } else {
                ...
                Result.failure(workDataOf(ADVANCE_OUTPUT_DATA to "already exist"))
            }
