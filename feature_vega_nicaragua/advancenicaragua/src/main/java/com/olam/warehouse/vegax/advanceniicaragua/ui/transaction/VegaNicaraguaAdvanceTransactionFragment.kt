package com.olam.warehouse.vegax.advanceniicaragua.ui.transaction
...
import com.olam.warehouse.vegax.advancenicaragua.databinding.FragamentVegaNicaraguaAdvanceTransactionBinding
import com.olam.warehouse.vegax.advancenicaragua.databinding.ItemVegaNicaraguaAdvanceTransactionBinding
import com.olam.warehouse.vegax.advanceniicaragua.data.domain.model.VegaNicaraguaAdvancePostRequest
import com.olam.warehouse.vegax.advanceniicaragua.ui.Callback
...
import com.olam.warehouse.vegax.advanceniicaragua.utils.postCopy
import com.olam.warehouse.vegax.forwardponicaragua.work.getForwardPoOneTimeRequestWorker
import org.koin.androidx.viewmodel.ext.android.viewModel

class VegaNicaraguaAdvanceTransactionFragment : BaseFragment() {
    override val layoutResourceId = R.layout.fragament_vega_nicaragua_advance_transaction
    private lateinit var binding: FragamentVegaNicaraguaAdvanceTransactionBinding
    private val vm: VegaNicaraguaAdvanceViewModel by viewModel()
    private var advanceList = mutableListOf<VegaNicaraguaAdvanceTransactionDetails>()
    private var advanceSearchList = arrayListOf<VegaNicaraguaAdvanceTransactionDetails>()
    private var advanceSortList = mutableListOf<VegaNicaraguaAdvanceTransactionDetails>()
    private var selectedAdvanceTransaction = VegaNicaraguaAdvanceTransactionDetails()
    ...
    companion object {
        fun newInstance() = VegaNicaraguaAdvanceTransactionFragment().putArgs {
        }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragamentVegaNicaraguaAdvanceTransactionBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        binding.tvSync.setOnClickListener {
            if (DateUtils.isFirstDayOfMonth(requireContext(), count)) {
                if (advanceList.filter { it.syncStatus == false }
        ...
        binding.etSearchVendor.onChange {
            if (it.isNotEmpty()) {
                advanceSearchList.clear()
                advanceSearchList.filter { it.syncStatus == true }.forEach { item ->
                    if (item.vendorName?.contains(it, true) == true || item.vendorCode?.contains(
                            it,
                            true
                        ) == true
                    ) advanceSearchList.add(item)
                }
                setupAdapter(advanceSearchList)
            } else {
                setupAdapter(advanceList.filter { it.syncStatus == true }.toMutableList())
            }
        }
            ...
            UIUtils.getMetirialCustomView(
                ...
                getString(R.string.cancel),
                {
                    ...
                    when (isSecurityPin) {
                        ...
                        else -> syncInitiateFirstTime()
                    }
//                startSync(advanceList.filter { it.syncStatus == false }.toMutableList(), 0)
//                checkQuickPin()
                },
                { dismiss() })
        }
    ...
    private fun syncInitiateFirstTime(){
        ...
        postCopy = VegaNicaraguaAdvancePostRequest()
        startSync(
            advanceList.filter { it.syncStatus == false }.toMutableList(),
            0
        )
    }
    ...
    var syncCount = 0
    private fun startSync(advanceList1: MutableList<VegaNicaraguaAdvanceTransactionDetails>, _index: Int) {
        if(addedWorkList.isEmpty()||!addedWorkList.contains(advanceList1[_index].tempId)) {
            if (_index == 0) {
                binding.clProgress.visible()
                binding.textViewCount.text = advanceList1.size.toString()
                binding.progressBar.max = advanceList1.size
                binding.progressBar.progress = advanceList1.size
            ...
            val _element = advanceList1[_index]
            //grnList1.forEachIndexed { _index, _element ->
            val input = workDataOf(UIUtils.ADVANCE_DATA to _element.tempId)
            val worker = getForwardPoOneTimeRequestWorker(input, _index)
            enQueueUniqueWorker(worker, _element.tempId, requireContext())
            addedWorkList.add(_element.tempId)
                        ...
                        when (workInfo.state) {
                            WorkInfo.State.SUCCEEDED -> {
                                ...
                                binding.textViewCount.text =
                                    advanceList1.size.minus(syncCount).toString()
                                binding.progressBar.progress = advanceList1.size.minus(syncCount)
                                if (syncCount != advanceList1.size) startSync(advanceList1, syncCount)
                                if (syncCount == advanceList1.size) {
                                    syncCount = 0
                                    binding.clProgress.gone()
                                }
//                                    hideLoading()
...
//                                    hideLoading()
    ...
    private fun updateUI(data: List<VegaNicaraguaAdvanceTransactionDetails>?) {
        advanceList.clear()
        data?.let { advanceList.addAll(it) }
        moveToPending()
    }
    ...
    private fun moveToPending() {
        ...
        val count = advanceList.filter { it.syncStatus == false }.toMutableList()
        ...
        setupAdapter(advanceList.filter { it.syncStatus == false }.toMutableList())
    }

    private fun sortByDate() {
        if (advanceList.filter { it.syncStatus == true }.isNotEmpty()) {
            advanceSortList = advanceList.filter { it.syncStatus == true }.toMutableList()
            if (advanceSortList.size > 0)
                setupAdapter(advanceSortList.asReversed())
        }
    }

    private fun setupAdapter(itemList: MutableList<VegaNicaraguaAdvanceTransactionDetails>) {
        if (itemList.size > 0) {
        ...
        binding.rvTransaction.setUpAdapter(
            itemList,
            R.layout.item_vega_nicaragua_advance_transaction,
            ItemVegaNicaraguaAdvanceTransactionBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvAdvanceNumber.text =
                    if (it.documentNumber?.isNotEmpty() == true) it.documentNumber else it.tempId
                bindItem.tvVendorValue.text = it.vendorName
                bindItem.tvRequestAdvanceAmount.text =
                    getString(R.string.c_doller) + " " + it.requestedAdvanceAmount
                ...
                if (it.syncStatusMsg.isNullOrEmpty() && it.syncStatus == false) bindItem.clErrorStatus.gone() else bindItem.clErrorStatus.visible()
    ...
    private fun moveToSummary(vegaReceiving: VegaNicaraguaAdvanceTransactionDetails) {
        priceEdit = false
        selectedAdvanceTransaction = vegaReceiving
        var data = Bundle()
        data.putParcelable(UIUtils.RECEIVING_DATA, selectedAdvanceTransaction)
    ...
    private fun moveEdit(it: VegaNicaraguaAdvanceTransactionDetails) {

        var data = Bundle()
        data.putParcelable(UIUtils.RECEIVING_DATA, it)
        callBack?.replaceFragment(ADVANCE_DETAILS_FRAG, data)
