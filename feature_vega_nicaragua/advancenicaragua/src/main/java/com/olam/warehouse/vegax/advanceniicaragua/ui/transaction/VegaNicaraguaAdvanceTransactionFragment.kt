...
import kotlinx.android.synthetic.main.item_vega_nicaragua_advance_transaction.view.*
...
class VegaNicaraguaAdvanceTransactionFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        vm.advanceTransList.observe(viewLifecycleOwner, androidx.lifecycle.Observer { updateUI(it) })
        ...
        binding.tvSync.setOnClickListener {
            if (DateUtils.isFirstDayOfMonth(context!!,count)) {
                if (advanceList.filter { it.syncStatus== false }.toMutableList().size > 0) showConfirmDialog()
                ...
                if (workInfo != null) {
                    when (workInfo.state) {
                        WorkInfo.State.SUCCEEDED -> {
                            ...
                            syncCount++
                            binding.textViewCount.text = advanceList1.size.minus(syncCount).toString()
...
//                                    hideLoading()
    ...
    private fun setupAdapter(itemList: MutableList<VegaNicaraguaAdvanceTransactionDetails>) {
        ...
        binding.rvTransaction.setUp(itemList, R.layout.item_vega_nicaragua_advance_transaction, { it, pos ->
            tvAdvanceNumber.text = if (it.documentNumber?.isNotEmpty() == true) it.documentNumber else it.tempId
            tvVendorValue.text = it.vendorName
            tvRequestAdvanceAmount.text = getString(R.string.c_doller) + " " + it.requestedAdvanceAmount
