...
class VegaNicaraguaMtntConsignmentFragment : BaseFragment(), VegaSingleSelectCommonListener {
    ...
    private fun initUI() {
        ...
        vm.allProduct.observe(viewLifecycleOwner, Observer {
            if (it != null) {
                if(isThirdParty)
                    productList = it.filter {  it.thirdPartyFlag.equals("X", true) }
                else
...
//        vm.stockLots.observe(viewLifecycleOwner, Observer { updateUI(it) })
    ...
    private fun getData() {
        if (!vm.mtnt.sendingPlant.isNullOrEmpty()) getPurchaseOrderByStorageLocation(
            vm.mtnt.sendingPlant
                ?: "-".split("-")[0]
        )
    ...
    private fun validateProceed() {
        when {
            ...
            isCertified && binding.tvCertificationValue.text.isEmpty() -> showSnack(getString(R.string.select_certification))
...
//            binding.tvVendorNameValue.text.isNullOrEmpty() -> showSnack(getString(R.string.enter_vendor_name))
    ...
    private fun prepareTruckWb() {
        if (vm.mtnt.tempId.isEmpty()) {
            val randomDouble = "TMP".plus(Random.nextLong().toString()).plus(
                DateUtils.getCurrentTimeInMills().toString().takeLast(5)
            )
        ...
        vm.mtnt.netWeight = selectedPurchaseOrder?.openQuantity
        ...
        vm.mtnt.vehicleNumber = binding.tvTruckNoValue.text.toString().trim()
        vm.mtnt.driverName = binding.tvDriverNameValue.text.toString().trim()
        vm.mtnt.contactNumber = binding.tvDriverNoValue.text.toString().trim()
    ...
    private fun filterSTONumber() {
        ...
        for (item in purchaseOrderList) {
            if(metrials.contains(item.materialCode?.takeLast(12)))
    ...
    private fun showSingleSelectDialog(title: String, currentFalg: String) {
        ...
        when (currentFalg) {
            ...
            MATERIAL -> {
                ...
                val item = materialModelList.map { it.materialName.toString().plus("--").plus(it.materialCode) }
                matList.addAll(item)
                list = matList
            }
            VENDORNAME -> {
    ...
    override fun clickOnItem(data: String, currentFlag: String) {
        ...
        when (currentFlag) {
            ...
            MATERIAL -> {
                ...
                //vm.mtnt.materialName = data.split("--").get(0)
                isCertified = data.contains("Certificado", false)
                val material = materialModelList.filter { it.materialCode.equals(data.split("--").get(1)) }
                if (material.size > 0) {
                    vm.mtnt.materialCode = material[0].materialCode
                    vm.mtnt.materialName = material[0].materialName
                    ...
                    if (vm.mtnt.materialCode.toString().startsWith("0000005")) {
                        if (isOnline()) vm.getStockListSap(arrayListOf(vm.mtnt.materialCode.toString())) else vm.getStockListOffline(
                ...
                vm.mtnt.plantId = selectedPurchaseOrder?.openQuantity
                getQualityGrades()
                enableProceed()
                binding.tvVendorNameValue.text = ""
                binding.tvQualityGradeValue.text = ""
    ...
    private fun enableProceed() {
        var enable =
            (binding.tvWhValue.text.isNotEmpty() && binding.tvstoValue.text.isNotEmpty() && binding.tvDriverName.text.isNotEmpty()
                    ...
                    && binding.tvMaterialValue.text.isNotEmpty())
        if (binding.tvMaterialValue.text.contains("Certificado", false))
            binding.tvCertification.text = getString(R.string.certification)
        else
            binding.tvCertification.text = getString(R.string.certification1)
        if (enable) {
