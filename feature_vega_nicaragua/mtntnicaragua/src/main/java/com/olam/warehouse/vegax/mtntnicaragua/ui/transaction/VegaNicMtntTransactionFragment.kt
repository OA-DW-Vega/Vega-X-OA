...
import kotlinx.android.synthetic.main.item_nic_mtnt_sync_status_progress_bar.view.*
import kotlinx.android.synthetic.main.item_vega_nic_mtnt_lot_transaction.view.*
import kotlinx.android.synthetic.main.item_vega_nicaragua_mtnt_transaction.view.*
import kotlinx.android.synthetic.main.item_vega_nicaragua_mtnt_transaction.view.ivScaleClose
...
class VegaNicMtntTransactionFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        binding.tvSync.setOnClickListener {
            if (DateUtils.isFirstDayOfMonth(context!!,count)) {
                if (mtntList.filter { it.mtnt.isOfflineData == true }.filter { it.mtnt.isSyncStatus == false }
    ...
    private fun setupAdapter(itemList: List<VegaMtntWithLotsWithBags>) {
        ...
        binding.rvTransaction.setUp(
            ...
            { it, pos ->
                tvMtntOBDIdValue.text = it.mtnt.purchaseDocNum
                tvTransNo.text =
                ...
                tvSendPlantValue.text = it.mtnt.storageLocationCode
                tvGradeValue.text = it.mtnt.qualityGrade
                ...
                if (it.mtnt.isSyncStatus == false && it.mtnt.isErrorStatus == false) clErrorStatus.visible() else clErrorStatus.gone()
                tvReadMore.setOnClickListener { view -> showErrorDialog(it.mtnt.message.toString()) }

                rvSyncLotStatus.setUp(it.lineItems.toMutableList(),
