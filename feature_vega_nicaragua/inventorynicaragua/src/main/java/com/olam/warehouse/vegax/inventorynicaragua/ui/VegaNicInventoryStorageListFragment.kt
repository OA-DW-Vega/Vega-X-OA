...
import com.olam.warehouse.vegax.inventorynicaragua.ui.dialog.VegaNicInventorySyncDetailsDailog
...
class VegaNicInventoryStorageListFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.tvSearchLot.setOnClickListener {
            var lotNo = ""
            showDialogWithInput(
                getString(R.string.search_lot_number),
                object : DialogClick {
                ...
                true,
                getString(R.string.enter_lot_no),
                lotNo
            )
...
//            }
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        try {
            ...
            searchView.queryHint = getString(R.string.search_stloc)
            getAllFilterValues()
                        ...
                        if (newText?.isEmpty() == true) {
                            ...
                            wareHouseList.forEach { qtyWb ->
                                newText?.let { text ->
                                    ...
                                    if (qtyWb.warehouseLocation.procureLocationCode.plus(qtyWb.warehouseLocation.procureLocationName).trim()!!.contains(text, true)) {
                                        mSearchList.add(qtyWb)
                                    }
                                }
                            }
    ...
    private fun syncDetails() {
        val bottomDialog = VegaNicInventorySyncDetailsDailog.newInstance(syncStatusList, materialSyncStatusList)
        ...
        data.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    ...
                    materialSyncStatusList.clear()
                    it.data?.data?.materialWiseSyncStatus?.let { it1 -> materialSyncStatusList.addAll(it1) }
                    getAllFilterValues()
                    setAdapter(wareHouseList)
                    syncDetails()
                }
