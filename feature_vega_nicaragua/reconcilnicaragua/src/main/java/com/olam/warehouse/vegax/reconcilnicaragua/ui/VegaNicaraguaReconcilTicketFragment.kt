...
import com.olam.warehouse.vegax.reconcilnicaragua.databinding.ItemVegaNicaraguaReconcilReportBinding
...
//            viewBinder.tvPtbfAdvance.text = vm.reportItem.ptbfAdvance
            ...
            for (item: Int in 0 until noOfBlock) {
                if (item == 0) {
                    ...
                    viewBinder.rvTableInfo1.setUpAdapter(
                        reports.toMutableList(),
                        R.layout.item_vega_nicaragua_reconcil_report,
                        ItemVegaNicaraguaReconcilReportBinding::inflate,
                        { it, pos, bindItem ->
                            bindItem.tvSno.text = it.itemPos
                            bindItem.tvDocType.text = it.documentType
                            bindItem.tvMoveType.text = it.movementType
                            bindItem.tvVendor.text = it.supplierName
                            bindItem.tvQtyGrade.text = it.grade.plus(" - ").plus(it.gradeDesc)
                            bindItem.tvNetWeight.text = it.netWeight
                            bindItem.NetAmt.text =
                                it.netPayment?.replace(getString(R.string.c_doller), "")
                        })
                    bitmapPrintKeys.add(
                        bitmapToString(
                            getBitmapFromView(
            ...
            if (noOfBlock == 0) {
                ...
                if (noOfBlockBalance != 0) {
                    ...
                    viewBinder.rvTableInfo2.setUpAdapter(
                        subItem,
                        R.layout.item_vega_nicaragua_reconcil_report,
                        ItemVegaNicaraguaReconcilReportBinding::inflate,
                        { it, pos, bindItem ->
