...
import com.olam.warehouse.vegax.reconcilnicaragua.databinding.ItemInvoiceLayoutBinding
...
//        vm.getInvoiceOfflineData(startTime, endTime)
    ...
    private fun setUpAdapter(list: List<VegaNicaraguaReportModel>) {
        ...
        binding.rvList.setUpAdapter(
            list.toMutableList(),
            R.layout.item_invoice_layout,
            ItemInvoiceLayoutBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvDocNoValue.text = it.documentType
                bindItem.tvMovementTypeValue.text = it.movementType
                bindItem.tvVendorNameValue.text = it.supplierName
                bindItem.tvnetWeightValue.text =
                    if (!it.netWeight.isNullOrEmpty()) it.netWeight?.trim().plus(" ")
                        .plus(it.unitOfMeasure) else "-"
                bindItem.tvQualityGradeValue.text = it.grade.plus(" - ").plus(it.gradeDesc)
                bindItem.tvNetPaymentValue.text =
