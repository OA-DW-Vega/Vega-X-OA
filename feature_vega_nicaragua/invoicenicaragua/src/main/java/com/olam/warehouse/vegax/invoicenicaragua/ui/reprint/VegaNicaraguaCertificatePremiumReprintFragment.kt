...
import com.olam.warehouse.login.databinding.VegaNicaraguaCertificatePremiumReceiptPrintingBinding
...
class VegaNicaraguaCertificatePremiumReprintFragment : BaseFragment() {
    ...
    companion object {
        fun newInstance() = VegaNicaraguaCertificatePremiumReprintFragment()
            .putArgs {
            }
    }
    ...
    private fun initUI() {
        ...
        vm.invoiceReceiptOffline.observe(
            viewLifecycleOwner,
            androidx.lifecycle.Observer { updateInvoiceReceiptOfflineUI(it) })
        vm.supplier.observe(viewLifecycleOwner, androidx.lifecycle.Observer {
        ...
        binding.tvGrnHeading.text=getString(R.string.certificate_premium_heading)
    }
    ...
    private fun updateGrnUI(it: List<VegaReceiving>?) {
        it?.let { item ->
            val data = item.filter {
                ( it.grnType.equals("spot" ,true) || it.grnType.equals("fixed"  , true)) && (covertToDouble(it.certificatePremium) >0.00)
            }
            data.forEach {
...
//        setupAdapter(invoiceList)
            ...
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    ...
                    response.data?.data?.let { it1 ->
                        ...
                        val data = resultData.filter {
                              (covertToDouble(it.certificatePremium) >0.00)
                        }
    ...
    private fun updateUI(data: List<VegaNicaraguaInvoiceDetails>?) {
        invoiceList.clear()
        data?.let {
            val data = it.filter { covertToDouble(it.certificatePremium) >0.0}
    ...
    private fun setupAdapter(itemList: MutableList<VegaNicaraguaInvoiceDetails>) {
        ...
        binding.rvTransaction.setUpAdapter(
            ...
            { it, pos, bindItem ->
                bindItem.tvCertificatePremium.visibility = View.VISIBLE
                bindItem.tvCertificatePremiumValue.visibility = View.VISIBLE
                bindItem.tvCertificatePremiumValue.text =
                    getString(R.string.c_doller) + " " + it.certificatePremium
                bindItem.tvInvoiceTempIdValue.text = it.tempId
        ...
        DoAsync {
            printableList.forEachIndexed { index, item ->
                ...
                val view = LayoutInflater.from(context)
                    .inflate(
                        com.olam.warehouse.login.R.layout.vega_nicaragua_certificate_premium_receipt_printing,
                        null
                    )
                ...
                when (selectedInvoice.grnType) {
                    getString(com.olam.warehouse.login.R.string.ptbf) -> {

                        getString(com.olam.warehouse.login.R.string.s_ptbf)
                    }
                    getString(com.olam.warehouse.login.R.string.spot) -> {

                        viewBinder.typeOfPurchase.text =
                            getString(com.olam.warehouse.login.R.string.s_spot)
                    ...
                    getString(com.olam.warehouse.login.R.string.spot_tolling) -> {
                        ...
                        viewBinder.typeOfPurchase.text =
                            getString(com.olam.warehouse.login.R.string.s_tolling)
                        viewBinder.typeOfPurchaseCopy.text =
                            getString(com.olam.warehouse.login.R.string.s_tolling)
                    }
                ...
                viewBinder.batchListCopy.text = grnData.charg
