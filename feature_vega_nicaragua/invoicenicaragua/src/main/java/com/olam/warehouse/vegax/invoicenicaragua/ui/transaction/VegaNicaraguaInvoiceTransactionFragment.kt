...
import kotlinx.android.synthetic.main.item_vega_nicaragua_invoice_transaction.view.*
...
class VegaNicaraguaTransactionFragment : BaseFragment() {
    ...
    private fun sortByDate() {
        if (invoiceList.isNotEmpty()) {
            ...
            if (invoiceSortList.size > 0)
                invoiceList = invoiceSortList.asReversed() as ArrayList<VegaNicaraguaInvoiceDetails>
            setupAdapter(invoiceList.filter { it.isSynced })
        }
    }
    ...
    private fun setupAdapter(itemList: List<VegaNicaraguaInvoiceDetails>) {
        ...
        binding.rvTransaction.setUp(
            ...
            { it, pos ->
                tvGrnTempIdValue.text = it.tempId
                tvBatchNoValue.text = it.grn
...
//            if(it.isSynced) ivScaleClose.gone() else ivScaleClose.visible()
            ...
            ivEdit.setOnClickListener { view ->
                ...
                popupMenu.menu.findItem(com.olam.warehouse.login.R.id.action_edit1).isVisible = false
            ...
            if (it.syncStatusMsg.isNullOrEmpty()) clErrorStatus.gone() else clErrorStatus.visible()
            ...
            UIUtils.getMetirialCustomView(
                ...
                getString(R.string.cancel),
                {
                    ...
                    when (isSecurityPin) {
                        ...
                        else -> startSync(invoiceList.filter { !it.isSynced } as ArrayList<VegaNicaraguaInvoiceDetails>,
                            0)
                    }
                ...
                if (workInfo != null) {
                    when (workInfo.state) {
                        WorkInfo.State.SUCCEEDED -> {
                            ...
                            syncCount++
                            binding.textViewCount.text = invoiceList1.size.minus(syncCount).toString()
                            binding.progressBar.progress = invoiceList1.size.minus(syncCount)
                            if (syncCount != invoiceList1.size) startSync(invoiceList1, syncCount)
