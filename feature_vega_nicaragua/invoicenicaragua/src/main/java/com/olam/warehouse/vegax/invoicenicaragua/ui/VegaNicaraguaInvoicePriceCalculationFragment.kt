...
import com.olam.warehouse.vegax.grnecuador.utils.PrepareInvoiceData
import com.olam.warehouse.vegax.grnecuador.utils.covertToDouble
import com.olam.warehouse.vegax.grnecuador.utils.getTmpId
import com.olam.warehouse.vegax.grnecuador.utils.prepareAdvanceLineItem
...
//                        UIUtils.showErrorDialog(requireContext(), it.error.toString())
    ...
    private fun getPricingInfo() {
        vm.priceConfigInfo.observe(this, androidx.lifecycle.Observer {
    ...
    private fun getQualityGradeDesc() {
        ...
        vm.getQualityGradeDesc(grnData.qualityGrade!!)
    }
                ...
                if (priceConfigDetails != null) {
                    try {
                        ...
                        selectedGrnCount.forEach {
                            ...
                            ftdc = ftdc.plus(
                                ((covertToDouble(priceConfigDetails?.ftdc!!)) * (covertToDouble(
                                    exchangeRate!!
    ...
    private fun setAdvanceLinesAdapter(data: List<AdvanceLineItemDetails>) {
        if (data != null && data.size > 0) {
            ...
            advanceLineAdaptor = VegaNicaraguaInvoicePriceCalculationAdvanceLineAdaptor(context!!, data)
    ...
    private fun saveData(isSync: Boolean, msg: String) {
        ...
        invoiceInfo.invoiceNo =
            if (invoiceInfo.invoiceNo?.isEmpty() == true) generateInvoiceRefNumber() else invoiceInfo.invoiceNo
    ...
    private fun preparingInvoiceData(): java.util.ArrayList<VegaNicaraguaInvoiceDetails> {
        ...
        invoiceDetails.tempNumber = generateInvoiceRefNumber()
        invoiceSequnceNo = generateInvoiceRefNumber()
