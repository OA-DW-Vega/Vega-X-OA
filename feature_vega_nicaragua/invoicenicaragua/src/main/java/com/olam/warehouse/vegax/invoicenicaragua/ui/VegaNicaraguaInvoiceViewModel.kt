...
class VegaNicaraguaInvoiceViewModel(
    ...
    private var grnTransListWithAdvanSource: LiveData<List<VegaNicGrnInvoiceWithAdvanceItems>> = MutableLiveData()
    private val _grnTransListWithAdvan = MediatorLiveData<List<VegaNicGrnInvoiceWithAdvanceItems>>()
    val grnTransListWithAdvan: LiveData<List<VegaNicGrnInvoiceWithAdvanceItems>> get() = _grnTransListWithAdvan

    private var invoiceWithAdvanOfflineSource: LiveData<List<VegaNicPtbfInvoiceWithAdvanceItems>> =
        MutableLiveData()
    private val _invoiceWithAdvanOffline = MediatorLiveData<List<VegaNicPtbfInvoiceWithAdvanceItems>>()
    val invoiceWithAdvanOffline: LiveData<List<VegaNicPtbfInvoiceWithAdvanceItems>> get() = _invoiceWithAdvanOffline
    ...
    fun getInvoiceWithAdvanOfflineData() = viewModelScope.launch(dispatchers.main) {
        _invoiceWithAdvanOffline.removeSource(invoiceWithAdvanOfflineSource) // We make sure there is only one source of livedata (allowing us properly refresh)
        withContext(dispatchers.io) {
            invoiceWithAdvanOfflineSource = useCase.getInvoiceWithAdvanOfflineData()
        }
        _invoiceWithAdvanOffline.addSource(invoiceWithAdvanOfflineSource) {
            _invoiceWithAdvanOffline.value = it
        }
    }

    fun getReceivingWithAdvanceLineItem() = viewModelScope.launch(dispatchers.main) {
        _grnTransListWithAdvan.removeSource(grnTransListWithAdvanSource) // We make sure there is only one source of livedata (allowing us properly refresh)
        withContext(dispatchers.io) {
            grnTransListWithAdvanSource = useCase.getReceivingWithAdvanceLineItem()
        }
        _grnTransListWithAdvan.addSource(grnTransListWithAdvanSource) {
            _grnTransListWithAdvan.value = it
        }
    }
