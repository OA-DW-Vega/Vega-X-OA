...
import com.olam.warehouse.vegax.invoicenicaragua.databinding.ItemVegaNicaraguaWithholdTaxReprintBinding
...
class VegaNicaraguaWithHoldTaxReprintFragment : BaseFragment() {
                    ...
                    newText.let {
                        if (newText?.isEmpty() == true) {
                            ...
                            invoiceList.forEach { qtyWb ->
                                newText?.let { text ->
                                    if (qtyWb.tempId.contains(text,true) == true ||
                                        qtyWb.grn?.contains(text,true) == true
                                    ) {
    ...
    private fun updateGrnUI(it: List<VegaReceiving>?) {
        it?.let { item ->
            var data = item.filter {( it.grnType.equals("spot" ,true) || it.grnType.equals("fixed"  , true)) && covertToDouble(it.withholdingTax) >0}
            data.forEach {
...
//        setupAdapter(invoiceList)
    ...
    private fun filterData(){
        val offlineList= invoiceList.filter {  it.grn.isNullOrEmpty() }.sortedByDescending { it.charg }
    ...
    private fun setupAdapter(itemList: MutableList<VegaNicaraguaInvoiceDetails>) {
        ...
        binding.rvTransaction.setUpAdapter(
            itemList,
            R.layout.item_vega_nicaragua_withhold_tax_reprint,
            ItemVegaNicaraguaWithholdTaxReprintBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvWithHoldTaxValue.text =
                    getString(R.string.c_doller) + " " + it.withholdingTax
                ...
                bindItem.tvGrossAmountValue.text = it.grossValue
