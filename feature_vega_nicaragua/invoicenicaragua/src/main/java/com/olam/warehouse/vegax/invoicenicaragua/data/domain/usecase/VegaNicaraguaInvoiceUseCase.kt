...
class VegaNicaraguaInvoiceUseCase(
    ...
    suspend  fun updatePostingDate(tmpWbId: String, postdate: String) =
        repository.updatePostingDate(tmpWbId,postdate)

    suspend fun getReceivingWithAdvanceLineItem()  = repository.getReceivingWithAdvanceLineItem()
    suspend fun getInvoiceWithAdvanOfflineData()  = repository.getInvoiceWithAdvanOfflineData()
    suspend fun getProducts(): LiveData<List<VegaMaterial>> = repository.getProducts()
}
