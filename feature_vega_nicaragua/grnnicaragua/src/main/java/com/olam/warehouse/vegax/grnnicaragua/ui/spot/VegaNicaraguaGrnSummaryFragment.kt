...
import kotlinx.android.synthetic.main.item_vega_nicaragua_quality_details.view.*
...
class VegaNicaraguaGrnSummaryFragment : BaseFragment(),
    ...
    private fun setAdvanceLinesAdapter(data: List<VegaNicaraguaAdvanceLineItemGrn>) {
        ...
        binding.advanceLineItems.setUp(data.toMutableList(), R.layout.item_vega_nicaragua_advance, { it, pos ->
            ...
            val totalAmt =
                ...
                )?.plus(if (it.commissionAmount?.isNotEmpty() == true) it.commissionAmount?.toDouble() ?: 0.0 else 0.0)
                    ?.plus(
    ...
    private fun setUpAdapter(qualityParameters: ArrayList<VegaQualityParameter?>) {
        binding.rvQuality.setUp(qualityParameters, R.layout.item_vega_nicaragua_quality_details, { it, pos ->
            if (pos % 2 == 0) {
            ...
            tvQualityNameApprove.text = it?.descrChar
            tvUnitApprove.text = it?.qualityParameterValue
        })
    }
