...
class VegaNicaraguaGrnPriceCalculationFragment : BaseFragment(),
    ...
    private fun initUI() {
        ...
        vm.advanceItems.observeOnce(viewLifecycleOwner, Observer {
            ...
            if (receivingData.finalPayment?.isNotEmpty() == true) {
                ...
                if (isOnline()) vm.getAdvanceDetailsByVendor(receivingData.supplierCode!!) else vm.getAdvanceDetailsByVendorOffline(
    ...
    private fun setPriceDetails() {
        ...
        if (priceConfigDetails != null) {
            try {
                ...
                var humidityDiscount = 0.0
                if (receivingData.grnType.equals(getString(R.string.fixed))) {

                     humidityDiscount =
                        formatDouble(
                            (totalPrice!! + certificatePremium!!) * (covertToDouble(priceConfigDetails?.moistureDiscount) / 100)
                        )
                ...
                if (receivingData.grnType.equals(getString(R.string.fixed))) {
                    ...
                    grossValue =
                        formatDouble((totalPrice!! + certificatePremium!!) - (qualityDiscount!! + humidityDiscount))
                ...
                var ftdcValue = if(receivingData.ftdcValue.isNullOrEmpty()) priceConfigDetails?.ftdc else receivingData.ftdcValue
                var ftdc: Double =
        ...
        if (receivingData.grnType.equals(getString(R.string.fixed))) {
            binding.volumeLL.gone()
            binding.humidityLL.gone()
        } else {
            binding.volumeLL.visible()
            binding.humidityLL.visible()
        }
    }
    ...
    private fun setAdvanceLinesAdapter(data1: List<AdvanceLineItemDetails>) {
        ...
        if (data != null && data.size > 0) {
            ...
            advanceLineAdaptor =
                VegaNicaraguaGrnPriceCalculationAdvanceLineAdaptor(requireContext(), data)
