...
import com.olam.warehouse.presentation.utils.extension.setBackground
...
var postCopy = VegaNicaraguaGrnPost()
fun getTmpId() = "TMP_".plus(Random.nextInt().toString()).plus(getCurrentTimeInMills().toString().takeLast(5))
...
fun saveLotSequence(batchNumber: String, transactionList: MutableList<VegaReceiving>, isOnline: Boolean) {
    val transbatchNos = transactionList.filter { it.status.equals(Status.SYNC_COMPLETED)||it.status.equals(Status.SYNC_PENDING) }.map { it.batchNumber?.substring((it.batchNumber?.length?:0) - 5)?.toInt() }.sortedBy { it }.asReversed()
    ...
    if (!isEditTrans) {
        ...
        if(transbatchNos.isNotEmpty()){
            ...
            if(isOnline)
                maxValue = transbatchNos.maxByOrNull { it?:0 }?.inc()?:lotSequence
            else
                maxValue = (transbatchNos.maxByOrNull { it?:0 }?.inc()?:lotSequence)/*.inc()*/
            when{
                lotSequence != maxValue -> lotSequence = maxValue
            }
        }
...
fun getFormatedDate(sdate: String): String {
    ...
    val newFormat = SimpleDateFormat("yyyyMMdd")
...
fun isdatefunc(postDate: String): Boolean {
    val millis = System.currentTimeMillis()
    val currentdate = Date(millis)
    if (!postDate.isNullOrEmpty()) {
        val postdateval = getStringtoDate(postDate)
        return !currentdate.before(postdateval)
    } else
        return true
}

private fun getStringtoDate(toString: String): Date? {
    val format = SimpleDateFormat("dd/MM/yyyy")
    var date: Date? = null
    try {
        date = format.parse(toString)
...
fun changeState(status: Int, root: ConstraintLayout, context: Context?) {
    for (i in 1..status) {
        val stateId = context?.resources?.getIdentifier("view_state$i", "id", context.packageName)
        val view: View? = stateId?.let { root.findViewById(it) }
        view?.let {
        ...
        val txtId = context?.resources?.getIdentifier("text_view_state$i", "id", context.packageName)
        val txtView: TextView? = txtId?.let { root.findViewById(it) }
        ...
        if (i < status) {
            val prgId = context?.resources?.getIdentifier("view_progress$i", "id", context.packageName)
            val prgView: View? = prgId?.let { root.findViewById(it) }
            prgView?.setBackgroundColor(context.getColor(com.olam.warehouse.presentation.R.color.colorPrimaryOfi1) ?: 0)
        }
    }
}
