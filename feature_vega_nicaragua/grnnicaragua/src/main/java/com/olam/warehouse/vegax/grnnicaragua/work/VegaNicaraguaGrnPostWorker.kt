...
import com.olam.warehouse.vegax.grnnicaragua.utils.postCopy
...
class VegaNicaraguaGrnPostWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params),
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        val vm: VegaNicaraguaGrnViewModel by inject()
        ...
        val ttFarmerList = dao.getTTFarmerData(wbid)
        val farmerTransDetails = dao.getFarmerTransData(wbid)
        ...
        post.ttFarmerList = ttFarmerList?: emptyList()
        post.farmerTransDetails = farmerTransDetails?: TrackTraceFarmerTransDetails()
        ...
        /*if (receivingData.syncStarted == 1) {
            ...
            try {
                if(!postCopy.equals(post)) {
                        ...
                        if (respData != null) {
                            if (resp?.data?.wbFlag == true && resp.data.qcFlag == true && resp.data.grnFlag == true) {
                                if (receivingData.grnType.equals("ptbf", true)) {
                                    ...
                                    if(!resp.data.grnNumber.toString().isNullOrEmpty())receivingData.grnNumber = resp.data.grnNumber.toString()
                                    if(!resp.data.poNumber.toString().isNullOrEmpty())receivingData.purchaseDocNum = resp.data.poNumber.toString()
                                ...
                                receivingData.batchNumber = resp?.data?.batchNumber.toString()
                                if(!resp?.data?.poNumber.toString().isNullOrEmpty())receivingData.purchaseDocNum = resp?.data?.poNumber.toString()
                                if (receivingData.deleteFlag == true) {
                                    receivingData.status = Status.SYNC_COMPLETED
                                    receivingData.isSynced = true
                                } else {
                                    receivingData.status = Status.SYNC_PENDING
                    ...
                    Result.failure(workDataOf(GRN_OUTPUT_DATA to "already exist"))
                }
    ...
    if (priceConfigDetails != null) {
        try {
           ...
           /* totalPrice = covertToDouble(priceConfigDetails.dailyPrice) * netWeight.toDouble()
            ...
            info.bagCount =
                bagItems.sumOf { if (it.bagCount.isNotEmpty()) it.bagCount.toInt() else 0 }.toString()
            info.grossWeight =
                bagItems.sumOf { if (it.grossWeight.isNotEmpty()) it.grossWeight.toDouble() else 0.0 }
                    .toString()
            info.tareWeight = bagItems.sumOf {
...
private fun preparingGrnData(
    ...
    val weight = bagItems.sumOf { if (it.netWeight.isNotEmpty()) it.netWeight.toDouble() else 0.0 }
