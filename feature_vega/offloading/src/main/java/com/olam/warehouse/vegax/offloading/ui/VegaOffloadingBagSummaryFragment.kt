...
import com.olam.warehouse.vegax.offloading.databinding.ItemVegaOffloadingSummaryBinding
...
class VegaOffloadingBagSummaryFragment : BaseFragment() {
    ...
    private fun setUpAdapter(data: MutableList<VegaQualityParameter>) {
        ...
        binding.rvBagDetailSummary.setUpAdapter(
            list,
            R.layout.item_vega_offloading_summary,
            ItemVegaOffloadingSummaryBinding::inflate,
            { it, pos, bindingItem ->
                bindingItem.tvItemName.text =
                    if (!it.qualityParamLabel.isNullOrEmpty()) it.qualityParamLabel else it.descrChar
                bindingItem.tvItemValue.text =
