...
import kotlinx.android.synthetic.main.item_vega_offloading_presampling.view.*
...
class VegaOffloadingPreSamplingFragment : BaseFragment() {
    ...
    private fun setUpPreAdapter(data: MutableList<VegaQualityParamsWithQualitative?>) {
        ...
        binding.rvPreSampling.setUp(preSamplingList, R.layout.item_vega_offloading_presampling, { item, pos ->
            tvDescription.text =
    ...
    private fun setUpSuggLocationAdapter(data: MutableList<VegaQualityParameter>) {
        ...
        binding.rvSuggLocation.setUp(data, R.layout.item_vega_offloading_presampling, { item, pos ->
            tvDescription.text =
...
//        data.let { it -> locationList = it }
        binding.rvLocation.setUp(data, R.layout.item_vega_offloading_presampling, { item, pos ->
            tvDescription.text =
                if (!item.qualityParamLabel.isNullOrEmpty()) with(UIUtils) { with(item.qualityParamLabel.toString()) { mandatoryStars() } } else with(
                    UIUtils
                ) { with(item.descrChar.toString()) { mandatoryStars() } }
    ...
    private fun setUpBagAdapter(data: MutableList<VegaQualityParameter>) {
        ...
        binding.rvBagRefraction.setUp(bagList, R.layout.item_vega_offloading_presampling, { item, pos ->
    ...
    private fun updateParams(data: Resource<GenericReqAndResp<List<VegaOffloadingLotQuality>>>) {
        data.let {
            when (it.status) {
                ...
                Resource.Status.ERROR -> {
                    hideLoading()
                    UIUtils.showErrorDialog(requireActivity(), "Quality Fetching Failed")
                }
                else -> {
                }
            }
