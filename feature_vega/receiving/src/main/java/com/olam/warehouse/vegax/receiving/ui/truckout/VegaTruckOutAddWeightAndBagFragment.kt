...
import com.olam.warehouse.vegax.receiving.databinding.FragmentVegaTruckoutAddWeightBinding
 ...
 */
class VegaTruckOutAddWeightAndBagFragment : BaseFragment() {

    private var isBagReturnable: Boolean= false
    ...
    private lateinit var binding: FragmentVegaTruckoutAddWeightBinding

    override val layoutResourceId = R.layout.fragment_vega_truckout_add_weight

    companion object {
        fun newInstance(
            receivingData: VegaReceiving,
            receiving: ArrayList<VegaReceiving>
        ) = VegaTruckOutAddWeightAndBagFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentVegaTruckoutAddWeightBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        binding.cbPo.setOnCheckedChangeListener { it, isChecked ->  isBagReturnable= isChecked }
    }
        ...
        when {
            binding.etTruckTarWeight.text.isNullOrEmpty() -> showSnack(getString(R.string.error_valid_tar_weight))
                ...
                if (binding.tvBagType1.text.isNotEmpty() || binding.etBagCount1.text.isNotEmpty()) {
                    ...
                    if (data.isNotEmpty()){
                        receiving.bagTareWeight = data[0].tareWeight
                        receiving.bagMaterialCode= data[0].bagMaterialCode
                    }
                    if(getCurrentKey().contains("VEGA_IV") && getCurrentKey().contains("CASH")){
                        if(receiving.bagMaterialCode=="000000500000000070" || receiving.bagMaterialCode=="000000200000013211")
                            receiving.bagReturnFlag=isBagReturnable
                    }
                ...
                if (binding.tvBagType2.text.isNotEmpty() || binding.etBagCount2.text.isNotEmpty()) {
                    ...
                    if (data.isNotEmpty()){
                        receiving1.bagTareWeight = data[0].tareWeight
                        receiving1.bagMaterialCode= data[0].bagMaterialCode
                    }
                    if(getCurrentKey().contains("VEGA_IV") && getCurrentKey().contains("CASH")){
                        if(receiving1.bagMaterialCode=="000000500000000070" || receiving1.bagMaterialCode=="000000200000013211")
                            receiving1.bagReturnFlag=isBagReturnable
                    }
                ...
                if (binding.tvBagType3.text.isNotEmpty() || binding.etBagCount3.text.isNotEmpty()) {
                    ...
                    if (data.isNotEmpty()){
                        receiving2.bagTareWeight = data[0].tareWeight
                        receiving2.bagTareWeight = data[0].bagMaterialCode
                    }
                    if(getCurrentKey().contains("VEGA_IV") && getCurrentKey().contains("CASH")){
                        if(receiving2.bagMaterialCode=="000000500000000070" || receiving2.bagMaterialCode=="000000200000013211")
                            receiving2.bagReturnFlag=isBagReturnable
                    }
