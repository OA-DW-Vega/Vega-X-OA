...
import com.olam.warehouse.vegax.inventory.databinding.ItemVegaInventorySyncDetailsBinding
...
class VegaInventorySyncDetailsDailog : BottomSheetDialogFragment() {
    ...
    private fun setUpAdapter(syncStatusList: java.util.ArrayList<SyncStatus>) {
        binding.rvSyncDetails.setUpAdapter(
            syncStatusList,
            R.layout.item_vega_inventory_sync_details,
            ItemVegaInventorySyncDetailsBinding::inflate,
            { it, pos, bindingItem ->
                bindingItem.tvPlantId.text = it.plant
                bindingItem.tvStatus.text = it.status
                bindingItem.tvSyncTime.text = it.updatedTimeInMS?.let { it1 ->
                ...
                when (it.status) {
                    "PEND" -> bindingItem.tvStatus.setTextColor(
                        ContextCompat.getColor(
                            bindingItem.tvStatus.context,
                            getSyncItemBackgroundColor(5)
                        )
                    )
                    "ERR" -> bindingItem.tvStatus.setTextColor(
                        ContextCompat.getColor(
                            bindingItem.tvStatus.context,
                            getSyncItemBackgroundColor(3)
                        )
                    )
                    else -> bindingItem.tvStatus.setTextColor(
                        ContextCompat.getColor(
                            bindingItem.tvStatus.context,
