...
import kotlinx.android.synthetic.main.item_vega_fgrn_summary.view.*
...
class VegaFgrnSummaryFragment : BaseFragment() {
    ...
    private fun updateDetails(poList: List<VegaDispatchLots>) {
        ...
        binding.rvFgrnSummary.setUp(data, R.layout.item_vega_fgrn_summary, { item, pos ->
            tvBatchNo.text = item.batchNumber
            tvStorageLocation.text = item.storageLocationCode
            tvOutputMaterial.text = item.materialName
            tvWeight.text = item.weight.plus(" ").plus(item.unitOfMeasure)
            tvNoOfBags.text = item.noOfBags
        }, {
            //showConfirmDialog()
        })
    }
