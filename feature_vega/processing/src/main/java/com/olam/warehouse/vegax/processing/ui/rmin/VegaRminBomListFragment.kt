...
import kotlinx.android.synthetic.main.item_vega_rmin_bom_list.view.*
...
class VegaRminBomListFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        binding.rvBomList.setUp(bomListData!!, R.layout.item_vega_rmin_bom_list, { it, pos ->
            tvBomNo.text = it.cfgno.toString()
            tvInputGrade.text = materialName.toString()
            tvOutputGrade.text = it.materialName.toString()
        }, {

            callBack?.replaceFragment(stageFevor!!, cfgNumber!!, materialName!!, materialNo!!, this)
        })
