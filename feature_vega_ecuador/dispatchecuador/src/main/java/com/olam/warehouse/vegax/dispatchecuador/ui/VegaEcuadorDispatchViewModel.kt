 ...
 */
class VegaEcuadorDispatchViewModel(
    private val useCase: VegaEcuadorDispatchUseCase,
    ...
    private var purchaseSourceLocal: LiveData<List<VegaEcuadorDispatchPurchaseOrders>> = MutableLiveData()
    private val _purchaseOrderLocal = MediatorLiveData<List<VegaEcuadorDispatchPurchaseOrders>>()
    val purchaseOrderLocal: LiveData<List<VegaEcuadorDispatchPurchaseOrders>> get() = _purchaseOrderLocal
    ...
    private var deliveryWbPostSource: LiveData<Resource<GenericReqAndResp<VegaEcuadorDeliveryPostResponse>>> =
        MutableLiveData()
    private val _deliveryWbPost = MediatorLiveData<Resource<GenericReqAndResp<VegaEcuadorDeliveryPostResponse>>>()
    val deliveryWbPost: LiveData<Resource<GenericReqAndResp<VegaEcuadorDeliveryPostResponse>>> get() = _deliveryWbPost
    ...
    fun postWbDeliveryDetail(vegaDeliveryPost: VegaEcuadorWbDeliveryPost) = viewModelScope.launch(dispatchers.main) {
        _deliveryWbPost.removeSource(deliveryWbPostSource) // We make sure there is only one source of livedata (allowing us properly refresh)
        withContext(dispatchers.io) {
            deliveryWbPostSource = useCase.postWbDeliveryDetail(vegaDeliveryPost)
        }
        _deliveryWbPost.addSource(deliveryWbPostSource) {
            _deliveryWbPost.value = it
        }
    }
    ...
    fun getPurchaseOrderLocal(plantId: String) = viewModelScope.launch(dispatchers.main) {
        ...
        withContext(dispatchers.io) {
            purchaseSourceLocal = useCase.getPurchaseOrderLocal(plantId)
        }
    ...
    private var dispatchTruckResources: LiveData<Resource<GenericReqAndResp<List<VegaCocoaDispatchWB>>>> = MutableLiveData()
    private val _truck = MediatorLiveData<Resource<GenericReqAndResp<List<VegaCocoaDispatchWB>>>>()
    val truck: LiveData<Resource<GenericReqAndResp<List<VegaCocoaDispatchWB>>>> get() = _truck

    fun getTruckList() = viewModelScope.launch(dispatchers.main) {
        _truck.removeSource(dispatchTruckResources)
        withContext(dispatchers.io) {
            dispatchTruckResources = useCase.getTruckLists()
        }
        _truck.addSource(dispatchTruckResources) {
            _truck.value = it
        }
    }
}
