...
import com.olam.warehouse.vegax.dispatchecuador.databinding.FragmentEcuadorMtntWeighbridgeAddLayoutBinding
...
import com.olam.warehouse.vegax.dispatchecuador.ui.VegaEcuadorCustomSingleSelectDialog
import com.olam.warehouse.vegax.dispatchecuador.ui.VegaEcuadorDispatchViewModel
import com.olam.warehouse.vegax.dispatchecuador.ui.VegaEcuadorRecyclerViewItemClickListener
import com.olam.warehouse.vegax.dispatchecuador.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import java.util.concurrent.TimeUnit

class VegaEcuadorMtntWeighBridgeAddLotFragment : BaseFragment(),VegaEcuadorRecyclerViewItemClickListener  {
    override val layoutResourceId = R.layout.fragment_ecuador_mtnt_weighbridge_add_layout
    private lateinit var binding: FragmentEcuadorMtntWeighbridgeAddLayoutBinding
    ...
    //private lateinit var adapter: VegaCoffeeMtntLotAdapter
    ...
    private var wbData: VegaEcuadorDispatch? = VegaEcuadorDispatch()
    ...
    companion object {
        fun newInstance(model: VegaCocoaDispatchWB) =
            VegaEcuadorMtntWeighBridgeAddLotFragment()
                .putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentEcuadorMtntWeighbridgeAddLayoutBinding.inflate(layoutInflater)
        initExtra()
        initUI()
        listener = this
        return binding.root
    ...
    private fun initUI() {
       ...
       vm.validateLot.observe(viewLifecycleOwner, Observer {
        ...
        if (dispatchLotsList.size > 0) {
            ...
            vm.getPurchaseOrderLocal(wbData!!.storageLocationCode.toString())
        } else {
    ...
    override fun clickOnItem(data: String, isWh: Boolean) {
        ...
        if (isWh) {
            ...
            enableStartLoad(false)
            wbData?.storageLocationCode = da[0]
                wbData?.plantName = da[1]
        } else {
            binding.tvstoValue.text = data
            binding.tvPoNoValue.text = data
            selectedSTO =
                purchaseOrder.filter { it.purchaseDocNum == data } as MutableList<VegaEcuadorDispatchPurchaseOrders>
            setUpAdapter(selectedSTO)
            enableStartLoad(true)
            wbData?.purchaseOrders?.clear()
            selectedSTO.let { wbData?.purchaseOrders?.addAll(it) }
            selectedSTO.distinct()
            wbData?.purchaseDocNum = data
            wbData?.unitsOfMeasure = selectedSTO[0].meins.toString()
        }
    }
    ...
    private fun updateAdapter(lot: VegaEcuadorDispatchLots) {
        ...
        if (!addedLotId.contains(lot.batchNumber)) {
            if (wbData?.wbTempId?.isEmpty()!!) wbData?.wbTempId = getTmpId() else wbData?.wbTempId
            lot.wbTempId = wbData?.wbTempId!!
            lot.wbTempId = getTmpId()
            wbData?.lotList?.add(lot)
    ...
    private fun updateLoadingState(isStarted: Boolean) {
        when (isStarted) {
            true -> {
                wbData?.isStarted = true
                startTime = System.currentTimeMillis()
                wbData?.startTime = startTime.toString()
            ...
            false -> {
                ...
                wbData?.endTime = endTime.toString()
                val duration = endTime?.minus(startTime ?: 0)
                wbData?.turnAroundTime = TimeUnit.MILLISECONDS.toMinutes(duration ?: 0).toString()
                binding.btStartLoad.text = resources.getString(R.string.end_loading)
    ...
    private fun moveToAddInventory() {
        if (!isLoadingStarted) {
         ...
         //model = vm.dispatchWh
         wbData?.lotList?.clear()
        addedLotList.let { wbData?.lotList?.addAll(it) }
        callBack?.replaceFragment(LOT_LIST, wbData!!, materialCodeList)
    }

    private fun moveToMergeList() {
        wbData?.erdat = "/Date(".plus(DateUtils.getCurrentTimeInMills()).plus(")/")
        vm.saveProcessOrderAndLotDetails(addedLotList, wbData!!)
        callBack?.replaceFragment(MERGE_LIST, wbData!!, addedLotList as ArrayList<VegaEcuadorDispatchLots>)
    }
    ...
    private fun updateUIWithLocalData() {
        //binding.llOffline.gone()
        isLoadingStarted = wbData!!.isStarted
        enableProceed(wbData!!.isStarted && wbData!!.isEnded)
        enableStartLoad(wbData!!.isStarted && !wbData!!.isEnded)
        updateLoadingState(!wbData!!.isEnded)
      //  binding.tvWhValue.text = model!!.storageLocationCode.plus("-").plus(getPlantDetails().plantName)
    ...
    private fun removeCurrentItem(pos: Int) {
        vm.deleteLot(addedLotList.get(pos).batchNumber, wbData?.wbTempId.toString())
