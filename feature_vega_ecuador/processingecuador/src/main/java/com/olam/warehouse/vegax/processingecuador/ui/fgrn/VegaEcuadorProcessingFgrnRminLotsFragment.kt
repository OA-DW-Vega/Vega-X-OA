...
import com.olam.warehouse.vegax.processingecuador.databinding.FragmentVegaEcuadorProcessingFgrnLocalRminLotsBinding
import com.olam.warehouse.vegax.processingecuador.databinding.ItemLotSummaryEcuadorProcessingBinding
import com.olam.warehouse.vegax.processingecuador.utils.*
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaEcuadorProcessingFgrnRminLotsFragment : BaseFragment() {
    override val layoutResourceId = R.layout.fragment_vega_ecuador_processing_fgrn_local_rmin_lots
    private lateinit var binding: FragmentVegaEcuadorProcessingFgrnLocalRminLotsBinding
    ...
    companion object {
        fun newInstance(model: VegaCocoaFgrnItems, id: String) =
            VegaEcuadorProcessingFgrnRminLotsFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentVegaEcuadorProcessingFgrnLocalRminLotsBinding.inflate(layoutInflater)
    ...
    private fun initUI() {
        ...
        vm.offlineRminLotsLocal.observe(viewLifecycleOwner, Observer {
            if (!it.isNullOrEmpty()) {
    ...
    private fun moveToNext() {
        when (getCurrentFragment()) {
            is VegaEcuadorProcessingFgrnRminLotsFragment -> {
    ...
    private fun updateFgrnAdapter(poList: MutableList<VegaCoffeeRminLots>) {
        ...
        binding.rvPoList.setUpAdapter(
            poList,
            R.layout.item_lot_summary_ecuador_processing,
            ItemLotSummaryEcuadorProcessingBinding::inflate,
            { item, pos, bindItem ->
                ...
                bindItem.ivSelect.isChecked = item.isAdded ?: false
                bindItem.llLotItem.setOnClickListener { view ->
                    item.isAdded = !item.isAdded!!
                    bindItem.ivSelect.isChecked = item.isAdded!!
    ...
    private fun enableDisableBtn(enable: Boolean) {
        if (enable) {
            binding.tvCreateNewFgrn.isEnabled = true
            binding.tvCreateNewFgrn.setBackgroundColor(getColor(com.olam.warehouse.presentation.R.color.colorPrimaryOfi))
