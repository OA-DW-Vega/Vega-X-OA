...
import com.olam.warehouse.vegax.processingecuador.databinding.FragmentVegaEcuadorProcessingFgrnPoDetailsBinding
import com.olam.warehouse.vegax.processingecuador.databinding.ItemVegaEcuadorProcessingFgrnPoDetailsBinding
import com.olam.warehouse.vegax.processingecuador.utils.FRAG_GRADES
import com.olam.warehouse.vegax.processingecuador.utils.FRAG_PENDING
import com.olam.warehouse.vegax.processingecuador.utils.getColor
import com.olam.warehouse.vegax.processingecuador.utils.weightToProcess
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper
import java.text.DecimalFormat

class VegaEcuadorProcessingFgrnPODetailsFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_vega_ecuador_processing_fgrn_po_details
    private lateinit var binding: FragmentVegaEcuadorProcessingFgrnPoDetailsBinding
    ...
    companion object {
        fun newInstance() = VegaEcuadorProcessingFgrnPODetailsFragment().putArgs {
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaEcuadorProcessingFgrnPoDetailsBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun moveToNext(offlineFgrnList: List<VegaCocoaFgrnItems>) {
        when (getCurrentFragment()) {
            is VegaEcuadorProcessingFgrnPODetailsFragment -> when (offlineFgrnList.size > 0) {
    ...
    private fun updageStageUI(stageList: List<VegaProcessingStage>) {
        ...
        val stageAdapter =
            ArrayAdapter(requireContext(), R.layout.item_vega_ecuador_processing_rmin_grade, stageListData)
                    ...
                    when (it.data?.success) {
                        true -> {
                            response.data?.data?.let {
                                poList.clear()
                                val datalist =
                                    it.filter { !it.rminList.isNullOrEmpty() } as MutableList<VegaCocoaFgrnItems>
                                poList = datalist.asReversed()
                                updateAdapter(poList)

                                /* it.forEach { item ->
                                     if (getCurrentKey().split("_")[1].contains("EC")) {
                               ...
                               /* if (getCurrentKey().split("_")[1].contains("EC") && plantList.contains(
                                        plantId
                                    )
                                ) {
    ...
    private fun updateAdapter(poList: MutableList<VegaCocoaFgrnItems>) {
        ...
        binding.rvPoList.setUpAdapter(
            poList,
            R.layout.item_vega_ecuador_processing_fgrn_po_details,
            ItemVegaEcuadorProcessingFgrnPoDetailsBinding::inflate,
            { item, pos, bindItem ->
...
//                if (getCurrentKey().split("_")[1].contains("EC") && plantList.contains(plantId)) {
