package com.olam.warehouse.vegax.processingecuador.utils
...
): VegaEcuadorProcessingCreatePoReq {
    val poReq = VegaEcuadorProcessingCreatePoReq()
...
//    poReq.poQuantity = model.poQuantity
    poReq.poQuantity = model.weightToProcess.replace(" QE","")
    ...
    lotDetails.forEach {
        ...
        if(model?.shift?.length ?:0 >= 4 ) processLot.shiftType = model?.shift?.substring(0,3) else processLot.shiftType = model?.shift
