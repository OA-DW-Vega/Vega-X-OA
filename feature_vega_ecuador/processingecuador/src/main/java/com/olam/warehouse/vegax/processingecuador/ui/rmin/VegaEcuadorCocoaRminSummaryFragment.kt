...
import com.olam.warehouse.vegax.processingecuador.data.domain.model.VegaEcuadorProcessingRMINLotDetails
import com.olam.warehouse.vegax.processingecuador.data.domain.model.VegaEcuadorProcessingRminResponse
import com.olam.warehouse.vegax.processingecuador.data.domain.model.VegaEcuadorRminProcessingPost
import com.olam.warehouse.vegax.processingecuador.databinding.FragmentEcuadorCocoaRminSummaryBinding
import com.olam.warehouse.vegax.processingecuador.databinding.ItemLotEcuadorCocoaRminSummaryBinding
...
import com.olam.warehouse.vegax.processingecuador.utils.SHIFT
import com.olam.warehouse.vegax.processingecuador.utils.prepareRminCreatePoRequest
import org.koin.androidx.viewmodel.ext.android.viewModel
import org.matomo.sdk.Tracker
import org.matomo.sdk.extra.TrackHelper

class VegaEcuadorCocoaRminSummaryFragment : BaseFragment() {

    override val layoutResourceId = R.layout.fragment_ecuador_cocoa_rmin_summary
    private lateinit var binding: FragmentEcuadorCocoaRminSummaryBinding
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        TrackHelper.track().screen("processingecuador/ui/rmin/VegaCocoaRminSummaryFragment")
            .title("Rmin Cocoa")
            .with(tracker)
    }
    ...
    companion object {
        fun newInstance(model: VegaCocoaRminProcessing) =
            VegaEcuadorCocoaRminSummaryFragment().putArgs {
                putParcelable(MODEL_BUNDLE, model)
            }
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentEcuadorCocoaRminSummaryBinding.inflate(layoutInflater)
        initExtra()
        initUI()
    ...
    private fun setupAdapter() {
        ...
        binding.rvLotList.setUpAdapter(
            list,
            R.layout.item_lot_ecuador_cocoa_rmin_summary,
            ItemLotEcuadorCocoaRminSummaryBinding::inflate,
            { it, _, bindItem ->
    ...
    private fun preparePostData(): VegaEcuadorRminProcessingPost {
        val postModel = VegaEcuadorRminProcessingPost()
        ...
        postModel.shiftType = model?.shift?.substring(0,3) ?: ""
        postModel.processingStage = model?.stage
        postModel.rmin = true
        val lots = model?.lotList
        val processList = mutableListOf<VegaEcuadorProcessingRMINLotDetails>()
        lots?.forEach {
            val processLot = VegaEcuadorProcessingRMINLotDetails()
            ...
            processLot.shiftType = model?.shift?.substring(0,3) ?: ""
    ...
    private fun updateUI(response: Resource<GenericReqAndResp<List<VegaEcuadorProcessingRminResponse>>>) {
        response.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    hideLoading()
    ...
    private fun moveToSuccessPage(success: List<VegaEcuadorProcessingRminResponse>?) {
