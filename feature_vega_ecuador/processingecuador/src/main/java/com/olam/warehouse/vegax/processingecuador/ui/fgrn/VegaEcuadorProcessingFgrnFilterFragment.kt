...
import com.olam.warehouse.vegax.processingecuador.data.domain.model.FilterList
import com.olam.warehouse.vegax.processingecuador.databinding.FragmentVegaEcuadorProcessingFgrnFilterBinding
import com.olam.warehouse.vegax.processingecuador.databinding.ItemVegaEcuadorProcessingFgrnInventoryFilterBinding
import com.olam.warehouse.vegax.processingecuador.utils.*
import java.util.*


class VegaEcuadorProcessingFgrnFilterFragment : BaseFragment() {

    private lateinit var binding: FragmentVegaEcuadorProcessingFgrnFilterBinding
    override val layoutResourceId = R.layout.fragment_vega_ecuador_processing_fgrn_filter
    private var callBack: CallBack? = null
    private var fullFilter = ArrayList<String>()
    private var listFliterWhLoc = ArrayList<FilterList>()
    private var startRange: String = ""
    private var endRange: String = ""
    private var aboveThirty: Boolean = false

    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaEcuadorProcessingFgrnFilterFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentVegaEcuadorProcessingFgrnFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        for (item in whLocations.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFliterWhLoc.add(FilterList(item, true))
                else listFliterWhLoc.add(FilterList(item))
            }
        }
    ...
    private fun setUpAdapter(listFliterWhLoc: ArrayList<FilterList>) {
        binding.rvWhLocation.setUpAdapter(
            listFliterWhLoc,
            R.layout.item_vega_ecuador_processing_fgrn_inventory_filter,
            ItemVegaEcuadorProcessingFgrnInventoryFilterBinding::inflate,
            { it, pos, bindItem ->
