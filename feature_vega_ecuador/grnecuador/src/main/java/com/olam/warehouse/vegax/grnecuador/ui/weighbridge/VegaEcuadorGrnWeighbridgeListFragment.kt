 ...
 */
class VegaEcuadorGrnWeighbridgeListFragment : BaseFragment() {
        ...
        data.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    hideLoading()
                    val weighBridge =
                        it.data?.data?.filter { it.grnNumber.isNullOrEmpty() && it.weighMethod.equals("WB") }
                    //it.qcStatus.isNullOrEmpty()
                    if (weighBridge?.size!! > 0) {
                        //weighBridgeList = weighBridge as MutableList<VegaGrnWeighBridgeId>
