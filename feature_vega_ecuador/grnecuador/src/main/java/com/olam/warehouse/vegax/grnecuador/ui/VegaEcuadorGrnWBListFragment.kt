...
import com.olam.warehouse.vegax.grnecuador.databinding.FragmentVegaEcuadorGrnWeighbridgeListBinding
import com.olam.warehouse.vegax.grnecuador.databinding.ItemVegaEcuadorWeighbridgeGrnBinding
 ...
 */
class VegaEcuadorGrnWBListFragment : BaseFragment() {
    ...
    private lateinit var binding: FragmentVegaEcuadorGrnWeighbridgeListBinding
    private var callBack: CallBack? = null
    override val layoutResourceId = R.layout.fragment_vega_ecuador_grn_weighbridge_list
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentVegaEcuadorGrnWeighbridgeListBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    companion object {
        fun newInstance(weighmentType: String) = VegaEcuadorGrnWBListFragment().putArgs {
            putString(WEIGH_TYPE, weighmentType)
        }
    }
    ...
    private fun initUI() {
        weighmentType = arguments?.getString(WEIGH_TYPE)!!
        if (isOnline()) {
//            if (weighmentType.equals(WEIGHSCALE)) {
                vm.weighBridge.observe(viewLifecycleOwner, Observer { updateUI(it) })
                vm.getWeighBridgeList()
//            } else if (weighmentType.equals(WEIGHBRIDGE)) {
//                vm.weighBridgeGrnPending.observe(viewLifecycleOwner, Observer {
//                    updateUI(it) })
//                vm.getWeighBridgeGrnPendingList()
//            }
            ...
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    ...
                    if (weighmentType.equals(WEIGHSCALE)) {
                        weighBridge =
                            it.data?.data?.filter {
                                it.qcStatus.isNullOrEmpty() && it.grnNumber.isNullOrEmpty() && it.weighMethod.equals(
                                    "WS"
                                )
                            }
                    } else if (weighmentType.equals(WEIGHBRIDGE)) {
                        weighBridge =
                            it.data?.data?.filter { it.weighMethod.equals("WB") && it.grnNumber.isNullOrEmpty() && it.netWeight != "0.000"}
                    }
    ...
    private fun updateWBUI(data: Resource<GenericReqAndResp<List<VegaGrnWeighBridgeId>>>) {
        data.let {
            when (it.status) {
                Resource.Status.SUCCESS -> {
                    hideLoading()
                    ...
                    if (weighmentType.equals(WEIGHSCALE)) {
                        ...
                        weighBridge =
                            it.data?.data?.filter { it.weighMethod.equals("WB") && it.grnNumber.isNullOrEmpty() }
                    }
    ...
    private fun setUpAdapter(data: List<VegaGrnWeighBridgeId>?) {
        ...
        binding.rvWeighBridgeId.setUpAdapter(
            ...
            ItemVegaEcuadorWeighbridgeGrnBinding::inflate,
            { it, pos, bindItem ->
                if (weighmentType.equals(WEIGHSCALE)) {
                    it.unitPrice = it.unitPrice.toString().replace("\\s".toRegex(), "")
                    bindItem.tvProcurementType.text =
                        if (it.purchaseDocNum.isNullOrEmpty()) getString(R.string.spot_purchase) else getString(
                            R.string.fixed_purchase
                        )
                } else {
                    bindItem.tvPoLabel.text = context.getString(R.string.material)
                    bindItem.tvProcurementType.text =
                        it.materialName
                }
                ...
                if(it.sourceLotId?.isNotEmpty() == true) {
                    bindItem.tvSourceLotIdLabel.visible()
                    bindItem.tvSourceLotId.text = it.sourceLotId
                } else {
                    bindItem.tvSourceLotIdLabel.gone()
                }
//                bindItem.tvSourceLotId.text = it.sourceLotId
            ...
            },
            {
               ...
               /* if (weighmentType.equals(WEIGHSCALE)) {
                    ...
                    callBack?.replaceFragment(GRN_WB_FRAG, item)
                }*/
            })
    }
}
