...
import com.olam.warehouse.vegax.inventoryecuador.databinding.ItemVegaEcuadorInventoryDetailsBinding
...
class VegaEcuadorInventoryDetailsFragment : BaseFragment() {
    ...
    private fun setUpAdapter(data: List<InventoryLots>) {
        ...
        binding.rvInventory.setUpAdapter(
            lotList,
            R.layout.item_vega_ecuador_inventory_details,
            ItemVegaEcuadorInventoryDetailsBinding::inflate,
            { it, pos, bindItem ->
                bindItem.tvImpurityValue.text = it.impurity
                bindItem.tvHumidityValue.text = it.humidity
                bindItem.tvMouldValue.text = it.mould
                bindItem.tvLotName.text = it.lotId
                bindItem.tvWeightData.text = it.stockQty.plus(" ").plus("QE")
    ...
    fun applyFilter(bundle: Bundle) {
       ...
       fullFilter.addAll(impurityList)
        ...
        if(filterData1.isNotEmpty()) filterData.addAll(filterData1)
        if(filterData2.isNotEmpty()) filterData.addAll(filterData2)
        if(filterData3.isNotEmpty()) filterData.addAll(filterData3)
        if(filterData4.isNotEmpty()) filterData.addAll(filterData4)

        //val result = HashSet<InventoryLots>()
        //result.addAll(filterData)
        //filterData = result.toMutableList() as ArrayList<InventoryLots>
        setUpAdapter(if (filterData.size == 0) lotList else filterData)
    }



    /*fun applyFilter1(bundle: Bundle) {
        val materialList = bundle.getStringArrayList(FILTER_MATERIAL) ?: ArrayList()
        val humidityList = bundle.getStringArrayList(FILTER_HUMIDITY) ?: ArrayList()
        val impurityList = bundle.getStringArrayList(FILTER_IMPURITY) ?: ArrayList()
        val mouldList = bundle.getStringArrayList(FILTER_MOULD) ?: ArrayList()
