...
class VegaEcuadorBcApproveRepositoryImpl(
    ...
    override suspend fun postApproval(postApprovalData: VegaEcuadorPostApprovalRequest): LiveData<Resource<GenericReqAndResp<VegaEcuadorBcApprovePostResponse>>> {

        return object : NetworkOnlyBoundResource<GenericReqAndResp<VegaEcuadorBcApprovePostResponse>>() {

            override suspend fun createCall(): GenericReqAndResp<VegaEcuadorBcApprovePostResponse> =
                api.postBcApproval(postApprovalData)
        }.build().asLiveData()
    }
