...
import com.olam.warehouse.vegax.pileecuador.databinding.FragmentEcuadorPileManagementFilterBinding
import com.olam.warehouse.vegax.pileecuador.databinding.ItemVegaEcuadorPileFilterBinding
import com.olam.warehouse.vegax.pileecuador.ui.data.domain.model.EcuadorFilterList
import com.olam.warehouse.vegax.pileecuador.ui.utils.*
import java.util.*

class VegaEcuadorPileFilterFragment : BaseFragment() {

    private lateinit var binding: FragmentEcuadorPileManagementFilterBinding
    override val layoutResourceId = R.layout.fragment_ecuador_pile_management_filter
    private var callBack: CallBack? = null
    private var fullFilter = ArrayList<String>()
    private var listFliterWhLoc = ArrayList<EcuadorFilterList>()
    private var startRange: String = ""
    private var endRange: String = ""
    private var aboveThirty: Boolean = false

    companion object {
        fun newInstance(bundle: Bundle, fullFilter: ArrayList<String>) = VegaEcuadorPileFilterFragment().putArgs {
    ...
    override fun onCreateView(
        ...
        binding = FragmentEcuadorPileManagementFilterBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        for (item in whLocations.distinct()) {
            if (!item.isNullOrEmpty()) {
                if (fullFilter.size > 0 && fullFilter.contains(item)) listFliterWhLoc.add(EcuadorFilterList(item, true))
                else listFliterWhLoc.add(EcuadorFilterList(item))
            }
        }
    ...
    private fun setUpAdapter(listFliterWhLoc: ArrayList<EcuadorFilterList>) {
        binding.rvWhLocation.setUpAdapter(
            listFliterWhLoc,
            R.layout.item_vega_ecuador_pile_filter_,
            ItemVegaEcuadorPileFilterBinding::inflate,
            { it, pos, bindItem ->
    ...
    private fun setupRangeBar() {
        binding.seekbarPlaceholder.setRangeValues(0, 50000)
