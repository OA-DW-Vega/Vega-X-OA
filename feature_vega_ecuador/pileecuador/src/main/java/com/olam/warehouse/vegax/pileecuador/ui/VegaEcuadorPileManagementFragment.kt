...
import com.olam.warehouse.vegax.pileecuador.databinding.FragmentEcuadorPileManagementBinding
import com.olam.warehouse.vegax.pileecuador.databinding.ItemEcuadorPileManagementBinding
...
import com.olam.warehouse.vegax.pileecuador.ui.utils.PILE_MANAGEMENT_EDIT
...
class VegaEcuadorPileManagementFragment : BaseFragment(), VegaSingleSelectListener {

    override val layoutResourceId = R.layout.fragment_ecuador_pile_management
    private lateinit var binding: FragmentEcuadorPileManagementBinding
    ...
    companion object {
        ...
        ) = VegaEcuadorPileManagementFragment().putArgs {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentEcuadorPileManagementBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun initUI() {
        ...
        binding.clInventory.setOnClickListener {
            ...
            callBack?.replaceFragment(
                INVENTORY_FRAG,
                VegaEcuadorThirdPartyPileLotListModel(
    ...
    override fun clickOnItem(data: String, isMaterial: Boolean, isVendor: Boolean, isGrade: Boolean) {
        ...
        if (isVendor) {
            ...
            fromVendor = vendorList.singleOrNull { it.vendorCode == code[0] } ?: VegaVendor()
    ...
    private fun setUpAdapter(list: ArrayList<VegaCocoaDispatchLots>) {
        ...
        binding.rvLots.setUpAdapter(
            lots,
            R.layout.item_ecuador_pile_management,
            ItemEcuadorPileManagementBinding::inflate,
            { item, pos, bindItem ->
                    ...
                    if (it.isNotEmpty()) {
                        ...
                        if (come ?: 0 <= 0) {
                            ...
                            vm.lots.forEach { item ->
                                if (item.batchNumber.equals(item.batchNumber) && item.materialCode.equals(
                                        item.materialCode
                                    )
                                ) {
                                    item.editedWeight = editVal
