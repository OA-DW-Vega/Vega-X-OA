...
class VegaEcuadorOffloadingSupplierSummaryFragment : BaseFragment() {
    ...
    /*private fun settingTrackTraceValue(){
        if(mReceiving.get(0).sourceLotId.isNotEmpty()) {
            binding.tvSourceLotValue.text = mReceiving.get(0).sourceLotId
           if(mReceiving.get(0).eudrStatus) updateEudrStatusFlag(true) else updateEudrStatusFlag(false)
        } else {
            binding.tvSourceLot.gone()
            binding.tvSourceLotValue.gone()
            binding.tvEudrStatus.gone()
            binding.eudrStatus.llEudrStatus.gone()
            binding.viSourceLot.gone()
        }
    }*/
    ...
    private fun settingTrackTracevalue(){
        ...
        if(receivingData.sourceLotId.isNotEmpty()){
            binding.tvSourceLot.visible()
            binding.tvSourceLotValue.visible()
        ...
        if(receivingData.farmerLessTransactionId?.isNotEmpty() == true){
            ...
            binding.tvEudrStatus.visible()
            binding.tvEudrStatusValue.visible()
            binding.tvSourceLot.text = getString(R.string.transaction_id)
...
//            binding.tvDirectEudrStatus.visible()
    ...
    private fun postReceiving() {
        if (AppUtils.isOnline()) {
            vm.postEcuadorOffloadingData(VegaReceivingPost(getCurrentKey(),
                getPlantDetails(),mReceiving,
    ...
    private fun moveToSuccessPage(wbId: String?, encodedImageContent: String?) {
        ...
        intent.putExtra(AppUtils.EUDR_STATUS,  if(mReceiving.get(0).eudrStatus)"1" else "0" )
